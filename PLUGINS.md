...
2. 打开 Jarvis → 设置 → 插件，把文件夹拖进安装区（或压缩成 .zip 拖入）
...
推荐先用 `jarvis-plugin-creator` 脚手架生成，再按需求删减：
...
当 Jarvis / Codex 这类 Agent 直接帮用户开发插件时，优先走 dev loop，而不是把半成品复制到正式插件目录：

1. 插件源码放在当前工作区，或 `${JARVIS_HOME}/plugin-dev-sources/`。
2. 调用 EventBus `plugin.dev.install` 或 HTTP `POST /api/plugins/dev/install`，把源码复制到 `${JARVIS_HOME}/plugins-dev/<pluginId>` 并加载。
...
开发态权限来自 Jarvis 记住的 dev slot，而不是 manifest 自己声明。`devRunId` 是一次 dev install/reload 的运行护栏，调用 enable/disable/reset/uninstall 时建议带上，避免旧上下文误操作新的开发槽。dev 操作只允许作用于 `${JARVIS_HOME}/plugins-dev/` 中的 runtime copy，不会写入 `${JARVIS_HOME}/plugins/`，也不会污染正式插件的禁用偏好。
...
- **手动安装**：将插件目录放到 `${JARVIS_HOME}/plugins/`。实际目录可在设置 → 插件页面或 `/api/plugins/settings` 的 `plugins_dir` 查看
...
插件私有数据自动存放在 `${JARVIS_HOME}/plugin-data/{pluginId}/`。删除插件时此目录保留，重新安装后配置还在。
...
`minAppVersion`（可选）声明插件运行所需的最低 Jarvis 版本。如果当前 app 版本低于该值，插件不会加载，状态标记为 `incompatible`。建议所有插件都声明此字段，避免用户在旧版本上遇到不兼容问题。
...
- 新插件可以使用 `@jarvis/plugin-runtime` 的 `defineTool()` 获得类型和默认参数；当前静态 `tools/*.js` loader 仍读取命名导出。
...
插件需要实时比分、天气、行情、外部搜索结果或第三方平台数据时，新代码应通过宿主提供的 `ctx.network.fetch()` 访问外部 HTTP API。iframe 页面只调用本插件自己的 route，例如 `jarvis.api.fetch("api/live-scores")`；route handler 再调用 `ctx.network.fetch("https://...")`。这样 iframe 认证、外部域名声明、超时、缓存和响应大小限制都在宿主边界内统一处理。
...
每个进入插件 route 的 HTTP 请求都会得到一份独立的请求级上下文。新 route 建议通过 `@jarvis/plugin-runtime` 的 `getPluginRequestContext(c)` 读取；老写法 `c.get("pluginRequestContext")` 仍兼容。

```js
import { getPluginRequestContext } from "@jarvis/plugin-runtime";
...
工厂函数在 session 创建时被 Pi SDK 调用，handler 在对应事件触发时执行。安装、启用或 reload full-access 插件后，Jarvis 会让当前空闲 session 重新绑定 extension runner；正在 streaming、compacting 或切换中的 session 会跳过重绑，下一次安全重建时生效。完整事件列表参见 Pi SDK extension 文档。
...
CLI provider 必须使用结构化参数绑定。不要拼 shell 字符串；Jarvis 会通过 `execFile` / `spawn` 的非 shell 模式运行命令，并把输出收束进媒体任务目录。
...
在 `manifest.json` 的 `contributes.configuration` 中声明配置 schema。Jarvis 会规范化字段、写入默认值、校验类型，并在设置 API 中自动隐藏敏感字段：
...
插件页面通过 WebView/iframe 渲染。旧 iframe 是兼容名称，新的插件设计可以把它理解成 WebView：适合展示已有 Web 应用、远程网站或单独 HTML。Jarvis 原生聊天 surface 不依赖 WebView/iframe。新插件建议使用 `@jarvis/plugin-sdk` 发送握手和宿主请求：
...
await jarvis.toast.show({ message: '已刷新', type: 'success' });
await jarvis.external.open('https://example.com');
await jarvis.clipboard.writeText('复制内容');
await jarvis.resources.open({ resource: { kind: 'session-file', fileId: 'sf_1' }, mode: 'preview' });
```

底层仍保留 `jarvis.host.request(type, payload)`，用于未来 capability 或实验能力；稳定能力优先使用 typed helper。
...
`jarvis.resources.*` 只是在 iframe 中向宿主发请求：可以请求打开资源、选择资源、申请访问权限，但不能直接读取或写入文件内容。真正的用户资源读写仍然放在插件服务端 route、tool 或 lifecycle 里，通过 `ctx.resources` 进入 ResourceIO。

宿主会在 iframe URL 上附加 `jarvis-theme` 和 `jarvis-css` 参数，插件可选择引用主题 CSS 以保持视觉一致：

```html
<link rel="stylesheet" href="${new URLSearchParams(location.search).get('jarvis-css')}">
```

静态前端资源放在插件目录的 `assets/` 下，由 Jarvis 宿主通过 `/api/plugins/{pluginId}/assets/...` 统一服务。这个模型参考 VS Code Webview 的资源边界：入口 route 通过本地 token 或 `pluginIframeTicket` 打开，成功返回页面后，宿主下发一个只作用于 `/api/plugins/{pluginId}/assets/` 的 HttpOnly 短会话 cookie。Vite split chunks、`React.lazy()`、CSS、字体、图片、JSON、wasm、MP4/WebM/MOV 等浏览器可播放视频请求不需要也不应该携带 `?token` 或 `pluginIframeTicket`。视频资源支持 HTTP Range，`<video>` 可以通过官方 assets 路由播放和 seek。

页面脚本调用本插件的动态 route API 时，优先使用 `@jarvis/plugin-sdk` 的 `jarvis.api.fetch()`。它会从当前 iframe route 推导 pluginId，并自动携带宿主随 iframe URL 下发的 surface session 凭证：
...
网页转换成插件时，要把同插件的 `fetch('/api/...')` 重写为 `jarvis.api.fetch(...)`，不要在浏览器代码里硬编码 `/api/plugins/{pluginId}/...`。底层协议是：宿主把 surface session 放在 query 参数 `pluginSurfaceSession`，页面调用本插件 route handler 时以 `X-Jarvis-Plugin-Surface-Session` 请求头（或同名 query 参数）回传：
...
Agent 新生成或重构插件时，不要为了 CSS、JS、图片、字体、MP4 这类静态资源额外注册 `/api/file`、`/api/video`、`/assets/*` 等 route。已有插件中的静态资源兼容 handler 继续可用；新工作遵循的正式契约是把这些资源放进 `assets/`，用 `jarvis.assets.url(...)` 或 route shell 中的官方 assets 路径引用。`pluginIframeTicket` 只用于 iframe 文档加载，不要手动拼到静态资源 URL。

React 插件 UI 建议使用 `@jarvis/plugin-components`，它提供和 Jarvis 当前控件接近的 Button、IconButton、TextInput、Textarea、Select、Switch、SettingRow、CardShell、List、EmptyState 等基础组件：
...
`JarvisThemeProvider` 支持三种模式：`inherit` 读取宿主 CSS 变量并走 SDK fallback；`jarvis` 固定使用某个 Jarvis 主题 token；`custom` 只覆盖插件显式传入的 token，未传字段继续 fallback。组件只依赖 `jarvis-plugin-*` class 和 CSS 变量，不导入 renderer 内部组件。
...
新插件建议使用 `@jarvis/plugin-runtime` 的 `definePlugin()`。它会返回兼容当前 PluginManager 的 class：
...
Plugin 间通信通过 EventBus 的请求-响应机制。`bus.handle` 需要 full-access 权限，`bus.request` 所有插件都可以用。`bus.listCapabilities()` / `bus.getCapability(type)` 可以读取当前稳定能力目录，目录记录能力名、输入输出 schema、权限要求、错误码、稳定性和当前是否有 handler 可用。新插件建议用 `@jarvis/plugin-runtime` 的 `defineBusHandler()`、`requestBus()` 和 `JARVIS_BUS_SKIP`，这样 handler 类型、请求参数和链式跳过语义都来自 SDK，而不是手写约定。
...
**SKIP 链**：同一事件类型可以注册多个 handler。系统按注册顺序调用，直到某个 handler 返回非 `JARVIS_BUS_SKIP` 的值。返回 `JARVIS_BUS_SKIP` 表示"我不处理，交给下一个"：
...
插件可以直接使用 `@jarvis/plugin-runtime` 的 typed helpers；底层对应 EventBus 能力如下：

| 能力 | 说明 |
|------|------|
| `session:create` | 创建不切换主界面焦点的普通 Jarvis session，可指定 `agentId`、`cwd`、`memoryEnabled`、`workspaceFolders`、`thinkingLevel`、`permissionMode`、`ownerPluginId`、`kind`、`visibility` |
...
```js
import { listUsageEntries, subscribeUsageEvents } from "@jarvis/plugin-runtime";
...
工具名自动加 `pluginId_` 前缀，通过 `register()` 在卸载时自动移除。Jarvis 自己触发这类工具（例如定时自动化或 dev smoke test）时，也会按 `execute(input, ctx)` 调用动态工具。Jarvis 内部桥接层如果已经暴露 Pi 工具签名，可以显式设置 `invocationStyle: "pi_tool"` 保留旧调用约定。
...
**重启恢复**：Jarvis 持久化任务与 schedule 元数据，不持久化插件函数。插件需要在 `onload()` 时重新注册 `task:register-handler`，然后调用 `task:list` 查询 `status: "recovering"` 的本插件任务，按自己的业务存储恢复或失败它们。

### 官方插件市场

设置 → 插件里的「打开插件市场」会进入独立的市场子页，该页面读取 `/api/plugins/marketplace`。Jarvis 采用 Obsidian 式官方社区插件目录：第三方开发者把插件提交到 `OH-Plugins`，用户只浏览、安装、启用、禁用，不管理市场源。
...
没有配置环境变量时，Jarvis 会先尝试读取 `${JARVIS_HOME}/plugin-marketplace/marketplace.json`（本地开发覆盖），如果不存在则读取官方 `OH-Plugins` URL。市场 index 的基本形状与 `OH-Plugins` 仓库一致：
...
市场版本管理以 `versions[]` 为长期契约：每一项声明 `version`、该版本的 `compatibility.minAppVersion` 和对应的 `distribution`。没有 `versions[]` 时，Jarvis 会把根级 `version` / `compatibility` / `distribution` 视为单版本条目。客户端会按 SemVer 选择“当前 app 能运行的最高版本”，同时保留 `latestVersion`、`selectedVersion`、`installedVersion`、`updateAvailable`、`downgrade`、`reinstall`、`compatible`、`installAction` 和 `canInstall` 给 UI 展示。

如果已安装版本高于当前 app 可兼容的最高市场版本，安装动作会被标记为 `downgrade`，必须显式传 `allowDowngrade: true` 才能继续。拖拽 / 本地路径安装同样会阻止隐式降级。更新安装会先备份旧目录到 `${JARVIS_HOME}/plugin-backups/<pluginId>/`，新版本加载失败时恢复旧目录并重新加载；成功后 `${JARVIS_HOME}/plugin-installs.json` 会记录来源、版本、release URL 和 sha256，供后续市场状态判断。
...
Jarvis 支持多 session / 多 agent 并行运行。插件开发时需注意：
