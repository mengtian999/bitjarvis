...
export function buildApprovalReviewContext({
  ...
  const dynamicApprovalHistory = typeof source?.getRecentApprovalHistory === "function"
    ? source.getRecentApprovalHistory(sessionPath, ctx)
    : null;
  return {
    sessionPath,
    ...
    recentApprovalHistory: Array.isArray(ctx?.recentApprovalHistory)
      ...
      : (Array.isArray(source?.recentApprovalHistory)
        ? source.recentApprovalHistory
        : (Array.isArray(dynamicApprovalHistory) ? dynamicApprovalHistory : [])),
    visibleTranscript,
