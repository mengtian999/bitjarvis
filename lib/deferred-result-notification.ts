...
export const DEFERRED_RESULT_MESSAGE_TYPE = "jarvis-background-result";
export const DEFERRED_RESULT_RECORD_TYPE = "jarvis-deferred-result";
...
function formatResultNotification(taskId, result, meta) {
  ...
  return `<jarvis-background-result task-id="${escapeXml(taskId)}" status="success" type="${type}">\n${body}\n</jarvis-background-result>`;
}

function formatFailNotification(taskId, reason, meta) {
  const type = escapeXml(meta?.type || "background-task");
  return `<jarvis-background-result task-id="${escapeXml(taskId)}" status="failed" type="${type}">\n${escapeXml(reason)}\n</jarvis-background-result>`;
}

function formatAbortNotification(taskId, reason, meta) {
  const type = escapeXml(meta?.type || "background-task");
  return `<jarvis-background-result task-id="${escapeXml(taskId)}" status="aborted" type="${type}">\n${escapeXml(reason || "task was stopped")}\n</jarvis-background-result>`;
}
...
export function parseDeferredResultNotification(content) {
  ...
  const match = /^<jarvis-background-result\b([^>]*)>([\s\S]*?)<\/jarvis-background-result>$/.exec(text);
