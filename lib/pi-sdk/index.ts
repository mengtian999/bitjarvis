...
/**
 * Jarvis 侧保持稳定的 Tool[] 调用契约，适配层负责转换 Pi SDK 版本差异。
 *
 * Pi SDK 0.68+ 将 `tools` 改成 string[] allowlist；Jarvis 的沙盒工具仍然
 ...
 * SDK upgrade fails Jarvis's typecheck at this boundary instead of at runtime.
 */
 ...
 * Jarvis 消费侧（core/model-image-preprocess.ts）契约保持不变：
...
/**
 * Register a provider through the ModelRegistry instance that owns Jarvis's
 ...
 *
 * 当 Jarvis 已经从自己的 allowlist 解析出 `allowedModel` 时，直接绑定该
 * ModelRegistry 刷新后对象；这避免 Pi 的私有刷新方法在 Jarvis 已禁用模型时
 * 找到并保留 Pi 内置目录中的同名模型。未传第二参数时保留旧 adapter 行为。
 *
 * @param {object} session - AgentSession 实例
 * @param {object} [allowedModel] - Jarvis 当前 allowlist 中、与 session 同身份的模型对象
