...
export function sessionFilesCacheDir(jarvisHome, sessionRef) {
  if (!jarvisHome) throw new Error("jarvisHome is required for session file cache");
  ...
  return path.join(jarvisHome, "session-files", hash);
}
