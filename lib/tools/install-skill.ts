 ...
 *   B. local_path / source(path) — 从当前 Jarvis server 可见路径安装完整 package
...
export function createInstallSkillTool({ getUserSkillsDir, getConfig, resolveUtilityConfig, onInstalled, registerSessionFile, resolveSessionFile }: any) {
  ...
  return {
    name: "install_skill",
    label: "Install Skill",
    description: "Install a complete skill package into the shared skill pool, enabled only for the current Agent by default. Pass exactly one source: github_url (GitHub repo), local_path (path visible to the current Jarvis server), fileId (uploaded SessionFile package), or source as a typed FileRef such as { type: 'path', path } or { type: 'session_file', fileId }. The whole package directory is installed — never pass raw skill_content or a lone SKILL.md. If the safety review returns requiresRiskConfirmation, explain the risk to the user and, only after explicit confirmation, call again with risk_accepted=true plus the returned risk_confirmation_token.",
    sessionPermission: {
      resolveInvocation: () => ({
        action: "install",
        kind: "review",
    ...
    parameters: Type.Object({
      ...
      local_path: Type.Optional(
        Type.String({ description: "Skill package path visible to the current Jarvis server. Can point to a folder containing SKILL.md, .zip, or .skill. Relative paths resolve from the current session cwd." })
      ),
