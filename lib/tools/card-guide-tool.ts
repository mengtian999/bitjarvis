/**
 * card-guide-tool.ts — Jarvis Interactive Card 设计手册工具
 *
...
// 视觉语言完全遵循 Jarvis 主题 + KAMI 墨染色系。

const HANDBOOK = `# Jarvis Interactive Card Design Handbook

You are generating HTML/SVG fragments that render inside an iframe in Jarvis's chat.
...
### KAMI extended palette (from Jarvis's notebook design language)
These are not injected as CSS variables but belong to Jarvis's full color spectrum. Use them directly as hex values.
...
### Chart palette (6 colors, all from Jarvis/KAMI ink-dyed family)
...
cursor: pointer;
\`\`\`
Hover: \`background: var(--accent-light)\`. No :active scale animation (Jarvis quiet motion principle).
...
| emoji | Jarvis uses SVG stroke icons only |
...
| Font-weight 700 | Jarvis allows 400/500 only. 600 for metrics only |
| Title Case / ALL CAPS | Jarvis uses sentence case exclusively |
...
- Default corner radius: \`rx="2"\` (Jarvis's square-corner style). Larger values only for deliberate pill shapes.
...
export function createCardGuideTool() {
  return {
    name: "jarvis_card_guide",
    label: "Interactive Card Guide",
    description:
      "Returns the Jarvis Interactive Card Design Handbook — CSS variables, colors, typography, layout rules, " +
    ...
    sessionPermission: {
      resolveInvocation: () => ({
        action: "read",
        kind: "read",
        capability: "jarvis_card_guide.read",
      }),
    },
