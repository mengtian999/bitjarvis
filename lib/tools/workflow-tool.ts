...
const WORKFLOW_DESCRIPTION = [
  ...
  "Always await agent(): const result = await agent('task prompt', { access: 'read', agentType: 'jarvis' }); agent() does not return { result }.",
...
export function createWorkflowTool(deps) {
  return {
    ...
    execute: async (_toolCallId, params, _signal, _onUpdate, ctx) => {
      ...
      // DeferredResultCoordinator 监听后以 <jarvis-background-result type="workflow"> steer 回灌主对话。
