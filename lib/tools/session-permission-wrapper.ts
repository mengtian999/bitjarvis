...
function captureSessionBinding(ctx: any, deps: any = {}) {
  try {
    ...
    // ID), not Jarvis's SessionManifest identity. Freeze it independently so
...
function toolActionName(params) {
  return params && typeof params.action === "string" && params.action.trim()
    ? params.action.trim()
    : "execute";
}

function buildToolApprovalRequest(confirmId: any, toolName: any, params: any, reviewContext: any = null) {
  const payload: any = { toolName, params };
  // The synthetic capability key this confirmation can record for the rest of
  // the session when the user picks "don't ask again for this action". It only
  // matches the same tool+action pair (see classifySyntheticSessionGrant).
  payload.sessionGrantCapability = `${toolName}:${toolActionName(params)}`;
  if (reviewContext && typeof reviewContext === "object") {
    if (nonEmptyText(reviewContext.reason)) payload.reviewReason = nonEmptyText(reviewContext.reason);
    if (nonEmptyText(reviewContext.reviewer)) payload.reviewer = nonEmptyText(reviewContext.reviewer);
    if (nonEmptyText(reviewContext.risk)) payload.risk = nonEmptyText(reviewContext.risk);
  }
  return {
...
async function askForToolApproval(toolName: any, params: any, sessionPath: any, deps: any, reviewContext: any = null) {
  ...
  deps.emitEvent?.({
    type: "session_confirmation",
    request: buildToolApprovalRequest(confirmId, toolName, params, reviewContext),
  }, sessionPath);
  const decision = await promise;
  const status = toStatus(decision?.action);
  const allowed = status === "confirmed";
  if (deps.recordApprovalOutcome) {
    try {
      const target = approvalTargetForTool(toolName, params);
      deps.recordApprovalOutcome(sessionPath, {
        toolName,
        action: toolActionName(params),
        ...(target?.label ? { target: target.label } : {}),
        status: allowed ? "approved" : "rejected",
        ...(reviewContext?.reason ? { reason: reviewContext.reason } : {}),
      });
    } catch (err) {
      // The confirmation decision stands; a failed history write must not
  ...
  return tools.map((tool: any) => {
    ...
    return {
      ...
      execute: async (...args: any[]) => {
        ...
        let reviewContextForAsk: any = null;
        if (decision.action === "review") {
          ...
          reviewContextForAsk = {
            ...
            reviewer: review.decision?.reviewer || null,
            risk: review.decision?.risk || null,
          };
        }
        ...
        const approval = await askForToolApproval(tool.name, approvalParams.value, sessionPath, deps, reviewContextForAsk);
        if (!approval.allowed) {
          return toolError("Tool action was not approved.", {
