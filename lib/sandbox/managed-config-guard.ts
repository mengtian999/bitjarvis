...
export function isManagedAgentConfigPath(filePath, { jarvisHome }: { jarvisHome?: string } = {}) {
  if (!filePath || !jarvisHome) return false;

  const target = resolveExistingOrNearest(filePath);
  const home = resolveExistingOrNearest(jarvisHome);
...
export function createManagedConfigWriteGuard({ jarvisHome }: { jarvisHome?: string } = {}) {
  return (absolutePath, operation) => {
    if (operation !== "write" && operation !== "delete") return { allowed: true };
    if (!isManagedAgentConfigPath(absolutePath, { jarvisHome })) return { allowed: true };
    return {
      allowed: false,
