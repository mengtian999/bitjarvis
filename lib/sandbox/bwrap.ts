...
function addPrivateRuntimeEnv(args) {
  const runtimeDirs = ["/tmp/jarvis-home", "/tmp/jarvis-cache", "/tmp/jarvis-npm-cache", "/tmp/jarvis-pip-cache"];
  for (const dir of runtimeDirs) args.push("--dir", dir);
  args.push(
    "--setenv", "HOME", "/tmp/jarvis-home",
    "--setenv", "XDG_CACHE_HOME", "/tmp/jarvis-cache",
    "--setenv", "npm_config_cache", "/tmp/jarvis-npm-cache",
    "--setenv", "PIP_CACHE_DIR", "/tmp/jarvis-pip-cache",
  );
}
