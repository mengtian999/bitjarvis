...
export function createSandboxedTools(cwd, customTools, {
  ...
  const checkManagedConfigWrite = createManagedConfigWriteGuard({ jarvisHome });
  ...
  const searchToolPaths = {
    managedBinDir: resolveJarvisPiSdkManagedBinDir(jarvisHome),
    legacyManagedBinDir: resolveLegacyPiSdkManagedBinDir(jarvisHome),
  };
