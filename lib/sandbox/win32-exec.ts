...
const PROBE_TOKEN = "__jarvis_probe_ok__";
...
const WIN32_SANDBOX_HELPER_LAUNCH_FAILURE_RE = /jarvis-win-sandbox:\s+CreateProcessAsUserW failed/i;
...
function getBundledGitRoots(env = process.env, deps: Record<string, any> = {}) {
  ...
  const roots = [
    ...
    env.JARVIS_ROOT ? resolve(env.JARVIS_ROOT, "..", "git") : null,
...
function findGitRuntime({ env = process.env, bundledOnly = false } = {}) {
  ...
  throw new Error(
    "[win32-exec] No usable git.exe found. Install Git for Windows or rebuild Jarvis with bundled MinGit."
  );
}
...
function withWin32SandboxRuntimeEnv(baseEnv, sandbox) {
  ...
  if (!sandboxIsEnabled(sandbox) || !sandbox?.jarvisHome) return env;

  const root = joinRuntimePath(sandbox.jarvisHome, ".ephemeral", WIN32_SANDBOX_ENV_DIR);
...
function findNodeRuntimeOnPath(commandName, args, env) {
  const candidates = [];
  if (env?.JARVIS_DEV_NODE_BIN) candidates.push(executableInfoFromPath(env.JARVIS_DEV_NODE_BIN, `JARVIS_DEV_NODE_BIN (${env.JARVIS_DEV_NODE_BIN})`));
...
function collectWin32EnvironmentDiagnostics(env) {
  ...
  return [
    ...
    diagnosticValueMetadata("JARVIS_ROOT", envValue(env, "JARVIS_ROOT")),
    diagnosticValueMetadata("JARVIS_SERVER_ENTRY", envValue(env, "JARVIS_SERVER_ENTRY")),
...
function emitWin32RuntimeFailureDiagnostic(onData, {
  ...
  const lines = [
    ...
    diagnosticValueMetadata("JARVIS_HOME", sandbox?.jarvisHome || envValue(env, "JARVIS_HOME")),
    ...
    "Next step: attach this diagnostic with the command log; for cached POSIX runtimes also clear JARVIS_HOME/.ephemeral/win32-sandbox-runtime and retry.",
    "",
  ].filter(Boolean);
...
function prepareRuntimeForSandbox(runtimeInfo, sandbox, kind) {
  if (!sandboxIsEnabled(sandbox) || !sandbox?.jarvisHome) return runtimeInfo;
  return prepareSandboxRuntime(runtimeInfo, {
    jarvisHome: sandbox.jarvisHome,
    kind,
  });
}
...
// (above) already points TEMP/TMP at <jarvisHome>/.ephemeral/win32-sandbox-env
...
// reasons.
function win32SandboxEnvRoot(jarvisHome) {
  if (!jarvisHome) return null;
  return joinRuntimePath(jarvisHome, ".ephemeral", WIN32_SANDBOX_ENV_DIR);
}

function withSandboxEnvRootAsRequiredWritePath(grants, jarvisHome) {
  const root = win32SandboxEnvRoot(jarvisHome);
  // No jarvisHome means withWin32SandboxRuntimeEnv never redirected TEMP in
...
async function spawnViaSandboxHelper({
  ...
  const grants = withSandboxEnvRootAsRequiredWritePath(grantsForSandbox(sandbox, cwd), sandbox.jarvisHome);
  ...
  try {
    ...
    if (!terminal) {
      ...
      error.code = "JARVIS_WIN32_SANDBOX_TERMINAL_PROTOCOL";
      throw error;
    }
    ...
    if (terminal.status === "termination_failed") {
      ...
      error.code = "JARVIS_WIN32_SANDBOX_TERMINATION_FAILED";
...
function throwSandboxPowerShellUnsupported() {
  ...
  error.code = "JARVIS_WIN32_SANDBOX_POWERSHELL_UNSUPPORTED";
  throw error;
}
