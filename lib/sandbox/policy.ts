...
export function deriveSandboxPolicy({
  ...
  return {
    ...
    writablePaths: [
      ...
      ...READ_WRITE_HOME_DIRS.map((d) => path.join(jarvisHome, d)),
      ...workspaceRoots,
      ...runtimeWritablePaths,
    ].filter(Boolean),
    ...
    readablePaths: [
      ...
      ...READ_ONLY_HOME_DIRS.map((d) => path.join(jarvisHome, d)),
    ].filter(Boolean),

    // OS 沙盒用：拒绝读取（文件 + 目录）
    denyReadPaths: [
      ...BLOCKED_FILES.map((f) => path.join(jarvisHome, f)),
      ...BLOCKED_DIRS.map((d) => path.join(jarvisHome, d)),
    ],

    // OS 沙盒用：写保护（在可写范围内再限制）
    protectedPaths: [
      ...workspaceRoots.map((root) => path.join(root, ".git")),
      path.join(jarvisHome, "session-files"),
    ].filter(Boolean),
  };
}
