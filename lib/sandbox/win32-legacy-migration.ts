...
import {
  buildWin32JarvisWriteAclCleanupArgs,
...
const LEGACY_PROFILE_PREFIX = "com.jarvis.sandbox.";
...
export function isLegacyWin32SandboxProfileName(name) {
  return typeof name === "string" && /^com\.jarvis\.sandbox\.\d+\.\d+$/i.test(name);
}
...
export function collectWin32LegacySandboxMigrationTargets({
  ...
  if (jarvisHome) {
    push(jarvisHome);
    push(joinWin32Aware(jarvisHome, ".ephemeral"));
    push(joinWin32Aware(jarvisHome, "agents"));
    push(joinWin32Aware(jarvisHome, "session-files"));
    push(joinWin32Aware(jarvisHome, "uploads"));
  }
...
function defaultMigrationMarkerPath(jarvisHome) {
  return jarvisHome
    ? path.join(jarvisHome, "user", "win32-sandbox-migration-v3.json")
    : null;
}

function defaultCleanupMarkerPath(jarvisHome) {
  return jarvisHome
    ? path.join(jarvisHome, "user", "win32-sandbox-cleanup-v4.json")
    : null;
}
...
function buildMigrationPhases({ targets, cleanup, cleanupProfiles = true }) {
  ...
  if (targets.aclPaths?.length) {
    const args = [];
    if (cleanup) {
      args.push(...buildWin32JarvisWriteAclCleanupArgs({ paths: targets.aclPaths }));
...
export async function runWin32LegacySandboxMigration({
  ...
  const completionMarkerPath = cleanup && !disableMarker
    ? (markerPath || defaultMigrationMarkerPath(jarvisHome))
    : null;
...
export async function runWin32LegacySandboxRootCleanup({
  ...
  const marker = markerPath || defaultCleanupMarkerPath(jarvisHome);
...
export class Win32LegacySandboxCleanupQueue {
  ...
  constructor({
    ...
    this.markerPath = markerPath || defaultCleanupMarkerPath(jarvisHome);
    this.env = env;
    this.spawn = spawn;
