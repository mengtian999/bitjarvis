...
export function buildWin32JarvisWriteAclCleanupArgs({ paths = [] } = {}) {
  const targets = [...new Set((paths || []).filter(Boolean))];
  if (targets.length === 0) {
    throw new Error("win32 Jarvis write ACL cleanup requires at least one path");
  }
  const out = [];
  for (const p of targets) {
    out.push("--cleanup-jarvis-write-acl", p);
  }
  return out;
}
