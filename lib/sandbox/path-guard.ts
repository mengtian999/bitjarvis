...
export class PathGuard {
  ...
  constructor(policy) {
    ...
    this.jarvisHome = this._resolveReal(policy.jarvisHome) || path.resolve(policy.jarvisHome);
  ...
  _getAccessLevelResolved(resolved) {
    // 1. BLOCKED 文件（jarvisHome 根）
    for (const f of BLOCKED_FILES) {
      if (resolved === path.join(this.jarvisHome, f)) return AccessLevel.BLOCKED;
    }

    // 2. BLOCKED 目录
    for (const d of BLOCKED_DIRS) {
      if (this._isInside(resolved, path.join(this.jarvisHome, d))) {
    ...
    // 8. jarvisHome 内未匹配 → 遵守 read-all 契约：非敏感路径可读，写仍需显式白名单。
    if (this._isInside(resolved, this.jarvisHome)) {
