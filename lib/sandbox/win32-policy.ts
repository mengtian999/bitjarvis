...
export function externalReadPathsFromSessionFiles(files = [], { workspaceRoots = [], jarvisHome }: { workspaceRoots?: any[]; jarvisHome?: any } = {}) {
  const workspaceRootKeys = identityKeys(uniqueNormalized(workspaceRoots));
  const homeKey = jarvisHome ? filesystemIdentityKeySync(jarvisHome) : null;
  const out = [];
