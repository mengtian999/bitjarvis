...
export const WIN32_SANDBOX_HELPER_NAME = "jarvis-win-sandbox.exe";
export const WIN32_SANDBOX_TERMINAL_PREFIX = "jarvis-win-sandbox: terminal-v1";
...
function legacyPackagedDesktopResourceRoots(env) {
  ...
  const roots = [];
  const appPath = env.JARVIS_DESKTOP_APP_PATH;
  ...
  const execPath = env.JARVIS_DESKTOP_EXEC_PATH;
  ...
  // JARVIS_DESKTOP_RESOURCES_PATH contract existed.
...
function resourceSiblingCandidates(segments, { env, resourcesPath }) {
  const roots = [];
  pushUniqueRuntimePath(roots, env.JARVIS_DESKTOP_RESOURCES_PATH);
  ...
  if (env.JARVIS_ROOT) {
    pushUniqueRuntimePath(roots, resolveRuntimePath(env.JARVIS_ROOT, ".."));
  }
...
function parseWin32SandboxTerminalRecordLine(line: string): Win32SandboxTerminalRecord | null {
  const match = line.match(
    /^jarvis-win-sandbox: terminal-v1 status="([^"]*)" exitCode="([^"]*)" timeoutMs="([^"]*)" win32Error="([^"]*)"$/,
  );
