...
function createOpenClawIssue({ marker, id, name, version }) {
  return {
    ...
    name,
    version,
    message: `This package looks like an OpenClaw plugin (${marker}). Jarvis cannot install OpenClaw plugin zips directly. Use a Jarvis plugin package with manifest.json, or port the plugin to Jarvis's plugin SDK.`,
  };
}
