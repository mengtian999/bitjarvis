...
export const DEFAULT_OFFICIAL_PLUGIN_MARKETPLACE_URL = "https://raw.githubusercontent.com/mengtian999/OH-Plugins/main/marketplace.json";
...
export function createDefaultPluginMarketplace({ jarvisHome, env = process.env, fetchImpl }: any = {}) {
  const envPath = normalizeOptionalText(env.JARVIS_PLUGIN_MARKETPLACE_FILE);
  const envUrl = normalizeOptionalText(env.JARVIS_PLUGIN_MARKETPLACE_URL);
  const localIndex = jarvisHome ? path.join(jarvisHome, "plugin-marketplace", "marketplace.json") : null;
