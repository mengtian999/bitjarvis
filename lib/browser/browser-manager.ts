...
// 冷保存文件：重启后恢复浏览器状态（由 setJarvisHome 注入路径）
let _jarvisHome = null;
const _coldStatePath = () => path.join(_jarvisHome, "user", "browser-sessions.json");
...
export class BrowserManager {
  ...
  /**
   * 注入用户数据根目录（由入口在启动时调用）
   * @param {string} home - engine.jarvisHome
   */
  static setJarvisHome(home) {
    _jarvisHome = home;
  }
