...
}: { jarvisHome?: string; pluginId?: string; pluginDir?: string; version?: string; maxBackups?: number } = {}) {
  if (!jarvisHome || !pluginId || !pluginDir || !fs.existsSync(pluginDir)) return null;
  const backupRoot = path.join(jarvisHome, "plugin-backups", safeSegment(pluginId));
