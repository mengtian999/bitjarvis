...
const VALID_YUAN = new Set(["jarvis", "butter", "ming", "kong"]);
...
function isZipPath(filePath) {
  ...
  return lower.endsWith(".zip") || lower.endsWith(".jarvis-package.zip");
}
...
function normalizeAgent(card) {
  ...
  const yuan = VALID_YUAN.has(agent.yuan || card?.yuan) ? (agent.yuan || card?.yuan) : "jarvis";
...
function ensurePreviewAssets(plan) {
  const yuan = plan.agent?.yuan || "jarvis";
  if (!plan.assets.avatar) {
    copyPlanAsset(plan, defaultAvatarForYuan(yuan), "avatar", "jarvis-default-avatar");
  }
  if (!plan.assets.cardFront && plan.assets.avatar) {
    plan.assets.cardFront = { ...plan.assets.avatar };
  }
  if (!plan.assets.cardBack) {
    copyPlanAsset(plan, defaultCardBackForYuan(yuan), "cardBack", "jarvis-default-card-back");
  }
  if (!plan.assets.yuanIcon) {
    copyPlanAsset(plan, defaultYuanIconForYuan(yuan), "yuanIcon", "jarvis-default-yuan-icon");
  }
}
...
export function createCharacterCardService(engine) {
  const stagingRoot = path.join(engine.jarvisHome, ".ephemeral", "character-card-imports");
  ...
  function readAgentExportSource(agentId) {
    ...
    const yuan = VALID_YUAN.has(config?.agent?.yuan) ? config.agent.yuan : "jarvis";
  ...
  async function exportAgentPackage(agentId, options: Record<string, any> = {}) {
    ...
    const token = randomToken();
    const exportRoot = path.join(engine.jarvisHome, ".ephemeral", "character-card-exports", token);
