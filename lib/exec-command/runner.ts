...
function normalizeThrownToolError(err: any, maxOutputTokens: number) {
  if (err?.jarvisCommandBlockedResult) {
    return firstText(err.jarvisCommandBlockedResult);
  }
...
function getTempFilePath() {
  ...
  return join(tmpdir(), `jarvis-exec-command-${id}.log`);
}
...
export async function runExecCommandDirect({
  ...
  try {
    ...
    collector.close();
    if (err?.jarvisCommandBlockedResult) {
      return mergeExecDetails(err.jarvisCommandBlockedResult, {
        ...execDetails,
        ok: false,
        transportError: false,
