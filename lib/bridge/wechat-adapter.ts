...
function resolveSyncBufPath(jarvisHome, botToken) {
  const dir = path.join(jarvisHome, "bridge", "wechat");
  fs.mkdirSync(dir, { recursive: true });
  return path.join(dir, `sync-${hash8(botToken)}.json`);
}

function resolveContextCachePath(jarvisHome, botToken) {
  const dir = path.join(jarvisHome, "bridge", "wechat");
  fs.mkdirSync(dir, { recursive: true });
 ...
 * @param {string} [opts.jarvisHome] - jarvisHome 路径，用于 cursor 持久化
 * @param {(msg: object) => void} opts.onMessage
 * @param {(status: string, error?: string) => void} [opts.onStatus]
 */
export function createWechatAdapter({ botToken, jarvisHome, agentId, onMessage, onStatus }) {
  ...
  const syncBufPath = jarvisHome ? resolveSyncBufPath(jarvisHome, botToken) : null;
  const contextCachePath = jarvisHome ? resolveContextCachePath(jarvisHome, botToken) : null;
