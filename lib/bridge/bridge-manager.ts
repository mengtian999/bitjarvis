...
import { createMatrixAppServiceAdapter } from "./matrix-appservice-adapter.ts";
...
const ADAPTER_REGISTRY = {
  ...
  wechat: {
    create: (creds, onMessage, hooks, agentId) => createWechatAdapter({
      ...
      jarvisHome: creds.jarvisHome,
      agentId,
      onMessage,
      onStatus: hooks?.onStatus,
    }),
    getCredentials: (cfg) => cfg?.enabled && cfg?.botToken ? { botToken: cfg.botToken, jarvisHome: cfg._jarvisHome || "" } : null,
    ownerSessionKey: (userId, agentId) => `wx_dm_${userId}@${agentId}`,
    connectsAsync: true,
  },
  matrix: {
    // AppService POLL 模式（P1）：一个 agent 一个虚拟用户，as_token + ?user_id= 长轮询 /sync。
    // ownerSessionKey 的 chatId 段是 owner 的 Matrix ID（@alice:server），
    // adapter.resolveOwnerChatId 负责把 userId 解析成实际 DM roomId（sendProactive 路径）。
    create: (creds, onMessage, hooks, agentId) => createMatrixAppServiceAdapter({
      homeserverUrl: creds.homeserverUrl,
      asToken: creds.asToken,
      senderUserId: creds.senderUserId,
      agentId,
      onMessage,
      onStatus: hooks?.onStatus,
    }),
    getCredentials: (cfg) => cfg?.enabled && cfg?.homeserverUrl && cfg?.asToken && cfg?.senderUserId
      ? { homeserverUrl: cfg.homeserverUrl, asToken: cfg.asToken, senderUserId: cfg.senderUserId }
      : null,
    ownerSessionKey: (userId, agentId) => `mx_dm_${userId}@${agentId}`,
    connectsAsync: true,
  },
};
...
export class BridgeManager {
  ...
  /**
   * @param {object} opts
   * @param {import('../../core/engine.ts').JarvisEngine} opts.engine
  ...
  constructor({ engine, hub }) {
    ...
    this.mediaPublisher = new MediaPublisher({
      baseUrl: engine.getBridgeMediaPublicBaseUrl?.() || process.env.JARVIS_BRIDGE_PUBLIC_BASE_URL || "",
      allowedRoots: roots,
    });
  ...
  autoStart(agents) {
    ...
    for (const [agentId, agent] of agents) {
      ...
      for (const [platform, spec] of Object.entries(ADAPTER_REGISTRY)) {
        ...
        if (platform === "wechat") cfg._jarvisHome = this.engine.jarvisHome;
        let creds = null;
        try {
