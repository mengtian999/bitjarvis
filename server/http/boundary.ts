...
export function createRequestContext(c, engine) {
  ...
  const request = {
    ...
    url: c?.req?.url || "http://jarvis.local/",
    path: safePathname(c?.req?.url || "http://jarvis.local/"),
  };
...
function getActiveGrants(engine, authPrincipal) {
  ...
  if (!engine?.jarvisHome) return implicit ? [implicit] : [];
  try {
    return [
      ...findActiveGrantsForPrincipal(engine.jarvisHome, authPrincipal.principalId),
      ...(implicit ? [implicit] : []),
    ];
  } catch {
