...
export function createDeskRoute(engine, hub) {
  ...
  // source: it is constructed unconditionally by JarvisEngine, so its absence
  ...
  function writeUploadedCoverImage(body) {
    ...
    const uploadRoot = path.join(engine.jarvisHome || os.tmpdir(), "tmp", "markdown-cover-uploads");
