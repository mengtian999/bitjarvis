...
async function listStudioWorkspaces(engine, requestContext) {
  ...
  for (const mount of listStudioMountsForStudio(engine.jarvisHome, studioId)) {
...
async function createLocalPathWorkspace(engine, requestContext, body) {
  ...
  const mount = upsertStudioMount(engine.jarvisHome, {
...
function disableLocalPathWorkspace(engine, requestContext, mountId) {
  const studioId = requireStudioId(requestContext);
  const activeMount = listStudioMountsForStudio(engine.jarvisHome, studioId)
  ...
  return disableStudioMount(engine.jarvisHome, mountId, { hostStudioId: studioId });
}
