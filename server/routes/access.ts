...
export function createAccessRoute({
  ...
  if (!engine?.jarvisHome) throw new Error("engine.jarvisHome required");
  ...
  route.put("/access/network", async (c) => {
    ...
    try {
      ...
      const existing = loadServerNetworkConfig(engine.jarvisHome);
      ...
      const network = saveServerNetworkConfig(engine.jarvisHome, {
        ...existing,
        mode,
        listenHost,
        listenPort,
  ...
  async function issueAccessCredential(c, profile) {
    ...
    try {
      ...
      const issued = createDeviceCredential(engine.jarvisHome, {
  ...
  route.put("/access/account/profile", async (c) => {
    ...
    try {
      const body = await safeJson(c);
      const account = updateLocalAccountProfile(engine.jarvisHome, {
  ...
  route.put("/access/account/password", async (c) => {
    ...
    try {
      const body = await safeJson(c);
      const account = setLocalAccountPassword(engine.jarvisHome, {
  ...
  route.delete("/access/account/password", async (c) => {
    ...
    if (denied) return denied;
    try {
      const account = clearLocalAccountPassword(engine.jarvisHome, { now: now() });
      recordSecurityAuditEvent(c, engine, {
...
export function createAccessSummary(engine, runtimeState, listLanAddresses = getLanAddresses) {
  const network = loadServerNetworkConfig(engine.jarvisHome);
  const registries = loadDeviceAccessRegistries(engine.jarvisHome);
  return {
    network: createNetworkSummary(network, runtimeState, listLanAddresses),
    account: getLocalAccountSummary(engine.jarvisHome),
...
function normalizeNetworkMode(value) {
  if (value === "loopback" || value === "lan" || value === "remote" || value === "custom_remote") return value;
  throw new Error("mode must be loopback, lan, remote, or custom_remote");
}
