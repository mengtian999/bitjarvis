...
export function createServerIdentityRoute({ jarvisHome, appVersion = "?", getRuntimeContext }: { jarvisHome?: string; appVersion?: string; getRuntimeContext?: () => any } = {}) {
  ...
  route.get("/server/identity", (c) => {
    try {
      const runtimeContext = typeof getRuntimeContext === "function"
        ? getRuntimeContext()
        : createServerRuntimeContext({ jarvisHome, appVersion });
