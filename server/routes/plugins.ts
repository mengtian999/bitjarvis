...
async function proxyToPlugin(c: any, pluginApp: any, pluginId: string, agentId?: string, requestPrincipal: any = null) {
  ...
  headers.delete("X-Jarvis-Agent-Id");
...
export function parsePluginIframeSurfaceRoute(routeUrl: any) {
  const parsed = new URL(String(routeUrl || ""), "http://jarvis.local");
...
function appendPluginAssetSessionCookie(c: any, engine: any, pluginId: string, response: Response, iframeTicket: any) {
  ...
  if (!engine?.jarvisHome) return response;
...
function assertPluginIframeSurfaceAllowed(surface: any) {
  ...
  const pathname = new URL(surface.surfacePath, "http://jarvis.local").pathname;
...
async function downloadMarketplaceRelease({ engine, plugin }: { engine: any; plugin: any }) {
  ...
  if (!engine.jarvisHome) {
    const err = new Error("JARVIS_HOME is unavailable for plugin release installation") as Error & { status: number };
    err.status = 500;
    throw err;
  }
  ...
  const downloadsDir = path.join(engine.jarvisHome, "plugin-install-sources", pluginId, version);
 ...
 * @param {import('../../core/engine.ts').JarvisEngine} engine
 */
