...
function getCachePath(engine: any) {
  return path.join(engine.jarvisHome, "models-cache.json");
}
...
export function createProvidersRoute(engine: any) {
  ...
  /**
   * 统一概览：合并 Provider Catalog + Jarvis provider defaults + OAuth status
  ...
  route.get("/providers/summary", async (c) => {
    ...
    // 追加没有 Provider Catalog overlay 的 OAuth provider。模型仍来自 Jarvis plugin 默认清单。
  ...
  function jarvisCatalogFallback(name: any) {
    if (!name) return { error: "name is required for Jarvis model catalog", models: [] };
    ...
    if (payload.models.length === 0) {
      return {
        source: "jarvis-catalog",
        error: `No models found in Jarvis catalog for provider "${sourceProviderId}"`,
        ...payload,
      };
    }
    return { source: "jarvis-catalog", ...payload };
  }
  ...
  route.post("/providers/fetch-models", async (c) => {
    ...
    // Jarvis-owned models-json OAuth catalogs never probe ChatGPT /models. Legacy
    ...
    if (chatProvider?.projection === "models-json" && chatProvider.credentialSource === "auth-storage") {
      return c.json(jarvisCatalogFallback(name));
    }
