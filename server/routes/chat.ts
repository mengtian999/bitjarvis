...
export function resolveTurnStallAbortMs(value = process.env.JARVIS_TURN_STALL_ABORT_MS) {
...
export function createChatRoute(engine: any, hub: any, { upgradeWebSocket }: any) {
  ...
  function scheduleDisconnectAbort() {
    ...
    if (!disconnectAbortEnabled) {
      // 默认断连不中断：agent 任务继续在后台跑完，用户重连后可接续查看。
      // 需要旧行为（无客户端就 abort_all）时设 JARVIS_WS_DISCONNECT_ABORT_ENABLED=1。
      debugLog()?.log("ws", "no clients; leaving streaming sessions running in background");
      return;
    }
  ...
  function scheduleTurnStallWatchdog(sessionPath, ss) {
    ...
    ss.turnStallTimer = setTimeout(() => {
      ...
      // 存在进行中的工具执行时不误判为「停滞」：长工具（长命令 / browser / computer-use）
      // 可能在执行期间长时间不产生流式事件。这里刷新活动时间并顺延看门狗，避免把
  ...
  hub.subscribe((event, sessionPath) => {
    ...
    // stop_reason 归一化：不同 provider 对「输出预算耗尽被截断」的表示不同
    // （OpenAI 系用 "length"，Anthropic 旧文档用 "max_tokens"），统一视为 "length"，
    // 否则 Claude 的 thinking-only 被截断时既不会标记 truncated，也不会触发空回复提示。
    ...
    const normalizeStopReason = (reason) => (
      typeof reason === "string" && LENGTH_STOP_REASONS.has(reason.toLowerCase()) ? "length" : reason
    );

    // 兜底：把分阶段正文缓冲里残留的内容也落盘（text_end 缺失时把正文显示出来）。
    // OpenAI 系响应（openai-responses / codex / azure-openai）的正文 text_delta 会先囤在
    // ss.pendingPhaseTextByIndex，等 text_end 才真正显示。若 text_end 没到，turn 收尾时
    // 这段缓冲会被清掉 —— 正文其实已生成却被丢弃，表现就是「思考完成但无正文，重试后才出现」。
    // 这里在 turn 结束兜底 flush，避免正文静默丢失。
    const flushPendingPhaseTextBuffer = () => {
      if (!(ss.pendingPhaseTextByIndex instanceof Map)) return;
      if (ss.pendingPhaseTextByIndex.size === 0) return;
      const pending = ss.pendingPhaseTextByIndex;
      for (const [, buffered] of pending) {
        if (typeof buffered === "string" && buffered.trim()) emitVisibleTextDelta(buffered);
      }
      pending.clear();
    };
    ...
    if (event.type === "message_update") {
      ...
      flushPendingTurnInputConsumptions(sessionPath, ss, event.message);
      ss.hasToolCall = true;
      ss.activeToolCount = Math.max(0, (ss.activeToolCount || 0) + 1);
      if (ss.isThinking) {
        ss.isThinking = false;
        emitStreamEvent(sessionPath, ss, { type: "thinking_end" });
      }
      ...
      flushTerminalParsers();
      // 兜底：text_end 缺失时把分阶段正文缓冲残留内容也落盘，
      // 避免 turn 收尾把这些（已经到手的）正文直接清掉。
      flushPendingPhaseTextBuffer();
      ...
      // 仅生成了 thinking 而无正文/无工具调用，同样按空回复处理
      // （与 core/llm-client.ts 的 empty_after_thinking 判定对齐，避免「思考完成却无正文」静默）
      const thinkingOnlyVisibleResult = ss.hasThinking && !ss.hasOutput && !ss.hasToolCall;
      const emptyReply =
        (!ss.hasOutput && !ss.hasToolCall && !ss.hasThinking)
        || truncatedWithoutVisibleResult
