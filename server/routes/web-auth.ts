...
export function createWebAuthRoute({
  ...
  route.post("/web-auth/login", async (c) => {
    ...
    const issued = createWebSession(jarvisHome, {
      principal,
  ...
  route.post("/web-auth/logout", async (c) => {
    revokeWebSession(jarvisHome, c.req.header("cookie"), { now: now() });
...
function authenticatePasswordLogin(c, {
  ...
  const verified = verifyLocalAccountPassword(jarvisHome, { username, password });
