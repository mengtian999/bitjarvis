...
export function createConfigRoute(engine: any) {
  ...
  route.put("/config", async (c) => {
    try {
      ...
      // onProviderChanged 涉及 Pi SDK 重新注册 provider，可能抛异常，
      // 但此时 provider 已经落盘——不该让运行时刷新失败把 200 变成 500，
      // 否则前端会弹错误提示但实际保存已成功。
      if (providersChanged) {
        try {
          ...
          debugLog()?.error("api", `onProviderChanged failed after provider change (provider already saved): ${refreshErr.message}`);
        }
