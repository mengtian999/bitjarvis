...
const TODO_COMPLETE_MESSAGE =
  "[Jarvis Todo] The user marked the current todo list as completed and removed it from the session UI. Treat every item in that list as completed. Create a new todo list only if new work needs tracking.";
...
export function createSessionsRoute(engine, hub = null) {
  ...
  route.post("/sessions/switch", async (c) => {
    try {
      ...
      try { appendFileSync(path.join(engine.jarvisHome, "switch-error.log"), `${new Date().toISOString()}\n${errDetail}\n---\n`); } catch {}
      return c.json({ error: err.message }, 500);
    }
  });
...
function patchSessionFileLifecycleBlocks(blocks, engine, sessionPath) {
  ...
  for (const block of blocks || []) {
    ...
    if (!file && block.type === "screenshot" && block.base64 && engine?.jarvisHome && typeof engine?.getSessionFileByPath === "function") {
      try {
        const filePath = browserScreenshotPath(engine.jarvisHome, sessionPath, {
          base64: block.base64,
