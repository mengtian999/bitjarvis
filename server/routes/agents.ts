...
import { deactivateMatrixProfileForAgent, syncMatrixProfileForAgent } from "./matrix-bind.ts";

/**
 * 改名 / 换头像后把新 displayname + avatar 异步同步到 Matrix 虚拟用户。
 * 仅当该 agent 已绑定 matrix 才真正执行（syncMatrixProfileForAgent 内部判空）；
 * fire-and-forget、失败静默——IM profile 同步失败不影响 agent 配置 / 头像保存。
 */
function triggerMatrixProfileSync(engine: any, agentId: string) {
  void syncMatrixProfileForAgent(engine, agentId).catch(() => {
    // 静默：IM profile 同步失败不影响 agent 配置 / 头像保存。
  });
}

/**
 * 删除 agent 后异步把其 Matrix 虚拟用户下线（停 adapter + presence offline）。
 * 仅当该 agent 曾绑定 matrix 才执行；fire-and-forget、失败静默——IM 下线失败不影响 agent 删除。
 */
function triggerMatrixDeactivate(engine: any, agentId: string, matrixCfg: any) {
  void deactivateMatrixProfileForAgent(engine?.getBridgeManager?.(), agentId, matrixCfg).catch(() => {
    // 静默：IM 下线失败不影响 agent 删除。
  });
}
...
function readAgentPersonaSource(engine, id, kind) {
  ...
  return resolvePersonaSource({
    ...
    yuanType: cfg.agent?.yuan || "jarvis",
...
function emitAgentConfigAppEvents(engine, agentId, { globalFields, agentPartial, providersChanged }) {
  ...
  if (hasOwn(agentPartial?.agent, "voice")) {
    agentPayload.voice = agentPartial.agent.voice ?? null;
    agentUpdated = true;
  }
  if (agentUpdated) {
...
export function createAgentsRoute(engine) {
  ...
  route.delete("/agents/:id", async (c) => {
    try {
      ...
      // 删除前捕获 matrix 绑定信息（deleteAgent 后 getAgent 返回 null，无法再读 cfg）；
      // 浅拷贝避免 agent dispose 后异步读到被清空的字段。
      const ag = engine.getAgent?.(id);
      const matrixCfg = ag?.config?.bridge?.matrix ? { ...ag.config.bridge.matrix } : null;
      const result = await engine.deleteAgent(id);
      const replacementAgentId = result?.replacementAgentId || null;
      // 删除后异步把 Matrix 虚拟用户下线（停 adapter + presence offline），仅已绑定 matrix 的 agent 生效。
      if (matrixCfg?.enabled && matrixCfg.senderUserId && matrixCfg.homeserverUrl && matrixCfg.asToken) {
        triggerMatrixDeactivate(engine, id, matrixCfg);
      }
  ...
  route.post("/agents/:id/avatar", bodyLimit({ maxSize: 15 * 1024 * 1024 }), async (c) => {
    ...
    triggerMatrixProfileSync(engine, id);
  ...
  route.put("/agents/:id/config", async (c) => {
    ...
    try {
      ...
      // 改名（agentPartial.agent.name 变更）后异步把新 displayname + avatar 同步到 Matrix 虚拟用户。
      if (agentPartial.agent?.name !== undefined) {
        triggerMatrixProfileSync(engine, id);
      }
      emitAgentConfigAppEvents(engine, id, { globalFields, agentPartial, providersChanged });
      recordSecurityAuditEvent(c, engine, {
