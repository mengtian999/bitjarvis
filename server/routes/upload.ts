 ...
 *
 * 存储位置：
 * - 无 sessionPath：{jarvisHome}/uploads/，按 24 小时清理旧临时文件
 * - 有 sessionPath：{jarvisHome}/session-files/<session-hash>/，跟随 session 冷却清理
 */
...
function resolveUploadTarget(engine, sessionPath) {
  if (sessionPath) {
    ...
    return {
      dir: sessionFilesCacheDir(engine.jarvisHome, { sessionId, sessionPath }),
      storageKind: "managed_cache",
      shouldCleanOldUploads: false,
    };
  }
  return {
    dir: path.join(engine.jarvisHome, "uploads"),
  ...
  route.post("/upload", async (c) => {
    ...
    for (const srcPath of paths) {
      ...
      try {
        ...
        if (isSensitivePath(srcPath, engine.jarvisHome)) {
