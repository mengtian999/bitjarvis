...
function interludeText({ receiverName, source, status }) {
  const receiver = compactLabel(receiverName, "Jarvis");
  const keyMap = {
...
export function buildDeferredResultInterludeBlock(event, { receiverName = "Jarvis", meta = null } = {}) {
...
export function resolveDeferredReceiverName(engine, sessionPath) {
  ...
  return agent?.agentName || engine?.agentName || "Jarvis";
}
