 ...
 *
 * 无 IPC 通道：就绪与端口写入 JARVIS_HOME/server-info.json，桌面端轮询该文件。
 */
...
import { JarvisEngine } from "../core/engine.ts";
...
// BrowserManager 用 setter 注入 jarvisHome/sessionIdResolver（而非在别处构造时
...
import { fromRoot } from "../shared/jarvis-root.ts";
...
export async function startServer(root: CompositionRoot = {}): Promise<void> {
  ...
  async function bindServerTransportOwnership(
    ...
    try {
      ...
      const jarvisOnPort = fallbackEligible ? await isJarvisServerListeningOnPort({ port, host }) : false;
      const decision = decideLoopbackBindFallback({ errCode, networkMode, envPortPinned, jarvisOnPort });
      ...
      if (decision === "fallback") {
        ...
        if (fallbackPort !== null) {
          try {
            ...
            saveServerNetworkConfig(jarvisHome, { ...config, listenPort: fallbackPort });
      ...
      if (decision === "fail-other-jarvis") {
        ...
        startupError.startupPayload.suggestions.unshift(
          "Another Jarvis server is already listening on this port. If you have a second Jarvis installation or data directory, give each one a distinct port; if this is a leftover process, quit jarvis-server from Task Manager.",
        );
  ...
  function createPortInUseStartupError(cause: any, { host, port, listenHost, networkMode }: any) {
    const payload = {
      ...
      suggestions: [
        ...
        "If this is another Jarvis server, restart that instance or quit it cleanly.",
  ...
  // 用户数据存放在 ~/.jarvis/（打包后与产品代码分离）
  // 开发时可通过 JARVIS_HOME 环境变量隔离数据目录，如：JARVIS_HOME=~/.jarvis-dev node server/index.js
  ...
  // ── 同宅互斥闸（同一 JARVIS_HOME 的内核互斥）──
  // 必须在任何端口监听、任何 store 打开之前跑：一台机器上的同一 JARVIS_HOME
  // 可能被两个内核并发打开（`jarvis serve` 先起、桌面后启动是最常见的触发
  // 路径），并发读写同一批 SQLite / session 文件会互相覆盖。这里用 token
  // 认证探测（shared/server-info-probe.cjs）确认 server-info.json 记录的
  // 内核是否仍然存活、且确实是同一个家；探测不通（not-jarvis / dead）视为
  ...
  // 相同时后到者的 listen() 会天然 EADDRINUSE。
  {
    ...
    if (existingServerInfo) {
      ...
      // not-jarvis / dead：残留锁已确认失效（自清，不需要用户手删）
  ...
  // 或猜测数据处于哪个版本。
  {
    const allowDataDowngrade = process.env.JARVIS_ALLOW_DATA_DOWNGRADE === "1";
    ...
    if (epochResult.allowed === false) {
      ...
      const stampRead = readDataEpochStamp(jarvisHome);
      ...
      if (!mustBlock) {
        console.warn(
          `JARVIS_DATA_EPOCH_BASELINE_WARNING reason=${epochResult.reason}\n`
        ...
        const marker = epochResult.reason === "epoch-downgrade-blocked"
          ? "JARVIS_DATA_EPOCH_BLOCKED"
          : "JARVIS_DATA_EPOCH_TRANSITION_INCOMPLETE";
  ...
  const SERVER_TOKEN = process.env.JARVIS_TOKEN || crypto.randomBytes(16).toString("hex");
  const envPort = Number.parseInt(process.env.JARVIS_PORT || "", 10);
  const envPortPinned = Number.isInteger(envPort) && envPort >= 0;
  if (!envPortPinned) {
    await ensureServerNetworkConfigWithPortSelection(jarvisHome, { log: (msg) => log.log(msg) });
  }
  const serverNetwork = resolveServerListenOptions(jarvisHome);
  ...
  const firstRunReport = ensureFirstRun(jarvisHome, productDir);
  ...
  ensureLocalIdentityRegistries(jarvisHome);
  log.log("① ensureLocalIdentityRegistries 完成");

  // ── 初始化 Debug 日志 ──
  const dlog = initDebugLog(path.join(jarvisHome, "logs"));

  // ── 初始化引擎 ──
  log.log("② 创建 JarvisEngine...");
  const engine: any = new JarvisEngine({
    jarvisHome,
    productDir,
    appVersion,
    builtinMediaAdapters: root.builtinMediaAdapters,
  } as any);
  log.log("② JarvisEngine 构造完成，开始 init...");
  ...
  BrowserManager.setJarvisHome(engine.jarvisHome);
  ...
  const deferredResultStore = new DeferredResultStore(
    hub.eventBus,
    path.join(jarvisHome, ".ephemeral", "deferred-tasks.json"),
  ...
  const loopStore = new LoopStore(
    path.join(jarvisHome, ".ephemeral", "loop-state.json"),
    { log },
  );
  ...
  // CORS（默认允许 localhost 开发前端和 production Electron file:// 前端；JARVIS_CORS_ORIGIN 可收紧到单一来源）+ 鉴权
  const corsAllowedOrigin = process.env.JARVIS_CORS_ORIGIN;
  ...
  const workflowActivityStore = new WorkflowActivityStore(
    path.join(jarvisHome, "workflow-activity.json"),
  );
  ...
  const shouldCreateStartupSession = process.env.JARVIS_CREATE_STARTUP_SESSION !== "0";
  ...
  // `/mobile`、`/desktop` 网页客户端入口的供货模式判定（JARVIS_RENDERER_DIST /
  ...
  app.get("/api/health", async (c) => {
    ...
    return c.json({
      ...
      agent: engine.agentName,
      agentYuan: engine.agent?.config?.agent?.yuan || "jarvis",
  ...
  try {
    ...
    browserWss.on("connection", (ws) => {
      ...
      // 调试：记录浏览器 WS 消息往返（异步写入 + 缓冲，仅 JARVIS_DEBUG=1 时启用）
      const _bwsEnabled = process.env.JARVIS_DEBUG === "1";
      let _bwsBuf = "";
      let _bwsFlushTimer = null;
      const _bwsLogPath = path.join(jarvisHome, "browser-ws.log");
    ...
    log.log(`Jarvis Server 运行在 http://${host}:${actualPort}`);
    ...
    try {
      ...
      fs.writeFileSync(serverInfoPath, JSON.stringify({
        ...
        token: SERVER_TOKEN,
        version: appVersion,
        ownerKind: process.env.JARVIS_SERVER_OWNER === "desktop" ? "desktop" : "standalone",
        ownerPid: Number.parseInt(process.env.JARVIS_SERVER_OWNER_PID || "", 10) || null,
