...
import { createMatrixBindRoute } from "../routes/matrix-bind.ts";
...
import { createSpeechGenerationRoute } from "../routes/speech-generation.ts";
import { createSherpaRoute } from "../routes/sherpa.ts";
import { createVoiceRoute } from "../routes/voice.ts";
...
export function registerOpenRoutes(app: Hono, ctx: CompositionContext): void {
  ...
  app.route("/api", createMatrixBindRoute({ engine, bridgeManagerRef }));
  ...
  app.route("/api", createSpeechGenerationRoute(engine));
  app.route("/api", createSherpaRoute(engine));
  app.route("/api", createVoiceRoute(engine, hub, { upgradeWebSocket }));
