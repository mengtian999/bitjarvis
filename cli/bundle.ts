/**
 * `jarvis bundle pull` / `jarvis bundle status` — the self-hosted form's manual
...
/**
 * `jarvis bundle pull [--channel <c>]`. Returns the process exit code.
...
export async function runBundlePull({
  ...
  console.log(`${ansi.green}Pulled and activated ${result.version}.${ansi.reset} Restart jarvis serve to take effect.`);
  return 0;
}

/**
 * `jarvis bundle status [--channel <c>]`. Pure local read — renderer
...
export async function runBundleStatus({
  ...
  const [current, next, otaState] = await Promise.all([
    pointerStore.readPointer(jarvisHome, rendererChannel, "current"),
    pointerStore.readPointer(jarvisHome, rendererChannel, "next"),
    otaCore.readOtaState(jarvisHome),
  ]);
  const state = (otaState && otaState[channel]) || {};

  if (!current) {
    console.log(`${ansi.dim}No web frontend has been pulled yet. Run:${ansi.reset} jarvis bundle pull`);
    return 0;
  }
