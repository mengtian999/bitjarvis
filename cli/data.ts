/**
 * `jarvis data diagnose` / `jarvis data checkpoints` / `jarvis data restore` —
...
import { resolveCliJarvisHome } from "./local-server.ts";
...
function checkpointsRootFor(jarvisHome: string): string {
  return path.join(jarvisHome, DATA_EPOCH_CHECKPOINTS_DIRNAME);
}

/**
 * Lists every published, complete checkpoint under
 * {jarvisHome}/data-epoch-checkpoints. `.tmp-*` staging and `.invalid-*`
 ...
 */
function listDataEpochCheckpoints(jarvisHome: string): { checkpoints: CheckpointSummary[]; skipped: number } {
  const root = checkpointsRootFor(jarvisHome);
...
/**
 * `jarvis data diagnose [--url ...]` — read-only. Never writes to disk:
 ...
 */
export async function runDataDiagnose({ jarvisHome = resolveCliJarvisHome() }: { jarvisHome?: string } = {}): Promise<number> {
  console.log(`Data-epoch diagnostics ${ansi.dim}(${jarvisHome})${ansi.reset}`);
  ...
  const forwardJournal = readDataEpochJournal(jarvisHome);
  const restoreJournal = readDataEpochRestoreJournal(jarvisHome);
  ...
  const maintenance = inspectDataEpochMaintenance(jarvisHome);
  ...
  const { checkpoints, skipped } = listDataEpochCheckpoints(jarvisHome);
  ...
  if (checkpoints.length === 0) {
    ...
    console.log(`  Checkpoints          ${checkpoints.length} available${skippedNote} — run \`jarvis data checkpoints\` for details`);
  }
...
/**
 * `jarvis data checkpoints` — lists every available recovery checkpoint.
 */
export async function runDataCheckpoints({ jarvisHome = resolveCliJarvisHome() }: { jarvisHome?: string } = {}): Promise<number> {
  const { checkpoints, skipped } = listDataEpochCheckpoints(jarvisHome);
  if (checkpoints.length === 0) {
  ...
  console.log(`Data-epoch checkpoints ${ansi.dim}(${jarvisHome})${ansi.reset}`);
...
/**
 * `jarvis data restore <transitionId> [--confirm-token "restore <transitionId>"]`
 *
...
export async function runDataRestore({
  transitionId,
  confirmToken = null,
  jarvisHome = resolveCliJarvisHome(),
  ...
  if (!transitionId) {
    console.error(`${ansi.red}data restore requires a transitionId: jarvis data restore <transitionId>${ansi.reset}`);
    return 1;
  }

  const checkpointDir = path.join(checkpointsRootFor(jarvisHome), transitionId);
  ...
  try {
    ...
    console.error(`${ansi.red}No checkpoint found for transitionId "${transitionId}" in ${checkpointsRootFor(jarvisHome)}.${ansi.reset}`);
    console.error(`${ansi.dim}Run \`jarvis data checkpoints\` to see what is available.${ansi.reset}`);
    return 1;
  }
