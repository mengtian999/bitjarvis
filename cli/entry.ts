...
import { JarvisCliClient } from "./client.ts";
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (!connection.ok && shouldAutoStartServer(args)) {
    console.error(`${ansi.dim}Starting local Jarvis Server...${ansi.reset}`);
    connection = await startLocalServerAndWait({ projectRoot: PROJECT_ROOT });
  }
  if (!connection.ok) {
    console.error(`${ansi.red}${connection.message}${ansi.reset}`);
    console.error(`${ansi.dim}Start one with: jarvis serve${ansi.reset}`);
    return 1;
  }

  const client = new JarvisCliClient(connection);
