...
export function resolveCliJarvisHome(env = process.env) {
  return resolveHomePath(env.JARVIS_HOME || path.join(os.homedir(), ".jarvis"));
}

function resolveHomePath(value) {
  const raw = String(value || "").trim();
  if (!raw) return path.join(os.homedir(), ".jarvis");
...
export function readLocalServerInfo({ jarvisHome = resolveCliJarvisHome(), checkProcess = true } = {}) {
  const filePath = path.join(jarvisHome, "server-info.json");
  if (!fs.existsSync(filePath)) {
    return {
      ok: false,
      reason: "missing_server_info",
      filePath,
      message: `No running Jarvis Server was found at ${filePath}`,
    };
  }
  ...
  if (checkProcess && Number.isInteger(info.pid) && !isProcessAlive(info.pid)) {
    return {
      ...
      filePath,
      message: `Jarvis Server process ${info.pid} is no longer running`,
    };
  }
...
export function resolveConnection({ url, token, jarvisHome }: { url?: string; token?: string; jarvisHome?: string } = {}) {
  if (url) {
    return {
      ok: true,
  ...
  const local = readLocalServerInfo({ jarvisHome });
