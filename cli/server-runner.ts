...
import { readLocalServerInfo, resolveCliJarvisHome } from "./local-server.ts";
...
/**
 * Resolves which already-activated renderer directory `jarvis serve` should
 * inject into the server as `JARVIS_RENDERER_DIST`. Reuses
 ...
 * `versionDir` (`valid: false`); the caller sets `JARVIS_RENDERER_DIST` to
 ...
 * guide page.
 *
 * If no pointer exists at all (`jarvis bundle pull` was never run on this
...
export async function resolveRendererDistPointer({
  jarvisHome,
  channel = "stable",
}: { jarvisHome: string; channel?: string }): Promise<RendererDistPointer | null> {
  const rendererChannel = rendererPointerChannel(channel);
  const boot = await activation.resolveBoot(rendererChannel, jarvisHome);
  if (boot) {
    return { distDir: boot.pointer.versionDir, version: boot.pointer.version ?? null, valid: true };
  }
  const current = await pointerStore.readPointer(jarvisHome, rendererChannel, "current");
...
export async function resolveServerSpawnSpec({
  ...
  const explicitRoot = env.JARVIS_ROOT && fs.existsSync(path.join(env.JARVIS_ROOT, "bootstrap.js"))
    ? env.JARVIS_ROOT
    : null;
  ...
  const rendererDist = await resolveRendererDistPointer({ jarvisHome: resolveCliJarvisHome(env), channel });

  if (packagedRoot) {
    const spawnEnv: NodeJS.ProcessEnv = {
      ...env,
      JARVIS_ROOT: packagedRoot,
      JARVIS_SERVER_ENTRY: path.join(packagedRoot, "bundle", "index.js"),
    };
    if (rendererDist) spawnEnv.JARVIS_RENDERER_DIST = rendererDist.distDir;
    return {
      mode: "packaged",
 ...
 * whatever server-info.json is on disk for `jarvisHome` (regardless of
...
}: { jarvisHome: string; probeImpl?: typeof probeServerInfo }): Promise<{ blocked: boolean; message: string | null }> {
  const local = readLocalServerInfo({ jarvisHome, checkProcess: false });
...
/**
 * Builds the env object `jarvis serve` spawns its server child with, applying
 * the `--allow-data-downgrade` override (threaded to the child as
 * `JARVIS_ALLOW_DATA_DOWNGRADE=1`, which server/index.ts's data-epoch gate
...
export function buildServeSpawnEnv({
  ...
  if (allowDataDowngrade) {
    spawnEnv.JARVIS_ALLOW_DATA_DOWNGRADE = "1";
    warn(
...
export async function spawnServerForeground({
  ...
  const guard = await guardAgainstForeignServer({ jarvisHome: resolveCliJarvisHome(env), probeImpl });
  if (guard.blocked) {
...
export async function startLocalServerAndWait({
  ...
  const jarvisHome = resolveCliJarvisHome(env);
  const existing = readLocalServerInfo({ jarvisHome });
  ...
  while (Date.now() - startedAt < timeoutMs) {
    const info = readLocalServerInfo({ jarvisHome });
    if (info.ok) return { ...info, started: true, serverMode: spec.mode };
    await delay(intervalMs);
  }

  throw new Error(`Jarvis Server did not become ready within ${Math.round(timeoutMs / 1000)}s`);
}
