...
2. Open Jarvis → Settings → Plugins, drag the folder into the install area (or drag a .zip)
...
Start with the `jarvis-plugin-creator` scaffold, then delete what you do not need:

```bash
python3 skills2set/jarvis-plugin-creator/scripts/create_jarvis_plugin.py "My Plugin" --path examples/plugins --kind full
```
...
When an Agent such as Jarvis or Codex is building a plugin on the user's behalf,
prefer the dev loop over copying a half-finished plugin into the real plugin
directory:

1. Keep the plugin source in the current workspace, or in `${JARVIS_HOME}/plugin-dev-sources/`.
2. Call the EventBus `plugin.dev.install` or HTTP `POST /api/plugins/dev/install` to copy the source into `${JARVIS_HOME}/plugins-dev/<pluginId>` and load it.
...
Dev-mode permissions come from the dev slot Jarvis remembers, not from what the
...
runtime copy under `${JARVIS_HOME}/plugins-dev/`. They never write to
`${JARVIS_HOME}/plugins/` and never pollute the disable preferences of a real
installed plugin.
    ...
    "scenarios": [
      {
        "id": "hello-tool",
        "steps": [
          { "invokeTool": { "name": "hello", "input": { "name": "Jarvis" } } },
          { "expectToolText": "hello Jarvis" }
        ]
      }
    ]
  }
...
- **Manual**: Place the plugin directory in `${JARVIS_HOME}/plugins/`. The actual path is shown in Settings → Plugins or via `/api/plugins/settings` as `plugins_dir`
...
Plugin private data is stored in `${JARVIS_HOME}/plugin-data/{pluginId}/`. This directory is preserved when the plugin is deleted, so config persists across reinstalls.
...
`minAppVersion` (optional) declares the minimum Jarvis version required to run the plugin. If the current app version is lower, the plugin will not load and its status is set to `incompatible`. All plugins should declare this field to prevent compatibility issues on older versions.
...
- New plugins can use `defineTool()` from `@jarvis/plugin-runtime` for types and default parameters. The current static `tools/*.js` loader still reads named exports.
...
```js
import { defineTool } from '@jarvis/plugin-runtime';
...
```js
import { createMediaDetails } from "@jarvis/plugin-runtime";
...
Both static `tools/*.js` exports and dynamic `ctx.registerTool()` tools receive the SDK-style `(input, ctx)` call. If the plugin is disabled, missing, or the tool cannot be found, the scheduled run fails explicitly and records the error in cron history. Jarvis does not silently fall back to an Agent session.
...
```js
import { createChatSurfaceCard, createSession } from "@jarvis/plugin-runtime";
...
Factory functions are invoked by Pi SDK at session creation time; handlers fire when the corresponding event occurs. After a full-access plugin is installed, enabled, or reloaded, Jarvis rebinds extension runners for currently idle sessions; sessions that are streaming, compacting, or switching are skipped and pick up the new extension set on the next safe rebuild. See Pi SDK extension documentation for the full event list.
...
CLI providers must use structured argument bindings. Do not build shell command strings; Jarvis runs commands through non-shell `execFile` / `spawn` paths and collects outputs into the media task directory.
...
Plugin pages are rendered via WebView/iframe. New plugins should use `@jarvis/plugin-sdk` for handshake and host requests:
...
await jarvis.toast.show({ message: 'Refreshed', type: 'success' });
...
The lower-level `jarvis.host.request(type, payload)` remains available for future or experimental capabilities. Prefer typed helpers for stable capabilities.
...
`jarvis.resources.*` only sends host-mediated requests from the iframe: it can ask Jarvis to open a resource, pick resources, or request access, but it cannot read or write file contents directly. Actual user-resource reads and writes stay in server-side plugin routes, tools, or lifecycle code through `ctx.resources` and ResourceIO.

The host appends `jarvis-theme` and `jarvis-css` query parameters to the iframe URL. Plugins can optionally reference the theme CSS for visual consistency:

```html
<link rel="stylesheet" href="${new URLSearchParams(location.search).get('jarvis-css')}">
```

Static frontend resources belong under the plugin's `assets/` directory and are served by the Jarvis host at `/api/plugins/{pluginId}/assets/...`. This follows the same boundary idea as VS Code Webview resources: the entry route is opened with the local token or `pluginIframeTicket`; after a successful page response, the host issues a short-lived HttpOnly cookie scoped only to `/api/plugins/{pluginId}/assets/`. Vite split chunks, `React.lazy()` imports, CSS, fonts, images, JSON, wasm, and browser-playable video files such as MP4/WebM/MOV should not depend on `?token` or `pluginIframeTicket`. Video assets support HTTP byte ranges for `<video>` playback and seeking.

When page scripts call the plugin's own dynamic route APIs, prefer `jarvis.api.fetch()` from `@jarvis/plugin-sdk`. It derives the current plugin id from the iframe route and sends the surface session credential that the host appended to the iframe URL:

```js
import { jarvis } from '@jarvis/plugin-sdk';

const res = await jarvis.api.fetch('create-session', {
  method: 'POST',
});
```

When converting a website into a plugin, rewrite same-plugin `fetch('/api/...')` calls to `jarvis.api.fetch(...)` instead of hard-coding `/api/plugins/{pluginId}/...` in browser code. The lower-level protocol is: the host appends the surface session as the `pluginSurfaceSession` query parameter, and page scripts send it back with the `X-Jarvis-Plugin-Surface-Session` header (or a query parameter of the same name):
...
const res = await fetch('/api/plugins/my-plugin/create-session', {
  method: 'POST',
  headers: { 'X-Jarvis-Plugin-Surface-Session': surfaceSession },
});
```
...
const iconUrl = jarvis.assets.url('images/icon.svg');
const bgVideoUrl = jarvis.assets.url('videos/background.mp4');
```
...
Agent-generated plugins and newly edited plugin UI should not register extra `/api/file`, `/api/video`, `/assets/*`, or similar routes only to serve CSS, JS, images, fonts, or MP4 files. Existing plugins that already use static-file compatibility handlers remain loadable. The documented contract for new work is to put those resources in `assets/` and reference them with `jarvis.assets.url(...)` or the official host-served assets path in the route shell.

React plugin UIs should use `@jarvis/plugin-components`. It provides Button, IconButton, TextInput, Textarea, Select, Switch, SettingRow, CardShell, List, EmptyState, and related primitives that match Jarvis's current controls:

```tsx
import { Button, CardShell, JarvisThemeProvider, SettingRow, Switch } from "@jarvis/plugin-components";
...
`JarvisThemeProvider` supports three modes: `inherit` reads host CSS variables and then uses SDK fallback tokens; `jarvis` pins the UI to a named Jarvis theme token set; `custom` only overrides explicitly provided tokens and lets missing fields continue through the fallback chain. Components depend only on `jarvis-plugin-*` classes and CSS variables, not renderer internals.
...
New plugins should use `definePlugin()` from `@jarvis/plugin-runtime`. It returns a class-compatible value for the current PluginManager:

```js
import { definePlugin } from '@jarvis/plugin-runtime';
...
```js
import { JARVIS_BUS_SKIP } from "@jarvis/plugin-runtime";
  ...
  async onload() {
    ...
    this.register(
      this.ctx.bus.handle("bridge:send", async (payload) => {
        if (payload.platform !== "feishu") return JARVIS_BUS_SKIP;
...
**SKIP chain**: Multiple handlers can be registered for the same event type. The system calls them in registration order until one returns a value other than `JARVIS_BUS_SKIP`. Returning `JARVIS_BUS_SKIP` means "I don't handle this, pass it on":
...
Plugins should prefer the typed helpers from `@jarvis/plugin-runtime`. They map to these EventBus capabilities:

| Capability | Description |
|------------|-------------|
| `session:create` | Create a normal Jarvis session without switching the main UI focus, with `agentId`, `cwd`, `memoryEnabled`, `workspaceFolders`, `thinkingLevel`, `permissionMode`, `ownerPluginId`, `kind`, and `visibility` |
...
Plugins can register background tasks so Jarvis can track and abort them. Runtime lifecycle is managed by `TaskRegistry`.
...
The "Open plugin marketplace" button in Settings -> Plugins opens a full marketplace subpage that reads `/api/plugins/marketplace`. Jarvis follows the Obsidian-style official community catalog model: third-party authors submit plugins to `OH-Plugins`, while users browse, install, enable, and disable plugins without managing marketplace sources.

Default official catalog:

```text
https://raw.githubusercontent.com/mengtian999/OH-Plugins/main/marketplace.json
```

Developer overrides remain available:

- `JARVIS_PLUGIN_MARKETPLACE_FILE=/path/to/marketplace.json`
- `JARVIS_PLUGIN_MARKETPLACE_URL=https://.../marketplace.json`

Without either environment variable, Jarvis first tries `${JARVIS_HOME}/plugin-marketplace/marketplace.json` for local development. If it does not exist, Jarvis reads the official `OH-Plugins` URL. The marketplace index shape matches the `OH-Plugins` repository:
...
```json
{
  ...
  "plugins": [{
    ...
    "distribution": {
      "kind": "release",
      "packageUrl": "https://github.com/mengtian999/OH-Plugins/releases/download/demo-v1.0.0/demo.zip",
      "sha256": "..."
    },
    "versions": [
      {
...
Marketplace version management uses `versions[]` as the long-term contract: each item declares `version`, that version's `compatibility.minAppVersion`, and its own `distribution`. If `versions[]` is absent, Jarvis treats the root-level `version` / `compatibility` / `distribution` as a single version entry. The client chooses the highest SemVer version compatible with the current app, while exposing `latestVersion`, `selectedVersion`, `installedVersion`, `updateAvailable`, `downgrade`, `reinstall`, `compatible`, `installAction`, and `canInstall` for UI state.

If the installed version is newer than the highest compatible marketplace version, the action is marked as `downgrade` and install requires explicit `allowDowngrade: true`. Drag-and-drop / local path installs also reject implicit downgrades. Updates back up the previous plugin directory under `${JARVIS_HOME}/plugin-backups/<pluginId>/`; if the new version fails to load, Jarvis restores and reloads the old directory. Successful installs are recorded in `${JARVIS_HOME}/plugin-installs.json` with source, version, release URL, and sha256 so later marketplace state is explicit.
...
Jarvis supports multiple sessions and multiple agents running in parallel. Keep the following in mind when developing plugins:
