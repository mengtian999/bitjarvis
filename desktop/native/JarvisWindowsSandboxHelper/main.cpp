...
struct Options {
    ...
    std::vector<std::wstring> jarvisWriteAclCleanupPaths;
...
static void fail(const std::wstring& message) {
    std::wcerr << L"jarvis-win-sandbox: " << message << std::endl;
}

static void debug(const std::wstring& message) {
    wchar_t enabled[8] = {};
    DWORD n = GetEnvironmentVariableW(L"JARVIS_WIN32_SANDBOX_DEBUG", enabled, 8);
...
static void emitTerminalRecord(
    ...
    std::wcerr
        << L"jarvis-win-sandbox: terminal-v1"
        << L" status=\"" << status << L"\""
...
static std::wstring sidForWritableRoot(const std::wstring& root) {
    return hashSidForWritableRoot(root, L"S-1-5-21-", L"jarvis-win32-write-root-v3:");
}

static std::wstring sidForWritableRootLegacyCapabilityNamespace(const std::wstring& root) {
    return hashSidForWritableRoot(root, L"S-1-15-3-4096-", L"jarvis-win32-write-root-v2:");
}

static std::wstring sidForWritableRootLegacyAccountNamespace(const std::wstring& root) {
    return hashSidForWritableRoot(root, L"S-1-5-21-", L"jarvis-win32-write-root:");
}
...
static Options parseArgs(int argc, wchar_t** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == L"--cleanup-jarvis-write-acl" && i + 1 < argc) {
            opts.jarvisWriteAclCleanupPaths.push_back(argv[++i]);
            continue;
        }
    ...
    bool maintenanceMode = !opts.jarvisWriteAclCleanupPaths.empty() ||
...
static bool generatePrivateDesktopName(std::wstring& name) {
    ...
    name = L"jarvis-win-sandbox-desktop-" + suffix;
    return true;
}
...
static void revertImpersonationOrTerminate() {
    ...
    std::wcerr
        << L"jarvis-win-sandbox: impersonation-revert-failure-v1"
        << L" error=\"" << rc << L"\""
...
static void emitCreateProcessLaunchFailureDiagnostic(
    ...
    std::wcerr
        ...
        << std::endl;
    std::wcerr
        << L"jarvis-win-sandbox: launch-failure-probes"
...
static void emitPrelaunchDesktopProbeFailureDiagnostic(const std::wstring& desktopProbe) {
    std::wcerr
        << L"jarvis-win-sandbox: prelaunch-probe-failure-v1"
...
static void emitPostCreateEarlyExitDiagnostic(
    ...
    std::wcerr
        << L"jarvis-win-sandbox: post-create-exit-v1"
...
static void emitTimeoutProcessSnapshot(HANDLE job) {
    ...
    if (!QueryInformationJobObject(
        ...
        const DWORD errorCode = GetLastError();
        std::wcerr
            << L"jarvis-win-sandbox: timeout-processes-v1"
...
static void emitGuardianRecord(
    ...
    std::wcerr
        << L"jarvis-win-sandbox: guardian-v1"
        << L" status=\"" << status << L"\""
...
static bool isLegacyAppContainerProfileName(const std::wstring& name) {
    const std::wstring prefix = L"com.jarvis.sandbox.";
...
static std::wstring probeNamedObjectNamespace(HANDLE restrictedToken) {
    ...
    std::wstring name = L"Local\\jarvis-win-sandbox-diagnose-" +
...
static int diagnoseRestrictedToken(const Options& opts) {
    ...
    std::wcerr
        << L"jarvis-win-sandbox: diagnose-token"
        << L" cwd=\"" << opts.cwd << L"\""
    ...
    for (const auto& root : opts.writableRoots) {
        std::wcerr
            << L"jarvis-win-sandbox: diagnose-token-writable-root"
    ...
    for (const auto& sid : restrictingSids) {
        std::wcerr
            << L"jarvis-win-sandbox: diagnose-token-restricting-sid"
    ...
    std::wcerr
        << L"jarvis-win-sandbox: diagnose-token-base-named-objects-probe"
...
static MigrationResult cleanupJarvisWriteAcls(const std::vector<std::wstring>& paths) {
    ...
    for (const auto& path : paths) {
        ...
        if (rc != ERROR_SUCCESS) {
            fail(L"jarvis-write-acl-cleanup path=\"" + path + L"\" error=\"" + win32Message(rc) + L"\"");
            result.failures++;
        ...
        if (!matchedSids.empty()) {
            if (revokeSidsFromPath(path, matchedSids, dacl)) {
                ...
                for (PSID sid : matchedSids) {
                    std::wcerr
                        << L"jarvis-win-sandbox: jarvis-write-acl-cleaned"
                        << L" path=\"" << path << L"\""
    ...
    for (const auto& path : opts.legacyAclDiagnosticPaths) {
        ...
        if (dacl) {
            for (DWORD i = 0; i < dacl->AceCount; i++) {
                ...
                std::wcerr
                    << L"jarvis-win-sandbox: legacy-appcontainer-acl"
                    << L" path=\"" << path << L"\""
                    << L" sid=\"" << sidString << L"\""
        ...
        if (opts.cleanupLegacyAcl) {
            if (legacySids.empty()) {
                debug(L"legacy ACL cleanup found no Jarvis-owned AppContainer SID for " + path);
...
static MigrationResult cleanupLegacyProfiles(const std::vector<std::wstring>& profileNames) {
    ...
    for (const auto& name : profileNames) {
        ...
        if (SUCCEEDED(hr)) {
            result.findings++;
            std::wcerr
                << L"jarvis-win-sandbox: legacy-appcontainer-profile-cleaned"
                << L" name=\"" << name << L"\""
                << std::endl;
            continue;
        }
...
int wmain(int argc, wchar_t** argv) {
    ...
    try {
        ...
        std::wcerr << L"jarvis-win-sandbox: " << wide << std::endl;
        return 2;
    }
    ...
    if (!opts.jarvisWriteAclCleanupPaths.empty() ||
        !opts.legacyAclDiagnosticPaths.empty() ||
        !opts.legacyProfileNames.empty() ||
        !opts.legacyProfileCleanupNames.empty() ||
        opts.cleanupLegacyAcl) {
        ...
        MigrationResult jarvisWriteResult;
        if (!opts.jarvisWriteAclCleanupPaths.empty()) {
            jarvisWriteResult = cleanupJarvisWriteAcls(opts.jarvisWriteAclCleanupPaths);
        }
        ...
        failures += jarvisWriteResult.failures + aclResult.failures;
        ...
        int findings = jarvisWriteResult.findings + aclResult.findings + profileResult.findings;
