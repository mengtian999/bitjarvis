...
@main
struct JarvisComputerUseHelper {
    static func main() {
        do {
            try run()
            ...
            fputs("jarvis-computer-use-helper failed: \(error)\n", stderr)
    ...
    private static func run() throws {
        ...
        case "status":
            guard DaemonClient.isDaemonListening(socketPath: socketPath) else {
                throw HelperError(code: ExitCode.toolError, message: "jarvis-computer-use-helper daemon is not running on \(socketPath).")
            }
            print("jarvis-computer-use-helper daemon running; CuaDriverCore \(CuaDriverCore.version)\n  socket: \(socketPath)")
        case "serve":
    ...
    private static func callTool(_ name: String, args: [String], raw: Bool, compact: Bool, socketPath: String) async throws {
        ...
        await MainActor.run {
            ...
            applyJarvisCursorRuntimeConfig(from: ProcessInfo.processInfo.environment)
        }
        ...
        case .ok(let response):
            guard response.ok else {
                throw HelperError(
                    ...
                    message: response.error ?? "jarvis-computer-use-helper daemon call failed."
                )
            }
            guard case .call(let result) = response.result else {
                throw HelperError(code: ExitCode.software, message: "jarvis-computer-use-helper daemon returned a non-call response.")
            }
            try emitToolResult(result, raw: raw, compact: compact)
            return true
        }
    ...
    private static func runDaemon(socketPath: String) async -> Int32 {
        do {
            ...
            fputs("jarvis-computer-use-helper daemon failed: \(error)\n", stderr)
            return ExitCode.software
        }
    }
    ...
    private static func stopDaemon(socketPath: String) throws {
        ...
        case .ok(let response):
            guard response.ok else {
                throw HelperError(code: response.exitCode ?? ExitCode.software, message: response.error ?? "Failed to stop jarvis-computer-use-helper daemon.")
            }
            print("jarvis-computer-use-helper daemon stopped.")
        case .noDaemon:
            throw HelperError(code: ExitCode.toolError, message: "jarvis-computer-use-helper daemon is not running on \(socketPath).")
        case .error(let message):
            throw HelperError(code: ExitCode.software, message: message)
    ...
    private static func defaultDaemonSocketPath() -> String {
        ...
        if let explicit = env[jarvisSocketPathEnvKey], !explicit.isEmpty {
            return expandHome(explicit)
        }
        let home = env["HOME"] ?? NSHomeDirectory()
        return home + "/Library/Caches/jarvis-computer-use/jarvis-computer-use-helper.sock"
    }
    ...
    private static func printHelp() {
        print("""
        jarvis-computer-use-helper status
        jarvis-computer-use-helper list_apps '{"bundle_id":"com.apple.finder"}' --raw --compact
        jarvis-computer-use-helper get_window_state '{"pid":844,"window_id":10725}' --raw --compact
        """)
    }
}
...
private let jarvisAgentCursorConfigEnvKey = "JARVIS_AGENT_CURSOR_CONFIG_JSON"
private let jarvisSocketPathEnvKey = "JARVIS_COMPUTER_USE_SOCKET_PATH"

private struct JarvisCursorRuntimeConfig: Decodable {
    let enabled: Bool?
    let style: Style?
    let motion: Motion?
...
@MainActor
private func applyJarvisCursorRuntimeConfig(from environment: [String: String]) {
    guard let raw = environment[jarvisAgentCursorConfigEnvKey],
          let data = raw.data(using: .utf8),
          let config = try? JSONDecoder().decode(JarvisCursorRuntimeConfig.self, from: data)
    else {
        return
    }
