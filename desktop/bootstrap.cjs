...
let diagnosticsDir = path.join(os.tmpdir(), "jarvis-desktop-launch");
...
let jarvisHome = null;
try {
  const { resolveJarvisHome } = require("../shared/jarvis-runtime-paths.cjs");
  jarvisHome = resolveJarvisHome(process.env.JARVIS_HOME || process.env.JARVIS_HOME);
  process.env.JARVIS_HOME = jarvisHome;
  process.env.JARVIS_HOME = jarvisHome;
  diagnosticsDir = path.join(jarvisHome, "diagnostics", "desktop-launch");
} catch (err) {
  const diagnosticPath = writeDiagnostic("jarvis-home-resolve-failed.json", "jarvis-home-resolve-failed", {
    phase: "desktop-bootstrap",
    error: serializeError(err),
  });
  showBootstrapError(
    "Jarvis Launch Failed",
    `Jarvis failed before JARVIS_HOME could be resolved.\n\n${err?.message || err}\n\nDiagnostic file:\n${diagnosticPath || diagnosticsDir}`,
  );
...
function verifyWindowsInstallSurfaceBeforeMain() {
  ...
  showBootstrapError("Jarvis Launch Failed", detail);
  exitAfterBootstrapFailure();
  return false;
}
...
function loadDesktopMain() {
  try {
    ...
    showBootstrapError(
      "Jarvis Launch Failed",
      `Jarvis failed before the desktop main process finished loading.\n\n${err?.message || err}\n\nDiagnostic file:\n${diagnosticPath || diagnosticsDir}`,
    );
    exitAfterBootstrapFailure();
  }
}
...
function tryStartOfficePdfHelper() {
  ...
  try {
    ...
    if (process.argv.some((arg) => arg === "--jarvis-office-html-to-pdf" || arg.startsWith("--jarvis-office-html-to-pdf="))) {
