/**
 * Jarvis Desktop — Electron 主进程
 *
 * 职责：
 * 1. 创建启动窗口（splash）
 * 2. spawn() 启动 Jarvis Server
...
const {
  resolveJarvisHome,
} = require("../shared/jarvis-runtime-paths.cjs");
const {
...
const APP_USER_MODEL_ID = "com.jarvis.app"; // Keep in sync with package.json build.appId.

// preload 缺失时 Electron 会静默忽略，renderer 拿不到 window.jarvis →
// onboarding/主窗口白屏且无前端报错。此处硬崩，拒绝以不可用状态启动。
{
  ...
  if (!fs.existsSync(preloadPath)) {
    ...
    try { dialog.showErrorBox("Jarvis failed to start", msg); } catch { }
...
const jarvisHome = resolveJarvisHome(process.env.JARVIS_HOME);
...
function redactMainLogText(value) {
  return redactLogText(value, { homeDir: os.homedir(), extraPaths: [jarvisHome] });
}

function readNetworkProxyPreference() {
  const prefsPath = path.join(jarvisHome, "user", "preferences.json");
...
// 按 JARVIS_HOME 隔离 Electron userData（localStorage / cache / session）
// 生产: ~/Library/Application Support/Jarvis（历史目录，随 Jarvis 显示名保留）
// 开发: ~/Library/Application Support/Jarvis-dev
const defaultHome = path.join(os.homedir(), ".jarvis");
...
if (!gpuStartupPolicy.hardwareAccelerationEnabled) {
  ...
  // 延迟 GPU 诊断：等待 gpu-info-update 事件后检查 GPU 功能状态
  // 启动早期 getGPUFeatureStatus() 可能返回空数据
  let gpuCheckDone = false;
  const checkGpuAndMaybeDisable = () => {
    if (gpuCheckDone || gpuStartupPolicy.hardwareAccelerationEnabled === false) return;
    gpuCheckDone = true;
    try {
      const gpuStatus = app.getGPUFeatureStatus();
      if (!gpuStatus) return;
      const criticalFeatures = ["webgl", "webgpu", "gpu_compositing", "rasterization"];
      const allUnavailable = criticalFeatures.every(f => {
        const status = gpuStatus[f];
        return status === "unavailable_software" || (typeof status === "string" && status.startsWith("disabled"));
      });
      if (allUnavailable) {
        console.warn("[desktop] GPU 关键功能全部不可用，自动降级到 software-safe 模式");
        app.disableHardwareAcceleration();
        app.commandLine.appendSwitch("disable-gpu");
        app.commandLine.appendSwitch("disable-gpu-compositing");
      }
    } catch (err) {
      console.warn("[desktop] GPU 诊断检查失败（非致命）:", err.message);
    }
  };
  // 监听 gpu-info-update 事件（GPU 信息就绪时触发）
  app.on("gpu-info-update", checkGpuAndMaybeDisable);
  // 超时兜底：3 秒后仍无事件则直接检查
  setTimeout(checkGpuAndMaybeDisable, 3000);
}
...
function _getMainI18n() {
  ...
  try {
    ...
    let locale = null;
    try {
      const prefs = JSON.parse(fs.readFileSync(path.join(jarvisHome, "user", "preferences.json"), "utf-8"));
...
function getCurrentAgentId() {
  const prefsPath = path.join(jarvisHome, "user", "preferences.json");
  const agentsDir = path.join(jarvisHome, "agents");
...
function hasExistingConfig() {
  try {
    ...
    const configPath = path.join(jarvisHome, "agents", agentId, "config.yaml");
...
function hasLegacyProviderConfig() {
  ...
  try {
    const modelsPath = path.join(jarvisHome, "added-models.yaml");
...
// 首启验签解压到 JARVIS_HOME/artifacts 后从版本化目录 spawn。dev 模式不经过它。
...
// JARVIS_SIGN_KEYSET 的构建期替换），运行时没有旁路。
...
// （JARVIS_DATA_EPOCH_BLOCKED / JARVIS_DATA_EPOCH_TRANSITION_INCOMPLETE），
...
function readDesiredServerNetworkConfig() {
  const filePath = path.join(jarvisHome, "server-network.json");
  try {
...
async function startServer() {
  const serverInfoPath = path.join(jarvisHome, "server-info.json");
    ...
    if (pidAlive) {
      ...
      if (!disposition.removeInfoFile) {
        ...
        if (disposition.failFast) {
          const err = new Error(
            `STALE_SERVER_UNCLEANED: residual Jarvis server (PID ${existingInfo.pid}) is still running and holds port ${Number.isInteger(stalePort) ? stalePort : "unknown"} (${verification.reason}). ` +
            `Quit it from Task Manager (look for jarvis-server.exe) or restart the computer, then launch Jarvis again.`
          );
          err.code = "STALE_SERVER_UNCLEANED";
          throw err;
        }
        // 端口不冲突不等于"可以安全共存"：残留进程可能是同一 JARVIS_HOME 上
        // 监听在别的端口的另一个内核（典型触发路径：`jarvis serve` 先起、桌面
        // 后启动）。用 token 认证探测确认它是否仍然是同一个家，是则拒绝
        // spawn 第二个内核；探测不通（not-jarvis / dead）才继续走原有 spawn。
  ...
  // JARVIS_HOME/artifacts 下按版本落盘；这里经 artifact-boot 决策出可 spawn
  ...
  if (!artifactBoot.hasSeed(resourcesPath, platformArch)) {
    if (app.isPackaged) {
      throw new Error(
        ...
        + "The installation is broken — please reinstall Jarvis.",
      );
    }
    return null;
  }
...
async function handleRendererArtifactLoadFailure({ win, pageName, opts, label, reason }) {
  ...
  await artifactBoot.writeBootSentinel(jarvisHome, _rendererBootChannel, resolved.train).catch((err) => {
...
async function triggerArtifactRepairFlow() {
  const result = await dialog.showMessageBox({
    ...
    detail: mt(
      ...
      null,
      "This resets Jarvis's app components to the originally installed version and restarts the app. Your data (agents, sessions, settings) is not affected.",
    ),
  });
...
async function _spawnServerOnce(serverInfoPath, artifactBootContext) {
  ...
  let serverEnv = {
    ...
    JARVIS_SERVER_OWNER: "desktop",
    JARVIS_SERVER_OWNER_PID: String(process.pid),
    JARVIS_DESKTOP_EXEC_PATH: process.execPath,
    JARVIS_DESKTOP_APP_PATH: app.getAppPath(),
    JARVIS_DESKTOP_IS_PACKAGED: app.isPackaged ? "1" : "0",
  };
  if (
    app.isPackaged
    ...
    serverEnv.JARVIS_DESKTOP_RESOURCES_PATH = process.resourcesPath;
  }
  // The server receives every ordinary desktop environment variable, but it
  // must not inherit Pi's global agent directory. Jarvis supplies all SDK paths
  // explicitly so a host-level Pi installation cannot redirect Jarvis's data.
  ...
  // 本次修复范围内。
  if (artifactBootContext) {
    serverEnv.JARVIS_RENDERER_DIST = _distRenderer;
  }
  ...
  if (artifactBootContext) {
    // 打包模式：从 JARVIS_HOME/artifacts 的版本化目录启动（首启已由
    // resolvePackagedArtifactBoot 解压 seed；目录布局与旧 Resources/server 一致）
    // macOS/Linux：jarvis-server 是 shell wrapper，内部调用 bootstrap.js，无需额外参数
    // Windows：jarvis-server.exe 是裸 Node 二进制（改名），需要显式传入 bootstrap.js
    const versionedServerRoot = artifactBootContext.serverRoot;
    const bundledServer = path.join(versionedServerRoot, "jarvis-server");
    ...
    serverArgs = process.platform === "win32"
      ...
      : [];
    serverEnv.JARVIS_ROOT = versionedServerRoot;
    serverEnv.JARVIS_SERVER_ENTRY = entry;
    ...
    serverEnv.JARVIS_CREATE_STARTUP_SESSION = "0";
  } else {
    ...
    serverBin = process.env.JARVIS_DEV_NODE_BIN || process.env.npm_node_execpath || "node";
    serverCwd = devRoot;
    serverArgs = [path.join(devRoot, "server", "bootstrap.ts")];
    serverEnv.JARVIS_ROOT = devRoot;
    ...
    serverEnv.JARVIS_SERVER_ENTRY = path.join(devRoot, "server", "main-full.ts");
  ...
  if (artifactBootContext) {
    await artifactBoot.writeBootSentinel(jarvisHome, artifactBootContext.channel, artifactBootContext.train);
  }
  ...
  if (process.platform === "win32") {
    ...
    if (!guardianBin) {
      throw new Error(
        "WINDOWS_SERVER_GUARDIAN_MISSING: jarvis-win-sandbox.exe is required to supervise the server process tree. Rebuild or reinstall Jarvis."
      );
    }
    serverEnv.JARVIS_WIN32_SANDBOX_HELPER = guardianBin;
  ...
  monitoredProcess.on("exit", async (code, signal) => {
    ...
    if (_serverRestartAttempts < 1) {
      ...
      try {
        ...
        dialog.showErrorBox("Jarvis Server", mt("dialog.serverRestartFailed", {
      ...
      // 壳身份用途：同上。
      dialog.showErrorBox("Jarvis Server", mt("dialog.serverMultipleCrash", {
        version: app?.getVersion?.() || "unknown",
        reason,
      }));
    }
...
function createTray() {
  ...
  tray.setToolTip(isDev ? "Bit Jarvis (dev)" : "Bit Jarvis");

  const buildMenu = () => Menu.buildFromTemplate([
    { label: mt("tray.show", null, "Show Bit Jarvis"), click: () => showPrimaryWindow() },
...
/**
 * 将崩溃日志写入 JARVIS_HOME/crash.log（默认 ~/.jarvis/crash.log）并返回日志内容
 */
function buildServerCrashDiagnostics() {
  // production 时 server 在 JARVIS_HOME/artifacts 的版本化目录（以最近一次
  ...
  if (process.platform === "win32" && isPackaged) {
    const exePath = path.join(serverDir, "jarvis-server.exe");
    const cmdPath = path.join(serverDir, "jarvis-server.cmd");
    const gitRoot = path.join(process.resourcesPath, "git");
    items.push(`jarvis-server.exe exists: ${fs.existsSync(exePath)}`);
    items.push(`jarvis-server.cmd exists (manual debug): ${fs.existsSync(cmdPath)}`);
    items.push(`PortableGit dir exists: ${fs.existsSync(gitRoot)}`);
    items.push(``);
    items.push(`Manual debug: open cmd.exe, cd to "${serverDir}", run jarvis-server.cmd`);
  }

  items.push(buildGpuStartupDiagnostics({ jarvisHome, policy: gpuStartupPolicy, app }));
  items.push(buildInstallAclHealDiagnostics({ jarvisHome }));
...
function detectDataEpochLaunchMarker(crashInfo) {
  ...
  const blocked = crashInfo.match(/JARVIS_DATA_EPOCH_BLOCKED reason=(\S+)/);
  if (blocked) return { kind: "blocked", reason: blocked[1] };
  const incomplete = crashInfo.match(/JARVIS_DATA_EPOCH_TRANSITION_INCOMPLETE reason=(\S+)/);
 ...
 * server/index.ts's JARVIS_DATA_EPOCH_TRANSITION_INCOMPLETE marker and
...
function buildLaunchFailureDialogDetail(err, crashInfo) {
  ...
  if (dataEpochMarker) {
    const specialized = dataEpochMarker.kind === "blocked"
      ? buildDataEpochBlockedDetail(jarvisHome)
...
function writeCrashLog(errorMessage) {
  ...
  const content = redactMainLogText([
    ...
    // getCurrentContentVersion() 声明处对这类诊断文案的例外说明。
    `Jarvis: v${app?.getVersion?.() || "unknown"}`,
    `Time: ${timestamp}`,
    `Error: ${errorMessage}`,
  ...
  // 写入文件（best effort）
  try {
    const crashLogPath = path.join(jarvisHome, "crash.log");
    // 数据目录存放凭证，只对当前用户开放（Windows 上 NTFS 忽略该位，由用户目录 ACL 兜底）
    fs.mkdirSync(jarvisHome, { recursive: true, mode: 0o700 });
...
function createSplashWindow() {
  ...
  splashWindow = new BrowserWindow({
    ...
    transparent: true,
    show: false,
    // 启动画面不得在 Windows 任务栏占用独立按钮：主窗口尚未就绪时，
    // 任务栏应只显示主窗口的图标，否则会出现"点击任务栏图标却在
    // 末尾新开一个图标"的双按钮问题（splash 与主窗口各自成组）。
    skipTaskbar: true,
    webPreferences: {
...
// ── 窗口状态记忆 ──
const windowStatePath = path.join(jarvisHome, "user", "window-state.json");

// ── 升级后首启公告：最后看过公告的版本记录 ──
const lastSeenVersionPath = path.join(jarvisHome, "user", "last-seen-version.json");
...
function computePendingAnnouncement() {
  ...
  const { pending, seedVersion } = resolvePostUpdateAnnouncement({
    ...
    lastSeenVersion,
    isPackagedLike: app.isPackaged || process.env.JARVIS_FORCE_ANNOUNCEMENT === "1",
...
const quickChatWindowStatePath = path.join(jarvisHome, "user", "quick-chat-window-state.json");
 ...
 * 后台网络请求）；唯一的例外是显式配置了 `JARVIS_ARTIFACT_MANIFEST` 排练
...
const SNAPSHOT_SCRIPT = `(function() {
  var ref = 0;
  var MAX_TREE = 30000;
  document.querySelectorAll('[data-jarvis-ref]').forEach(function(el) {
  ...
  function compact(el, depth) {
    ...
    function collect(node) {
      ...
      if (isInteractive(node)) {
        ref++;
        node.setAttribute('data-jarvis-ref', String(ref));
...
const DEFAULT_BROWSER_WORKSPACE_KEY = "__jarvis_default_browser__";
...
function _browserPartitionName(sessionPath) {
  return `persist:jarvis-browser-${_browserProfileKey(sessionPath)}`;
}
...
async function handleBrowserCommand(cmd, params) {
  switch (cmd) {
    ...
    case "browserSearch": {
      ...
      const ses = session.fromPartition("jarvis-search");
      const view = new WebContentsView({
        webPreferences: {
          session: ses,
          contextIsolation: true,
    ...
    case "click": {
      return await _withLiveWebContents(params.sessionPath, async (wc) => {
        ...
        await wc.executeJavaScript(
          "(function(){ var el = document.querySelector('[data-jarvis-ref=\"" + clickRef + "\"]');" +
    ...
    case "type": {
      return await _withLiveWebContents(params.sessionPath, async (wc) => {
        if (params.ref != null) {
          ...
          await wc.executeJavaScript(
            "(function(){ var el = document.querySelector('[data-jarvis-ref=\"" + typeRef + "\"]');" +
    ...
    case "select": {
      return await _withLiveWebContents(params.sessionPath, async (wc) => {
        ...
        await wc.executeJavaScript(
          "(function(){ var el = document.querySelector('[data-jarvis-ref=\"" + selRef + "\"]');" +
...
function setupBrowserCommands() {
  ...
  function connect() {
    ...
    ws.on("message", async (data) => {
      ...
      const _bLog = (line) => { try { require("fs").appendFileSync(require("path").join(jarvisHome, "browser-cmd.log"), `${new Date().toISOString()} ${redactMainLogText(line)}\n`); } catch { } };
...
function createOnboardingWindow(query = {}) {
  ...
  onboardingWindow.once("ready-to-show", () => {
    console.log("[debug] onboarding ready-to-show fired, isVisible:", onboardingWindow.isVisible());
    console.log("[debug] splash exists:", !!splashWindow, "splash destroyed:", splashWindow?.isDestroyed());
    // 关闭 splash，显示 onboarding
    if (splashWindow && !splashWindow.isDestroyed()) {
      splashWindow.close();
    }
    console.log("[debug] calling onboardingWindow.show(), tray exists:", !!tray, "tray destroyed:", tray?.isDestroyed());
    onboardingWindow.show();
    console.log("[debug] after show(), isVisible:", onboardingWindow.isVisible());
    try { console.log("[debug] bounds:", JSON.stringify(onboardingWindow.getBounds())); } catch (e) { }
    console.log("[debug] all windows:", require('electron').BrowserWindow.getAllWindows().map(w => ({ id: w.id, label: w.webContents.getTitle?.() || '', visible: w.isVisible(), destroyed: w.isDestroyed() })));
  });

  onboardingWindow.on("closed", () => {
    console.log("[debug] onboarding window closed");
    onboardingWindow = null;
  });
  onboardingWindow.on("blur", () => {
    console.log("[debug] onboarding window blurred");
  });
  onboardingWindow.on("focus", () => {
    console.log("[debug] onboarding window focused");
  });
}
...
<body>
  ...
  <footer class="watermark">
    ...
    <span class="watermark-text">Jarvis</span>
  </footer>
</body>
</html>`;
}
...
async function screenshotCapture(htmlContent, width) {
  ...
  const tmpHtml = path.join(tmpDir, `jarvis-ss-${Date.now()}.html`);
...
async function applyTrainUpdateNow(senderWebContents) {
  ...
  const result = await trainUpdateApply.runApplyNowSequence({
    ...
    verifyStaged: async () => {
      const staged = await artifactOta.readStagedTrainStatus(jarvisHome, { channel });
  ...
  if (!result.ok) {
    ...
    if (result.step === "shutdown-server" || result.step === "start-server") {
      ...
      dialog.showErrorBox(mt("dialog.installFailedTitle", null, "Jarvis Update"), mt(
        "dialog.trainUpdateApplyFailedBody",
        { version: app?.getVersion?.() || "unknown", error: result.error },
        `Jarvis update failed to apply: ${result.error}\n\nPlease restart the app.`,
      ));
    }
...
wrapIpcHandler("train-update-status", async () => {
  const status = await artifactOta.readStagedTrainStatus(jarvisHome, { channel: readUpdateChannelPreference() });
...
wrapIpcOn("settings-changed", (_event, type, data) => {
  ...
  if (type === "locale-changed") {
    ...
    if (tray && !tray.isDestroyed()) {
      const buildMenu = () => Menu.buildFromTemplate([
        { label: mt("tray.show", null, "Show Jarvis"), click: () => showPrimaryWindow() },
        { label: mt("tray.settings", null, "Settings"), click: () => createSettingsWindow() },
        { type: "separator" },
        { label: mt("tray.repairArtifacts", null, "Repair Components…"), click: () => { triggerArtifactRepairFlow().catch((err) => console.error(`[desktop] repair flow failed: ${err.message}`)); } },
        { type: "separator" },
...
wrapIpcHandler("get-avatar-path", (_event, role) => {
  ...
  const baseDir = role === "user"
    ? path.join(jarvisHome, "user")
    : agentId ? path.join(jarvisHome, "agents", agentId) : null;
...
wrapIpcHandler("get-splash-info", () => {
  try {
    const agentId = getCurrentAgentId();
    if (!agentId) return { agentName: null, locale: "zh-CN", yuan: "jarvis" };
    const configPath = path.join(jarvisHome, "agents", agentId, "config.yaml");
    const text = fs.readFileSync(configPath, "utf-8");
    // 统一换行符，避免 CRLF/LF 差异导致正则匹配失败
    const clean = text.replace(/\r\n/g, '\n');
    const agentMatch = clean.match(/^agent:\s*\n\s+name:\s*([^#\n]+)/m);
    const localeMatch = clean.match(/^locale:\s*(.+)/m);
    const yuanMatch = clean.match(/^\s+yuan:\s*([^#\n]+)/m);
    return {
      agentName: agentMatch?.[1]?.trim() || null,
      locale: localeMatch?.[1]?.trim() || null,
      yuan: yuanMatch?.[1]?.trim() || "jarvis",
    };
  } catch {
    return { agentName: null, locale: "zh-CN", yuan: "jarvis" };
  }
});
...
wrapIpcBestEffortHandler("open-skill-viewer", (_event, data) => {
  ...
  if (data.skillPath && path.isAbsolute(data.skillPath)) {
    ...
    if (fileExt === ".skill" || fileExt === ".zip") {
      ...
      const installedDir = path.join(jarvisHome, "skills", baseName);
      ...
      try {
        ...
        const tmpDir = path.join(app.getPath("temp"), "jarvis-skill-preview-" + Date.now());
  ...
  return withScreenshotLock(async () => {
    try {
      ...
      const segmentSuffix = Number.isInteger(segmentTotal) && segmentTotal > 1 && Number.isInteger(segmentIndex) && segmentIndex > 0
        ...
        : "";
      const filePath = path.join(dir, `jarvis-${timestamp}${segmentSuffix}.png`);
...
wrapIpcBestEffortHandler("show-notification", (_event, title, body, agentId, rawOptions) => {
  ...
  if (shouldSuppressDesktopNotification(notificationOptions, { getFocusedWindow: () => BrowserWindow.getFocusedWindow() })) {
    return { shown: false, reason: "jarvis_focused" };
  }
  ...
  const avatarPath = resolveAgentAvatarPath(jarvisHome, agentId);
  if (avatarPath) {
...
wrapIpcBestEffortHandler("app-ready", (event) => {
  ...
  if (!_startHiddenAtLogin && process.platform === "darwin" && Notification.isSupported()) {
    ...
    if (settings && status === "not-determined") {
      const notif = new Notification({ title: "Jarvis", body: mt("notification.ready", null, "Notifications enabled"), silent: true });
      notif.show();
    }
  }
...
app.whenReady().then(async () => {
  try {
    ...
    console.log("[desktop] 启动 Jarvis Server...");
    await startServer();
    ...
    dialog.showErrorBox(
      mt("dialog.launchFailedTitle", null, "Jarvis Launch Failed"),
      mt("dialog.launchFailedBody", {
        version: app?.getVersion?.() || "unknown",
        detail,
        logPath: path.join(jarvisHome, "crash.log"),
      })
    );
    forceQuitApp = true;
    app.quit();
...
app.on("window-all-closed", () => {
  console.log("[debug] window-all-closed fired, tray:", !!tray, "trayDestroyed:", tray?.isDestroyed(), "onboarding:", !!onboardingWindow, "splash:", !!splashWindow, "windows:", require('electron').BrowserWindow.getAllWindows().length);
  // 有托盘时保持常驻：macOS 通过 dock 重新打开，Windows 通过托盘双击
  // 托盘不存在时（创建失败或未初始化）直接退出，避免幽灵进程
  if (!tray || tray.isDestroyed()) {
    console.log("[debug] no tray, quitting");
    forceQuitApp = true;
    app.quit();
  } else {
    console.log("[debug] tray exists, keeping alive");
  }
});

app.on("will-quit", (event) => {
  console.log("[debug] will-quit event, forceQuitApp:", forceQuitApp);
});
...
process.on('unhandledRejection', (reason) => {
  console.error("[debug] UNHANDLED REJECTION:", reason?.message || reason);
});
...
async function shutdownServer() {
  ...
  if (removeServerInfo) {
    try { fs.unlinkSync(path.join(jarvisHome, "server-info.json")); } catch { }
  } else {
