...
// 或用 JARVIS_INVITE_API_URL 覆盖。
const DEFAULT_INVITE_API_URL = "";
const DEFAULT_GITHUB_OWNER = "mengtian999";
const DEFAULT_GITHUB_REPO = "Bitjarvis";

let _mainWindow = null;
let _setIsUpdating = null;  // 由 main.cjs 注入
let _jarvisHome = null;     // 由 main.cjs 注入
...
// ── 更新通道状态文件（{JARVIS_HOME}/update-channel.json）──

function updateChannelFilePathOrNull() {
  if (!_jarvisHome) return null;
  return path.join(_jarvisHome, UPDATE_CHANNEL_FILE_NAME);
}
...
function resolveUpdateFeedConfig(env = process.env) {
  const explicitFeedUrl = env.JARVIS_UPDATE_FEED_URL || "";
  const source = String(env.JARVIS_UPDATE_SOURCE || env.JARVIS_UPDATE_PROVIDER || "").trim().toLowerCase();
  const digestBaseUrl = env.JARVIS_UPDATE_DIGEST_BASE_URL || "";
...
function isAutoCheckEnabled() {
  try {
    const prefsPath = path.join(_jarvisHome || "", "user", "preferences.json");
...
function logUpdate(message) {
  ...
  if (!_jarvisHome) return;
  try {
    const logDir = path.join(_jarvisHome, "logs");
...
async function cleanUpdateCache() {
  const dataDir = _jarvisHome;
  const versionFile = path.join(dataDir, "last-update-version");

  // 迁移：旧版 bug 把 last-update-version 写到了 ~/.jarvis-dev/（生产环境误用）
  // 搬过来后尝试清理孤儿目录
  try {
    const wrongDir = path.join(require("os").homedir(), ".jarvis-dev");
    if (wrongDir !== dataDir) {
      ...
      if (fs.existsSync(wrongFile)) {
        ...
        console.log("[auto-updater] 已清理旧版误写的 ~/.jarvis-dev/last-update-version");
      }
    }
  } catch {}
...
function resolveInviteApiUrl(env = process.env) {
  return trimTrailingSlash(env.JARVIS_INVITE_API_URL || DEFAULT_INVITE_API_URL);
}
...
function initAutoUpdater(mainWindow, {
  setIsUpdating, jarvisHome,
} = {}) {
  _mainWindow = mainWindow;
  _setIsUpdating = setIsUpdating;
  _jarvisHome = jarvisHome;
