 ...
 * platform.js — 平台适配层
 *
 * Electron 环境：直接转发给 preload 注入的 window.jarvis（IPC）
 * Web 环境：降级到 HTTP API + 浏览器原生 API
 *
 * 使用方式：所有前端代码调 platform.xxx()，不再直接碰 window.jarvis。
 */
(function () {
  if (window.jarvis) {
    // Electron — 直接用 preload 注入的 IPC bridge
    window.platform = window.jarvis;
    return;
  }

  // Web / 非 Electron 环境 — HTTP fallback
  const params = new URLSearchParams(location.search);
  const devWeb = normalizeDevWebConfig(window.__JARVIS_DEV_WEB__);
  const token = params.get("token") || localStorage.getItem("jarvis-token") || "";
...
(async function initPlatform() {
  ...
  if (plat === "win32") {
    ...
    document.addEventListener("scroll", (event) => {
      ...
      target.classList.add("jarvis-scroll-active");
      const previousTimer = scrollIdleTimers.get(target);
      if (previousTimer) clearTimeout(previousTimer);
      const nextTimer = setTimeout(() => {
        target.classList.remove("jarvis-scroll-active");
