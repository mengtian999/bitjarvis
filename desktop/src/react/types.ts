...
export interface Agent {
  ...
  name: string;
  yuan: string;
  /** 该 agent 配置的音色（agent.voice），未配置时为 null。 */
  voice?: string | null;
  isPrimary: boolean;
  hasAvatar?: boolean;
...
export interface Model {
  ...
  /** 输入模态数组（Pi SDK 标准字段）。包含 "image" / "video" 表示模型支持对应媒体输入；音频走 Jarvis 兼容能力字段。 */
...
export type TabType = 'chat' | 'im' | 'channels' | `plugin:${string}`;
