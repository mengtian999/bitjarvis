...
export function QuickChatApp() {
    ...
    async function bootstrap() {
      try {
        const [serverPort, serverToken] = await Promise.all([
          window.jarvis?.getServerPort?.(),
          window.jarvis?.getServerToken?.(),
        ]);
  ...
  const resetDetachedSession = useCallback(() => {
    ...
    window.jarvis?.quickChatResize?.('compact');
  }, []);

  useEffect(() => {
    const dispose = window.jarvis?.onQuickChatShown?.(() => {
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      setDraft('');
      setAttachments([]);
      window.jarvis?.quickChatResize?.('chat');
  ...
  const openFullSession = useCallback(() => {
    ...
    markHidden();
    window.jarvis?.quickChatOpenSession?.(sessionPathRef.current);
  }, [markHidden]);
  ...
  useLayoutEffect(() => {
    ...
    window.jarvis?.quickChatResize?.({ mode, height });
