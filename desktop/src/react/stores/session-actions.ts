...
import { jarvisFetch, jarvisUrl } from '../hooks/use-jarvis-fetch';
...
function isDesktopShell(): boolean {
  return typeof window !== 'undefined' && !!(window as unknown as { jarvis?: unknown }).jarvis;
}
...
export async function loadMessages(forPath?: string): Promise<void> {
  ...
  try {
    const res = await jarvisFetch(sessionMessagesUrl(targetPath));
...
export async function completeSessionTodos(sessionPath: string): Promise<boolean> {
  ...
  try {
    await jarvisFetch('/api/sessions/todos/complete', {
      method: 'POST',
...
export async function loadMoreMessages(forPath?: string): Promise<void> {
  ...
  try {
    ...
    const res = await jarvisFetch(sessionMessagesUrl(targetPath, { before }));
    const data = await res.json();
...
async function fetchSessionMetaRecoveryStatus(): Promise<SessionMetaRecoveryStatus | null> {
  try {
    const res = await jarvisFetch('/api/health');
    const data = await res.json();
...
export async function loadSessions(): Promise<void> {
  ...
  // 第一个 jarvisFetch 调用——调用方/测试对"/api/sessions 是这次加载的第一个
  // 请求"这个顺序有既有假设），紧接着不等待地发起 /api/health 探测：两个请求
  // 仍然背靠背同时打到网络上，只是调用顺序固定，不会因为并行而变得不确定。
  const sessionsFetchPromise = jarvisFetch('/api/sessions');
...
export async function switchSession(path: string): Promise<void> {
  ...
  try {
    const res = await jarvisFetch('/api/sessions/switch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        path,
    ...
    if (switchedAgent) {
      ...
      agentPatch.agentYuan = ag?.yuan || 'jarvis';
      agentPatch.agentAvatarUrl = ag?.hasAvatar ? jarvisUrl(`/api/agents/${data.agentId}/avatar?t=${Date.now()}`) : null;
...
async function postPendingSessionCreate(body: PendingSessionCreateBody): Promise<any> {
  const res = await jarvisFetch('/api/sessions/new-detached', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function loadPendingNewSessionPermissionDefault(): Promise<SessionPermissionMode> {
  try {
    const res = await jarvisFetch('/api/preferences/session-permission-default');
    const data = await res.json();
...
export async function createNewSession(options: CreateNewSessionOptions = {}): Promise<void> {
  ...
  try {
    const res = await jarvisFetch('/api/session-thinking-level?pendingNewSession=1');
    const data = await res.json();
...
export async function continueDeletedAgentSession(path: string): Promise<boolean> {
  try {
    const res = await jarvisFetch('/api/sessions/continue-deleted-agent', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function archiveSession(path: string): Promise<void> {
  try {
    ...
    const res = await jarvisFetch('/api/sessions/archive', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        path,
...
export async function listArchivedSessions(): Promise<ArchivedSession[]> {
  try {
    const res = await jarvisFetch('/api/sessions/archived');
    if (!res.ok) return [];
...
export async function restoreSession(target: string | Pick<ArchivedSession, 'path' | 'sessionId'>): Promise<RestoreResult> {
  ...
  try {
    const res = await jarvisFetch('/api/sessions/restore', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        path: sessionPath,
...
export async function deleteArchivedSession(target: string | Pick<ArchivedSession, 'path' | 'sessionId'>): Promise<boolean> {
  ...
  const sessionId = typeof target === 'string' ? null : normalizeSessionId(target.sessionId);
  try {
    const res = await jarvisFetch('/api/sessions/archived/delete', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        path: sessionPath,
...
export async function cleanupArchivedSessions(maxAgeDays: 30 | 90): Promise<{ deleted: number }> {
  try {
    const res = await jarvisFetch('/api/sessions/cleanup', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function renameSession(path: string, title: string): Promise<boolean> {
  try {
    const res = await jarvisFetch('/api/sessions/rename', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function pinSession(path: string, pinned: boolean): Promise<boolean> {
  try {
    const localSessionId = sessionIdForPathFromState(useStore.getState() as Record<string, any>, path);
    const res = await jarvisFetch('/api/sessions/pin', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        path,
...
export async function reorderPinnedSessions(orderedSessionIds: string[]): Promise<boolean> {
  ...
  try {
    const res = await jarvisFetch('/api/sessions/pin-order', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function refreshSessionCapabilities(path: string): Promise<boolean> {
  ...
  try {
    const res = await jarvisFetch('/api/sessions/fresh-compact', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ path }),
      // Fresh compact runs an LLM summarization over the whole conversation;
      // long sessions routinely exceed the 30s jarvisFetch default. A premature
