...
export async function refreshPluginUI(): Promise<void> {
  try {
    ...
    const [pagesResult, widgetsResult, grantsResult, prefsResult] = await Promise.allSettled([
      jarvisFetch('/api/plugins/pages').then(r => r.json()),
      jarvisFetch('/api/plugins/widgets').then(r => r.json()),
      jarvisFetch('/api/plugins/ui-host-capabilities').then(r => r.json()),
      jarvisFetch('/api/preferences/plugin-ui').then(r => r.json()),
    ]);
...
function persistField(field: Record<string, unknown>): void {
  jarvisFetch('/api/preferences/plugin-ui', {
    method: 'PUT',
