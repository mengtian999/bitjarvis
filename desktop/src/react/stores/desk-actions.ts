...
async function pruneStaleLocalDeskRoot(dir: string): Promise<void> {
  ...
  try {
    const res = await jarvisFetch(url, {
      method: 'DELETE',
...
export async function loadStudioWorkspaces(): Promise<StudioWorkspace[]> {
  ...
  if (!hasServerConnection(s)) return [];
  try {
    const res = await jarvisFetch('/api/studio/workspaces');
...
export async function removeStudioWorkspace(mountId: string): Promise<boolean> {
  ...
  try {
    const res = await jarvisFetch(`/api/studio/workspaces/${encodeURIComponent(normalized)}`, {
      method: 'DELETE',
    });
    const data = await res.json();
    if (data.error) throw new Error(String(data.error));
...
export async function loadDeskFiles(subdir?: string, overrideDir?: string | null, overrideMountId?: string | null): Promise<void> {
  ...
  try {
    ...
    const res = await jarvisFetch(`${mountId ? '/api/workbench/files' : '/api/desk/files'}${qs}`);
    const data = await responseJsonOrEmpty(res);
...
export async function searchDeskFiles(query: string): Promise<DeskSearchResult[]> {
  ...
  try {
    ...
    const res = await jarvisFetch(`${mountId ? '/api/workbench/search' : '/api/desk/search-files'}?${params}`);
    const data = await res.json();
    if (data.error) throw new Error(String(data.error));
...
export async function loadJianContent(): Promise<void> {
  ...
  try {
    ...
    const qs = params.toString() ? `?${params}` : '';
    const res = await jarvisFetch(`${mountId ? '/api/workbench/content' : '/api/desk/jian'}${qs}`);
    if (mountId) {
      if (res.status === 404) {
...
export async function saveJianContent(content?: string): Promise<void> {
  ...
  try {
    const mountId = activeDeskMountId(s);
    await jarvisFetch(mountId ? '/api/workbench/actions' : '/api/desk/jian', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const qs = params.toString() ? `?${params}` : '';
    const res2 = await jarvisFetch(`${activeMountId ? '/api/workbench/files' : '/api/desk/files'}${qs}`);
...
export async function deskUploadFiles(paths: string[]): Promise<void> {
  const s = useStore.getState();
  try {
    const res = await jarvisFetch('/api/desk/files', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function deskUploadBrowserFilesToSubdir(files: File[], subdir: string): Promise<boolean> {
  ...
  try {
    ...
    const res = await jarvisFetch('/api/workbench/upload', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        mountId,
...
export async function deskCreateFileInSubdir(subdir: string, name: string, text: string): Promise<boolean> {
  ...
  const mountId = activeDeskMountId(s);
  try {
    const res = await jarvisFetch(mountId ? '/api/workbench/actions' : '/api/desk/files', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(mountId
  ...
  try {
    if (mountId) {
      for (const name of names) {
        ...
        const res = await jarvisFetch('/api/workbench/actions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
...
export function toggleJianSidebar(forceOpen?: boolean): void {
  ...
  localStorage.setItem('jarvis-jian', newOpen ? 'open' : 'closed');
  if (forceOpen === undefined) s.setJianAutoCollapsed(false);
}

export function initJian(): void {
  const legacy = localStorage.getItem('jarvis-jian');
  const savedJian = legacy ?? localStorage.getItem('jarvis-jian-chat');
  if (savedJian !== null && legacy === null) {
    localStorage.setItem('jarvis-jian', savedJian);
  }
