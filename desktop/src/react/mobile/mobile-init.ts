...
export async function initializeMobileRuntime(principal: MobilePrincipal): Promise<{
  ...
  const bootstrapRes = await jarvisFetch('/api/mobile/bootstrap');
  ...
  await applyAgentIdentity({
    agentName: bootstrap.agentName || 'Jarvis',
  ...
  if (currentAgent) {
    ...
    useStore.setState({
      ...
      agentYuan: currentAgent.yuan || bootstrap.agentYuan || 'jarvis',
      homeFolder,
...
export async function loadMobileSessions({
  ...
  const res = await jarvisFetch('/api/sessions');
...
export async function switchMobileSession(
  ...
  useStore.setState((state) => {
    ...
    ...(session?.agentId ? {
      ...
      agentYuan: sessionAgent?.yuan || 'jarvis',
...
export async function createMobileSession(): Promise<string | null> {
  ...
  const res = await jarvisFetch('/api/sessions/new', {
    method: 'POST',
