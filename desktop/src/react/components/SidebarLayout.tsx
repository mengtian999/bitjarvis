  ...
  if (contentW < CHAT_MIN_WIDTH) {
    ...
    if (s.sidebarAutoCollapsed) {
      ...
      if (w - rightW - previewW - channelInspectorW - neededForLeft >= CHAT_MIN_WIDTH) {
        ...
        const savedLeft = localStorage.getItem(`jarvis-sidebar-${tab}`);
        if (savedLeft !== 'closed') {
    ...
    if (s2.jianAutoCollapsed) {
      ...
      if (w - leftW2 - previewW - channelInspectorW - neededForRight >= CHAT_MIN_WIDTH) {
        const savedRight = localStorage.getItem('jarvis-jian');
...
export function toggleSidebar(forceOpen?: boolean): void {
  ...
  const tab = s.currentTab || 'chat';
  localStorage.setItem(`jarvis-sidebar-${tab}`, open ? 'open' : 'closed');
...
export function SidebarLayout() {
  ...
  useEffect(() => {
    ...
    // 迁移 localStorage
    const legacy = localStorage.getItem('jarvis-sidebar');
    if (legacy && !localStorage.getItem('jarvis-sidebar-chat')) {
      localStorage.setItem('jarvis-sidebar-chat', legacy);
    }
    const savedOpen = localStorage.getItem('jarvis-sidebar-chat');
