...
const SESSION_FILE_SORT_KEY = 'jarvis-session-file-sort';
    ...
    try {
      ...
      await Promise.all(bridgeAgents.map(async agent => {
        await Promise.all(BRIDGE_PLATFORMS.map(async platform => {
          const res = await jarvisFetch(`/api/bridge/sessions?platform=${encodeURIComponent(platform)}&agentId=${encodeURIComponent(agent.id)}`);
  ...
  const sendFilesToBridge = useCallback(async (filesToSend: FileRef[], target: BridgeSendTarget) => {
    ...
    try {
      for (const file of sendableFiles) {
        const res = await jarvisFetch(`/api/bridge/send-media?agentId=${encodeURIComponent(target.agentId)}`, {
          method: 'POST',
