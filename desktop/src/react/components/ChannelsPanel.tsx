...
import { speakTts, stopTts, subscribeTts, stripMarkdownForSpeech } from './tts/tts-controller';
import { isChannelTtsAutoPlayEnabled } from './tts/tts-preferences';
import {
  addChannelMember,
...
import { useVoiceDictation, VoiceModePills, VoiceInputOverlay } from './voice/VoiceDictationBar';
...
const CHANNEL_COMPOSER_FOCUS_EVENT = 'jarvis-channel-composer-focus';
...
export function ChannelMessages() {
  ...
  // 消息级 TTS 播放态(messageId = 消息 key),供气泡上的播放按钮显示进行中
  const [ttsPlayingId, setTtsPlayingId] = useState<string | null>(null);
  const [ttsError, setTtsError] = useState<string | null>(null);
  useEffect(() => {
    let errorTimer: ReturnType<typeof setTimeout> | null = null;
    const unsub = subscribeTts(s => {
      setTtsPlayingId(s.playing ? s.messageId : null);
      if (s.error) {
        setTtsError(s.error);
        if (errorTimer) clearTimeout(errorTimer);
        errorTimer = setTimeout(() => setTtsError(null), 6000);
      }
    });
    return () => { unsub(); if (errorTimer) clearTimeout(errorTimer); };
  }, []);

  // 频道自动朗读:新到达的非自己消息在开关开启时自动播报;
  // 切换频道/历史水合(初次填充)/消息变少(重载)不触发
  const autoPlayTrackerRef = useRef<{ channel: string | null; count: number }>({ channel: null, count: 0 });
  useEffect(() => {
    const tracker = autoPlayTrackerRef.current;
    if (tracker.channel !== currentChannel || messages.length < tracker.count) {
      autoPlayTrackerRef.current = { channel: currentChannel, count: messages.length };
      return;
    }
    if (messages.length === tracker.count) return;
    const startIdx = tracker.count;
    tracker.count = messages.length;
    if (!isChannelTtsAutoPlayEnabled()) return;
    if (typeof document !== 'undefined' && document.visibilityState !== 'visible') return;
    const store = useStore.getState();
    const ch = store.channels.find((c) => c.id === currentChannel);
    const isDM = ch?.isDM ?? false;
    const dmOwnerId = resolveDmOwnerId(ch, store.currentAgentId);
    const agentMap = buildAgentMap(store.agents);
    // 一批新消息只播报最后一条可播的(agent 回复),避免连发时排队轰炸
    for (let i = messages.length - 1; i >= startIdx; i--) {
      const msg = messages[i];
      const body = stripMarkdownForSpeech(msg.body || '');
      if (!body) continue;
      const senderInfo = resolveChannelMember(
        msg.sender, store.userName, store.userAvatarUrl, store.agents,
        isDM ? dmOwnerId : store.currentAgentId, agentMap,
      );
      const isSelf = senderInfo.isUser || (isDM && msg.sender === dmOwnerId);
      if (isSelf) continue;
      // messageId 与渲染侧 msgKey 规则一致,保证气泡按钮能显示播放中
      const msgKey = `${msg.timestamp}-${msg.sender}-${i}`;
      void speakTts(body, { messageId: msgKey, voice: agentMap.get(msg.sender)?.voice || undefined });
      break;
    }
    ...
    <>
      <div ref={wrapperRef}>
        {messages.map((msg, idx) => {
          ...
          const isSelf = senderInfo.isUser || (isDM && msg.sender === dmOwnerId);
          const msgKey = `${msg.timestamp}-${msg.sender}-${idx}`;
          const el = (
            <div
              key={msgKey}
              className={
            ...
            >
              ...
              <div className={styles.channelMsgBody}>
                ...
                />
                {!isSelf && (msg.body || '').trim() && (
                  <ChannelSpeakButton
                    messageKey={msgKey}
                    text={msg.body}
                    voice={agentMap.get(msg.sender)?.voice || undefined}
                    playing={ttsPlayingId === msgKey}
                  />
                )}
              </div>
            </div>
      ...
      {ttsError && (
        <div className={styles.channelTtsError} role="status">{ttsError}</div>
      )}
      {showNewMessages && (
        <button
          type="button"
...
/** Agent 消息气泡上的朗读按钮:点击合成播报,再次点击停止;同一时间只播一条。 */
function ChannelSpeakButton({ messageKey, text, voice, playing }: {
  messageKey: string;
  text: string;
  voice?: string;
  playing: boolean;
  ...
  const handle = useCallback(() => {
    ...
    void speakTts(stripMarkdownForSpeech(text), { messageId: messageKey, voice });
  }, [playing, messageKey, text, voice]);
  return (
    <button
      type="button"
      className={styles.channelMsgSpeak}
      onClick={handle}
      data-playing={playing ? 'true' : undefined}
      aria-label={playing ? window.t?.('channel.stopPlayback') : window.t?.('channel.playMessage')}
      title={playing ? window.t?.('channel.stopPlayback') : window.t?.('channel.playMessage')}
    >
      {playing ? (
...
export function AgentPhoneSessionPreview({ sessionPath, agentId, agentYuan }: {
  ...
  const moodYuan = agentYuan || 'jarvis';
...
export function ChannelInput() {
  ...
  // ── 语音免提听写/对话(与主输入区 InputArea 同架构) ──
  // 直接用 useVoiceDictation 返回的 voiceState(useMemo 稳定)作为渲染源,
  // 不再 useState+useEffect 镜像——镜像会引入一拍延迟,导致点听写后 overlay 不即时显示。
  const handleChannelDictationFinal = useCallback((text: string) => {
    const trimmed = (text || '').trim();
    if (!trimmed) return;
    setInputValue(prev => (prev ? `${prev}${prev.endsWith(' ') ? '' : ' '}${trimmed}` : trimmed));
    // 程序性追加后把光标移到末尾并滚动,确保超过 max-height(120px≈5行)时最新行可见
  ...
  // conversationTarget 必须 memoize: 它在 useVoiceDictation 的 state useMemo 依赖数组里,
  // 若每次渲染都是新对象引用会破坏 memo 稳定化 → 配合下方 useEffect+setState 反馈环
  // 引发无限渲染循环(Maximum update depth exceeded → "此区域暂时无法显示")。
  const conversationTarget = useMemo(
    () => currentChannel ? { kind: 'channel' as const, channelName: currentChannel } : undefined,
    [currentChannel],
  );
  const { state: voiceState, toggle: voiceToggle, switchMode: voiceSwitchMode } = useVoiceDictation({
    onFinalText: handleChannelDictationFinal,
    conversationTarget,
    onChannelMessageSent: (msg) => {
      // 语音路径的用户消息回显:与打字路径 sendChannelMessage 的本地落库一致
      if (msg.channel && msg.channel === currentChannel) {
        appendChannelMessage(currentChannel, {
          sender: msg.sender,
  ...
  const t = window.t?.bind(window) ?? ((key: string) => key);
  const voiceActive = voiceState?.active ?? false;
  ...
  return (
    <div className={styles.channelInputWrapper}>
      {/* 列容器: overlay 在上,textarea 在下。
          channelInputWrapper 是水平 flex(row),若 overlay(width:100%) 直接放行内会把
          textarea 挤到 0 宽度,导致转写文字不可见——必须用 column 包裹让 overlay 位于 textarea 上方。 */}
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minWidth: 0 }}>
        {voiceActive && voiceState && (
          <VoiceInputOverlay
            ...
            dictationHint={voiceState.dictationHint}
            conversationHint={voiceState.conversationHint}
            t={t}
          />
        )}
        <textarea
          ...
          style={{ display: voiceActive && voiceState?.mode === 'conversation' ? 'none' : '' }}
        />
      </div>
      ...
      {voiceState && (
        <VoiceModePills
          ...
          hasConversationTarget={voiceState.hasConversationTarget}
          disabled={sending}
          onToggle={voiceToggle}
          onSwitchMode={voiceSwitchMode}
          t={t}
          variant="icon"
        />
      )}
