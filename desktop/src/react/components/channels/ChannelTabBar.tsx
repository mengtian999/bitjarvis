...
export function switchTab(tab: TabType) {
  ...
  localStorage.setItem('jarvis-tab', tab);

  // IM tab 全屏：强制关闭左侧栏
  if (tab === 'im') {
    if (s.sidebarOpen) toggleSidebar(false);
    return;
  }
...
function buildTabList(pluginPages: PluginPageInfo[], tabOrder: string[]): TabType[] {
  ...
  const draggable: TabType[] = ['channels' as TabType, 'im' as TabType, ...pluginTabs];
...
function getTabLabel(tab: TabType, pluginPages: PluginPageInfo[], locale: string): string {
  if (tab === 'chat') return t('channel.chatTab') || '🤖 Agent 工作台';
...
export function ChannelTabBar() {
  ...
  useEffect(() => {
    const savedTab = localStorage.getItem('jarvis-tab');
