...
export async function loadMcpApprovalIndex(): Promise<StateConnector[]> {
  const res = await jarvisFetch('/api/mcp/state');
...
export async function grantForSession(sessionId: string, capability: string): Promise<void> {
  const res = await jarvisFetch('/api/mcp/session-permissions', {
    method: 'POST',
...
/**
 * The synthetic capability key a non-MCP tool approval carries when the wrapper
 * built its confirmation request. It is `${toolName}:${action}` — the same key
 * `classifySyntheticSessionGrant` honours in Auto mode, so granting it stops
 * the same tool+action pair from being re-asked for the rest of this session.
 *
 * Returns null for MCP tools (they carry a real descriptor capability via
 * `resolveMcpApprovalTarget`) and for blocks without the key.
 */
export function sessionGrantCapabilityFromBlock(block: SessionConfirmationBlock): string | null {
  if (block.kind !== 'tool_action_approval') return null;
  const capability = block.payload?.sessionGrantCapability;
  return typeof capability === 'string' && capability.trim() ? capability.trim() : null;
}
...
export async function grantPermanently(target: McpApprovalTarget): Promise<void> {
  const res = await jarvisFetch(`/api/mcp/connectors/${encodeURIComponent(target.connectorId)}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
