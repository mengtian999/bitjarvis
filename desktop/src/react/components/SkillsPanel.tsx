...
export function SkillsPanel() {
  ...
  const installSkillFromPath = useCallback(async (filePath: string) => {
    ...
    try {
      const res = await jarvisFetch(`/api/skills/install?agentId=${encodeURIComponent(targetAgentId)}`, {
        method: 'POST',
  ...
  const deleteSkill = useCallback(async (name: string) => {
    ...
    try {
      const res = await jarvisFetch(`/api/skills/${encodeURIComponent(name)}?agentId=${encodeURIComponent(agentId)}`, {
        method: 'DELETE',
      });
      const data = await readJsonObject(res);
      const error = responseError(data);
