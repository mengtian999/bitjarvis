...
function initialBridgePlatform(): BridgePlatform {
  const saved = localStorage.getItem('jarvis_bridge_tab');
...
export function BridgePanel() {
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      const res = await jarvisFetch(`/api/bridge/status?agentId=${encodeURIComponent(snapshotId)}`);
  ...
  const loadPlatformData = useCallback(async (plat: BridgePlatform) => {
    ...
    try {
      ...
      const [statusRes, sessionsRes] = await Promise.all([
        jarvisFetch(`/api/bridge/status?agentId=${encodeURIComponent(snapshotId)}`),
        jarvisFetch(`/api/bridge/sessions?platform=${plat}${agentQuery}`),
      ]);
      if (bridgeAgentIdRef.current !== snapshotId) return; // stale
  ...
  // 订阅 bridge status 变化（代替 window.__jarvisBridgeLoadStatus）
  ...
  // 订阅 bridge 消息（代替 window.__jarvisBridgeOnMessage）— 按 agent 过滤
  ...
  const switchTab = useCallback((plat: BridgePlatform) => {
    ...
    setCurrentSessionPath(null);
    setChatOpen(false);
    localStorage.setItem('jarvis_bridge_tab', plat);
  ...
  const resetSession = useCallback(async () => {
    ...
    if (!snapshotId) return;
    try {
      await jarvisFetch(`/api/bridge/sessions/${encodeURIComponent(currentKey)}/reset?agentId=${encodeURIComponent(snapshotId)}`, { method: 'POST' });
      if (bridgeAgentIdRef.current !== snapshotId) return; // stale
