...
export function AppPages() {
  ...
  const [hasVisitedIm, setHasVisitedIm] = useState(false);

  useEffect(() => {
    if (currentTab === 'im' && !hasVisitedIm) {
      setHasVisitedIm(true);
    }
  }, [currentTab, hasVisitedIm]);
  ...
  return (
    <>
      ...
      {/* IM overlay：覆盖整个 app 区域（含侧栏位置），cross-fade。
          sidebar 不收缩，避免切换跳变；FluffyChat 全窗口渲染。 */}
      {hasVisitedIm && (
        <div
          className={`im-tab-container${currentTab === 'im' ? ' active' : ''}`}
        >
          <ImView />
        </div>
      )}
      ...
      {currentTab !== 'im' && <WorkspaceCompanionRail />}
    </>
  );
}
