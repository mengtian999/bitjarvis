...
export function ActivityPanel() {
  ...
  const loadData = useCallback(() => {
    jarvisFetch('/api/desk/activities')
      .then(r => r.json())
  ...
  const promoteActivity = useCallback(async (activityId: string) => {
    try {
      const res = await jarvisFetch(`/api/desk/activities/${activityId}/promote`, { method: 'POST' });
        ...
        <div id="activityListView" style={FLEX_COLUMN_STYLE}>
          <div className={fp.floatingPanelHeader}>
              ...
              <div className={fp.activityHbToggle}>
                <span className="jarvis-toggle-label">{t('activity.heartbeat')}</span>
                <button
                  className={'jarvis-toggle' + (hbEnabled ? ' on' : '')}
...
function ActivityCard({
  ...
  useEffect(() => {
    ...
    void jarvisFetch(`/api/desk/activities/${a.id}/session`)
      .then(res => res.json())
      .then(data => {
        if (cancelled) return;
