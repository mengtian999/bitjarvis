...
const SESSION_VIEW_MODE_KEY = 'jarvis-session-sidebar-view-mode';
const SESSION_DRAG_MIME = 'application/x-jarvis-session-path';
const PROJECT_DRAG_MIME = 'application/x-jarvis-project-id';
const FOLDER_DRAG_MIME = 'application/x-jarvis-project-folder-id';
...
function SessionListInner() {
  ...
  useEffect(() => {
    ...
    jarvisFetch('/api/browser/session-states')
      .then(r => r.json())
      .then(data => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const titleRes = await jarvisFetch(`/api/sessions/search?q=${encodedQuery}&phase=title&limit=20`, {
        ...
        const contentRes = await jarvisFetch(`/api/sessions/search?q=${encodedQuery}&phase=content&limit=20`, {
          signal: controller.signal,
          timeout: 12_000,
        });
  ...
  const handleCloseBrowserSession = useCallback(async (sessionPath: string) => {
    ...
    try {
      const res = await jarvisFetch('/api/browser/close-session', {
        method: 'POST',
...
const SessionItem = memo(function SessionItem({ session: s, isActive, isPending, isStreaming, isPinned, hasUnreadOutput, agents, browserState, rowMode, onCloseBrowser, draggable = false, onDragStart, onDragEnd }: {
  ...
  const handleBrowserOpen = useCallback(async (e: React.MouseEvent | React.KeyboardEvent) => {
    ...
    e.stopPropagation();
    try {
      await jarvisFetch('/api/browser/open-session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
const SessionSummaryPreviewCard = memo(function SessionSummaryPreviewCard({
  ...
  useEffect(() => {
    ...
    jarvisFetch(`/api/sessions/summary?path=${encodeURIComponent(session.path)}`)
      .then(res => res.json())
