...
export function SkillViewerOverlay() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const items = await jarvis?.listSkillFiles?.(data.baseDir) as TreeItem[] | undefined;
      setFiles(items || []);
  ...
  async function loadFile(filePath: string, name: string) {
    ...
    setFileName(name);
    const text = await window.jarvis?.readSkillFile?.(filePath);
  ...
  async function onCopy() {
    ...
    try {
      ...
      const res = await jarvisFetch(`/api/skills/install?agentId=${encodeURIComponent(agentId)}`, {
        method: 'POST',
