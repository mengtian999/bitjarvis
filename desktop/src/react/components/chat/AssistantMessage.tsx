...
import { isChatTtsAutoPlayEnabled } from '../tts/tts-preferences';
...
export const AssistantMessage = memo(function AssistantMessage({
  ...
  // 音色：本消息所属 agent 的 config.agent.voice；未配置时回落全局 tts.voice。
  ...
  useEffect(() => {
    ...
    const unsub = subscribeTts((s) => {
      setTtsSpeaking(s.playing && s.messageId === message.id);
      if (s.error && s.messageId === message.id) {
        setTtsError(s.error);
  ...
  const assistantText = useMemo(() => {
    ...
    const plain = extractTextBlockPlainText(textBlocks);
    return plain.trim() || '';
  }, [blocks]);
  ...
  const handleSpeak = useCallback(() => {
    ...
    void speakTts(stripMarkdownForSpeech(assistantText), { messageId: message.id, voice: agentVoice || undefined });
  }, [assistantText, isStreaming, message.id, ttsSpeaking, agentVoice]);

  // 自动朗读:本消息从流式中结束(true→false)且开关开启时自动播报;
  // 历史消息(未曾流式)/只读会话/非最新消息不触发
  const prevStreamingRef = useRef(isStreaming);
  useEffect(() => {
    const wasStreaming = prevStreamingRef.current;
    prevStreamingRef.current = isStreaming;
    if (!wasStreaming || isStreaming) return;
    if (readOnly || !isLatestAssistantMessage || !assistantText) return;
    if (!isChatTtsAutoPlayEnabled()) return;
    void speakTts(stripMarkdownForSpeech(assistantText), { messageId: message.id, voice: agentVoice || undefined });
  }, [isStreaming, readOnly, isLatestAssistantMessage, assistantText, message.id, agentVoice]);
  ...
  const standardMessageActions = useMessageFooterActions({
    ...
    onSpeak: assistantText ? handleSpeak : undefined,
    ...
    <div className={`${styles.messageGroup} ${styles.messageGroupAssistant}${isInterludeOnly ? ` ${styles.messageGroupInterludeOnly}` : ''}${isSelected ? ` ${styles.messageGroupSelected}` : ''}`}
      ...
      {showAvatar && !isInterludeOnly && (
        <div className={styles.avatarRow}>
          <AgentAvatar
            info={displayInfo}
            className={`${styles.avatar} ${styles.jarvisAvatar}`}
            alt={displayName}
          />
      ...
      {ttsError && (
        <div style={{ fontSize: '0.75em', color: 'var(--color-error, #ef4444)', marginTop: 4, padding: '2px 8px' }}>
          {ttsError}
        </div>
      )}
    </div>
...
const MediaGenerationBlock = memo(function MediaGenerationBlock({ block, sessionPath, readOnly }: { block: any; sessionPath: string; readOnly: boolean }) {
  ...
  const handleRetry = useCallback(async () => {
    ...
    setRetryError('');
    try {
      const res = await jarvisFetch(`/api/media/tasks/${encodeURIComponent(viewBlock.taskId)}/retry`, {
        method: 'POST',
      });
...
const CronConfirmBlock = memo(function CronConfirmBlock({ block, sessionPath }: { block: any; sessionPath?: string }) {
  ...
  const fallbackAgentName = useStore(s => s.agentName) || 'Jarvis';
  const fallbackAgentYuan = useStore(s => s.agentYuan) || 'jarvis';
  ...
  const submitDraftJob = async (editedJobData: Record<string, unknown>) => {
    if (isSuggestionCard) {
      ...
      const response = await jarvisFetch('/api/desk/cron', {
        method: 'POST',
