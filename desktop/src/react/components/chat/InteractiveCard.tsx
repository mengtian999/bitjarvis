 ...
 *
 * 加载方式（关键架构）：
 *   卡片 HTML 由本地 Jarvis server 经 http 提供（server/routes/cards.ts），
...
export const InteractiveCard = memo(function InteractiveCard({ block }: InteractiveCardProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await jarvisFetch(`/api/cards/${encodeURIComponent(cardId)}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ code: block.code, title: block.title, varsCss }),
        });
        if (cancelled) return;
        const base = jarvisUrl(`/api/cards/${encodeURIComponent(cardId)}`);
        verRef.current += 1;
  ...
  const handleMessage = useCallback((e: MessageEvent) => {
    ...
    if (e.data?.type !== 'jarvis.card-resize') return;
  ...
  useEffect(() => {
    ...
    if (!iframe) return;
    const onLoad = () => {
      iframe.contentWindow?.postMessage('jarvis.card-ping', '*');
    };
