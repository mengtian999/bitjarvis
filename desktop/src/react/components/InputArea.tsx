...
import { markUserAbort } from '../services/user-abort-registry';
...
import { VoiceDictationBar, VoiceModePills, VoiceInputOverlay, type VoiceDictationState } from './voice/VoiceDictationBar';
...
function InputAreaInner({ surface }: Required<InputAreaProps>) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis-inline-notice', handler);
    return () => window.removeEventListener('jarvis-inline-notice', handler);
  }, []);
  ...
  const handleDictationFinal = useCallback((text: string) => {
    ...
    if (!editor || editor.isDestroyed || !trimmed) return;
    const current = serializeEditor(editor.getJSON()).text;
    const next = current.trim() ? current + '\n' + trimmed : trimmed;
    applyComposerDocument(editor, plainTextToEditorDocument(next), setInputText);
    // 程序性 setContent 不移动光标也不滚动,需主动定位到文档末尾并滚入视口,
    // 否则超过 5 行(.input-box max-height:120px)后新增行在视口外不可见。
    editor.commands.focus('end');
    editor.commands.scrollIntoView();
  }, [editor]);

  // ── 语音免提听写/对话 ──
  const [voiceDictationState, setVoiceDictationState] = useState<VoiceDictationState | null>(null);

  // ── 全屏模式检测 ──
  const [fullscreen, setFullscreen] = useState(false);
  useEffect(() => {
    const check = () => setFullscreen(window.innerWidth > 1200 || window.innerHeight > 800);
    check();
      ...
      for (const file of files) {
        ...
        try {
          ...
          const waveform = mimeType.startsWith('audio/')
            ...
            : undefined;
          const res = await jarvisFetch('/api/upload-blob', {
            method: 'POST',
  ...
  const loadVisionAuxiliaryConfig = useCallback(async () => {
    if (surface === 'mobile') {
      const res = await jarvisFetch('/api/models/auxiliary-vision');
      const data = await res.json();
    ...
    const res = await jarvisFetch('/api/preferences/models');
    const data = await res.json();
    return {
  ...
  useEffect(() => {
    if (activeServerConnection && surface !== 'mobile') {
      const query = pendingNewSession
        ...
        : currentSessionPath
          ...
          : '';
      jarvisFetch(`/api/session-thinking-level${query}`)
        .then(r => r.json())
    ...
    window.addEventListener('jarvis-plan-mode', handler);
    return () => window.removeEventListener('jarvis-plan-mode', handler);
  ...
  const submitEditorMessage = useCallback(async (type: 'prompt' | 'interject') => {
    ...
    try {
      ...
      // 图片 / 视频读 base64。统一走 platform 层：Electron 里 platform 代理到 jarvis，
  ...
  const handleStop = useCallback(() => {
    ...
    if (!request) return;
    // 登记“这是用户主动停止”，后续 aborted 事件据此不再弹「任务已中断」。
    markUserAbort(request.sessionPath, request.streamId ?? null);
    ws.send(JSON.stringify(request));
  }, [isStreaming]);

  /**
   * 用最小路径（文本-only prompt）直发一条消息，供「重试 / 继续任务」使用。
   * 与 submitEditorMessage 相同的 WS 协议，但绕过编辑器与附件流程。
   */
  const sendPromptText = useCallback(async (text: string) => {
    const trimmed = text.trim();
    if (!trimmed || sending || inputLocked || !connected) return;
    const state = useStore.getState();
    const sessionId = state.currentSessionId;
    const sessionPath = state.currentSessionPath;
    ...
    const clientMessageId = createClientUserMessageId();
    useStore.getState().appendOptimisticUserMessage(sessionPath, {
      id: clientMessageId,
      role: 'user',
      text: trimmed,
      textHtml: renderMarkdown(trimmed),
      timestamp: Date.now(),
      sendStatus: 'pending',
    });
    const ws = getWebSocket();
    if (!ws) {
      useStore.getState().markOptimisticUserMessageFailed(sessionPath, clientMessageId, 'websocket_unavailable');
      return;
    }
    try {
      ws.send(JSON.stringify({
        ...
        sessionId,
        sessionPath,
        displayMessage: { text: trimmed },
      }));
      upsertOptimisticSessionFirstMessage(sessionPath, trimmed, new Date().toISOString());
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      useStore.getState().markOptimisticUserMessageFailed(sessionPath, clientMessageId, message);
    }
  }, [connected, inputLocked, sending]);

  /** 重试：重发当前 session 最后一条用户消息。 */
  const handleRetryTurn = useCallback(() => {
    ...
    for (let i = items.length - 1; i >= 0; i--) {
      ...
      if (item?.type === 'message' && item.data?.role === 'user'
        ...
        void sendPromptText(item.data.text);
        return;
      }
    }
  }, [sendPromptText]);

  /** 继续任务：注入一条“接着未完成任务继续”的 prompt。 */
  const handleContinueTask = useCallback(() => {
    void sendPromptText(t('chat.continueTaskPrompt'));
  }, [sendPromptText, t]);
  ...
  return (
    ...
    >
      ...
      <InputStatusBars
        ...
        onDismissError={currentSessionPath ? () => useStore.getState().clearInlineError(currentSessionPath) : undefined}
      ...
      <div className={styles['input-stack']}>
        ...
        <div className={styles['input-wrapper']} ref={inputCardRef}>
          ...
          >
            {/* 语音活跃时: overlay 作为顶部状态条(状态标签+当前半句预览+波形)。
                听写模式编辑器保持可见,已确认句子像普通输入一样逐行持续展示;
                对话模式隐藏编辑器避免手动输入干扰,overlay 占据编辑器位置。 */}
            {voiceDictationState?.active && (
              <VoiceInputOverlay
                state={voiceDictationState.voiceState}
                mode={voiceDictationState.mode}
                interimText={voiceDictationState.interimText}
                errorText={voiceDictationState.errorText}
                dictationHint={voiceDictationState.dictationHint}
                conversationHint={voiceDictationState.conversationHint}
                t={t}
                fullscreen={fullscreen}
              />
            )}
            <div style={{ display: voiceDictationState?.active && voiceDictationState.mode === 'conversation' ? 'none' : '' }}>
          ...
          <InputControlBar
            ...
            onSteer={handleSteer}
            onStop={handleStop}
            voiceModePills={voiceDictationState ? (
              <VoiceModePills
                mode={voiceDictationState.mode}
                voiceState={voiceDictationState.voiceState}
                hasConversationTarget={voiceDictationState.hasConversationTarget}
                disabled={inputLocked || isStreaming}
                onToggle={voiceDictationState.toggle}
                onSwitchMode={voiceDictationState.switchMode}
                t={t}
                variant="icon"
                fullscreen={fullscreen}
              />
          ...
          />
          {/* 语音状态管理器(不渲染 UI,仅通过回调暴露状态给父组件) */}
          <VoiceDictationBar
            t={t}
            disabled={inputLocked || isStreaming}
            onFinalText={handleDictationFinal}
            onVoiceStateChange={setVoiceDictationState}
          />
