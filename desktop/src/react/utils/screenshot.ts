...
async function buildScreenshotPayloadForMessages(
  ...
  const assistantName = state.agentName || 'Jarvis';
...
export async function takeScreenshot(targetMessageId: string, sessionPath: string): Promise<void> {
  ...
  // 2. 读取截图设置
  const color = localStorage.getItem('jarvis-screenshot-color') || 'light';
  const width = localStorage.getItem('jarvis-screenshot-width') || 'mobile';
  const theme = buildThemeName(color, width);

  const t = window.t ?? ((p: string) => p);
  const jarvis = (window as any).jarvis;
  if (!jarvis?.screenshotRender) {
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i += 1) {
      ...
      const result = await jarvis.screenshotRender(payload) as ScreenshotRenderResult;
      if (!result.success) {
...
export async function takeArticleScreenshot(markdown: string, options: ArticleScreenshotOptions = {}): Promise<void> {
  ...
  try {
    const result = await jarvis.screenshotRender({
      mode: 'article',
      theme,
      markdown,
...
async function resolveAssistantFallbackAvatar(state: StoreSnapshot): Promise<string | null> {
  ...
  try {
    ...
    return userFallbackAvatar(state.agentName || 'Jarvis');
  }
}

async function fetchAvatarAsDataUrl(role: string, agentId: string | null): Promise<string | null> {
  const port = await (window as any).jarvis?.getServerPort?.();
  const token = await (window as any).jarvis?.getServerToken?.();
