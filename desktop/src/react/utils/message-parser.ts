...
const TAG_TO_YUAN: Record<string, string> = { mood: 'jarvis', pulse: 'butter', reflect: 'ming' };
// 当前块头是静态的；`at <时间戳>` 是历史 JSONL 里的旧块头，剥离端必须继续认
const SESSION_REMINDER_HEADER_RE = /^\[jarvis_reminder(?: at \d{4}-\d{2}-\d{2} \d{2}:\d{2})?\]\r?\n/;
const SESSION_REMINDER_END = '[/jarvis_reminder]';
...
export function parseMoodFromContent(content: string): { mood: string | null; yuan: string | null; text: string } {
  ...
  const yuan = TAG_TO_YUAN[match[1]] || 'jarvis';
