export type BridgePlatform = 'feishu' | 'dingtalk' | 'telegram' | 'whatsapp' | 'qq' | 'wechat' | 'matrix';
...
export const BRIDGE_PLATFORM_DESCRIPTORS: readonly BridgePlatformDescriptor[] = [
  ...
  // IM（Matrix）：通过 IM Tab 登录后自动绑定（POST /api/bridge/matrix/bind），无手动凭证表单
  { id: 'matrix', labelKey: 'settings.bridge.matrix', fallbackLabel: 'IM', panelOrder: 5, settingsEnabled: false },
];
