...
export interface ChatImageModel {
  ...
  compat?: {
    jarvisVideoInput?: boolean;
    jarvisAudioInput?: boolean;
    audioTransport?: string;
    jarvisAudioTransport?: string;
  } | null;
}
...
export function getModelVideoInputMode(model: ChatImageModel | null | undefined): ModelVideoInputMode {
  const explicitVideo = model?.video === true || model?.compat?.jarvisVideoInput === true;
...
export function getModelAudioInputMode(model: ChatImageModel | null | undefined): ModelAudioInputMode {
  ...
  const explicitAudio = model.audio === true || model.compat?.jarvisAudioInput === true;
  const transport = model.audioTransport || model.compat?.audioTransport || model.compat?.jarvisAudioTransport;
  if (explicitAudio) {
