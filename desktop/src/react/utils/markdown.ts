...
function footnoteIdPrefixForSource(src: string): string {
  return `jarvis-fn-${hashStringBase36(src)}`;
}
...
function footnoteDefinitions(md: MarkdownItInstance): void {
  md.block.ruler.before('reference', 'jarvis_footnote_definitions', (
    state: StateBlock,
    startLine: number,
    _endLine: number,
    silent: boolean,
...
function footnoteReferences(md: MarkdownItInstance): void {
  md.inline.ruler.before('link', 'jarvis_footnote_refs', (state: StateInline, silent: boolean) => {
    const start = state.pos;
    if (state.src.slice(start, start + 2) !== '[^') return false;
...
function appendFootnoteList(md: MarkdownItInstance): void {
  md.core.ruler.after('inline', 'jarvis_footnote_tail', (state: StateCore) => {
    const markdownEnv = state.env as MarkdownRenderEnv;
...
function markdownHeadingAnchors(md: MarkdownItInstance): void {
  md.core.ruler.push('jarvis_heading_anchors', (state: StateCore) => {
