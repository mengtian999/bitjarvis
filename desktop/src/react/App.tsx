...
function App() {
  ...
  // IM tab：侧栏保持宽度但变透明（内容隐藏），避免切换时宽度跳变。
  const isImTab = currentTab === 'im';
  ...
  useEffect(() => {
    console.info('[jarvis-launch] init-start');
    initApp()
      .then(() => {
        console.info('[jarvis-launch] init-finished');
      })
      .catch((err: unknown) => {
        console.error('[init] 初始化异常:', err);
        console.error('[jarvis-launch] init-failed', err);
        console.info('[jarvis-launch] app-ready', JSON.stringify({ reason: 'init-failed' }));
  ...
  return (
    <ErrorBoundary>
      ...
      <div className="app-shell">
        ...
        <div className="app">
          <ChatSidebar
            open={sidebarOpen && !isPluginTab && !isImTab}
