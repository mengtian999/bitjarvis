...
export function OnboardingApp({ preview, skipToTutorial }: OnboardingAppProps) {
  ...
  const jarvisFetch: JarvisFetch = useCallback((path, opts = {}) => {
    if (!serverConnection) {
      throw new Error(`onboarding jarvisFetch ${path}: server connection not ready`);
    }
  ...
  const connectLanServer = useCallback(async (baseUrl: string, credential: string) => {
    ...
    if (!preview) {
      await window.jarvis.onboardingComplete?.();
    }
  }, [preview]);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const port = await window.jarvis.getServerPort();
        const token = await window.jarvis.getServerToken();
        ...
        setServerConnection(connection);
        const splashInfo = await window.jarvis.getSplashInfo?.();
        ...
        try {
          const localPath = await window.jarvis.getAvatarPath?.('agent');
          if (localPath) setAvatarSrc(window.platform?.getFileUrl?.(localPath) ?? '');
        } catch { /* ignore */ }
        if (!preview) {
          const connectionFetch: JarvisFetch = (path, opts = {}) => {
  ...
  return (
    <div className="onboarding">
      ...
      {activeStep === 0 && <LocaleStep key={`step-0-${stepKey}`} preview={preview} jarvisFetch={jarvisFetch} agentId={agentId} verificationPlan={verificationPlan} avatarSrc={avatarSrc} initialLocale={locale} goToStep={goToStep} showError={showError} onLocaleChange={onLocaleChange} onConnectLanServer={connectLanServer} />}
