...
export type JarvisFetch = (path: string, opts?: RequestInit) => Promise<Response>;
...
export async function resolveOnboardingAgentId(jarvisFetch: JarvisFetch): Promise<string> {
  const res = await jarvisFetch('/api/agents?fresh=1');
...
export async function verifyOnboardingPersistence({
  ...
  const configRes = await jarvisFetch(agentConfigPath(agentId));
  ...
  const preferencesRes = await jarvisFetch('/api/preferences/models');
...
export async function testConnection({ jarvisFetch, providerUrl, providerApi, apiKey }: TestConnectionParams): Promise<TestResult> {
  const res = await jarvisFetch('/api/providers/test', {
    method: 'POST',
...
export async function saveProvider({
  ...
  const res = await jarvisFetch(agentConfigPath(agentId), {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export async function applyGatewayDefaultModels({
  ...
  const utilRes = await jarvisFetch('/api/preferences/models', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(utilityPatch),
  });
  await requireMutation(utilRes, 'Saving utility models');
  recordPreferenceModels(verificationPlan, utilityPatch);
}
...
export async function loadModels({ jarvisFetch, providerName, providerUrl, providerApi, apiKey }: LoadModelsParams): Promise<LoadModelsResult> {
  const res = await jarvisFetch('/api/providers/fetch-models', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      name: providerName,
...
export async function saveModel({
  ...
  const chatModelRes = await jarvisFetch(agentConfigPath(agentId), {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
  ...
  const providerModelsRes = await jarvisFetch(agentConfigPath(agentId), {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export async function loadDefaultWorkspace(jarvisFetch: JarvisFetch): Promise<string> {
  const res = await jarvisFetch('/api/config/default-workspace');
...
async function ensureDefaultWorkspace(jarvisFetch: JarvisFetch): Promise<string> {
  const res = await jarvisFetch('/api/config/default-workspace', { method: 'POST' });
...
export async function saveWorkspace({
  ...
  if (selected === defaultPath) {
    await ensureDefaultWorkspace(jarvisFetch);
  }
