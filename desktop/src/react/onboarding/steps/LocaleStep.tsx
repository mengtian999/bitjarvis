...
export function LocaleStep({
  preview, jarvisFetch, agentId, verificationPlan, avatarSrc, initialLocale,
  ...
  const onNext = useCallback(async () => {
    if (!preview) {
      try {
        await saveLocale(jarvisFetch, agentId, locale, verificationPlan);
      } catch (err) {
    ...
    goToStep(1);
  }, [preview, jarvisFetch, agentId, locale, verificationPlan, goToStep, showError]);
