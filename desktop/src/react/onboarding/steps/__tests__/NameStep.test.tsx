...
describe('NameStep', () => {
  ...
  it('renders user and agent placeholders with memory enabled by default', () => {
    render(<NameStep preview jarvisFetch={vi.fn<JarvisFetch>()} agentId="jarvis-primary" verificationPlan={createOnboardingVerificationPlan()} goToStep={vi.fn()} showError={vi.fn()} />);
  ...
  it('saves identity and memory settings before moving to provider setup', async () => {
    const jarvisFetch = vi.fn<JarvisFetch>(async () => ({ ok: true, status: 200, json: async () => ({ ok: true }) } as Response));
    const goToStep = vi.fn();

    render(<NameStep preview={false} jarvisFetch={jarvisFetch} agentId="jarvis-primary" verificationPlan={createOnboardingVerificationPlan()} goToStep={goToStep} showError={vi.fn()} />);
    fireEvent.change(screen.getByPlaceholderText('你的名字'), { target: { value: '测试用户' } });
    fireEvent.change(screen.getByPlaceholderText('小花'), { target: { value: 'Jarvis' } });
