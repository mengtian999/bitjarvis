...
export function WorkspaceStep({ preview, jarvisFetch, agentId, verificationPlan, goToStep, showError, backStep = 3 }: WorkspaceStepProps) {
  ...
  useEffect(() => {
    if (preview) return;
    let cancelled = false;
    loadDefaultWorkspace(jarvisFetch)
      .then(path => {
        if (cancelled) return;
        setDefaultPath(path);
      })
  ...
  const onNext = useCallback(async () => {
    ...
    setSaving(true);
    try {
      await saveWorkspace({ jarvisFetch, agentId, workspacePath: visiblePath, defaultPath, verificationPlan });
      goToStep(5);
    } catch (err) {
  ...
  }, [preview, goToStep, defaultPath, visiblePath, jarvisFetch, agentId, verificationPlan, showError]);
