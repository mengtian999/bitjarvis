...
import type { AddedModelEntry, AddedModelObject, DiscoveredModel, JarvisFetch, OnboardingVerificationPlan } from '../onboarding-actions';
...
export function ModelStep({
  preview, jarvisFetch, agentId, verificationPlan, providerName, providerUrl, providerApi, apiKey,
  goToStep, showError,
}: ModelStepProps) {
  ...
  useEffect(() => {
    const doLoad = async () => {
      ...
      try {
        const result = await loadModelsAction({ jarvisFetch, providerName, providerUrl, providerApi, apiKey });
        if (result.error) {
    ...
    doLoad();
  }, [preview, jarvisFetch, providerName, providerUrl, providerApi, apiKey]);
  ...
  const onNext = useCallback(async () => {
    ...
    try {
      await saveModelAction({
        jarvisFetch, agentId, selectedModel, providerName,
  ...
  }, [preview, canContinue, jarvisFetch, agentId, selectedModel, providerName, addedModels, selectedUtility, selectedUtilityLarge, verificationPlan, goToStep, showError]);
