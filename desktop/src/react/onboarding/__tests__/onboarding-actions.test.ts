...
describe('onboarding agent resolution', () => {
  it('uses the renamed primary agent instead of a hardcoded jarvis id', async () => {
    const jarvisFetch = vi.fn<JarvisFetch>(async path => {
    ...
    const agentId = await resolveOnboardingAgentId(jarvisFetch);
    ...
    expect(jarvisFetch).toHaveBeenLastCalledWith('/api/agents/renamed-primary/config', expect.objectContaining({
      method: 'PUT',
    }));
  });
...
describe('onboarding saveModel', () => {
  it('persists only models the user explicitly added to the provider', async () => {
    const jarvisFetch = vi.fn<JarvisFetch>(async () => jsonResponse({ ok: true }));
    ...
    const providerSaveCall = jarvisFetch.mock.calls.find(([path, options]) => {
      const body = JSON.parse(String(options?.body));
      return path === '/api/agents/jarvis-primary/config' && body.providers;
    });
...
describe('onboarding saveOnboardingIdentity', () => {
  it('persists user name, optional agent name, and the memory master switch together', async () => {
    ...
    expect(jarvisFetch).toHaveBeenCalledWith('/api/agents/jarvis-primary/config', {
      method: 'PUT',
  ...
  it('keeps the current agent name when the agent name input is left blank', async () => {
    ...
    const body = JSON.parse(String(jarvisFetch.mock.calls[0][1]?.body));
    expect(body).toEqual({
      user: { name: '测试用户' },
...
describe('onboarding saveWorkspace', () => {
  it('creates the default workspace before saving the agent desk config', async () => {
    ...
    await saveWorkspace({
      jarvisFetch,
      agentId: 'jarvis-primary',
      workspacePath: '/Users/test/Desktop/JARVIS-WorkSpace',
      defaultPath: '/Users/test/Desktop/JARVIS-WorkSpace',
    });

    expect(jarvisFetch).toHaveBeenNthCalledWith(1, '/api/config/default-workspace', {
      method: 'POST',
    });
    expect(jarvisFetch).toHaveBeenNthCalledWith(2, '/api/agents/jarvis-primary/config', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        desk: {
          home_folder: '/Users/test/Desktop/JARVIS-WorkSpace',
...
describe('onboarding mutation failures', () => {
  it('rejects a non-2xx response with the server error instead of continuing', async () => {
    const jarvisFetch = vi.fn<JarvisFetch>(async () => jsonResponse({ error: 'settings were not written' }, 500));

    await expect(saveLocale(jarvisFetch, 'jarvis-primary', 'zh-CN')).rejects.toThrow('settings were not written');
  });
});
...
describe('onboarding persistence verification', () => {
  it('reads back acknowledged provider settings without retaining the raw API key', async () => {
    ...
    const jarvisFetch = vi.fn<JarvisFetch>(async (path, options) => {
    ...
    await verifyOnboardingPersistence({ jarvisFetch, agentId: 'renamed-primary', verificationPlan });
