...
describe('OnboardingApp locale switching', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal('jarvis', {
      ...
      getSplashInfo: vi.fn(async () => ({ locale: 'zh-CN', agentName: 'Jarvis' })),
    ...
    vi.stubGlobal('fetch', vi.fn(async (url: string) => {
      if (url === 'http://127.0.0.1:62950/api/agents?fresh=1') {
        return {
          ok: true,
          status: 200,
          json: async () => ({ agents: [{ id: 'jarvis-primary', isPrimary: true, isCurrent: true }] }),
        } as Response;
      }
  ...
  it('lets first-run users connect to an existing LAN server from the welcome page', async () => {
    ...
    await waitFor(() => {
      expect(window.jarvis.onboardingComplete).toHaveBeenCalledTimes(1);
    });
