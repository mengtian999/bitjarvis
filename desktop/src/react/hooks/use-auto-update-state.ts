...
function devWebPreviewState(): AutoUpdateState | null {
  if (!window.__JARVIS_DEV_WEB__) return null;
  const params = new URLSearchParams(window.location.search);
  if (params.get('jarvis_update_preview') !== 'downloaded') return null;
  return {
    status: 'downloaded',
    version: params.get('jarvis_update_version') || '0.237.14',
    releaseNotes: null,
    releaseUrl: null,
    downloadUrl: null,
    progress: null,
    ...
    digest: {
      schemaVersion: 1,
      tag: `v${params.get('jarvis_update_version') || '0.237.14'}`,
      version: params.get('jarvis_update_version') || '0.237.14',
    ...
    digestUrl: null,
    digestError: null,
    updateSource: { provider: 'github', owner: 'mengtian999', repo: 'Bitjarvis' },
  };
}
...
export function useAutoUpdateState(): AutoUpdateState | null {
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.jarvis?.onAutoUpdateState?.((nextState) => {
      setState(nextState);
    });
