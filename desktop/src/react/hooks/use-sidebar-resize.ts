...
export function useSidebarResize(): void {
  ...
  useEffect(() => {
    ...
    // 恢复保存的宽度
    const savedLeft = localStorage.getItem('jarvis-sidebar-width');
    const savedRight = localStorage.getItem('jarvis-jian-width');
    const savedChannelInspector = localStorage.getItem('jarvis-channel-inspector-width');
    const savedPreview = localStorage.getItem('jarvis-preview-width');
    ...
    setupHandle(
      ...
      LEFT_MIN, LEFT_MAX, 'jarvis-sidebar-width', false,
    );
    ...
    setupHandle(
      ...
      (w) => applyJianWidth(w),
      RIGHT_MIN, RIGHT_MAX, 'jarvis-jian-width', true,
    );
    ...
    setupHandle(
      ...
      CHANNEL_INSPECTOR_MIN, CHANNEL_INSPECTOR_MAX, 'jarvis-channel-inspector-width', true,
    );
    ...
    setupHandle(
      ...
      PREVIEW_MIN, getPreviewMaxWidth, 'jarvis-preview-width', true,
    );
