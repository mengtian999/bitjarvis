...
async function queryStatus(): Promise<TrainUpdateStatus | null> {
  try {
    return (await window.jarvis?.trainUpdateStatus?.()) ?? null;
  } catch {
    return null;
  }
}
...
export function useTrainUpdateState(): UseTrainUpdateStateResult {
  ...
  useEffect(() => {
    const unsubscribe = window.jarvis?.onTrainUpdateAvailable?.((payload) => {
      setSnapshot((s) => ({
        ...s,
  ...
  useEffect(() => {
    const unsubscribe = window.jarvis?.onTrainUpdateProgress?.((p) => {
  ...
  // "窗口已经在跑时才发生"的那一种。
  useEffect(() => {
    const unsubscribe = window.jarvis?.onTrainFallbackNotice?.((payload) => {
  ...
  const checkNow = useCallback(async () => {
    setPhase('checking');
    try {
      const result = await window.jarvis?.trainUpdateCheck?.();
  ...
  const applyNow = useCallback(async () => {
    ...
    setProgress(null);
    try {
      const result = await window.jarvis?.trainUpdateApply?.();
      if (result && !result.ok) {
        setSnapshot((s) => ({ ...s, lastError: result.error || null }));
      }
      return result;
  ...
  const ackFallbackNotice = useCallback(async () => {
    ...
    try {
      await window.jarvis?.ackTrainFallbackNotice?.();
    } catch {
