...
export async function jarvisFetch(
  ...
  const connection = requireServerConnection(
    useStore.getState(),
    `jarvisFetch ${path}: server connection not ready`,
  );
  ...
  try {
    ...
    if (throwOnHttpError && !res.ok) {
      throw new Error(`jarvisFetch ${path}: ${res.status} ${res.statusText}`);
    }
    return res;
  } finally {
    clearTimeout(timer);
