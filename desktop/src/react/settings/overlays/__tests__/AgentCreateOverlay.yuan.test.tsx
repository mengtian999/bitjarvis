...
vi.mock("../../api", () => ({ jarvisFetch: vi.fn(), isJarvisFetchTimeout: () => false }));
vi.mock("../../actions", () => ({ loadAgents: vi.fn(), switchToAgent: vi.fn() }));
...
async function openOverlay() {
  ...
  window.dispatchEvent(new Event("jarvis-show-agent-create"));
...
function descFor(container: HTMLElement, yuan: string): string | undefined {
  ...
  const targetName = yuan === "jarvis" ? "jarvis" : yuan;
  const chip = chips.find((el) => el.querySelector(".yuan-chip-name")?.textContent === targetName);
