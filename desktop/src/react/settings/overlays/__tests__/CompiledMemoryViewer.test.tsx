...
describe('CompiledMemoryViewer editable facts', () => {
  beforeEach(() => {
    ...
    useSettingsStore.setState({
      ...
      agents: [{ id: 'jarvis', name: 'Jarvis', isPrimary: true }],
    } as never);
    vi.mocked(jarvisFetch).mockImplementation(async (url: string, init?: RequestInit) => {
      if (url === '/api/memories/compiled?agentId=jarvis' && !init) {
      ...
      if (url === '/api/memories/compiled/week/days?agentId=jarvis' && !init) {
        return new Response(JSON.stringify({
          days: [
      ...
      if (url === '/api/memories/compiled/facts?agentId=jarvis' && init?.method === 'PUT') {
        return new Response(JSON.stringify({ ok: true, facts: '用户喜欢清晰边界。\n用户关注记忆系统。' }));
      }
      if (url === '/api/memories/compiled/today?agentId=jarvis' && init?.method === 'PUT') {
        return new Response(JSON.stringify({ ok: true, today: '编辑后的今天。' }));
      }
      if (url === '/api/memories/compiled/longterm?agentId=jarvis' && init?.method === 'PUT') {
        return new Response(JSON.stringify({ ok: true, longterm: '编辑后的长期情况。' }));
      }
      if (url === '/api/memories/compiled/week/days/2026-07-01?agentId=jarvis' && init?.method === 'PUT') {
  ...
  it('renders read-only memory sections outside edit mode', async () => {
    ...
    window.dispatchEvent(new Event('jarvis-view-compiled-memory'));
  ...
  it('switches into edit mode with four editable sections and saves them from the header', async () => {
    ...
    await waitFor(() => {
      expect(jarvisFetch).toHaveBeenCalledWith('/api/memories/compiled/today?agentId=jarvis', {
        method: 'PUT',
    ...
    await waitFor(() => {
      expect(jarvisFetch).toHaveBeenCalledWith('/api/memories/compiled/facts?agentId=jarvis', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
    ...
    await waitFor(() => {
      expect(jarvisFetch).toHaveBeenCalledWith('/api/memories/compiled/longterm?agentId=jarvis', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
    ...
    await waitFor(() => {
      expect(jarvisFetch).toHaveBeenCalledWith('/api/memories/compiled/week/days/2026-07-01?agentId=jarvis', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
