...
export function CompiledMemoryViewer() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis-view-compiled-memory', handler);
    return () => window.removeEventListener('jarvis-view-compiled-memory', handler);
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await jarvisFetch(`/api/memories/compiled?agentId=${aid}`);
  ...
  const loadWeekDays = async (aid: string | null) => {
    try {
      const res = await jarvisFetch(`/api/memories/compiled/week/days?agentId=${aid}`);
      const data = await res.json();
  ...
  const clearCompiled = async () => {
    try {
      const aid = useSettingsStore.getState().getSettingsAgentId();
      await jarvisFetch(`/api/memories/compiled?agentId=${aid}`, { method: 'DELETE' });
  ...
  const saveCompiledEdits = async () => {
    ...
    try {
      ...
      if (changedWeekDays.length > 0) {
        const refreshed = await jarvisFetch(`/api/memories/compiled?agentId=${aid}`);
