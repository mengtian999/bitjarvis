...
export function CharacterCardPreviewOverlay({
  ...
  const yuanKey = (plan.agent.yuan || 'jarvis').toLowerCase();
  ...
  const assetUrl = (key: string) => (
    plan.assets?.[key]
      ? mode === 'export' && plan.agentId
        ? jarvisUrl(`/api/character-cards/export/${encodeURIComponent(plan.agentId)}/assets/${key}`)
        : plan.token
          ? jarvisUrl(`/api/character-cards/plans/${plan.token}/assets/${key}`)
          : ''
      : ''
  );
