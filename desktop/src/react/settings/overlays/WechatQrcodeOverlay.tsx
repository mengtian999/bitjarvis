...
export function WechatQrcodeOverlay() {
  ...
  const fetchQrcode = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await jarvisFetch('/api/bridge/wechat/qrcode', { method: 'POST' });
  ...
  const startPolling = useCallback((id: string) => {
    ...
    (async () => {
      while (!stoppedRef.current) {
        try {
          const res = await jarvisFetch('/api/bridge/wechat/qrcode-status', {
            method: 'POST',
          ...
          if (data.status === 'scanned') {
            ...
            if (data.userId) {
              await jarvisFetch(`/api/bridge/owner${agentQuery}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis-show-wechat-qrcode', show);
    return () => {
      window.removeEventListener('jarvis-show-wechat-qrcode', show);
      stopPolling();
    };
