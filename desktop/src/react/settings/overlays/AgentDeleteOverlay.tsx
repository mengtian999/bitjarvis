...
export function AgentDeleteOverlay() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis-show-agent-delete', handler);
    return () => window.removeEventListener('jarvis-show-agent-delete', handler);
  }, []);
  ...
  const confirmDelete = async () => {
    ...
    setError('');
    try {
      const res = await jarvisFetch(`/api/agents/${targetId}`, { method: 'DELETE' });
