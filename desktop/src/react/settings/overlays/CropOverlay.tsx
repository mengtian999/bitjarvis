...
export function CropOverlay() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis-open-cropper', handler);
    return () => window.removeEventListener('jarvis-open-cropper', handler);
  }, []);
...
async function uploadCroppedAvatar(role: string, dataUrl: string) {
  ...
  try {
    ...
    const res = await jarvisFetch(uploadUrl, {
      method: 'POST',
    ...
    if (role === 'agent') {
      ...
      const url = jarvisUrl(`/api/avatar/${role}?t=${ts}`);
