...
export function ClearMemoryConfirm() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis-show-clear-confirm', handler);
    return () => window.removeEventListener('jarvis-show-clear-confirm', handler);
  }, []);
  ...
  const doClear = async () => {
    ...
    try {
      ...
      const res = await jarvisFetch(`/api/memories?agentId=${aid}`, { method: 'DELETE' });
