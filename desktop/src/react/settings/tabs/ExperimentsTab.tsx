...
function SwitchButton({
  ...
  return (
    <button
      ...
      className={`jarvis-toggle${checked ? ' on' : ''}${disabled ? ' disabled' : ''}`}
      disabled={disabled}
      onClick={onClick}
    />
  );
...
function CacheSnapshotExperiment({ experiment, onValueChange }: {
  ...
  const loadObservation = async () => {
    ...
    const res = await jarvisFetch(observationUrl);
  ...
  const clearObservation = async () => {
    if (!observationUrl) return;
    await jarvisFetch(observationUrl, { method: 'DELETE' });
    setObservation(null);
  };
...
export function ExperimentsTab() {
  ...
  const updateExperimentValue = async (id: string, value: unknown) => {
    const res = await jarvisFetch(`/api/experiments/${id}`, {
      method: 'PATCH',
