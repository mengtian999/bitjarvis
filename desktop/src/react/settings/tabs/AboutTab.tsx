...
function InviteChannelSection() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const next = await jarvis?.inviteStatus?.();
  ...
  const handleRedeem = useCallback(async () => {
    ...
    setErrorDetail(null);
    try {
      const result = await jarvis?.inviteRedeem?.(trimmed);
      if (!result) {
...
export function AboutTab() {
  ...
  const handleCheck = useCallback(() => {
    void jarvis?.autoUpdateCheck?.();
    void checkTrainNow();
  }, [checkTrainNow, jarvis]);
  ...
  const handleInstallShell = useCallback(async () => {
    await jarvis?.autoUpdateInstall?.();
  }, [jarvis]);
  ...
  return (
    <div className={`${styles['settings-tab-content']} ${styles['active']}`} data-tab="about">
      ...
      <div className={styles['about-hero']}>
        <img className={styles['about-icon']} src={appIconUrl} alt="Bit Jarvis" />
        <div className={styles['about-name']}>Bit Jarvis</div>
      ...
      <SettingsSection>
        ...
        <SettingsRow
          ...
          control={<span>© 2026 mengtian</span>}
        />
        <SettingsRow
          label="GitHub"
          control={
            <a
              ...
              onClick={(e) => {
                e.preventDefault();
                jarvis?.openExternal?.('https://github.com/mengtian999/Bitjarvis');
              }}
            >
              github.com/mengtian999/Bitjarvis
