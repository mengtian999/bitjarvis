...
describe('ComputerUseSection', () => {
  it('hydrates the enabled switch from the unified settings snapshot before refresh completes', async () => {
    ...
    jarvisFetchMock.mockImplementation(() => new Promise<Response>((resolve) => {
      resolveLoad = resolve;
    }));
    useSettingsStore.setState({
      settingsSnapshot: {
    ...
    await waitFor(() => expect(jarvisFetchMock).toHaveBeenCalledWith('/api/preferences/computer-use'));
  });

  it('renders the experimental risk warning near the top of the Computer Use page', async () => {
    jarvisFetchMock.mockResolvedValue(jsonResponse({
      selectedProviderId: 'macos:cua',
  ...
  it('shows a toast when requesting permissions fails', async () => {
    jarvisFetchMock
      ...
      .mockRejectedValueOnce(new Error('jarvisFetch /api/preferences/computer-use/request-permissions: 400 Bad Request'));
