...
const jarvisFetch = vi.fn();

vi.mock('../../api', () => ({
  jarvisFetch: (...args: unknown[]) => jarvisFetch(...args),
}));
...
function mockConfigurablePlugin(initialValues: Record<string, unknown> = {
  ...
  jarvisFetch.mockImplementation(async (path: string, options?: RequestInit) => {
...
describe('PluginsTab', () => {
  beforeEach(() => {
    jarvisFetch.mockResolvedValue(jsonResponse([]));
  ...
  it('preserves Backspace and multiline Enter edits, then saves parsed array and object values', async () => {
    ...
    expect(tags).toHaveValue('[\n  "alpha"\n');
    expect(jarvisFetch.mock.calls.filter(([, options]) => options?.method === 'PUT')).toHaveLength(0);
    ...
    await waitFor(() => expect(jarvisFetch.mock.calls.some(([, options]) => options?.method === 'PUT')).toBe(true));
