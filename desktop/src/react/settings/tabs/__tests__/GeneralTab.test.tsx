...
describe('GeneralTab', () => {
  ...
  it('renders three completion categories and saves the chat preference through the notification route', async () => {
    ...
    await waitFor(() => expect(jarvisFetch).toHaveBeenLastCalledWith('/api/preferences/notifications', {
      method: 'PUT',
  ...
  it('records and registers the quick chat shortcut', async () => {
    ...
    await waitFor(() => expect(jarvisFetch).toHaveBeenLastCalledWith('/api/preferences/quick-chat', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
