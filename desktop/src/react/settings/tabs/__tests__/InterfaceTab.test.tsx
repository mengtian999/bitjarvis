...
function setAppearanceGlobals() {
  ...
  (globalThis as AppearanceGlobals).setPaperTexture = vi.fn((enabled: boolean) => {
    localStorage.setItem('jarvis-paper-texture', enabled ? '1' : '0');
  });
}
...
describe('InterfaceTab appearance state', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    jarvisFetchMock.mockImplementation(async (path: string, init?: RequestInit) => {
  ...
  it('updates the reading font card from component state after the preference changes', () => {
    localStorage.setItem('jarvis-font-serif', '1');
    ...
    expect(localStorage.getItem('jarvis-font-serif')).toBe('0');
  });

  it('recomputes paper texture availability when the selected theme changes', () => {
    localStorage.setItem('jarvis-theme', registry.DEFAULT_THEME);
    localStorage.setItem('jarvis-paper-texture', '1');
