...
type AccessMode = 'loopback' | 'lan' | 'remote';
...
export function AccessTab() {
  ...
  const loadSummary = useCallback(async () => {
    ...
    try {
      const res = await jarvisFetch('/api/access/summary');
  ...
  const qrUrl = useMemo(() => {
    ...
    const query = summary?.network.actualPort
      ...
      : '';
    return jarvisUrl(`/api/access/mobile-qr.svg${query}`);
  ...
  const saveNetworkSettings = useCallback(async (nextMode: AccessMode, nextPort: string) => {
    ...
    try {
      const res = await jarvisFetch('/api/access/network', {
        method: 'PUT',
  ...
  const generateMobileKey = useCallback(async () => {
    ...
    try {
      const res = await jarvisFetch('/api/access/mobile-credentials', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const generateDesktopKey = useCallback(async () => {
    ...
    try {
      const res = await jarvisFetch('/api/access/desktop-credentials', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const connectRemoteServer = useCallback(async () => {
    ...
    try {
      ...
      window.jarvis?.reloadMainWindow?.();
    } catch (err: any) {
  ...
  const saveAccount = useCallback(async () => {
    try {
      const res = await jarvisFetch('/api/access/account/profile', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const savePassword = useCallback(async () => {
    try {
      const res = await jarvisFetch('/api/access/account/password', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const clearPassword = useCallback(async () => {
    try {
      const res = await jarvisFetch('/api/access/account/password', { method: 'DELETE' });
      const data = await res.json();
      setSummary(prev => prev ? { ...prev, account: data.account } : prev);
      setPasswordDraft('');
  ...
  const revokeDevice = useCallback(async (deviceId: string) => {
    try {
      await jarvisFetch(`/api/devices/${encodeURIComponent(deviceId)}/revoke`, { method: 'POST' });
      await loadSummary();
  ...
  const revokeCredential = useCallback(async (credentialId: string) => {
    try {
      await jarvisFetch(`/api/devices/credentials/${encodeURIComponent(credentialId)}/revoke`, { method: 'POST' });
      await loadSummary();
