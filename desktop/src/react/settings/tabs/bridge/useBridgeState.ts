...
export function useBridgeState() {
  ...
  useEffect(() => {
    ...
    jarvisFetch(`/api/agents/${selectedAgentId}/public-ishiki`, { signal: ac.signal })
      .then(r => r.json())
  ...
  const savePublicIshiki = async () => {
    ...
    try {
      await jarvisFetch(`/api/agents/${agentId}/public-ishiki`, {
        method: 'PUT',
  ...
  const fetchStatusForAgent = useCallback(async (
    ...
    try {
      const res = await jarvisFetch(`/api/bridge/status?agentId=${encodeURIComponent(agentId)}`, signal ? { signal } : undefined);
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis-bridge-reload', handler);
    return () => window.removeEventListener('jarvis-bridge-reload', handler);
  ...
  const saveBridgeConfig = async (plat: string, credentials: Record<string, string> | null, enabled?: boolean) => {
    ...
    try {
      ...
      await jarvisFetch(`/api/bridge/config${agentQuery}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const testPlatform = async (
    ...
    try {
      const agentQuery = agentId ? `?agentId=${encodeURIComponent(agentId)}` : '';
      const res = await jarvisFetch(`/api/bridge/test${agentQuery}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const setOwner = async (plat: string, userId: string) => {
    ...
    try {
      const agentQuery = agentId ? `?agentId=${encodeURIComponent(agentId)}` : '';
      const res = await jarvisFetch(`/api/bridge/owner${agentQuery}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const saveGlobalSettings = async (partial: { permissionMode?: BridgePermissionMode; readOnly?: boolean; receiptEnabled?: boolean; richStreamingEnabled?: boolean }) => {
    ...
    try {
      const res = await jarvisFetch('/api/bridge/settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
