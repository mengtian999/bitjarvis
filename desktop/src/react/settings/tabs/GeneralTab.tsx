...
export function GeneralTab() {
  ...
  useEffect(() => {
    ...
    let alive = true;
    jarvisFetch('/api/preferences/quick-chat')
      .then(res => res.json())
      .then((data) => {
        if (!alive) return;
  ...
  useEffect(() => {
    ...
    let alive = true;
    jarvisFetch('/api/preferences/notifications')
      .then(res => res.json())
      .then((data) => {
        if (!alive) return;
  ...
  const saveQuickChatPreferences = useCallback(async (
    ...
    try {
      const res = await jarvisFetch('/api/preferences/quick-chat', {
        method: 'PUT',
      ...
      if (options.reloadShortcut) {
        const registration = await jarvis?.quickChatReloadShortcut?.();
        if (registration && registration.ok === false) {
          throw new Error(registration.error || t('settings.general.quickChat.registrationFailed'));
        }
      }
      if (options.eventName) jarvis?.settingsChanged?.(options.eventName, { quickChat: saved });
    } catch (err: any) {
      setQuickChatPrefs(previous);
      try {
        await jarvisFetch('/api/preferences/quick-chat', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ quickChat: previous }),
        });
        if (options.reloadShortcut) await jarvis?.quickChatReloadShortcut?.();
      } catch {}
      showToast(t('settings.saveFailed') + ': ' + (err?.message || String(err)), 'error');
    } finally {
      setQuickChatSaving(false);
    }
  }, [jarvis, quickChatPrefs, showToast]);
  ...
  const handleAutoLaunchToggle = useCallback(async (on: boolean) => {
    if (!jarvis?.setAutoLaunchEnabled) return;
    const previous = autoLaunch;
    setAutoLaunchSaving(true);
    try {
      const next = await jarvis.setAutoLaunchEnabled(on);
  ...
  const saveNotificationPreference = useCallback(async (
    ...
    try {
      const res = await jarvisFetch('/api/preferences/notifications', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        ...
        <SettingsRow
          ...
          control={
            <Toggle
              ...
              disabled={keepAwakeSaving || !jarvis?.setKeepAwakeEnabled}
            />
          }
        />
      </SettingsSection>
