...
export function SkillsTab() {
  ...
  const loadSkills = useCallback(async () => {
    ...
    try {
      ...
      const [skillsRes, bundlesRes] = await Promise.all([
        jarvisFetch(`/api/skills?agentId=${encodeURIComponent(agentId)}&runtime=1`),
        jarvisFetch(`/api/skills/bundles?agentId=${encodeURIComponent(agentId)}`),
      ]);
  ...
  const loadExternalPaths = useCallback(async () => {
    try {
      const res = await jarvisFetch('/api/skills/external-paths');
  ...
  useEffect(() => {
    ...
    window.addEventListener('jarvis-skills-changed', refresh);
    return () => window.removeEventListener('jarvis-skills-changed', refresh);
  ...
  useEffect(() => {
    ...
    jarvisFetch('/api/skills/translate', {
      method: 'POST',
  ...
  const installSkillFromPath = async (filePath: string) => {
    try {
      const res = await jarvisFetch('/api/skills/install', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const submitCreateBundle = async (name: string) => {
    ...
    if (!agentId) return;
    try {
      const res = await jarvisFetch(`/api/skills/bundles?agentId=${encodeURIComponent(agentId)}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const submitRenameBundle = async (bundle: SkillBundleInfo, name: string) => {
    ...
    try {
      const res = await jarvisFetch(`/api/skills/bundles/${encodeURIComponent(bundle.id)}?agentId=${encodeURIComponent(agentId)}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const exportBundle = async (bundle: SkillBundleInfo) => {
    try {
      const res = await jarvisFetch(`/api/skills/bundles/${encodeURIComponent(bundle.id)}/export`, {
        method: 'POST',
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
  ...
  const submitDeleteBundle = async (bundle: SkillBundleInfo) => {
    try {
      const res = await jarvisFetch(`/api/skills/bundles/${encodeURIComponent(bundle.id)}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
  ...
  const reorderBundles = async (bundleIds: string[]) => {
    ...
    if (!agentId) return;
    try {
      const res = await jarvisFetch(`/api/skills/bundles/order?agentId=${encodeURIComponent(agentId)}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const toggleSkill = async (name: string, enable: boolean) => {
    ...
    try {
      const res = await jarvisFetch(`/api/agents/${encodeURIComponent(agentId)}/skills/${encodeURIComponent(name)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const toggleBundle = async (bundle: SkillBundleInfo, enable: boolean) => {
    ...
    try {
      const res = await jarvisFetch(`/api/agents/${encodeURIComponent(agentId)}/skill-bundles/${encodeURIComponent(bundle.id)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ enabled: enable }),
      });
  ...
  const addExternalPath = async () => {
    ...
    try {
      await jarvisFetch('/api/skills/external-paths', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
