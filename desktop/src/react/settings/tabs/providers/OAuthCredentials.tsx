...
export function OAuthCredentials({ providerId, summary, onRefresh }: {
  ...
  const login = async () => {
    try {
      const res = await jarvisFetch('/api/auth/oauth/start', {
        method: 'POST',
  ...
  const submitCode = async () => {
    ...
    if (!code) return;
    try {
      const res = await jarvisFetch('/api/auth/oauth/callback', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const pollLogin = async (sessionId: string) => {
    for (let i = 0; i < 100; i++) {
      ...
      try {
        const res = await jarvisFetch(`/api/auth/oauth/poll/${sessionId}`);
        const data = await res.json();
        if (data.status === 'done') {
          showToast(t('settings.oauth.success'), 'success');
          setDeviceCode(null);
  ...
  const logout = async () => {
    try {
      await jarvisFetch('/api/auth/oauth/logout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
