...
describe('ProviderModelList', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.jarvisFetch.mockResolvedValue(jsonResponse({ models: [{ id: 'kimi-for-coding' }] }));
  ...
  it('opens fetched models in the add-model dropdown so they can be enabled', async () => {
    ...
    await waitFor(() => expect(mocks.jarvisFetch).toHaveBeenCalledWith('/api/config', expect.objectContaining({
      method: 'PUT',
  ...
  it('writes OAuth custom models through Provider Catalog instead of the legacy preferences route', async () => {
    ...
    expect(mocks.jarvisFetch.mock.calls.some(([url]) => String(url).includes('/auth/oauth/'))).toBe(false);
    expect(onRefresh).toHaveBeenCalled();
  });
  ...
  it('persists discovered model metadata when enabling a fetched model', async () => {
    ...
    await waitFor(() => {
      expect(mocks.jarvisFetch).toHaveBeenCalledWith('/api/config', expect.objectContaining({
        method: 'PUT',
        body: JSON.stringify({
          providers: {
            'custom-vllm': {
  ...
  it('does not serialize untouched capability defaults as explicit false overrides', async () => {
    const onRefresh = vi.fn(async () => {});
    mocks.jarvisFetch.mockResolvedValue(jsonResponse({ models: [] }));
    mocks.lookupModelMeta.mockImplementation((id: unknown, provider: unknown) => {
    ...
    await waitFor(() => {
      const updateCall = mocks.jarvisFetch.mock.calls.find(([url, options]) => (
