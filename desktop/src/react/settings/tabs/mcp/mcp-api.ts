...
export async function loadMcpState(agentId: string): Promise<McpState> {
  const res = await jarvisFetch(`/api/mcp/state?agentId=${encodeURIComponent(agentId)}`);
...
export async function setMcpEnabled(enabled: boolean): Promise<void> {
  const res = await jarvisFetch('/api/mcp/settings/enabled', {
    method: 'PUT',
...
export async function addMcpConnector(input: McpConnectorInput): Promise<void> {
  const res = await jarvisFetch('/api/mcp/connectors', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function addMcpConnectorsBulk(inputs: McpConnectorInput[]): Promise<McpBulkResult[]> {
  const res = await jarvisFetch('/api/mcp/connectors/bulk', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function setMcpDeferSettings(
  ...
  const res = await jarvisFetch('/api/mcp/settings/defer', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export async function updateMcpConnector(connectorId: string, input: McpConnectorInput): Promise<void> {
  const res = await jarvisFetch(`/api/mcp/connectors/${encodeURIComponent(connectorId)}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(input),
  });
...
export async function runMcpConnectorAction(
  ...
  const res = await jarvisFetch(`/api/mcp/connectors/${encodeURIComponent(connectorId)}/${action}`, {
    method: 'POST',
  });
  await jsonOrError(res);
}
...
export async function setAgentMcpConnector(
  ...
  const res = await jarvisFetch(`/api/mcp/agents/${encodeURIComponent(agentId)}/connectors/${encodeURIComponent(connectorId)}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ enabled }),
  });
...
export async function startMcpOAuth(connectorId: string): Promise<{ sessionId: string; url: string }> {
  const res = await jarvisFetch(`/api/mcp/connectors/${encodeURIComponent(connectorId)}/oauth/start`, {
    method: 'POST',
  });
...
export async function pollMcpOAuth(sessionId: string): Promise<{ status: string; error?: string }> {
  const res = await jarvisFetch(`/api/mcp/oauth/poll/${encodeURIComponent(sessionId)}`);
...
export async function cancelMcpOAuth(connectorId: string): Promise<void> {
  const res = await jarvisFetch(`/api/mcp/connectors/${encodeURIComponent(connectorId)}/oauth/cancel`, {
    method: 'POST',
  });
  await jsonOrError(res);
}
...
export async function logoutMcpOAuth(connectorId: string): Promise<void> {
  const res = await jarvisFetch(`/api/mcp/connectors/${encodeURIComponent(connectorId)}/oauth/logout`, {
    method: 'POST',
  });
  await jsonOrError(res);
}
