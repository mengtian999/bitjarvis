...
export function MemorySection({ agentId, hasUtilityModel, memoryEnabled, currentPins }: {
  ...
  useEffect(() => {
    ...
    jarvisFetch(`/api/memories/health?agentId=${encodeURIComponent(agentId)}`, {
        ...
        <div className={hasUtilityModel !== true || memoryEnabled !== true ? 'settings-disabled' : ''}>
          ...
          <div className={styles['settings-subsection']}>
            ...
            <button
              ...
              onClick={() => window.dispatchEvent(new Event('jarvis-view-compiled-memory'))}
            >
          ...
          <div className={styles['settings-subsection']}>
            ...
            <div className={`${styles['memory-actions-row']} ${styles['memory-actions-spaced']}`}>
              <button
                ...
                onClick={() => window.dispatchEvent(new Event('jarvis-view-memories'))}
              >
                {t('settings.memory.actions.view')}
              </button>
              <button
                className={`${styles['memory-action-btn']} ${styles['danger']}`}
                onClick={() => window.dispatchEvent(new Event('jarvis-show-clear-confirm'))}
              >
