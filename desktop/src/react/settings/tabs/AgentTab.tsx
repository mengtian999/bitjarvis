...
import { SelectWidget, Toggle, ProviderIcon, ProviderGroupHeader, selectWidgetStyles, type SelectOption } from '@/ui';
...
import { deriveVoiceCatalog, resolveVoiceLanguage, useVoiceCatalog, voiceLabel } from './voice/voice-catalog';
...
export function AgentTab() {
  ...
  const currentYuan = settingsConfig?.agent?.yuan || 'jarvis';
  ...
  useEffect(() => {
    jarvisFetch('/api/models').then(r => r.json()).then(data => {
  ...
  // 音色：per-agent 覆盖值（agent.voice），未配置时回退全局默认音色。
  const { providers: voiceProviders, config: ttsConfig, ready: voiceReady } = useVoiceCatalog();
  const voiceCatalog = useMemo(
    () => deriveVoiceCatalog(voiceProviders, ttsConfig, resolveVoiceLanguage(ttsConfig)),
    [voiceProviders, ttsConfig],
  );
  const agentVoice = typeof settingsConfig?.agent?.voice === 'string' && settingsConfig.agent.voice.trim()
    ? settingsConfig.agent.voice.trim()
    : '';
  const voiceValue = agentVoice && voiceCatalog.voices.includes(agentVoice) ? agentVoice : '';
  const voiceOptions: SelectOption[] = [
    { value: '', label: t('settings.agent.ttsVoiceDefault') },
    ...voiceCatalog.voices.map(v => ({ value: v, label: voiceLabel(v, voiceCatalog.providerVoices) })),
  ];
  ...
  const saveAgentName = async () => {
    try {
      ...
      const res = await jarvisFetch(`/api/agents/${agentId}/config`, {
        method: 'PUT',
  ...
  const saveAgent = async () => {
    try {
      ...
      if (identityChanged) {
        requests.push(jarvisFetch(`${agentBase}/identity`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (ishikiChanged) {
        requests.push(jarvisFetch(`${agentBase}/ishiki`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const openAgentExportPreview = async (agentId: string) => {
    ...
    try {
      const res = await jarvisFetch('/api/character-cards/export/preview', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const confirmAgentExport = async () => {
    ...
    try {
      const res = await jarvisFetch('/api/character-cards/export', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  return (
    <div className={`${styles['settings-tab-content']} ${styles['active']}`} data-tab="agent">
        ...
        <AgentCardStack
          ...
          onAvatarClick={() => {
            ...
            input.addEventListener('change', () => {
              if (input.files?.[0]) {
                window.dispatchEvent(new CustomEvent('jarvis-open-cropper', {
          ...
          onDelete={(id) => window.dispatchEvent(new CustomEvent('jarvis-show-agent-delete', {
            detail: { agentId: id },
          }))}
          onExport={openAgentExportPreview}
          onAdd={() => window.dispatchEvent(new Event('jarvis-show-agent-create'))}
      ...
      <SettingsSection title={t('settings.about.title')} surface="plain">
        <div className={`${styles['settings-form-field']} ${styles['settings-form-field-center']}`}>
          ...
          <YuanSelector
            ...
            onChange={async (key) => {
              const agentId = getSettingsAgentId()!;
              try {
                await jarvisFetch(`/api/agents/${agentId}/config`, {
                  method: 'PUT',
                  headers: { 'Content-Type': 'application/json' },
        ...
        <SettingsRow
          label={t('settings.agent.ttsVoice')}
          hint={t('settings.agent.ttsVoiceHint')}
          control={(
            <SelectWidget
              value={voiceValue}
              disabled={!voiceReady}
              options={voiceOptions}
              onChange={(val) => void autoSaveConfig({ agent: { voice: val ? val : null } }, { silent: true })}
            />
          )}
        />
