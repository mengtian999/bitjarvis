...
function readAppearancePrefs(): AppearancePrefs {
  ...
  return {
    ...
    serifEnabled: localStorage.getItem('jarvis-font-serif') !== '0',
    paperTextureEnabled: isPaperTextureEnabled(localStorage),
    paperTextureBlocked: isPaperTextureBlockedTheme(concreteTheme),
    leavesOverlayEnabled: localStorage.getItem('jarvis-leaves-overlay') === '1',
  };
}
...
export function InterfaceTab() {
  ...
  useEffect(() => {
    let cancelled = false;
    jarvisFetch('/api/preferences/sidebar-ui')
      .then(res => res.json())
      .then(data => {
        if (cancelled) return;
  ...
  const saveSessionListRowMode = useCallback(async (singleLine: boolean) => {
    ...
    try {
      const res = await jarvisFetch('/api/preferences/sidebar-ui', {
        method: 'PUT',
      ...
      window.dispatchEvent(new CustomEvent('jarvis-settings', {
        ...
        <SettingsRow
          ...
          control={
            <Toggle
              ...
              onChange={(next) => {
                localStorage.setItem('jarvis-leaves-overlay', next ? '1' : '0');
                window.dispatchEvent(new CustomEvent('jarvis-settings', {
