...
const MEDIA_CACHE_IMAGE = '/api/media/image/providers';
const MEDIA_CACHE_VIDEO = '/api/media/video/providers';
const MEDIA_CACHE_SPEECH = '/api/speech-recognition/providers';

function readMediaCache(key: string): any | null {
  const entry = useSettingsStore.getState().mediaCache[key];
  if (entry && Date.now() - entry.at < MEDIA_CACHE_TTL_MS) return entry.data;
  return null;
}

function writeMediaCache(key: string, data: any) {
  const state = useSettingsStore.getState();
  state.set({ mediaCache: { ...state.mediaCache, [key]: { at: Date.now(), data } } });
}

function clearMediaCache(key: string) {
  const state = useSettingsStore.getState();
  const next = { ...state.mediaCache };
  delete next[key];
  state.set({ mediaCache: next });
}
...
export function MediaTab() {
  ...
  const loadImageProviders = useCallback(async () => {
    ...
    try {
      let data: any = readMediaCache(MEDIA_CACHE_IMAGE);
      if (!data) {
        const res = await jarvisFetch(MEDIA_CACHE_IMAGE);
        data = await res.json();
        writeMediaCache(MEDIA_CACHE_IMAGE, data);
      }
  ...
  const loadVideoProviders = useCallback(async () => {
    ...
    try {
      let data: any = readMediaCache(MEDIA_CACHE_VIDEO);
      if (!data) {
        const res = await jarvisFetch(MEDIA_CACHE_VIDEO);
        data = await res.json();
        writeMediaCache(MEDIA_CACHE_VIDEO, data);
      }
      const nextProviders = data.providers || {};
  ...
  const loadSpeechProviders = useCallback(async () => {
    try {
      let data: any = readMediaCache(MEDIA_CACHE_SPEECH);
      if (!data) {
        const res = await jarvisFetch(MEDIA_CACHE_SPEECH);
        data = await res.json();
        writeMediaCache(MEDIA_CACHE_SPEECH, data);
      }
      const nextProviders = data.providers || {};
  ...
  useEffect(() => {
    ...
    const refreshRuntimeMediaProviders = () => {
      // 焦点回归时强制刷新运行时模型（CLI 可能已安装/升级），绕过前端短缓存。
      clearMediaCache(MEDIA_CACHE_IMAGE);
      clearMediaCache(MEDIA_CACHE_VIDEO);
      loadImageProviders();
      loadVideoProviders();
    };
  ...
  const saveConfig = async (updates: Partial<MediaConfig>) => {
    try {
      const res = await jarvisFetch('/api/media/image/config', {
        method: 'PUT',
  ...
  const saveVideoConfig = async (updates: Partial<MediaConfig>) => {
    try {
      const res = await jarvisFetch('/api/media/video/config', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ values: encodeConfigPatch(updates) }),
      });
  ...
  const saveSpeechConfig = async (updates: SpeechConfigPatch) => {
    try {
      ...
      clearMediaCache(MEDIA_CACHE_SPEECH);
      showToast(t('settings.saved'), 'success');
    } catch (err: any) {
      showToast(err.message || 'Save failed', 'error');
    }
  ...
  return (
    <div className={`${styles['settings-tab-content']} ${styles['active']}`} data-tab="media">
      ...
      {/* 语音识别/合成/本地引擎面板已迁至独立的「语音」设置页(VoiceTab) */}
    </div>
  );
}
