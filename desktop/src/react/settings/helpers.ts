...
export async function refreshSettingsConfigSnapshot(): Promise<void> {
  ...
  if (!ownerId) return;
  const cfgRes = await jarvisFetch(`/api/agents/${ownerId}/config`);
...
export async function autoSaveGlobalModels(
  ...
  try {
    ...
    if (data.error) throw new Error(data.error);
    if (!opts.silent) store.showToast(t('settings.autoSaved'), 'success');
    const refreshRes = await jarvisFetch('/api/preferences/models');
...
export function savePins() {
  ...
  _savePinsTimer = setTimeout(async () => {
    ...
    try {
      const agentId = store.getSettingsAgentId();
      const res = await jarvisFetch(`/api/agents/${agentId}/pinned`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
