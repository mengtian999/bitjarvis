...
import { jarvisFetch, jarvisUrl } from './api';
...
export async function loadAvatars() {
  ...
  try {
    ...
    store.set({
      userAvatarUrl: avatars.user ? jarvisUrl(`/api/avatar/user?t=${ts}`) : null,
      agentAvatarUrl: agentId && agent?.hasAvatar
        ? jarvisUrl(`/api/avatar/agent?agentId=${encodeURIComponent(agentId)}&t=${ts}`)
        : null,
    });
  } catch {}
}
...
export async function loadSettingsConfig() {
  ...
  try {
    ...
    const [configRes, identityRes, ishikiRes, publicIshikiRes, userProfileRes, pinnedRes, globalModelsRes] =
      await Promise.all([
        jarvisFetch(`${agentBase}/config`, { signal: controller.signal }),
        jarvisFetch(`${agentBase}/identity`, { signal: controller.signal }),
        jarvisFetch(`${agentBase}/ishiki`, { signal: controller.signal }),
        jarvisFetch(`${agentBase}/public-ishiki`, { signal: controller.signal }),
        jarvisFetch('/api/user-profile', { signal: controller.signal }),
        jarvisFetch(`${agentBase}/pinned`, { signal: controller.signal }),
        jarvisFetch('/api/preferences/models', { signal: controller.signal }),
      ]);
    ...
    if (config.experience?.enabled === true) {
      const experienceRes = await jarvisFetch(`${agentBase}/experience`, { signal: controller.signal });
...
export async function loadSettingsSnapshot(options: { retainSameKeyData?: boolean } = {}) {
  ...
  try {
    const res = await jarvisFetch(`/api/settings/snapshot?agentId=${encodeURIComponent(agentId)}`, {
...
export async function loadPluginSettings() {
  ...
  try {
    const settingsRes = await jarvisFetch('/api/plugins/settings');
...
export async function switchToAgent(agentId: string) {
  const store = useSettingsStore.getState();
  try {
    const res = await jarvisFetch('/api/agents/switch', {
      method: 'POST',
...
export async function setPrimaryAgent(agentId: string) {
  const store = useSettingsStore.getState();
  try {
    const res = await jarvisFetch('/api/agents/primary', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: agentId }),
    });
