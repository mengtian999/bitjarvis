...
function subscribeEntry(entry: WatchEntry): Promise<void> {
  ...
  return jarvisFetch('/api/resource-io/subscribe', {
    method: 'POST',
...
function releaseEntry(entry: WatchEntry): void {
  ...
  entry.released = true;
  void jarvisFetch(`/api/resource-io/subscriptions/${encodeURIComponent(entry.subscriptionId)}`, {
    method: 'DELETE',
    throwOnHttpError: false,
  }).catch((err) => {
...
async function resubscribeActiveWatches(): Promise<void> {
  ...
  await Promise.all(entries.map(async (entry) => {
    ...
    if (previousSubscriptionId) {
      await jarvisFetch(`/api/resource-io/subscriptions/${encodeURIComponent(previousSubscriptionId)}`, {
        method: 'DELETE',
        throwOnHttpError: false,
      }).catch((err) => {
