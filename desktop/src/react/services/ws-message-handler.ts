...
import { isUserAbortRecent } from './user-abort-registry';

declare function t(key: string, vars?: Record<string, string>): any;

/**
 * 服务端系统性中止（turn_stall_timeout / 断线宽限 abort_all / 用户停止等）都会
 * 广播 `status(isStreaming:false, aborted:true, reason)`。用户自己点的停止不提示
...
export function handleServerMessage(msg: any): void {
  ...
  switch (msg.type) {
    ...
    case 'notification':
      if (window.jarvis?.showNotification) {
        // agentId 标识触发通知的助手，主进程据此读取该 agent 头像作为通知 icon。
        // 缺失时透传 null，主进程退回无 icon，禁止从当前焦点 agent 兜底。
        window.jarvis.showNotification(msg.title, msg.body, msg.agentId ?? null, {
    ...
    case 'status': {
      ...
      if (sp && applied) {
        if (msg.isStreaming) {
          // 新 turn 开始：上一轮遗留的中断提示已经过时（用户已经重试/继续了），清掉。
          const existing = sessionScopedValue(useStore.getState(), useStore.getState().inlineErrors, sp) ?? null;
          if (existing?.code === TURN_INTERRUPTED_ERROR_CODE) {
            useStore.getState().clearInlineError(sp);
          }
          streamBufferManager.beginTurn(sp, sid);
        } else {
          streamBufferManager.finishTurn(sp, sid);
          applyTurnInterruptedNotice(sp, msg);
        }
      }
      break;
    }
