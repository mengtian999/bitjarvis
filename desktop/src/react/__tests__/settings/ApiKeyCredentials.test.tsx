...
describe('ApiKeyCredentials', () => {
  beforeEach(() => {
    ...
    mocks.jarvisFetch.mockResolvedValue(jsonResponse({ ok: true }));
  ...
  it('sends the provider id when verifying saved credentials', async () => {
    ...
    await waitFor(() => expect(mocks.jarvisFetch).toHaveBeenCalledWith(
      '/api/providers/test',
      expect.objectContaining({ method: 'POST' }),
    ));
    const [, options] = mocks.jarvisFetch.mock.calls[0];
  ...
  it('reveals a masked saved api key through the explicit provider endpoint', async () => {
    const onRefresh = vi.fn(async () => {});
    mocks.jarvisFetch.mockResolvedValueOnce(jsonResponse({ api_key: 'sk-real-provider-key' }));
    ...
    await waitFor(() => expect(mocks.jarvisFetch).toHaveBeenCalledWith('/api/providers/deepseek/api-key'));
  ...
  it('keeps revealed saved api keys out of ordinary state and blocks copy', async () => {
    ...
    const testCall = mocks.jarvisFetch.mock.calls.find(([path]) => path === '/api/providers/test');
  ...
  it('lets a revealed saved api key be replaced directly', async () => {
    ...
    const configCall = mocks.jarvisFetch.mock.calls.find(([path]) => path === '/api/config');
  ...
  it('persists edited Kimi API keys on input blur without requiring connection verification', async () => {
    ...
    expect(mocks.jarvisFetch).not.toHaveBeenCalledWith(
      '/api/providers/test',
      expect.objectContaining({ method: 'POST' }),
    );
    const configCall = mocks.jarvisFetch.mock.calls.find(([path]) => path === '/api/config');
  ...
  it('saves discovered Gemini models during preset setup instead of static defaults', async () => {
    ...
    expect(mocks.jarvisFetch).toHaveBeenCalledWith('/api/providers/fetch-models', expect.objectContaining({
      method: 'POST',
    }));
  ...
  it('shows the server rejection and preserves the API draft for retry', async () => {
    mocks.jarvisFetch.mockResolvedValueOnce(jsonResponse({ error: 'unsupported provider API type' }));
    render(
      <ApiKeyCredentials
        providerId="deepseek"
        summary={providerSummary({ api: 'openai-completions', has_credentials: true })}
