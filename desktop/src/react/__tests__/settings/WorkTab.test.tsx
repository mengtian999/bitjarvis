...
describe('WorkTab workspace persistence', () => {
  ...
  it('saves the selected agent workspace without sending frontend business IPC', async () => {
    ...
    await waitFor(() => {
      expect(mockJarvisFetch).toHaveBeenCalledWith('/api/agents/agent-a/config', expect.objectContaining({
        method: 'PUT',
  ...
  it('does not refresh the settings snapshot when saving a non-owner agent config', async () => {
    ...
    await waitFor(() => {
      expect(mockJarvisFetch).toHaveBeenCalledWith('/api/agents/agent-b/config', expect.objectContaining({
        method: 'PUT',
      }));
    });
