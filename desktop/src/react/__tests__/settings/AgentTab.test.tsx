...
const jarvisFetchMock = vi.fn(async (_url: string, _opts?: RequestInit): Promise<MockResponse> => ({
  json: async () => ({ models: [] }),
}));
const showInFinderMock = vi.fn();

vi.mock('../../settings/api', () => ({
  jarvisFetch: (url: string, opts?: RequestInit) => jarvisFetchMock(url, opts),
  jarvisUrl: (path: string) => path,
...
describe('AgentTab settings agent selection', () => {
  beforeEach(() => {
    jarvisFetchMock.mockImplementation(async (_url: string, _opts?: RequestInit): Promise<MockResponse> => ({
      json: async () => ({ models: [] }),
    }));
  ...
  it('rerenders when browsing a different settings agent', async () => {
    ...
    expect(screen.getByTestId('selected-agent')).toHaveTextContent('jarvis');
  ...
  it('confirms character-card export from the live preview overlay', async () => {
    jarvisFetchMock.mockImplementation(async (url: string, _opts?: RequestInit): Promise<MockResponse> => {
        ...
        return {
          json: async () => ({
            ...
            plan: {
              mode: 'export',
              agentId: 'jarvis',
              packageName: 'jarvis-charactercard.zip',
              agent: { name: 'Jarvis', yuan: 'jarvis', description: '花名册描述' },
      ...
      if (url === '/api/character-cards/export') {
        return {
          json: async () => ({
            ok: true,
            filePath: '/tmp/jarvis-charactercard.zip',
            fileName: 'jarvis-charactercard.zip',
          }),
        };
      }
    ...
    const exportCall = jarvisFetchMock.mock.calls.find((call) => {
    ...
    expect(showInFinderMock).toHaveBeenCalledWith('/tmp/jarvis-charactercard.zip');
  });
  ...
  it('saves the agent name when pressing Enter in the name field (#1306)', async () => {
    ...
    const cfgCall = jarvisFetchMock.mock.calls.find((call) => {
      const [url, opts] = call as [string, RequestInit | undefined];
      return url === '/api/agents/jarvis/config' && opts?.method === 'PUT';
    }) as [string, RequestInit | undefined] | undefined;
  ...
  it('saves only the agent name from the compact name save button', async () => {
    ...
    const identityCall = jarvisFetchMock.mock.calls.find((call) => {
      const [url, opts] = call as [string, RequestInit | undefined];
      return url === '/api/agents/jarvis/identity' && opts?.method === 'PUT';
    });
