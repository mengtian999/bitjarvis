...
vi.mock('../../settings/api', () => ({
  ...
  jarvisUrl: (path: string) => `http://127.0.0.1:14500${path}?token=local`,
}));
...
describe('AccessTab', () => {
  beforeEach(() => {
    ...
    mockJarvisFetch.mockImplementation((url: string, options?: RequestInit) => {
      ...
      if (url === '/api/access/mobile-credentials' && options?.method === 'POST') {
        return Promise.resolve(jsonResponse({
          ok: true,
          secret: 'jarvis_dev_visible_once',
      ...
      if (url === '/api/access/desktop-credentials' && options?.method === 'POST') {
        return Promise.resolve(jsonResponse({
          ok: true,
          secret: 'jarvis_dev_desktop_visible_once',
      ...
      if (url === '/api/access/account/profile' && options?.method === 'PUT') {
        return Promise.resolve(jsonResponse({
          ok: true,
          account: { ...baseSummary.account, username: 'jarvis-owner', displayName: 'Jarvis Owner' },
        }));
      }
    ...
    Object.assign(window, {
      jarvis: { reloadMainWindow: vi.fn(async () => {}) },
    });
  });
  ...
  it('shows the stable mobile URL and saves LAN network settings', async () => {
    ...
    await waitFor(() => {
      expect(mockJarvisFetch).toHaveBeenCalledWith('/api/access/network', expect.objectContaining({
        method: 'PUT',
  ...
  it('uses snapshot access summary for the LAN switch before the refresh request settles', async () => {
    ...
    await waitFor(() => {
      expect(mockJarvisFetch).toHaveBeenCalledWith('/api/access/summary');
    });
  });
  ...
  it('generates a mobile access key and keeps the returned secret visible once', async () => {
    ...
    expect(await screen.findByDisplayValue('jarvis_dev_visible_once')).toBeInTheDocument();
    expect(mockJarvisFetch).toHaveBeenCalledWith('/api/access/mobile-credentials', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({
  ...
  it('generates a desktop access key from the manual computer section', async () => {
    ...
    expect(await screen.findByDisplayValue('jarvis_dev_desktop_visible_once')).toBeInTheDocument();
    expect(mockJarvisFetch).toHaveBeenCalledWith('/api/access/desktop-credentials', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({
  ...
  it('connects to an existing LAN server from the client connection section', async () => {
    ...
    await waitFor(() => {
      ...
      expect(window.jarvis.reloadMainWindow).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it('renders remote connections as local-only management and can return to the local server', async () => {
    ...
    expect(mockJarvisFetch).not.toHaveBeenCalledWith('/api/access/summary');
  ...
  it('saves the local owner profile and password from the account section', async () => {
    ...
    await waitFor(() => {
      expect(mockJarvisFetch).toHaveBeenCalledWith('/api/access/account/profile', expect.objectContaining({
        method: 'PUT',
        body: JSON.stringify({ username: 'jarvis-owner', displayName: 'Jarvis Owner' }),
      }));
    });
    ...
    await waitFor(() => {
      expect(mockJarvisFetch).toHaveBeenCalledWith('/api/access/account/password', expect.objectContaining({
        method: 'PUT',
  ...
  it('revokes individual credentials without requiring whole-device revocation', async () => {
    ...
    await waitFor(() => {
      expect(mockJarvisFetch).toHaveBeenCalledWith('/api/devices/credentials/cred_1/revoke', { method: 'POST' });
    });
  });
  ...
  it('clears the local password from the password section', async () => {
    ...
    await waitFor(() => {
      expect(mockJarvisFetch).toHaveBeenCalledWith('/api/access/account/password', { method: 'DELETE' });
    });
  });
});
