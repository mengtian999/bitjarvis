...
import { jarvisFetch } from '../../settings/api';
...
vi.mock('../../settings/api', () => ({
  ...
  yuanFallbackAvatar: (yuan?: string) => `fallback:${yuan || 'jarvis'}`,
}));
...
const agents = [
  { id: 'jarvis', name: '小花', yuan: 'jarvis', isPrimary: true, hasAvatar: false },
...
describe('AgentCardStack actions', () => {
  ...
  it('keeps the existing drag reorder commit semantics on pointerup', () => {
    vi.mocked(jarvisFetch).mockResolvedValue(new Response('{}'));
    render(React.createElement(AgentCardStack, {
      agents,
      selectedId: 'jarvis',
      currentAgentId: 'jarvis',
    ...
    const stack = screen.getByText('DeepSeek').closest('[class*="agent-cards"]') as HTMLElement;
    const card = screen.getByText('小花').closest('[data-agent-id="jarvis"]') as HTMLElement;
    vi.spyOn(stack, 'matches').mockReturnValue(true);
    Object.defineProperty(card, 'setPointerCapture', { configurable: true, value: vi.fn() });
    ...
    expect(jarvisFetch).toHaveBeenCalledWith('/api/agents/order', expect.objectContaining({
      method: 'PUT',
      body: JSON.stringify({ order: ['deepseek', 'jarvis', 'maomao'] }),
    }));
  });
