...
describe('MeTab', () => {
  ...
  it('saves the user name globally, not against whichever agent settings is showing', async () => {
    ...
    await waitFor(() => expect(jarvisFetchMock).toHaveBeenCalled());

    const configCalls = jarvisFetchMock.mock.calls.filter(([url]) => url.includes('/config'));
    expect(configCalls).toHaveLength(1);
    expect(configCalls[0][0]).toBe('/api/config');
    expect(JSON.parse(String(configCalls[0][1]?.body))).toEqual({ user: { name: 'New Name' } });
    expect(jarvisFetchMock.mock.calls.some(([url]) => url.startsWith('/api/agents/'))).toBe(false);
  });
  ...
  it('still saves the user profile through the user-level route', async () => {
    ...
    const profileCall = jarvisFetchMock.mock.calls.find(([url]) => url === '/api/user-profile');
