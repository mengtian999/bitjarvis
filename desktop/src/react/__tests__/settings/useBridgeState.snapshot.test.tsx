...
vi.mock('../../settings/api', () => ({
  jarvisFetch: (...args: unknown[]) => mockJarvisFetch(...args),
}));
...
function TelegramSecretProbe() {
  ...
  return (
    <div>
      ...
      <button type="button" onClick={() => bridge.setSelectedAgentId('jarvis')}>switch to jarvis</button>
    </div>
  );
}
...
function bridgeStatus(overrides: Record<string, unknown> = {}) {
  return {
    ...
    whatsapp: { enabled: false, status: 'disconnected', agentId: 'jarvis' },
    qq: {
      enabled: false,
      status: 'disconnected',
      appID: 'qq-app',
    ...
    wechat: { enabled: false, status: 'disconnected', token: '', agentId: 'jarvis' },
...
describe('useBridgeState snapshot hydration', () => {
  beforeEach(() => {
    ...
    mockJarvisFetch.mockImplementation((url: string) => {
      if (url === '/api/bridge/status?agentId=jarvis') {
  ...
  it('uses snapshot bridge status before the status refresh request settles', () => {
    ...
    expect(mockJarvisFetch).toHaveBeenCalledWith(
      '/api/bridge/status?agentId=jarvis',
  ...
  it('keeps reconciling WeChat after QR confirmation until the selected Agent connects', async () => {
    ...
    mockJarvisFetch.mockImplementation((url: string) => {
      ...
      const wechat = statusRequests === 1
        ? { enabled: false, status: 'disconnected', token: '', agentId: 'jarvis' }
        : statusRequests === 2
          ? { enabled: true, status: 'connecting', token: '********', agentId: 'jarvis' }
          : { enabled: true, status: 'connected', token: '********', agentId: 'jarvis' };
    ...
    await act(async () => {
      window.dispatchEvent(new Event('jarvis-bridge-reload'));
  ...
  it('cancels WeChat reconciliation when the selected Agent changes', async () => {
    vi.useFakeTimers();
    let jarvisStatusRequests = 0;
    mockJarvisFetch.mockImplementation((url: string) => {
      if (url === '/api/bridge/status?agentId=jarvis') {
        jarvisStatusRequests += 1;
        const wechat = jarvisStatusRequests === 1
          ? { enabled: false, status: 'disconnected', token: '', agentId: 'jarvis' }
          : { enabled: true, status: 'connecting', token: '********', agentId: 'jarvis' };
        return Promise.resolve(new Response(JSON.stringify(bridgeStatus({ wechat }))));
      }
    ...
    expect(screen.getByTestId('wechat-status')).toHaveTextContent('connecting');
    expect(jarvisStatusRequests).toBe(2);
  ...
  it('keeps saved public ishiki in the settings snapshot for remounts', async () => {
    ...
    mockJarvisFetch.mockImplementation((url: string, opts?: RequestInit) => {
      if (url === '/api/bridge/status?agentId=jarvis') {
        return new Promise<Response>(() => {});
      }
      if (url === '/api/agents/jarvis/public-ishiki') {
  ...
  it('applies owner status returned by setOwner without waiting for a later refresh', async () => {
    mockJarvisFetch.mockImplementation((url: string, opts?: RequestInit) => {
      ...
      if (url === '/api/bridge/owner?agentId=jarvis') {
        expect(opts).toMatchObject({
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  it('clears the submitted plaintext after save, ignores the returned mask, and tests with saved credentials', async () => {
    ...
    mockJarvisFetch.mockImplementation((url: string, opts?: RequestInit) => {
      ...
      if (url === '/api/bridge/config?agentId=jarvis') {
        expect(opts).toMatchObject({
          method: 'POST',
          body: JSON.stringify({
            platform: 'telegram',
      ...
      if (url === '/api/bridge/test?agentId=jarvis') {
        expect(opts).toMatchObject({
          method: 'POST',
          body: JSON.stringify({
            platform: 'telegram',
        ...
        return Promise.resolve(new Response(JSON.stringify({ ok: true, info: { username: 'jarvis_bot' } })));
      }
      throw new Error(`unexpected request: ${url}`);
    });
    ...
    await waitFor(() => {
      expect(mockState.showToast).toHaveBeenCalledWith('settings.bridge.testOk @jarvis_bot', 'success');
    });
  });
  ...
  it('keeps connection-test state and results owned by the Agent and request generation', async () => {
    let resolveJarvisTest!: (response: Response) => void;
    ...
    mockJarvisFetch.mockImplementation((url: string) => {
      ...
      if (url === '/api/bridge/test?agentId=jarvis') {
        return new Promise<Response>((resolve) => { resolveJarvisTest = resolve; });
      }
    ...
    await act(async () => {
      resolveJarvisTest(new Response(JSON.stringify({
        ok: true,
        info: { username: 'old_jarvis_bot' },
      })));
    });
  ...
  it('does not reuse a revision after switching away and back while a save is in flight', async () => {
    ...
    render(<TelegramSecretProbe />);
    fireEvent.change(screen.getByTestId('telegram-secret-input'), { target: { value: 'old-jarvis-secret' } });
    fireEvent.blur(screen.getByTestId('telegram-secret-input'));
    fireEvent.click(screen.getByRole('button', { name: 'switch to mio' }));
    fireEvent.click(screen.getByRole('button', { name: 'switch to jarvis' }));
    fireEvent.change(screen.getByTestId('telegram-secret-input'), { target: { value: 'new-jarvis-secret' } });
    ...
    await waitFor(() => {
      expect(screen.getByTestId('telegram-secret-input')).toHaveValue('new-jarvis-secret');
    });
    expect(screen.getByTestId('telegram-secret-revision')).toHaveTextContent('2');
    expect(screen.getByTestId('telegram-secret-dirty')).toHaveTextContent('true');
  });
  ...
  it('drops plaintext drafts when the selected Agent changes', async () => {
    ...
    render(<TelegramSecretProbe />);
    fireEvent.change(screen.getByTestId('telegram-secret-input'), { target: { value: 'jarvis-only-secret' } });
    fireEvent.click(screen.getByRole('button', { name: 'switch to mio' }));
