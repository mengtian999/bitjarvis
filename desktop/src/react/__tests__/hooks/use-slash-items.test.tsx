...
const mocks = vi.hoisted(() => ({
  jarvisFetch: vi.fn<(path: string, opts?: RequestInit) => Promise<Response>>(async (path: string) => {
...
describe('useSkillSlashItems', () => {
  ...
  it('fetches skills for the explicit target agent instead of the current focused agent', async () => {
    ...
    expect(mocks.jarvisFetch.mock.calls[0]?.[0]).toBe('/api/skills?agentId=mio&runtime=1');
    expect(mocks.jarvisFetch.mock.calls.map(call => call[0])).not.toContain('/api/skills?agentId=jarvis&runtime=1');
  });
});
...
describe('useServerSlashCommandItems', () => {
  ...
  it('fetches non-core server slash commands for the target agent', async () => {
    ...
    expect(mocks.jarvisFetch.mock.calls[0]?.[0]).toBe('/api/commands?agentId=mio');
  });
});
