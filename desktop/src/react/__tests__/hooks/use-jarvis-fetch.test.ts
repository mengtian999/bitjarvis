...
// Mock useStore before importing jarvisFetch
vi.mock('../../stores', () => ({
  useStore: {
    getState: () => ({
      serverPort: '3210',
...
describe('jarvisFetch', () => {
  ...
  it('非 2xx 状态码抛出错误', async () => {
    ...
    await expect(jarvisFetch('/api/missing')).rejects.toThrow('404');
  });
  ...
  it('允许调用方显式读取非 2xx 响应体', async () => {
    ...
    const res = await jarvisFetch('/api/diary/write', { throwOnHttpError: false });
