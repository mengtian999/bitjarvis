...
describe('Windows auto-hiding scrollbar contract', () => {
  ...
  afterEach(() => {
    ...
    vi.useRealTimers();
    Reflect.deleteProperty(window, 'jarvis');
    Reflect.deleteProperty(window, 'platform');
  });
  ...
  it('marks only the active Windows scroll target and clears it after inactivity', async () => {
    ...
    expect(scroller.classList.contains('jarvis-scroll-active')).toBe(true);

    vi.advanceTimersByTime(799);
    expect(scroller.classList.contains('jarvis-scroll-active')).toBe(true);

    vi.advanceTimersByTime(1);
    expect(scroller.classList.contains('jarvis-scroll-active')).toBe(false);
  ...
  it('keeps the visual override scoped to Windows while preserving scrollbar width', () => {
    ...
    expect(globalCss).toMatch(/html\[data-platform="win32"\][^{]*\.jarvis-scroll-active\s*\{[^}]*scrollbar-color:\s*var\(--overlay-strong\) transparent/s);
    expect(globalCss).toMatch(/\.jarvis-scroll-active::-webkit-scrollbar-thumb\s*\{[^}]*background:\s*var\(--overlay-strong\)/s);
