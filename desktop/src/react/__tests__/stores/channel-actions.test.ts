...
describe('channel-actions', () => {
  ...
  describe('loadChannels', () => {
    it('加载频道和 DM 列表', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ ownerAgentId: 'jarvis', dms: [{ ownerAgentId: 'jarvis', peerId: 'agent1', peerName: 'Agent 1', messageCount: 5 }] }),
        } as Response);
      ...
      expect(channels[1].dmOwnerId).toBe('jarvis');
    });
  ...
  describe('openChannel', () => {
    it('opens DM history with the stored owner agent id', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('/api/dm/agent1?agentId=jarvis');
  ...
  describe('loadConversationAgentActivities', () => {
    it('loads and keys agent phone activities by conversation and agent', async () => {
      ...
      expect((mockState.channelAgentActivities as any).ch1.jarvis[0]).toMatchObject({
        state: 'idle',
        summary: '已回复',
      });
    });
  ...
  describe('DM phone settings owner', () => {
    it('loads DM phone settings with the stored owner agent id', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('/api/conversations/dm%3Aagent1/agent-phone-settings?agentId=jarvis');
    ...
    it('saves DM phone settings with the stored owner agent id', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('/api/conversations/dm%3Aagent1/agent-phone-settings?agentId=jarvis', expect.objectContaining({
        method: 'POST',
      }));
      expect(mockState.channelAgentPhoneToolMode).toBe('write');
    });
  ...
  describe('channel member management', () => {
    it('adds a member and updates the current channel projection', async () => {
      ...
      mockState.channelMembers = ['jarvis', 'butter'];
      mockState.channels = [{
        id: 'ch1',
        name: 'general',
        members: ['jarvis', 'butter'],
        lastMessage: '',
        lastSender: '',
        lastTimestamp: '',
        newMessageCount: 0,
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ ok: true, members: ['jarvis', 'butter', 'ming'] }),
      } as Response);
      ...
      expect(mockState.channelMembers).toEqual(['jarvis', 'butter', 'ming']);
      expect((mockState.channels as any[])[0].members).toEqual(['jarvis', 'butter', 'ming']);
    ...
    it('surfaces backend member removal errors without mutating local members', async () => {
      ...
      expect(mockState.channelMembers).toEqual(['jarvis', 'butter']);
    });
  });
  ...
  describe('sendChannelMessage', () => {
    ...
    it('发送成功后按发送开始时的频道同步消息 cache 和频道列表投影', async () => {
      mockState.currentChannel = 'ch1';
      mockState.channelMessages = [
        { sender: 'jarvis', timestamp: '2026-05-07 17:00:00', body: 'old ch1' },
      ];
      mockState.channelMessageCache = {
  ...
  describe('exportCurrentConversation', () => {
    it('downloads the Markdown response using the server-provided filename', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        blob: async () => blob,
        headers: new Headers({
          'content-disposition': "attachment; filename=\"archive.md\"; filename*=UTF-8''jarvis-chat-archive.md",
        }),
      } as Response);
      ...
      try {
        ...
        await expect(exportCurrentConversation()).resolves.toBe('jarvis-chat-archive.md');
      ...
      mockFetch
        .mockResolvedValueOnce({
          ...
          json: async () => ({
            ...
            messages: [
              { sender: 'testuser', timestamp: '2026-05-07 17:00:00', body: 'old' },
              { sender: 'jarvis', timestamp: '2026-05-07 17:01:00', body: 'reloaded reply' },
            ],
          }),
        } as Response)
        .mockResolvedValue({
