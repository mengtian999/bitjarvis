...
describe('reorderPinnedSessions', () => {
  ...
  it('renumbers the pinned sessions immediately and posts the submitted order', async () => {
    ...
    jarvisFetchMock.mockImplementation(() => new Promise((resolve) => { resolveFetch = resolve; }));
    ...
    expect(jarvisFetchMock).toHaveBeenCalledWith('/api/sessions/pin-order', expect.objectContaining({
      method: 'POST',
    }));
    expect(JSON.parse(jarvisFetchMock.mock.calls[0][1].body)).toEqual({
  ...
  it('restores the previous order and warns when the server rejects the reorder', async () => {
    jarvisFetchMock.mockResolvedValue({
      ok: false,
  ...
  it('restores the previous order when the request throws', async () => {
    jarvisFetchMock.mockRejectedValue(new Error('offline'));
  ...
  it('does nothing when the submitted list is empty', async () => {
    ...
    expect(jarvisFetchMock).not.toHaveBeenCalled();
  });
});
