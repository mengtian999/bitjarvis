...
describe('message turn actions', () => {
  ...
  it('retries an arbitrary persisted user node with explicit session identity and display envelope', async () => {
    vi.mocked(jarvisFetch).mockResolvedValueOnce(new Response(JSON.stringify({ ok: true }), { status: 200 }));
    const message = {
      id: 'client-u1',
    ...
    expect(ok).toBe(true);
    expect(jarvisFetch).toHaveBeenCalledWith('/api/sessions/turns/retry', expect.objectContaining({
      method: 'POST',
      timeout: 30 * 60 * 1000,
      throwOnHttpError: false,
    }));
    const init = vi.mocked(jarvisFetch).mock.calls[0][1];
  ...
  it('forks an assistant node and normalizes the returned child locator', async () => {
    vi.mocked(jarvisFetch).mockResolvedValueOnce(new Response(JSON.stringify({
      ok: true,
