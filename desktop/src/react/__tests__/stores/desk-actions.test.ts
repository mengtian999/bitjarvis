...
// importing the store now reaches use-jarvis-fetch during that hoisted phase.
const mockJarvisFetch = vi.hoisted(() => vi.fn());

vi.mock('../../hooks/use-jarvis-fetch', () => ({
  jarvisFetch: mockJarvisFetch,
}));
...
describe('desk-actions workspace roots', () => {
  beforeEach(() => {
    ...
    mockJarvisFetch.mockReset();
    mockJarvisFetch.mockImplementation(async (url: string) => {
  ...
  it('loads the selected/home folder when no explicit override is passed', async () => {
    ...
    expect(mockJarvisFetch).toHaveBeenNthCalledWith(
      1,
  ...
  it('prunes a stale local workspace history entry when the selected local root is missing', async () => {
    ...
    expect(mockJarvisFetch).toHaveBeenNthCalledWith(
      2,
      '/api/config/workspaces/recent?agentId=jarvis',
  ...
  it('keeps local workspace history on permission errors', async () => {
    ...
    mockJarvisFetch.mockResolvedValueOnce(jsonStatusResponse({
    ...
    expect(mockJarvisFetch).toHaveBeenCalledTimes(1);
  });
  ...
  it('keeps the visible mounted workspace files when a refresh fails', async () => {
    ...
    mockJarvisFetch.mockResolvedValueOnce(jsonResponse({ error: 'workspace_not_found' }));
  ...
  it('removes recent workspaces locally and persists the history deletion', async () => {
    ...
    mockJarvisFetch.mockResolvedValueOnce(jsonResponse({ cwd_history: ['/workspace/Novel'] }));
    ...
    expect(mockJarvisFetch).toHaveBeenCalledWith(
      '/api/config/workspaces/recent?agentId=jarvis',
      expect.objectContaining({
        method: 'DELETE',
  ...
  it('does not record recent-workspace changes against a guessed agent when none is current', async () => {
    ...
    expect(mockJarvisFetch.mock.calls.some(([url]) => String(url).includes('/api/config/workspaces/recent'))).toBe(false);
  ...
  it('removes a Studio workspace mount and clears the selected mount when it was active', async () => {
    ...
    mockJarvisFetch.mockImplementation(async (url: string, opts?: RequestInit) => {
  ...
  it('clears recent workspace history through the server API', async () => {
    ...
    mockJarvisFetch.mockResolvedValueOnce(jsonResponse({ cwd_history: [] }));
    ...
    expect(mockJarvisFetch).toHaveBeenCalledWith(
      '/api/config/workspaces/recent/all?agentId=jarvis',
      expect.objectContaining({ method: 'DELETE' }),
    );
  });
  ...
  it('persists the selected workspace before refreshing the visible desk root', async () => {
    ...
    expect(useStore.getState().deskFiles).toEqual([]);
    expect(mockJarvisFetch).toHaveBeenCalledTimes(2);
    expect(mockJarvisFetch).toHaveBeenNthCalledWith(
      1,
  ...
  it('persists preview panel layout as part of the workspace UI state', async () => {
    ...
    expect(mockJarvisFetch).toHaveBeenCalledWith('/api/preferences/workspace-ui-state', expect.objectContaining({
      method: 'PUT',
    }));
    const [, requestInit] = mockJarvisFetch.mock.calls.at(-1) || [];
  ...
  it('persists the workspace snapshot captured before switching roots', async () => {
    ...
    try {
      ...
      const putCall = mockJarvisFetch.mock.calls.find(([url, init]) =>
  ...
  it('restores persisted workspace preview state from the backend when memory has no entry', async () => {
    mockJarvisFetch.mockResolvedValueOnce(jsonResponse({
      state: {
    ...
    expect(mockJarvisFetch).toHaveBeenCalledWith('/api/preferences/workspace-ui-state?workspace=%2Fworkspace&surface=electron');
    expect(useStore.getState().deskCurrentPath).toBe('');
  ...
  it('renames a tree item by explicit parent subdir and updates that tree cache', async () => {
    ...
    expect(ok).toBe(true);
    expect(mockJarvisFetch).toHaveBeenCalledWith('/api/desk/files', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({
        action: 'rename',
        dir: '/workspace',
  ...
  it('trashes tree items through the platform trash API and refreshes affected parents', async () => {
    ...
    expect(mockJarvisFetch).toHaveBeenCalledWith('/api/desk/files?dir=%2Fworkspace&subdir=notes');
  ...
  it('uses the mobile surface bucket for persisted workspace state in the PWA shell', async () => {
    ...
    expect(mockJarvisFetch).toHaveBeenCalledWith('/api/preferences/workspace-ui-state?workspace=%2Fworkspace&surface=pwa');
  ...
  it('safe-deletes tree items through the mobile workbench route in the PWA shell', async () => {
    ...
    expect(ok).toBe(true);
    expect(mockJarvisFetch).toHaveBeenCalledWith('/api/workbench/actions', expect.objectContaining({
      method: 'POST',
  ...
  it('safe-deletes default workspace tree items through ResourceIO for remote desktop clients', async () => {
    ...
    useStore.setState({
      activeServerConnection: {
        ...
        studioId: 'studio_lan',
        label: 'LAN Jarvis',
        baseUrl: 'http://jarvis.local:14500',
        wsUrl: 'ws://jarvis.local:14500',
        token: null,
        authState: 'paired',
        trustState: 'lan',
  ...
  it('uploads browser File objects through the workbench upload route', async () => {
    ...
    expect(ok).toBe(true);
    const call = mockJarvisFetch.mock.calls[0];
  ...
  it('searches workspace files against the active desk root', async () => {
    ...
    expect(mockJarvisFetch).toHaveBeenCalledWith('/api/desk/search-files?dir=%2Fworkspace&q=Desk');
  ...
  it('jumps to a search result by expanding ancestors and selecting the real tree path', async () => {
    ...
    expect(mockJarvisFetch).toHaveBeenNthCalledWith(1, '/api/desk/files?dir=%2Fworkspace&subdir=src');
    expect(mockJarvisFetch).toHaveBeenNthCalledWith(2, '/api/desk/files?dir=%2Fworkspace&subdir=src%2Fcomponents');
  ...
  it('reveals an absolute workspace directory by expanding tree paths without replacing the desk root', async () => {
    ...
    mockJarvisFetch
      .mockResolvedValueOnce(jsonResponse({
        files: [{ name: 'JARVIS-Works', isDir: true }],
        basePath: '/workspace',
      }))
      .mockResolvedValueOnce(jsonResponse({
      ...
      .mockResolvedValueOnce(jsonResponse({
        files: [{ name: 'jarvis.png', isDir: false }],
        basePath: '/workspace',
      }));

    const { revealDeskDirectory } = await import('../../stores/desk-actions');
    const ok = await revealDeskDirectory('/workspace/JARVIS-Works/Screenshots');

    expect(ok).toBe(true);
    expect(mockJarvisFetch).toHaveBeenNthCalledWith(1, '/api/desk/files?dir=%2Fworkspace');
    expect(mockJarvisFetch).toHaveBeenNthCalledWith(2, '/api/desk/files?dir=%2Fworkspace&subdir=JARVIS-Works');
    expect(mockJarvisFetch).toHaveBeenNthCalledWith(3, '/api/desk/files?dir=%2Fworkspace&subdir=JARVIS-Works%2FScreenshots');
    expect(useStore.getState().deskBasePath).toBe('/workspace');
    expect(useStore.getState().deskCurrentPath).toBe('');
    expect(useStore.getState().deskFiles).toEqual([{ name: 'JARVIS-Works', isDir: true }]);
    expect(useStore.getState().deskTreeFilesByPath).toMatchObject({
      '': [{ name: 'JARVIS-Works', isDir: true }],
      'JARVIS-Works': [{ name: 'Screenshots', isDir: true }],
      'JARVIS-Works/Screenshots': [{ name: 'jarvis.png', isDir: false }],
    });
    expect(useStore.getState().deskExpandedPaths).toEqual(['JARVIS-Works', 'JARVIS-Works/Screenshots']);
    expect(useStore.getState().deskSelectedPath).toBe('JARVIS-Works/Screenshots');
  ...
  it('does not reveal a directory outside the active desk root', async () => {
    ...
    const { revealDeskDirectory } = await import('../../stores/desk-actions');
    const ok = await revealDeskDirectory('/other/JARVIS-Works');

    expect(ok).toBe(false);
    expect(mockJarvisFetch).not.toHaveBeenCalled();
