...
describe('sidebar-ui-slice', () => {
  beforeEach(() => {
    ...
    jarvisFetchMock.mockResolvedValue({ json: async () => ({}) });
  });
  ...
  it('setSidebarProjectViewPrefs updates optimistically and persists only the project view', async () => {
    ...
    expect(jarvisFetchMock).toHaveBeenCalledTimes(1);
    const [url, options] = jarvisFetchMock.mock.calls[0] as [string, { method: string; body: string }];
  ...
  it('loadSidebarUiPrefs retries with bounded backoff and applies the result to the store', async () => {
    vi.useFakeTimers();
    let attempts = 0;
    jarvisFetchMock.mockImplementation(async () => {
      attempts += 1;
  ...
  it('loadSidebarUiPrefs de-dupes concurrent calls into a single request', async () => {
    jarvisFetchMock.mockImplementation(async () => ({ json: async () => singleLinePrefsPayload() }));

    await Promise.all([loadSidebarUiPrefs(), loadSidebarUiPrefs(), loadSidebarUiPrefs()]);

    expect(jarvisFetchMock).toHaveBeenCalledTimes(1);
    expect(jarvisFetchMock).toHaveBeenCalledWith('/api/preferences/sidebar-ui');
  });
});
