...
const mockFetch = vi.mocked(jarvisFetch);
...
function mockPermissionDefault(mode = 'ask') {
  ...
  beforeEach(() => {
    ...
    (globalThis.window as unknown as { jarvis?: unknown }).jarvis = {};
    installStoreMethods();
    mockFetch.mockReset();
  ...
  it('continues a deleted-agent session by creating and switching to the returned primary-agent session', async () => {
    ...
    const newPath = '/tmp/agents/jarvis/sessions/new.jsonl';
    mockFetch.mockImplementation(async (url: string) => {
      if (url === '/api/sessions/continue-deleted-agent') {
        return jsonResponse({ ok: true, path: newPath, agentId: 'jarvis', agentName: 'Jarvis' });
      }
      if (url === '/api/sessions') {
        return jsonResponse([{ path: newPath, agentId: 'jarvis', agentName: 'Jarvis', cwd: '/tmp/work' }]);
      }
      if (url === '/api/sessions/switch') {
        return jsonResponse({
          ok: true,
  ...
  describe('createNewSession cwd draft', () => {
    it('resets a global new-session draft to the primary agent and its effective workspace', async () => {
      ...
      expect(mockState.selectedAgentId).toBe('jarvis');
    ...
    it('uses the primary agent effective default workspace instead of the active session cwd', async () => {
      ...
      expect(mockState.selectedAgentId).toBe('jarvis');
      expect(mockState.selectedFolder).toBe('/home/test/Desktop/JARVIS-WorkSpace');
      expect(mockState.deskBasePath).toBe('/home/test/Desktop/JARVIS-WorkSpace');
      expect(mockState.deskCurrentPath).toBe('');
      expect(mockLoadDeskFiles).toHaveBeenCalledWith('', '/home/test/Desktop/JARVIS-WorkSpace', null);
    });

    it('invalidates an in-flight session switch so the new-session desk stays on the agent home folder', async () => {
      (mockState as Record<string, unknown>).currentSessionPath = '/session/jarvis.jsonl';
      (mockState as Record<string, unknown>).deskBasePath = '/workspace/Desktop/project-jarvis';
      (mockState as Record<string, unknown>).homeFolder = '/workspace/Desktop/project-jarvis';
      ...
      expect(mockState.pendingNewSession).toBe(true);
      expect(mockState.selectedFolder).toBe('/workspace/Desktop/project-jarvis');
      expect(mockState.deskBasePath).toBe('/workspace/Desktop/project-jarvis');
    ...
    it('uses the runtime new-session permission default instead of the old active session mode', async () => {
      ...
      const permissionEvent = dispatchedEvents.filter(e => e.type === 'jarvis-plan-mode').at(-1);
    ...
    it('consumes home draft on activation without copying text into the new session draft', async () => {
      ...
      Object.assign(mockState, {
        ...
        attachedFiles: [{
          fileId: 'sf_home',
          path: '/tmp/jarvis/session-files/home.png',
          name: 'home.png',
          isDirectory: false,
        }],
      });
    ...
    it('posts one new-session request at send time using the current pending draft', async () => {
      ...
      mockFetch.mockImplementation(async (url: string, opts?: RequestInit) => {
        ...
        if (url === '/api/sessions/switch') {
          return jsonResponse({ ok: true, sessionId: 'sess_fresh', agentId: 'jarvis', workspaceFolders: [] });
        }
        throw new Error(`unexpected fetch: ${url}`);
      });
    ...
    it('does not let a late detached create response activate a replacement pending draft (#2078 ABA)', async () => {
      ...
      expect(ref).toEqual({ sessionId: 'sess_a', sessionPath: '/session/a.jsonl', agentId: 'jarvis' });
    ...
    it('carries an explicit project id from the new-session draft into session creation', async () => {
      ...
      await createNewSession({ projectId: 'project-jarvis', cwd: '/workspace/project-jarvis' });
      mockFetch.mockResolvedValueOnce(jsonResponse({
        ok: true,
        path: '/session/new.jsonl',
        sessionId: 'sess_project',
        agentId: 'jarvis',
        cwd: '/workspace/project-jarvis',
  ...
  describe('switchSession 的 hasData 语义（#405 直接回归）', () => {
    ...
    it('switching across agents synchronizes the active agent home while showing the session cwd', async () => {
      Object.assign(mockState, {
        agents: [
          {
            ...
            yuan: 'jarvis',
            isPrimary: true,
            homeFolder: '/workspace/jarvis-home',
            effectiveHomeFolder: '/workspace/jarvis-home',
          },
          {
            id: 'mio',
            name: 'Mio',
        ...
        currentAgentId: 'jarvis',
        homeFolder: '/workspace/jarvis-home',
        deskBasePath: '/workspace/jarvis-session',
      });
      mockFetch.mockResolvedValueOnce(jsonResponse({
        agentId: 'mio',
        agentName: 'Mio',
