...
const { jarvisFetchMock, switchSessionMock } = vi.hoisted(() => ({
  jarvisFetchMock: vi.fn(),
  switchSessionMock: vi.fn(),
}));

vi.mock('../../hooks/use-jarvis-fetch', () => ({ jarvisFetch: jarvisFetchMock }));
...
const PATH = '/tmp/agents/jarvis/sessions/a.jsonl';
...
describe('chat-find-actions', () => {
  ...
  it('runChatFind 写结果并把定位意图指向最新命中', async () => {
    ...
    jarvisFetchMock.mockResolvedValue(findResponse(SAMPLE));
  ...
  it('runChatFind 竞态护栏：返回时 query 已变化则丢弃结果', async () => {
    ...
    jarvisFetchMock.mockReturnValue(new Promise((r) => { resolveFetch = r; }));
  ...
  it('runChatFind 接口失败置 error 状态、不发定位', async () => {
    useStore.getState().openChatFind(PATH, 'x');
    jarvisFetchMock.mockResolvedValue(findResponse({ error: 'boom' }, false));
    await runChatFind(PATH, 'x');
  ...
  it('locateSearchHit：find 空命中时不开查找条不定位（console.warn 路径）', async () => {
    ...
    jarvisFetchMock.mockResolvedValue(findResponse({ ...SAMPLE, total: 0, matches: [], bestIndex: null }));
    await locateSearchHit(PATH, 'x');
    expect(useStore.getState().chatFindBySession[PATH]).toBeUndefined();
    expect(useStore.getState().pendingMessageLocate).toBeNull();
  });
