...
vi.mock('../../hooks/use-jarvis-fetch', () => ({
  jarvisFetch: vi.fn(),
  jarvisUrl: (path: string) => `http://127.0.0.1:1234${path}`,
}));
...
describe('loadAvatars agent ownership', () => {
  ...
  it('falls back to the agent the client already selected', () => {
    useStore.setState({ currentAgentId: 'jarvis' } as never);
    loadAvatars({ agent: true, user: true });
    expect(useStore.getState().agentAvatarUrl).toContain('agentId=jarvis');
  });
