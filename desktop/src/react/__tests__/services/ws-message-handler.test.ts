...
import { markUserAbort, resetUserAbortRegistryForTest } from '../../services/user-abort-registry';
...
describe('ws-message-handler session-scoped desktop events', () => {
  ...
  it('drops a websocket event whose sessionId is already bound to a different known path (#2078)', () => {
    useStore.setState({
      ...
      currentSessionPath: '/session/a.jsonl',
      sessions: [
        { path: '/session/a.jsonl', sessionId: 'sess_a', agentId: 'a1', agentName: 'Jarvis' },
        { path: '/session/b.jsonl', sessionId: 'sess_b', agentId: 'a1', agentName: 'Jarvis' },
      ],
      sessionLocatorsById: {
  ...
  it('session_user_message 透传 origin 到 store 里的 ChatMessage.origin', () => {
    handleServerMessage({
      ...
      message: {
        text: '干净正文',
        origin: { kind: 'agent', agentId: 'jarvis', agentName: 'Jarvis' },
      },
    });
    ...
    expect(first?.type).toBe('message');
    if (!first || first.type !== 'message') throw new Error('expected message item');
    expect(first.data.origin).toEqual({ kind: 'agent', agentId: 'jarvis', agentName: 'Jarvis' });
  });
...
describe('ws-message-handler permission mode events', () => {
  ...
  it('keeps explicit auto mode from legacy plan_mode messages instead of collapsing to operate', () => {
    ...
    windowTarget.addEventListener('jarvis-plan-mode', (event) => {
...
describe('ws-message-handler turn_end side effects', () => {
  ...
  it('shows focused slash_result through the inline notice bridge', () => {
    ...
    window.addEventListener('jarvis-inline-notice', listener);
    ...
    try {
      ...
      window.removeEventListener('jarvis-inline-notice', listener);
      vi.unstubAllGlobals();
    }
  });
...
describe('ws-message-handler · 系统性中断提示', () => {
  beforeEach(() => {
    // node 环境（无 jsdom / 无 window.t）：applyTurnInterruptedNotice 会走内置
    // 中文兜底文案，这里只断言 code / detail，不锁具体文案。
    useStore.setState({
      currentSessionPath: '/session/a.jsonl',
      currentSessionId: 'sess_a',
  ...
  function inlineErrorFor(path: string) {
    return sessionScopedValue(useStore.getState(), useStore.getState().inlineErrors, path) ?? null;
  }

  it('status aborted=true（非用户停止）写入持久中断提示', () => {
    handleServerMessage({
      type: 'status',
      sessionPath: '/session/a.jsonl',
      streamId: 'stream-1',
  ...
  it('用户主动停止后同一 aborted 事件不再提示', () => {
    markUserAbort('/session/a.jsonl', 'stream-1');
  ...
  it('status aborted=false 的正常结束不产生中断提示', () => {
    handleServerMessage({
      type: 'status',
      sessionPath: '/session/a.jsonl',
      streamId: 'stream-1',
  ...
  it('新的 turn 开始（isStreaming=true）时清除上一轮的中断提示', () => {
    handleServerMessage({
      type: 'status',
      sessionPath: '/session/a.jsonl',
      streamId: 'stream-1',
    ...
    expect(inlineErrorFor('/session/a.jsonl')?.code).toBe(TURN_INTERRUPTED_ERROR_CODE);
