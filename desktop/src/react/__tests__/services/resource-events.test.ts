...
const jarvisFetch = vi.hoisted(() => vi.fn(async (path: string) => ({
  json: async () => (path.endsWith('/subscribe') ? { ok: true, subscriptionId: 'sub-1' } : { ok: true }),
})));

vi.mock('../../hooks/use-jarvis-fetch', () => ({ jarvisFetch }));

describe('resource-events', () => {
  afterEach(() => {
    vi.resetModules();
    jarvisFetch.mockClear();
  });
  ...
  it('shares one backend resource subscription per local file and releases it after the last subscriber leaves', async () => {
    ...
    expect(jarvisFetch).toHaveBeenCalledTimes(1);
    expect(jarvisFetch).toHaveBeenCalledWith('/api/resource-io/subscribe', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({
    ...
    expect(jarvisFetch).toHaveBeenCalledWith('/api/resource-io/subscriptions/sub-1', expect.objectContaining({
      method: 'DELETE',
    }));
  });
