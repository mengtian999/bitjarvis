...
const jarvisFetchMock = vi.fn<(path: string, opts?: RequestInit) => Promise<Response>>(
  async () => new Response('{}', { status: 200 }),
);

vi.mock('../../hooks/use-jarvis-fetch', () => ({
  jarvisFetch: (path: string, opts?: RequestInit) => jarvisFetchMock(path, opts),
  ...
  useStore.getState().initSession('/session/a.jsonl', [{
    ...
    data: {
      ...
      blocks: [{
        ...
        surface: 'input',
        status: 'pending',
        title: '允许 Jarvis 使用电脑',
        body: 'Jarvis 想控制这个应用来继续当前任务。',
...
describe('computer app approval prompt', () => {
  ...
  it('renders the current session pending input confirmation above the input box and posts confirmation', async () => {
    ...
    expect(screen.getByText('是否允许 Jarvis 控制 Mock Notes')).toBeTruthy();
    expect(screen.queryByText('Jarvis 想控制这个应用来继续当前任务。')).toBeNull();

    fireEvent.click(screen.getByRole('button', { name: '同意' }));

    await waitFor(() => {
      expect(jarvisFetchMock).toHaveBeenCalledWith('/api/confirm/confirm-computer-1', expect.objectContaining({
        method: 'POST',
  ...
  it('re-enables approval actions when a new pending confirmation replaces the submitted one', async () => {
    ...
    await waitFor(() => {
      expect(jarvisFetchMock).toHaveBeenCalledWith('/api/confirm/confirm-computer-1', expect.any(Object));
    });
    ...
    await waitFor(() => {
      expect(jarvisFetchMock).toHaveBeenCalledWith('/api/confirm/confirm-computer-2', expect.any(Object));
    });
  });
  ...
  it('can switch the current ask-mode conversation to operate before confirming a tool action', async () => {
    ...
    try {
      ...
      fireEvent.click(screen.getByRole('menuitem', { name: '本对话全部自动执行' }));
      ...
      const callFor = (path: string) => jarvisFetchMock.mock.calls.find(call => call[0] === path);
      await waitFor(() => {
  ...
  it('portals the ask-mode bypass menu outside the confirmation card', () => {
    ...
    const menuItem = screen.getByRole('menuitem', { name: '本对话全部自动执行' });
  ...
  it('does not duplicate input-surface confirmations inside assistant message content', () => {
    ...
    expect(screen.queryByText('允许 Jarvis 使用电脑')).toBeNull();
