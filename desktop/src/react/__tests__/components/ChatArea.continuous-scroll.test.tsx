...
describe('ChatArea continuous bottom scroll', () => {
  beforeEach(() => {
    ...
    useStore.setState({
      ...
      sessions: [{ path: '/chat/scroll.jsonl', agentId: 'jarvis', title: null, firstMessage: '', modified: '', messageCount: 1 }],
      streamingSessions: ['/chat/scroll.jsonl'],
      agents: [{ id: 'jarvis', name: 'Jarvis', yuan: 'jarvis' }],
    } as never);
  ...
  it('mounts a current empty session panel instead of leaving the chat area blank', async () => {
    useStore.setState({
      ...
      sessions: [{ path: '/chat/empty.jsonl', agentId: 'jarvis', title: null, firstMessage: '', modified: '', messageCount: 0 }],
      streamingSessions: [],
    } as never);
