...
describe('MainContent app file drag attachments', () => {
  beforeEach(() => {
    ...
    useStore.setState({
      ...
      agentName: 'Jarvis',
      agentYuan: 'jarvis',
      agents: [{ id: 'agent-a', name: 'Jarvis', yuan: 'jarvis', isPrimary: true }],
      attachedFiles: [],
  ...
  it('ends the global drag lifecycle when SkillsPanel exclusively consumes the nested drop', async () => {
    vi.mocked(jarvisFetch).mockImplementation(async (url: string) => {
    ...
    await waitFor(() => expect(vi.mocked(jarvisFetch).mock.calls.filter(([url]) =>
    ...
    expect(vi.mocked(jarvisFetch).mock.calls.some(([url]) => url === '/api/upload')).toBe(false);
  });
  ...
  it('keeps an ordinary root drop on the chat attachment path exactly once', async () => {
    ...
    expect(vi.mocked(jarvisFetch).mock.calls.filter(([url]) => url === '/api/upload')).toHaveLength(1);
    expect(overlay).not.toHaveClass('visible');
  });
  ...
  it('attaches workspace files dragged from a native-root mount directly by absolute path', async () => {
    vi.mocked(jarvisFetch).mockClear();
    useStore.setState({
      currentTab: 'chat',
    ...
    await attachAppFileDragPayloadToInput({
      dragId: 'jarvis-drag-mount-workspace',
      source: 'workspace',
      files: [{
    ...
    expect(jarvisFetch).not.toHaveBeenCalled();
  });
  ...
  it('does not attach dragged files to the chat input while viewing channels', async () => {
    ...
    await attachAppFileDragPayloadToInput({
      dragId: 'jarvis-drag-channel-session-file',
      source: 'session-file',
      files: [{
        id: 'sf_channel',
        fileId: 'sf_channel',
