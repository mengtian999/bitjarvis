...
describe('ComputerUseOverlay', () => {
  beforeEach(() => {
    ...
    useStore.setState({
      ...
      sessions: [{ path: '/session/a.jsonl', sessionId: 'sess_a', agentId: 'jarvis' }],
  ...
  it('does not draw background action cursors in the Jarvis window', () => {
  ...
  it('does not draw done pulses in the Jarvis window', () => {
    useStore.getState().setComputerOverlayForSession('/session/a.jsonl', {
      phase: 'done',
      action: 'click_element',
      target: { coordinateSpace: 'element', elementId: 'mock-button' },
  ...
  it('keeps the Jarvis overlay reserved for foreground takeover UI only', () => {
    useStore.getState().setComputerOverlayForSession('/session/a.jsonl', {
      phase: 'running',
      action: 'click_point',
