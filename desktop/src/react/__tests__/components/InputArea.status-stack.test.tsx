...
vi.mock('../../hooks/use-jarvis-fetch', () => ({
  jarvisFetch: vi.fn(async () => new Response('{}', { status: 200 })),
...
describe('InputArea status stack', () => {
  ...
  it('reveals screenshot notice directories in the workspace tree without replacing the desk root', async () => {
    ...
    window.dispatchEvent(new CustomEvent('jarvis-inline-notice', {
      detail: {
        text: '截图已保存到工作目录下的「JARVIS-Works」文件夹',
        type: 'success',
        deskDir: '/workspace/JARVIS-Works',
      },
    }));
    ...
    expect(deskActionMocks.revealDeskDirectory).toHaveBeenCalledWith('/workspace/JARVIS-Works');
  ...
  it('opens the generated screenshot image directly when the notice carries a file path', async () => {
    ...
    expect(openFile).toHaveBeenCalledWith('/workspace/JARVIS-Works/截图/jarvis-20260617.png');
