...
describe('PreviewRenderer HTML isolation', () => {
  ...
  it('registers HTML and renders the local preview document in a sandboxed iframe', async () => {
    ...
    expect(mocks.jarvisFetch).toHaveBeenCalledWith('/api/preview/html', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        title: 'demo.html',
  ...
  it('passes the HTML preview asset root to the server registration route', () => {
    ...
    expect(mocks.jarvisFetch).toHaveBeenCalledWith('/api/preview/html', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({
        title: 'demo.html',
        content: htmlContent,
  ...
  it('applies a workspace image dropped on the markdown cover', async () => {
    ...
    await waitFor(() => {
      expect(mocks.jarvisFetch).toHaveBeenCalledWith('/api/desk/beautify/cover/apply', expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({
          filePath: '/tmp/workspace/demo.md',
