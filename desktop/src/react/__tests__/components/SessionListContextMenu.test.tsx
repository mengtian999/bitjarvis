...
vi.mock('../../hooks/use-jarvis-fetch', () => ({
  jarvisFetch: (...args: unknown[]) => jarvisFetchMock(...args),
...
function seedSessions() {
  useStore.setState({
    sessions: [
      {
        path: '/tmp/agents/jarvis/sessions/with-summary.jsonl',
      ...
      {
        path: '/tmp/agents/jarvis/sessions/no-summary.jsonl',
        title: 'No summary',
        firstMessage: 'hello',
...
describe('SessionList context menu', () => {
  beforeEach(() => {
    window.localStorage.removeItem('jarvis-session-sidebar-view-mode');
    window.localStorage.removeItem('jarvis-sidebar-ui-prefs');
    ...
    jarvisFetchMock.mockImplementation(async (url: string) => {
  ...
  it('keeps summaryless session rows readable and disables only the summary menu item', () => {
    ...
    expect(jarvisFetchMock).not.toHaveBeenCalledWith(
      '/api/sessions/summary?path=%2Ftmp%2Fagents%2Fjarvis%2Fsessions%2Fno-summary.jsonl',
    );
  });
  ...
  it('keeps the right-click menu as a shared narrow menu and opens summary as a click-through preview card', async () => {
    ...
    expect(screen.queryByTestId('session-summary-card')).not.toBeInTheDocument();
    expect(jarvisFetchMock).not.toHaveBeenCalledWith(
      '/api/sessions/summary?path=%2Ftmp%2Fagents%2Fjarvis%2Fsessions%2Fwith-summary.jsonl',
    );
  ...
  it('routes context menu actions through the existing session operations', async () => {
    ...
    expect(pinSessionMock).toHaveBeenCalledWith('/tmp/agents/jarvis/sessions/with-summary.jsonl', true);
    ...
    expect(renameSessionMock).toHaveBeenCalledWith(
      '/tmp/agents/jarvis/sessions/no-summary.jsonl',
      'Renamed summaryless session',
    );

    fireEvent.contextMenu(sessionButton('Has summary'), { clientX: 24, clientY: 32 });
    fireEvent.click(await screen.findByText('session.archive'));
    expect(archiveSessionMock).toHaveBeenCalledWith('/tmp/agents/jarvis/sessions/with-summary.jsonl');
  });
  ...
  it('opens the session browser from a left click on the sidebar badge', async () => {
    const browserStates = {
      '/tmp/agents/jarvis/sessions/with-summary.jsonl': {
    ...
    await waitFor(() => {
      expect(jarvisFetchMock).toHaveBeenCalledWith('/api/browser/open-session', expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ sessionPath: '/tmp/agents/jarvis/sessions/with-summary.jsonl' }),
      }));
      expect(openBrowserViewerMock).toHaveBeenCalledWith({
        sessionPath: '/tmp/agents/jarvis/sessions/with-summary.jsonl',
      });
    });
    expect(jarvisFetchMock).not.toHaveBeenCalledWith('/api/browser/close-session', expect.anything());
  ...
  it('closes the session browser from the badge context menu', async () => {
    ...
    await waitFor(() => {
      expect(jarvisFetchMock).toHaveBeenCalledWith('/api/browser/close-session', expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ sessionPath: '/tmp/agents/jarvis/sessions/with-summary.jsonl' }),
      }));
    });
  ...
  it('applies the persisted single-line row mode from the store to regular session rows', () => {
    ...
    expect(row).toHaveAttribute('title', expect.stringContaining('Jarvis'));
  });
  ...
  it('never fetches sidebar UI preferences itself', async () => {
    ...
    await waitFor(() => {
      expect(jarvisFetchMock).toHaveBeenCalledWith('/api/browser/session-states');
    });
    expect(jarvisFetchMock).not.toHaveBeenCalledWith('/api/preferences/sidebar-ui');
  });
  ...
  it('shows title search results first and then content results', async () => {
    jarvisFetchMock.mockImplementation(async (url: string) => {
      ...
      if (url.includes('phase=title')) {
        return jsonResponse({
          results: [{
            path: '/tmp/agents/jarvis/sessions/title-search.jsonl',
            title: '聊天记录搜索',
            firstMessage: 'hello',
      ...
      if (url.includes('phase=content')) {
        return jsonResponse({
          results: [{
            path: '/tmp/agents/jarvis/sessions/content-search.jsonl',
            title: '排查记录',
            firstMessage: 'hello',
    ...
    const searchCalls = jarvisFetchMock.mock.calls
      .map(([url]) => String(url))
    ...
    expect(switchSessionMock).toHaveBeenCalledWith('/tmp/agents/jarvis/sessions/title-search.jsonl');
  });
  ...
  it('renders unread output and running status as row-level status signals', async () => {
    useStore.setState({
      currentSessionPath: '/tmp/agents/jarvis/sessions/no-summary.jsonl',
      streamingSessions: ['/tmp/agents/jarvis/sessions/with-summary.jsonl'],
      unreadOutputSessionPaths: ['/tmp/agents/jarvis/sessions/with-summary.jsonl'],
    } as never);
  ...
  it('marks the pending switch row immediately without changing the committed session path', () => {
    useStore.setState({
      currentSessionPath: '/tmp/agents/jarvis/sessions/no-summary.jsonl',
      pendingSessionSwitchPath: '/tmp/agents/jarvis/sessions/with-summary.jsonl',
      streamingSessions: [],
      unreadOutputSessionPaths: [],
    } as never);
  ...
  it('creates a project directly through the project heading button', async () => {
    makeSessionsToday();
    jarvisFetchMock.mockImplementation(async (url: string, init?: RequestInit) => {
      if (url === '/api/browser/session-states') return jsonResponse({});
      if (url === '/api/session-projects') {
        return jsonResponse({ catalog: { folders: [], projects: [] } });
      }
    ...
    await waitFor(() => {
      expect(jarvisFetchMock).toHaveBeenCalledWith('/api/session-projects/projects', expect.objectContaining({
        method: 'POST',
  ...
  it('renames a project from the project row context menu', async () => {
    ...
    await waitFor(() => {
      expect(jarvisFetchMock).toHaveBeenCalledWith('/api/session-projects/projects/project-root', expect.objectContaining({
        method: 'PATCH',
  ...
  it('deletes a project and moves its visible sessions to uncategorized', async () => {
    ...
    useStore.setState({
      sessions: [{
        path: '/tmp/agents/jarvis/sessions/project-1.jsonl',
    ...
    jarvisFetchMock.mockImplementation(async (url: string, init?: RequestInit) => {
      ...
      if (url === '/api/session-projects/projects/project-root' && init?.method === 'DELETE') {
        return jsonResponse({
          ...
          assignment: { projectId: 'cwd:', sessionPaths: ['/tmp/agents/jarvis/sessions/project-1.jsonl'] },
        });
      }
      return jsonResponse({});
    });
  ...
  it('starts a new session draft inside a cwd project by carrying only cwd', async () => {
    useStore.setState({
      sessions: [{
        path: '/tmp/agents/jarvis/sessions/cwd-project.jsonl',
  ...
  it('shows five project sessions by default and persists the show-all expansion', async () => {
    useStore.setState({
      sessions: Array.from({ length: 6 }, (_, index) => ({
        path: `/tmp/agents/jarvis/sessions/project-${index + 1}.jsonl`,
    ...
    await waitFor(() => {
      ...
      expect(jarvisFetchMock).toHaveBeenCalledWith('/api/preferences/sidebar-ui', expect.objectContaining({
        method: 'PUT',
        body: JSON.stringify({
          projectView: {
            collapsedProjectIds: [],
  ...
  it('assigns a session when dragged onto a project row', async () => {
    ...
    await waitFor(() => {
      expect(jarvisFetchMock).toHaveBeenCalledWith('/api/session-projects/session-assignment', expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({
          sessionPath: '/tmp/agents/jarvis/sessions/with-summary.jsonl',
  ...
  it('reorders projects when a project is dragged onto another project at the same level', async () => {
    ...
    await waitFor(() => {
      expect(jarvisFetchMock).toHaveBeenCalledWith('/api/session-projects/projects/reorder', expect.objectContaining({
        method: 'POST',
  ...
  it('materializes cwd projects before reordering them by drag', async () => {
    useStore.setState({
      sessions: [
        {
          path: '/tmp/agents/jarvis/sessions/alpha.jsonl',
          title: 'Alpha session',
          firstMessage: 'hello',
        ...
        {
          path: '/tmp/agents/jarvis/sessions/beta.jsonl',
          title: 'Beta session',
          firstMessage: 'hello',
          modified: new Date().toISOString(),
          messageCount: 1,
    ...
    await waitFor(() => {
      expect(jarvisFetchMock).toHaveBeenCalledWith(`/api/session-projects/projects/${encodeURIComponent(alphaId)}`, expect.objectContaining({
        method: 'PATCH',
        body: JSON.stringify({ name: 'alpha-project', folderId: null }),
      }));
      expect(jarvisFetchMock).toHaveBeenCalledWith(`/api/session-projects/projects/${encodeURIComponent(betaId)}`, expect.objectContaining({
        method: 'PATCH',
  ...
  describe('pinned strip reordering', () => {
    function seedPinnedSessions(options: { withSessionIds?: boolean } = {}) {
      ...
      useStore.setState({
        sessions: [
          {
            path: '/tmp/agents/jarvis/sessions/pin-a.jsonl',
          ...
          {
            path: '/tmp/agents/jarvis/sessions/pin-b.jsonl',
          ...
          {
            path: '/tmp/agents/jarvis/sessions/pin-c.jsonl',
