...
describe('FileHistoryModal', () => {
  ...
  it('loads versions when a file is selected and restores on confirm', async () => {
    ...
    await waitFor(() => expect(mocks.fetchHistorySnapshot).toHaveBeenCalledWith('jarvis', 7));
    fireEvent.click(screen.getByTestId('fh-restore'));
    await waitFor(() => expect(mocks.restoreHistorySnapshot).toHaveBeenCalledWith('jarvis', 7));
  });
});
