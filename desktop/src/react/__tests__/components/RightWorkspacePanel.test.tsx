...
const tMap: Record<string, string> = {
  ...
  'rightWorkspace.sessionFiles.sendSuccess': '已发送到 Jarvis：飞书 · 小群',
  'rightWorkspace.sessionFiles.sendFailed': '发送到 Jarvis：飞书 · 小群 失败：boom',
...
function resetStore(items: ChatListItem[] = []) {
  useStore.setState({
    ...
    jianDrawerOpen: true,
    deskBasePath: '/tmp/jarvis-work',
    deskCurrentPath: '',
    deskFiles: [],
    deskJianContent: '',
    agents: [{ id: 'jarvis', name: 'Jarvis', yuan: 'jarvis', hasAvatar: false }],
...
describe('RightWorkspacePanel', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(jarvisFetch).mockImplementation(async () => jsonResponse({ sessions: [] }));
  ...
  it('renders extensible right workspace tabs and keeps workspace as the compatibility default', () => {
    ...
    expect(screen.getByText('jarvis-work')).toBeInTheDocument();
  ...
  it('renders a download-to-device action for resource-backed session files', () => {
    ...
    expect(download).toHaveAttribute('href', 'http://jarvis.local:14500/api/resources/res_sf_report/content');
  ...
  it('previews remote resource-backed session files through ResourceIO without local path controls', async () => {
    vi.mocked(jarvisFetch).mockImplementation(async (url) => {
    ...
    await waitFor(() => {
      expect(jarvisFetch).toHaveBeenCalledWith('/api/resources/res_sf_report/content');
    });
  ...
  it('sends session files to an existing Bridge target from the context submenu', async () => {
    ...
    vi.mocked(jarvisFetch).mockImplementation(async (path, init) => {
    ...
    fireEvent.click(await screen.findByText('Jarvis：飞书 · 小群'));
    ...
    expect(jarvisFetch).toHaveBeenCalledWith('/api/bridge/send-media?agentId=jarvis', expect.objectContaining({
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    }));
  });
  ...
  it('hides raw Jian execution status while preserving it when editing the instruction body', async () => {
    ...
    try {
      ...
      const saveCall = vi.mocked(jarvisFetch).mock.calls.find(([url, init]) => (
