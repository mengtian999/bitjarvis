...
vi.mock('../../hooks/use-jarvis-fetch', () => ({
  jarvisFetch: vi.fn(async () => new Response(JSON.stringify({ ok: true, result: null }), { status: 200 })),
...
function sendBlock(overrides: Record<string, unknown> = {}) {
  return {
    ...
    target: { type: 'session', sessionId: 'sid-target-1', sessionTitle: null, agentId: 'jarvis', agentName: 'Jarvis' },
    detail: {
      kind: 'session_send_draft',
...
function createBlock(overrides: Record<string, unknown> = {}) {
  return {
    ...
    target: { type: 'agent', agentId: 'jarvis', agentName: 'Jarvis' },
    detail: {
      kind: 'session_create_draft',
      draft: { agentId: 'jarvis', model: 'claude', title: '', firstMessage: 'first message body' },
    },
    actions: [{ id: 'view', kind: 'open' }],
    ...overrides,
  };
...
describe('SessionCollabDraftCard', () => {
  beforeEach(() => {
    ...
    vi.mocked(jarvisFetch).mockResolvedValue(new Response(JSON.stringify({ ok: true, result: null }), { status: 200 }));
  });
  ...
  it('renders a send draft card with the target agent avatar and the store-resolved session title', () => {
    useStore.setState({
      sessions: [
        { path: '/sessions/target.jsonl', sessionId: 'sid-target-1', title: 'Project kickoff', firstMessage: '', modified: '', messageCount: 0, agentId: 'jarvis', agentName: 'Jarvis', cwd: null },
      ],
    } as never);
  ...
  it('falls back to agent name + short id tail (never the raw sessionId) when no matching session is in the store', () => {
    ...
    expect(screen.queryByText('sid-target-1')).not.toBeInTheDocument();
    const fallbackTitle = screen.getByText('Jarvis …et-1');
  ...
  it('submits the edited message to /api/session-collab/apply and shows the sent state on success', async () => {
    ...
    await waitFor(() => {
      expect(jarvisFetch).toHaveBeenCalledWith('/api/session-collab/apply', expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({
          suggestionId: 'suggestion_send_1',
  ...
  it('shows the expired message and disables the confirm button on 404 draft_expired', async () => {
    vi.mocked(jarvisFetch).mockResolvedValue(
  ...
  it('shows a retryable error on 500 apply_failed and allows re-submitting', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'sessionCollab.confirmSend' }));
    await waitFor(() => {
      expect(vi.mocked(jarvisFetch).mock.calls.length).toBe(2);
    });
  });
  ...
  it('posts /api/session-collab/reject on ignore and converges to the rejected state on success', async () => {
    ...
    await waitFor(() => {
      expect(jarvisFetch).toHaveBeenCalledWith('/api/session-collab/reject', expect.objectContaining({
        method: 'POST',
