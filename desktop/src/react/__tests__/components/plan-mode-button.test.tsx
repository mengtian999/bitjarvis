...
describe('PlanModeButton', () => {
  ...
  it('opens a menu and marks permission changes from the pending new-session surface explicitly', async () => {
    vi.mocked(jarvisFetch).mockResolvedValueOnce(jsonResponse({ mode: 'auto' }));
    ...
    await waitFor(() => {
      expect(jarvisFetch).toHaveBeenCalledWith('/api/preferences/session-permission-default', expect.objectContaining({
        method: 'PUT',
  ...
  it('targets the active session when changing an existing conversation permission mode', async () => {
    vi.mocked(jarvisFetch).mockResolvedValueOnce(jsonResponse({ mode: 'operate' }));
    useStore.setState({
      currentSessionPath: '/tmp/jarvis-session.jsonl',
    ...
    await waitFor(() => {
      expect(jarvisFetch).toHaveBeenCalledWith('/api/session-permission-mode', expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({
          mode: 'operate',
          pendingNewSession: false,
          sessionPath: '/tmp/jarvis-session.jsonl',
        }),
      }));
    });
