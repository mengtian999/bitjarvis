...
function bodyFor(path: string) {
  const call = jarvisFetchMock.mock.calls.find(entry => entry[0] === path);
  return call?.[1]?.body ? JSON.parse(String(call[1].body)) : null;
}

describe('MCP tool approval prompt', () => {
  beforeEach(() => {
    jarvisFetchMock.mockClear();
  ...
  it('offers the two remembering options alongside the whole-conversation one', async () => {
    ...
    expect(screen.getByRole('menuitem', { name: '本对话全部自动执行' })).toBeTruthy();
  });
  ...
  it('records a permanent grant against the connector without erasing its peers', async () => {
    jarvisFetchMock.mockImplementation(async (path: string) => (path === '/api/mcp/state'
  ...
  it('leaves a non-MCP tool approval exactly as it was', async () => {
    ...
    await waitFor(() => expect(jarvisFetchMock).toHaveBeenCalledWith('/api/mcp/state', undefined));
    openMenu();
  ...
  it('offers a session grant for a non-MCP boundary tool that carries a synthetic capability', async () => {
    render(React.createElement(SessionConfirmationPrompt, {
      block: {
        ...approvalBlock({ toolName: 'browser', params: { action: 'click' } }),
        subject: { label: 'browser' },
        payload: { toolName: 'browser', params: { action: 'click' }, sessionGrantCapability: 'browser:click' },
      },
    }));

    // The session grant lives in the overflow menu (no visible button, to keep
    // the title from being squeezed).
    openMenu();
    expect(screen.getByTestId('mcp-approve-session')).toBeTruthy();
    // No permanent grant for non-MCP tools.
    expect(screen.queryByTestId('mcp-approve-always')).toBeNull();
  });
