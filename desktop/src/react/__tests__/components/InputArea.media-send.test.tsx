...
function seedSession() {
  useStore.setState({
    ...
    attachedFiles: [{
      fileId: 'sf_pasted',
      path: '/tmp/jarvis/session-files/pasted.png',
      name: 'pasted.png',
      isDirectory: false,
    }],
...
describe('InputArea media send', () => {
  ...
  beforeEach(() => {
    ...
    seedSession();
    mocks.jarvisFetch.mockResolvedValue(new Response(JSON.stringify({
      models: {
        vision_enabled: true,
  ...
  it('sends pasted image bytes through the platform API when window.jarvis is unavailable', async () => {
    ...
    expect(window.platform.readFileBase64).toHaveBeenCalledWith('/tmp/jarvis/session-files/pasted.png');
    ...
    expect(mocks.jarvisFetch).toHaveBeenCalledWith('/api/preferences/models', undefined);
  });
  ...
  it('keeps an async pending-session submission bound to its original session and leaves the new composer intact (#2078)', async () => {
    ...
    mocks.ensureSession.mockImplementationOnce(async (draftId: string) => {
      ...
      await createGate;
      return { sessionId: 'sess_a', sessionPath: '/session/a.jsonl', agentId: 'jarvis' };
    });
    useStore.setState({
      ...
      attachedFiles: [{
        fileId: 'sf_a',
        path: '/tmp/jarvis/session-files/a.png',
        name: 'a.png',
        isDirectory: false,
      }],
      attachedFilesBySession: {},
    ...
    useStore.setState({
      ...
      pendingNewSession: false,
      pendingDraftId: null,
      sessions: [{ path: '/session/b.jsonl', sessionId: 'sess_b', agentId: 'jarvis', agentName: 'Jarvis' }],
    ...
    expect(payload.displayMessage.attachments).toEqual([
      expect.objectContaining({ fileId: 'sf_a', path: '/tmp/jarvis/session-files/a.png' }),
    ]);
  ...
  it('uses the chat-scoped auxiliary vision route for mobile image preflight', async () => {
    ...
    expect(mocks.jarvisFetch).toHaveBeenCalledWith('/api/models/auxiliary-vision', undefined);
    expect(mocks.jarvisFetch.mock.calls.some(([path]) => path === '/api/preferences/models')).toBe(false);
  });
  ...
  it('sends audio bytes natively for official MiMo audio models', async () => {
    useStore.setState({
      attachedFiles: [{
        fileId: 'sf_voice',
        path: '/tmp/jarvis/session-files/voice.wav',
        name: 'voice.wav',
        isDirectory: false,
      }],
      attachedFilesBySession: {
    ...
    const payload = JSON.parse(String(mocks.wsSend.mock.calls[0][0]));
    expect(window.platform.readFileBase64).toHaveBeenCalledWith('/tmp/jarvis/session-files/voice.wav');
  ...
  it('uses the switched session model audio snapshot when sending recorded audio', async () => {
    useStore.setState({
      attachedFiles: [{
        fileId: 'sf_recording',
        path: '/tmp/jarvis/session-files/recording.wav',
        name: '录音 1.wav',
        isDirectory: false,
        mimeType: 'audio/wav',
      }],
    ...
    const payload = JSON.parse(String(mocks.wsSend.mock.calls[0][0]));
    expect(window.platform.readFileBase64).toHaveBeenCalledWith('/tmp/jarvis/session-files/recording.wav');
    expect(payload.text).toBe('');
    expect(payload.audios).toEqual([{
      type: 'audio',
      data: 'AUDIO_BASE64',
  ...
  it('sends recorded audio immediately after saving the recording', async () => {
    ...
    mocks.jarvisFetch.mockImplementation(async (path: string) => {
      if (path === '/api/upload-blob') {
        return new Response(JSON.stringify({
          uploads: [{
            fileId: 'sf_recording',
            dest: '/tmp/jarvis/session-files/recording.wav',
            name: '录音 1.wav',
    ...
    const uploadBody = JSON.parse(String(mocks.jarvisFetch.mock.calls[0][1]?.body));
  ...
  it('keeps video attachments on the file-only path for unsupported models', async () => {
    useStore.setState({
      attachedFiles: [{
        fileId: 'sf_clip',
        path: '/tmp/jarvis/session-files/clip.mp4',
        name: 'clip.mp4',
        mimeType: 'video/mp4',
        isDirectory: false,
      }],
