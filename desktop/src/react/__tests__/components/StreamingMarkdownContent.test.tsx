...
describe('StreamingMarkdownContent', () => {
  ...
  it('keeps stream motion off React animation frames and limits CSS to opacity or tiny transforms', () => {
    ...
    expect(tailBlock).toContain('jarvis-stream-tail-in');
    expect(tailBlock).not.toContain('requestAnimationFrame');
    expect(cardBlock).toContain('jarvis-chat-soft-up-in');
    expect(animations).toContain('@keyframes jarvis-stream-tail-in');
    expect(animations).toContain('@keyframes jarvis-chat-soft-down-in');
    expect(animations).toContain('@keyframes jarvis-chat-soft-up-in');
  });
});
