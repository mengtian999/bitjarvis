...
describe('AppPages page ownership', () => {
  ...
  it('keeps the workspace companion on plugin pages without carrying the file preview', () => {
    useStore.setState({ currentTab: 'plugin:jarvis-hyperframes' } as never);

    render(<AppPages />);

    expect(screen.getByTestId('plugin-page')).toHaveTextContent('jarvis-hyperframes');
