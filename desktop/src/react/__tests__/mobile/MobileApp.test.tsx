...
describe('MobileApp', () => {
  ...
  beforeEach(() => {
    ...
    delete window.__jarvisMobileUpdateAvailable;
  ...
  it('loads locale before showing the access-key login when no browser session exists', async () => {
    ...
    expect(await screen.findByText('手机访问 Jarvis')).toBeInTheDocument();
  ...
  it('can submit a username and password login without sending a device credential', async () => {
    ...
    fireEvent.change(screen.getByLabelText('用户名'), { target: { value: 'jarvis-owner' } });
    ...
    await waitFor(() => {
      ...
      expect(body).toEqual({ username: 'jarvis-owner', password: 'secret-password' });
  ...
  it('loads chat sessions, desktop input surface, and workbench files for an authenticated phone', async () => {
    ...
    expect(useStore.getState().sessions.some(session => session.path === '/jarvis/sessions/one.jsonl')).toBe(true);
    expect(useStore.getState().sessionLocatorsById.sess_mobile_one).toEqual({ path: '/jarvis/sessions/one.jsonl' });
  ...
  it('shows a controlled reload action when the PWA has an update ready', async () => {
    ...
    window.addEventListener('jarvis-mobile-apply-update', applyUpdate);
    fetchMock.mockImplementation((input: RequestInfo | URL, options?: RequestInit) => {
      const url = String(input);
      if (url.includes('/api/web-auth/session')) {
        return Promise.resolve(jsonResponse({ authenticated: true, principal: principal(['chat', 'resources.read', 'files.read', 'files.write']) }));
    ...
    try {
      ...
      act(() => {
        window.dispatchEvent(new Event('jarvis-mobile-update-available'));
      });
      ...
      window.removeEventListener('jarvis-mobile-apply-update', applyUpdate);
    }
  });
  ...
  it('keeps a PWA update notice that arrives before the mobile shell is ready', async () => {
    const applyUpdate = vi.fn();
    window.addEventListener('jarvis-mobile-apply-update', applyUpdate);
    window.__jarvisMobileUpdateAvailable = true;
    fetchMock.mockImplementation((input: RequestInfo | URL, options?: RequestInit) => {
      const url = String(input);
      if (url.includes('/api/web-auth/session')) {
        return Promise.resolve(jsonResponse({ authenticated: true, principal: principal(['chat', 'resources.read', 'files.read', 'files.write']) }));
  ...
  it('re-pulls the open session on foreground when its revision drifted while backgrounded (#1610)', async () => {
    const sessionPath = '/jarvis/sessions/rc.jsonl';
    ...
    fetchMock.mockImplementation((input: RequestInfo | URL, options?: RequestInit) => {
      ...
      if (url.includes('/api/sessions') && !url.includes('/api/sessions/')) {
        return Promise.resolve(jsonResponse([
          { path: sessionPath, title: 'RC 会话', firstMessage: '', modified: '2026-06-10T00:00:00.000Z', messageCount: 2, agentId: 'jarvis', agentName: 'Jarvis', cwd: '/workspace', revision: diskRevision },
        ]));
      }
      return Promise.resolve(jsonResponse(jsonResponseForMobile(url, options)));
    });
  ...
  it('does not re-pull the open session on foreground when its revision is unchanged', async () => {
    const sessionPath = '/jarvis/sessions/idle.jsonl';
    fetchMock.mockImplementation((input: RequestInfo | URL, options?: RequestInit) => {
      const url = String(input);
      if (url.includes('/api/web-auth/session')) {
        return Promise.resolve(jsonResponse({ authenticated: true, principal: principal(['chat', 'resources.read', 'files.read', 'files.write']) }));
      ...
      if (url.includes('/api/sessions') && !url.includes('/api/sessions/')) {
        return Promise.resolve(jsonResponse([
          { path: sessionPath, title: '安静会话', firstMessage: '', modified: '2026-06-10T00:00:00.000Z', messageCount: 2, agentId: 'jarvis', agentName: 'Jarvis', cwd: '/workspace', revision: 'rev-stable' },
        ]));
      }
      return Promise.resolve(jsonResponse(jsonResponseForMobile(url, options)));
    });
  ...
  it('syncs the selected session permission mode from the mobile session list', async () => {
    ...
    window.addEventListener('jarvis-plan-mode', listener);
    fetchMock.mockImplementation((input: RequestInfo | URL, options?: RequestInit) => {
      const url = String(input);
      if (url.includes('/api/web-auth/session')) {
        return Promise.resolve(jsonResponse({ authenticated: true, principal: principal(['chat', 'resources.read', 'files.read', 'files.write']) }));
      ...
      if (url.includes('/api/sessions')) {
        return Promise.resolve(jsonResponse([
          { path: '/jarvis/sessions/one.jsonl', title: '日常记录', firstMessage: '', modified: '2026-05-16T00:00:00.000Z', messageCount: 2, agentId: 'jarvis', agentName: 'Jarvis', cwd: '/workspace', permissionMode: 'read_only' },
        ]));
      }
      return Promise.resolve(jsonResponse(jsonResponseForMobile(url, options)));
    });
    ...
    try {
      ...
      window.removeEventListener('jarvis-plan-mode', listener);
    }
  });
  ...
  it('resets mobile global new chat to the primary agent effective default workspace', async () => {
    ...
    expect(useStore.getState().selectedAgentId).toBe('jarvis');
    ...
    act(() => {
      MockWebSocket.instances[0]?.onmessage?.({
        data: JSON.stringify({
          ...
          sessionPath: '/jarvis/sessions/one.jsonl',
  ...
  it('shows server-created sessions from another LAN client without waiting for a refetch payload', async () => {
    ...
    act(() => {
      MockWebSocket.instances[0]?.onmessage?.({
        data: JSON.stringify({
          ...
          sessionPath: '/jarvis/sessions/from-desktop.jsonl',
          session: {
            path: '/jarvis/sessions/from-desktop.jsonl',
            title: '电脑新会话',
    ...
    await waitFor(() => {
      expect(useStore.getState().sessions.some(session => session.path === '/jarvis/sessions/from-desktop.jsonl')).toBe(true);
...
function jsonResponseForMobile(
  ...
  if (url.includes('/api/sessions')) {
    return [
      { path: '/jarvis/sessions/one.jsonl', sessionId: 'sess_mobile_one', title: '日常记录', firstMessage: '', modified: '2026-05-16T00:00:00.000Z', messageCount: 2, agentId: 'jarvis', agentName: 'Jarvis', cwd: '/workspace' },
    ];
  }
  return {};
}
