...
describe('Mobile PWA entry split', () => {
  ...
  it('wires service worker update detection to an explicit mobile refresh event', () => {
    ...
    expect(entrySource).toContain('jarvis-mobile-update-available');
    expect(entrySource).toContain('jarvis-mobile-apply-update');
    expect(entrySource).toContain('__jarvisMobileUpdateAvailable');
  ...
  it('serves mobile PWA assets explicitly in Vite dev instead of falling back to HTML', () => {
    ...
    expect(viteConfig).toContain("name: 'jarvis-serve-mobile-pwa-static-files'");
