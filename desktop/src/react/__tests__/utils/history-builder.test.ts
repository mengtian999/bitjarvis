...
describe('buildItemsFromHistory user image restoration', () => {
    ...
    const items = buildItemsFromHistory({
      messages: [
        ...
        },
        {
          ...
          role: 'user',
          content: '<jarvis-background-result task-id="task-1">done</jarvis-background-result>',
        },
        {
          id: '3',
  ...
  it('把 media-only attached_image 占位恢复成裸图片附件', () => {
    const items = buildItemsFromHistory({
      messages: [{
        id: 'u1',
        role: 'user',
        content: '[attached_image: /Users/test/.jarvis/attachments/upload-abc.png]\n(看图)',
      }],
    });
    ...
    expect(first.data.attachments).toEqual([{
      path: '/Users/test/.jarvis/attachments/upload-abc.png',
      name: 'upload-abc.png',
      isDir: false,
    }]);
  });
  ...
  it('原生 image block 与 attached_image 路径合并为一个图片附件', () => {
    const items = buildItemsFromHistory({
      messages: [{
        id: 'u1',
        role: 'user',
        content: '[attached_image: /Users/test/.jarvis/attachments/upload-native.png]\n看看这个',
    ...
    expect(first.data.attachments).toEqual([{
      path: '/Users/test/.jarvis/attachments/upload-native.png',
  ...
  it('把 attached_audio 标记恢复成音频附件，并从正文隐藏', () => {
    const items = buildItemsFromHistory({
      messages: [{
        id: 'u1',
        role: 'user',
        content: '[attached_audio: /Users/test/.jarvis/session-files/voice.wav]\n听一下',
      }],
    });
    ...
    expect(first.data.attachments).toEqual([{
      path: '/Users/test/.jarvis/session-files/voice.wav',
      name: 'voice.wav',
      isDir: false,
    }]);
  });
  ...
  it('从 SessionFile 账本恢复历史音频附件的展示名和元信息', () => {
    const filePath = '/Users/test/.jarvis/session-files/hash/录音 1_mpykkdz7_35680467.wav';
    const items = buildItemsFromHistory({
      messages: [{
        id: 'u1',
        role: 'user',
  ...
  it('用 Fork 文件的旧 id 和旧路径别名恢复未改写的历史附件', () => {
    const parentPath = '/Users/test/.jarvis/session-files/parent/voice.wav';
    const childPath = '/Users/test/.jarvis/session-files/child/voice.wav';
    const items = buildItemsFromHistory({
      messages: [{
        id: 'u-forked-file',
        role: 'user',
  ...
  it('从 SessionFile 账本恢复旧版图片附件，并把模型传输说明排除出可见正文', () => {
    const filePath = '/Users/test/.jarvis/uploads/粘贴图片_mpyjx6zr_fc3d70a9.png';
    const items = buildItemsFromHistory({
      messages: [{
        id: 'u1',
        role: 'user',
  ...
  it('从 SessionFile 机器上下文恢复展示名附件，并把机器行排除出可见正文', () => {
    const filePath = '/Users/test/.jarvis/uploads/报告2026_mq6l.txt';
    const items = buildItemsFromHistory({
      messages: [{
        id: 'u-session-file',
        role: 'user',
