...
describe('remote file preview workbench refs', () => {
  beforeEach(() => {
    ...
    mockJarvisFetch.mockResolvedValue({
  ...
  it('saves remote workbench content by mountId instead of producing a legacy rootId-only request', async () => {
    ...
    expect(String(mockJarvisFetch.mock.calls[0][1].body)).not.toContain('"rootId"');
  });
  ...
  it('uses the content response version when opening a remote workbench document', async () => {
    ...
    mockJarvisFetch.mockResolvedValueOnce(new Response('# fresh\n', {
      status: 200,
      headers: {
        'X-Jarvis-File-MtimeMs': '1781989413542',
        'X-Jarvis-File-Size': '8',
      },
    }));
  ...
  it('does not downgrade a remote workbench refresh to a stale target version', async () => {
    ...
    mockJarvisFetch.mockResolvedValueOnce(new Response('fresh external body', {
      status: 200,
      headers: {
        'X-Jarvis-File-MtimeMs': String(diskVersion.mtimeMs),
        'X-Jarvis-File-Size': String(diskVersion.size),
      },
    }));
  ...
  it('opens remote default workbench documents through ResourceIO content even in desktop runtime', async () => {
    ...
    mockJarvisFetch.mockResolvedValueOnce(new Response('# remote\n', { status: 200 }));
    ...
    expect(mockJarvisFetch).toHaveBeenCalledWith(expect.stringMatching(
  ...
  it('opens remote session FileRefs through ResourceIO content instead of native paths', async () => {
    ...
    mockJarvisFetch.mockResolvedValueOnce(new Response('# file ref\n', { status: 200 }));
    ...
    expect(mockOpenFilePreview).not.toHaveBeenCalled();
    expect(mockJarvisFetch).toHaveBeenCalledWith('/api/resources/res_sf_report/content');
    expect(useStore.getState().previewItems[0]).toMatchObject({
      title: 'report.md',
      content: '# file ref\n',
      storageKind: 'remote-content',
