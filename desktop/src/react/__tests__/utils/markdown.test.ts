...
describe('renderMarkdown', () => {
  ...
  it('renders markdown footnotes with clickable refs and backrefs', () => {
    ...
    const refMatch = html.match(
      /<sup class="footnote-ref"><a href="#(?<footnoteId>fn-jarvis-fn-[a-z0-9]+-1)" id="(?<refId>fnref-jarvis-fn-[a-z0-9]+-1)" role="doc-noteref">1<\/a><\/sup>/,
    );
  ...
  it('uses stable but source-scoped footnote ids', () => {
    ...
    const firstId = first.match(/id="(?<id>fn-jarvis-fn-[a-z0-9]+-1)"/)?.groups?.id;
    const firstAgainId = firstAgain.match(/id="(?<id>fn-jarvis-fn-[a-z0-9]+-1)"/)?.groups?.id;
    const secondId = second.match(/id="(?<id>fn-jarvis-fn-[a-z0-9]+-1)"/)?.groups?.id;
  ...
  it('preserves safe generated footnote markup in markdown preview mode', () => {
    ...
    expect(html).toMatch(/<sup class="footnote-ref"><a href="#fn-jarvis-fn-[a-z0-9]+-1" id="fnref-jarvis-fn-[a-z0-9]+-1" role="doc-noteref" rel="noopener noreferrer">1<\/a><\/sup>/);
    expect(html).toMatch(/<li id="fn-jarvis-fn-[a-z0-9]+-1">/);
    expect(html).toMatch(/<a href="#fnref-jarvis-fn-[a-z0-9]+-1" class="footnote-backref" role="doc-backlink" title="Jump back to reference" rel="noopener noreferrer">(?:↩|&#8617;)<\/a>/);
  });
  ...
  it('strips unsafe raw footnote ids classes and roles in markdown preview mode', () => {
    const html = renderMarkdownPreview([
      ...
      '<a id="fnref-jarvis-fn-safe-1" class="footnote-backref evil" href="javascript:alert(2)" role="button">back</a>',
      '</li>',
      '</ol>',
      '</section>',
    ].join(''));
    ...
    expect(html).toContain('<a id="fnref-jarvis-fn-safe-1" class="footnote-backref">back</a>');
