...
describe('parseMoodFromContent', () => {
  ...
  it('解析 <mood> 标签', () => {
    ...
    expect(result.yuan).toBe('jarvis');
...
describe('parseUserAttachments', () => {
  ...
  const reminder = [
    '[jarvis_reminder at 2026-07-10 09:05]',
  ...
  it('未闭合或畸形 reminder 原样保留', () => {
    const unclosed = '[jarvis_reminder at 2026-07-10 09:05]\n- Current time: 2026-07-10 09:05\nhello';
    const unclosedStatic = '[jarvis_reminder]\n- Current time: 2026-07-10 09:05\nhello';
    const malformed = '[jarvis_reminder sometime]\nsecret\n[/jarvis_reminder]\nhello';
  ...
  it('解析内部 attached_image 标记为图片引用，并从正文隐藏', () => {
    const input = '[attached_image: /Users/test/.jarvis/attachments/upload-abc.png]\n(看图)';
    const result = parseUserAttachments(input);
...
describe('moodLabel', () => {
  it('jarvis 返回 MOOD', () => {
    expect(moodLabel('jarvis')).toContain('MOOD');
  });
