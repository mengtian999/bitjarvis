...
describe('screenshot utils', () => {
  ...
  beforeEach(() => {
    ...
    window.addEventListener('jarvis-inline-notice', noticeHandler);
  ...
  afterEach(() => {
    vi.unstubAllGlobals();
    window.removeEventListener('jarvis-inline-notice', noticeHandler);
    delete (window as any).jarvis;
  ...
  it('主进程 IPC reject 时，给用户发出明确失败提示而不是变成未处理异常', async () => {
    ...
    expect((window as any).jarvis.screenshotRender).toHaveBeenCalledOnce();
  ...
  it('Markdown article screenshots carry source file context for relative attachments', async () => {
    (window as any).jarvis = {
      screenshotRender: vi.fn().mockResolvedValue({ success: true, dir: '/tmp/jarvis-home/截图' }),
    };
    ...
    expect((window as any).jarvis.screenshotRender).toHaveBeenCalledWith(expect.objectContaining({
      mode: 'article',
      markdown: '![](<文本附件/a.png>)',
      filePath: '/vault/note.md',
      articleType: 'markdown',
  ...
  it('article screenshot saved notices carry the generated image path', async () => {
    (window as any).jarvis = {
      screenshotRender: vi.fn().mockResolvedValue({
        success: true,
        dir: '/workspace/JARVIS-Works/截图',
        filePath: '/workspace/JARVIS-Works/截图/jarvis-20260617.png',
      }),
    };
    ...
    expect(notices).toEqual([
      expect.objectContaining({
        type: 'success',
        deskDir: '/workspace/JARVIS-Works/截图',
        filePath: '/workspace/JARVIS-Works/截图/jarvis-20260617.png',
      }),
    ]);
  });
  ...
  it('Markdown file screenshots read the file and save under the provided workspace', async () => {
    const jarvis = {
      screenshotRender: vi.fn().mockResolvedValue({ success: true, dir: '/workspace/JARVIS-Works/截图' }),
    };
    ...
    expect(jarvis.screenshotRender).toHaveBeenCalledWith(expect.objectContaining({
      mode: 'article',
      markdown: '# Report',
      filePath: '/workspace/report.md',
      articleType: 'markdown',
  ...
  it('screenshot font follows the reading font when no screenshot override is selected', async () => {
    localStorage.setItem('jarvis-font-serif', '0');
    (window as any).jarvis = {
      screenshotRender: vi.fn().mockResolvedValue({ success: true, dir: '/tmp/jarvis-home/截图' }),
    };
  ...
  it('screenshot font can override the reading font explicitly', async () => {
    localStorage.setItem('jarvis-font-serif', '0');
    localStorage.setItem('jarvis-screenshot-font', 'serif');
    (window as any).jarvis = {
      screenshotRender: vi.fn().mockResolvedValue({ success: true, dir: '/tmp/jarvis-home/截图' }),
    };
  ...
  it('按截图页更新页码，并按选中的消息块数推进总进度', async () => {
    ...
    expect((window as any).jarvis.screenshotRender).toHaveBeenCalledTimes(2);
    expect((window as any).jarvis.screenshotRender).toHaveBeenNthCalledWith(1, expect.objectContaining({
      locale: 'zh',
      segmentIndex: 1,
      segmentTotal: 2,
    }));
    expect((window as any).jarvis.screenshotRender).toHaveBeenNthCalledWith(2, expect.objectContaining({
      segmentIndex: 2,
      segmentTotal: 2,
    }));
  });
  ...
  it('自定义头像缺失时，截图 payload 仍烧录普通聊天 UI 的默认头像', async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.includes('assets/Jarvis.png')) {
    ...
    const payload = (window as any).jarvis.screenshotRender.mock.calls[0][0];
    ...
    expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('assets/Jarvis.png'));
  });
