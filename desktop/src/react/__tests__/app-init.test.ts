...
describe('initApp bridge indicator', () => {
  ...
  it('treats wechat as a connected bridge when bootstrapping the sidebar dot', async () => {
    ...
    mockJarvisFetch
      ...
      .mockResolvedValueOnce(jsonResponse({ agentId: 'jarvis', agent: 'Jarvis', user: 'User', avatars: {} }))
      .mockResolvedValueOnce(jsonResponse({ locale: 'zh-CN' }))
      .mockResolvedValueOnce(jsonResponse({ agents: [{ id: 'jarvis', isPrimary: true }] }))
    ...
    expect(mockJarvisFetch).toHaveBeenCalledWith('/api/bridge/status?agentId=jarvis');
  });
  ...
  it('refreshes the HttpOnly device web session before opening WebSocket for a persisted LAN frontend', async () => {
    ...
    (globalThis as Record<string, unknown>).window = {
      ...
      localStorage: {
        getItem: vi.fn((key: string) => key === 'jarvis-server-connections-v1' ? persistedLanConnectionJson() : null),
        setItem: vi.fn(),
        removeItem: vi.fn(),
      },
      platform: {
    ...
    expect(mockJarvisFetch).toHaveBeenNthCalledWith(1, '/api/web-auth/login', expect.objectContaining({
      method: 'POST',
      credentials: 'include',
      body: JSON.stringify({ credential: 'fixture-key' }),
    }));
    expect(mockJarvisFetch).toHaveBeenNthCalledWith(2, '/api/server/identity');
    ...
    expect((mockJarvisFetch.mock.invocationCallOrder[0] ?? 0)).toBeLessThan(mockConnectWebSocket.mock.invocationCallOrder[0] ?? 0);
  });
  ...
  it('stops startup explicitly when server identity cannot be loaded', async () => {
    ...
    mockJarvisFetch.mockRejectedValueOnce(new Error('identity unavailable'));

    const { initApp } = await import('../app-init');
    await initApp();

    expect(mockJarvisFetch).toHaveBeenCalledWith('/api/server/identity');
  ...
  it('initializes the pending workspace from agent home even when cwd history points elsewhere', async () => {
    ...
    expect(mockJarvisFetch).toHaveBeenCalledWith('/api/agents/jarvis/config');
