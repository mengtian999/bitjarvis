 ...
 *   t("sidebar.title")               // → "对话"
 *   t("tool.bash.running", { name }) // → "💻 Jarvis 正在小心翼翼地用你的电脑"
 *
...
const i18n = {
  ...
  async _fetchJson(relativePath) {
    ...
    try {
      ...
      // 在 Electron file:// 协议下 fetch 可能会被浏览器安全策略拦截，继续尝试 XHR 回落
    }

    // 2. XMLHttpRequest 读取 (Electron file:// 协议兼容性极好)
    try {
      ...
      // 3. Electron 渲染进程环境 IPC 读取回落
      if (typeof window !== "undefined" && window.jarvis?.readFile) {
        try {
          let filePath = decodeURIComponent(url.replace(/^file:\/\/\/?/, ""));
          if (/^[A-Za-z]:\//.test(filePath)) {
            filePath = filePath.replace(/\//g, "\\");
          }
          const content = await window.jarvis.readFile(filePath);
    ...
    throw new Error(`Unable to load JSON at ${relativePath}`);
  },
  ...
  async load(locale) {
    ...
    this.locale = key;
    try {
      this._data = await this._fetchJson(`./locales/${key}.json`);
    } catch (err) {
      console.error(`[i18n] Failed to load locale "${key}":`, err);
      if (key !== "en") {
        try {
          this._data = await this._fetchJson("./locales/en.json");
        } catch { this._data = {}; }
      } else {
        this._data = {};
      }
   ...
   * @param {object} [vars] 占位符变量，如 { name: "Jarvis", n: 3 }
   * @returns {string}
   */
  t(path, vars) {
