...
const JARVIS_PDF_FONT_FAMILIES = ["EB Garamond", "Noto Serif SC", "JetBrains Mono"];
 ...
 * 通过 JARVIS_RENDERER_DIST 显式注入，不能在这里从相对位置猜测。
 */
...
function locateThemesDir(candidates = defaultThemesDirCandidates()) {
  ...
  throw new Error(
    `Jarvis font css (${FONTS_CSS_FILENAME}) not found; looked in: ${candidates.join(", ")}. ` +
 ...
 *
 * JARVIS_RENDERER_DIST 一旦注入就是生产真值源：错误指针必须显式失败，不能
...
function resolveThemesDir({ env = process.env, fallbackCandidates = defaultThemesDirCandidates() } = {}) {
  ...
  if (injected !== undefined) {
    ...
    if (!rendererDist || !path.isAbsolute(rendererDist)) {
      throw new Error(`JARVIS_RENDERER_DIST must be an absolute path: ${injected}`);
    }
    ...
    if (!hasFontsCss(themesDir)) {
      throw new Error(
        `JARVIS_RENDERER_DIST is set to ${rendererDist}, but its Jarvis font css is missing or not a file: ${cssPath}`,
      );
    }
    return themesDir;
  }
...
function rewriteAndValidateFontUrls(block, { cssPath, fontsDir, family }) {
  ...
  const rewritten = block.replace(
    ...
    (_match, _quote, rawUrl) => {
      ...
      if (!fontUrl.startsWith("./fonts/")) {
        throw new Error(
          `Jarvis font css at ${cssPath} has an unsupported font URL for ${family}: ${fontUrl}`,
        );
      }
      ...
      if (!relativeFontPath || !isPathInside(fontsDir, fontPath)) {
        throw new Error(
          `Jarvis font css at ${cssPath} has a font URL outside themes/fonts for ${family}: ${fontUrl}`,
        );
      }
      ...
      try {
        ...
        throw new Error(
          `Jarvis font css at ${cssPath} references a missing font file for ${family}: ${fontPath}`,
        );
      }
      if (!stat.isFile() || stat.size <= 0) {
        throw new Error(
          `Jarvis font css at ${cssPath} references a font that is not a non-empty file for ${family}: ${fontPath}`,
        );
      }
  ...
  if (declaredUrlCount === 0 || matchedUrlCount !== declaredUrlCount) {
    throw new Error(`Jarvis font css at ${cssPath} has an invalid font URL declaration for ${family}`);
  }
  return rewritten;
}

function validatedFontInjectionCss({ themesDir, families = JARVIS_PDF_FONT_FAMILIES }) {
  const cssPath = path.join(themesDir, FONTS_CSS_FILENAME);
  ...
  if (missing.length > 0) {
    throw new Error(`Jarvis font css at ${cssPath} is missing families: ${missing.join(", ")}`);
  }
 ...
 */
function assertOfficePdfFontAssets({ themesDir, families = JARVIS_PDF_FONT_FAMILIES } = {}) {
  if (typeof themesDir !== "string" || !themesDir) {
    throw new Error("themesDir is required to validate Jarvis PDF font assets");
  }
 ...
 * 也不静默产出回退字体的 PDF。
 */
function buildFontInjectionCss({ themesDir = resolveThemesDir(), families = JARVIS_PDF_FONT_FAMILIES } = {}) {
  return validatedFontInjectionCss({ themesDir, families });
}

module.exports = {
  FONTS_CSS_FILENAME,
  JARVIS_PDF_FONT_FAMILIES,
