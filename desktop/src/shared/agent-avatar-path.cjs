 ...
 *   {jarvisHome}/agents/{agentId}/avatars/agent.{png|jpg|jpeg|webp}
...
/**
 * @param {string|null|undefined} jarvisHome - resolved JARVIS_HOME data root
 ...
 */
function resolveAgentAvatarPath(jarvisHome, agentId) {
  if (typeof jarvisHome !== "string" || jarvisHome.length === 0) return null;
  if (!isSafeAgentId(agentId)) return null;

  const avatarDir = path.join(jarvisHome, "agents", agentId, "avatars");
