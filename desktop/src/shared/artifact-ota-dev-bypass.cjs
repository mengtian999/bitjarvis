 ...
 *
 * `JARVIS_ARTIFACT_MANIFEST=<path|url>` lets a developer rehearse the full
 ...
 * unconditional alias entry there) — the string "JARVIS_ARTIFACT_MANIFEST"
 ...
 * reference `process.env.JARVIS_ARTIFACT_MANIFEST` from any other file in
 * the OTA module — this file is the ONLY place that name may appear.
 */

function resolveDevManifestOverride() {
  const value = process.env.JARVIS_ARTIFACT_MANIFEST;
