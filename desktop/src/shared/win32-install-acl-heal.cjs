...
function getInstallAclHealStatePath(jarvisHome) {
  return path.join(jarvisHome, STATE_FILE);
}

// 该文件只是"是否已 grant 过"的幂等记账；损坏时按空处理并在下次写入时重建，
// 代价是多跑一次幂等 icacls，自我修正，不构成静默降级。
function readHealState(jarvisHome) {
  try {
    const parsed = JSON.parse(fs.readFileSync(getInstallAclHealStatePath(jarvisHome), "utf-8"));
    if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) return parsed;
  } catch {}
  return { version: STATE_VERSION, ineffectiveCount: 0, heal: null };
}

function writeHealState(jarvisHome, state) {
  const filePath = getInstallAclHealStatePath(jarvisHome);
...
function judgeProbeOutcome({ jarvisHome, state, heal, policy, now }) {
  const evidence = policy.getGpuRecoveryEvidence(jarvisHome);
  ...
  if (stableHardwareStartup && (state.ineffectiveCount || 0) !== 0) {
    writeHealState(jarvisHome, { ...state, ineffectiveCount: 0 });
...
function maybeHealWin32InstallAcl({
  ...
  if (!jarvisHome) throw new Error("maybeHealWin32InstallAcl requires jarvisHome");
  if (platform !== "win32") return { status: "skipped", reason: "platform" };
  if (envFlag(env?.JARVIS_DISABLE_INSTALL_ACL_HEAL)) return { status: "skipped", reason: "disabled" };
  if (!isPackaged && !envFlag(env?.JARVIS_FORCE_INSTALL_ACL_HEAL)) {
  ...
  const state = readHealState(jarvisHome);
  ...
  if (heal?.status === "ok") {
    ...
    // 这里续跑探测而不是把它误判成"已处理完"。
    if (!heal.probedAt) return runProbe({ jarvisHome, state, heal, policy, now });
    return judgeProbeOutcome({ jarvisHome, state, heal, policy, now });
  }
  ...
  try {
    ...
    writeHealState(jarvisHome, { ...state, heal: nextHeal });
  ...
  writeHealState(jarvisHome, grantedState);
  return runProbe({ jarvisHome, state: grantedState, heal: grantedHeal, policy, now });
}

function buildInstallAclHealDiagnostics({ jarvisHome } = {}) {
  const state = jarvisHome ? readHealState(jarvisHome) : { heal: null };
  return [
    "",
    "--- Install ACL Heal ---",
    ...
    "Env switches: JARVIS_DISABLE_INSTALL_ACL_HEAL=1 disables the heal; JARVIS_FORCE_INSTALL_ACL_HEAL=1 forces it in unpackaged builds",
  ].join("\n");
}
