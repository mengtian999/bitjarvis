...
Answers `{"ok":true,"codes":["JARVIS-XXXX-XXXX-XXXX", ...],"generation":0,"total":5}`.
...
curl -sS -X POST "$GATE/redeem" \
  -H "Content-Type: application/json" \
  -d '{"code":"JARVIS-XXXX-XXXX-XXXX","deviceIdHash":"'"$(printf smoke | shasum -a 256 | cut -d' ' -f1)"'"}'
```
...
The client reads the endpoint from `JARVIS_INVITE_API_URL`, falling back to the
...
- For your own testing, launch the app with `JARVIS_INVITE_API_URL=$GATE`.
...
curl -sS -X POST "$GATE/admin/revoke" -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" -d '{"code":"JARVIS-XXXX-XXXX-XXXX"}'

# Burn a single code but leave its children alive
curl -sS -X POST "$GATE/admin/revoke" -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" -d '{"code":"JARVIS-XXXX-XXXX-XXXX","cascade":false}'
