 ...
 * - 占用者是另一个 Jarvis server 时禁止 fallback：同 JARVIS_HOME 双开会并发写坏
...
> {
  ...
  const existing = readExistingConfigOrNull(jarvisHome);

  if (!existing) {
    const selected = await select({});
    if (selected === null) {
      log(`首启随机选港全部失败，回退到默认端口 ${DEFAULT_SERVER_LISTEN_PORT}`);
      ensureServerNetworkConfig(jarvisHome, { now: nowIso });
      return { created: true, migrated: false, port: DEFAULT_SERVER_LISTEN_PORT };
    }
    saveServerNetworkConfig(jarvisHome, {
      schemaVersion: 1,
      mode: "loopback",
  ...
  if (existing.mode === "loopback" && existing.listenPort === DEFAULT_SERVER_LISTEN_PORT) {
    ...
    saveServerNetworkConfig(jarvisHome, { ...existing, listenPort: selected }, { now: nowIso });
...
function readExistingConfigOrNull(jarvisHome: string) {
  ...
  return loadServerNetworkConfig(jarvisHome);
}

export async function isJarvisServerListeningOnPort({
  port,
  host = "127.0.0.1",
  fetchImpl = fetch,
  timeoutMs = 1500,
