...
export function createPluginDevTools({ pluginDevService, getAgentId }: { pluginDevService?: any; getAgentId?: any } = {}) {
  ...
  return [
    createPluginDevTool({
      ...
      description: "Install a Jarvis plugin source directory into the isolated development plugin slot. Requires the Agent plugin dev tools setting to be enabled by the user.",
