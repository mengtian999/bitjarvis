/**
 * InputDraftsStore — 输入框草稿落盘（{JARVIS_HOME}/input-drafts.v1.json）
 *
...
export class InputDraftsStore {
  ...
  constructor({ jarvisHome }) {
    if (!jarvisHome) throw new Error("InputDraftsStore requires jarvisHome");
    this._path = path.join(jarvisHome, "input-drafts.v1.json");
    this._cache = null;
  }
