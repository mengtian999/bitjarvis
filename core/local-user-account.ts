...
export function getLocalAccountSummary(jarvisHome) {
  const { user } = loadDefaultUser(jarvisHome);
  const auth = loadLocalUserAuth(jarvisHome);
  const credential = findCredential(auth, user.userId);
  return sanitizeAccount(user, { passwordSet: !!credential });
}

export function updateLocalAccountProfile(jarvisHome, {
  username,
  displayName,
  now = new Date().toISOString(),
}: { username?: string; displayName?: string; now?: string } = {}) {
  const users = loadUsers(jarvisHome);
  ...
  user.updatedAt = now;
  users.updatedAt = now;
  writeSecretJson(path.join(jarvisHome, USERS_FILE), users);
  const auth = loadLocalUserAuth(jarvisHome);
  return sanitizeAccount(user, { passwordSet: !!findCredential(auth, user.userId) });
}

export function setLocalAccountPassword(jarvisHome, {
  password,
  now = new Date().toISOString(),
  ...
  auth.updatedAt = now;
  writeSecretJson(path.join(jarvisHome, LOCAL_USER_AUTH_FILE), auth);
  return sanitizeAccount(user, { passwordSet: true });
}

export function clearLocalAccountPassword(jarvisHome, {
  now = new Date().toISOString(),
} = {}) {
  const { user } = loadDefaultUser(jarvisHome);
  const auth = loadLocalUserAuth(jarvisHome);
...
export function verifyLocalAccountPassword(jarvisHome, { username, password }: { username?: string; password?: string } = {}) {
...
export function loadLocalUserAuth(jarvisHome, { now = new Date().toISOString() } = {}) {
  const filePath = path.join(jarvisHome, LOCAL_USER_AUTH_FILE);
...
function loadDefaultUser(jarvisHome) {
  const users = loadUsers(jarvisHome);
  return { users, user: getDefaultUser(users) };
}

function loadUsers(jarvisHome) {
  return validateUsers(readJsonRequired(path.join(jarvisHome, USERS_FILE), USERS_FILE), USERS_FILE);
}
