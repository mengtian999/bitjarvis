...
const STORAGE_PROVIDERS = ["local_fs", "webdav", "s3", "google_drive", "jarvis_cloud"];
...
export function ensureStudioMountRegistry(jarvisHome, { now = new Date().toISOString() } = {}) {
  const filePath = path.join(jarvisHome, STUDIO_MOUNTS_FILE);
...
export function loadStudioMountRegistry(jarvisHome) {
  ensureStudioMountRegistry(jarvisHome);
  const registry = readJsonRequired(path.join(jarvisHome, STUDIO_MOUNTS_FILE), STUDIO_MOUNTS_FILE);
  validateStudioMountRegistry(registry);
  return registry;
}

export function upsertStudioMount(jarvisHome, mount, { now = new Date().toISOString() } = {}) {
  const registry = loadStudioMountRegistry(jarvisHome);
  ...
  validateStudioMountRegistry(registry);
  writeJsonAtomic(path.join(jarvisHome, STUDIO_MOUNTS_FILE), registry);
...
export function listStudioMountsForStudio(jarvisHome, hostStudioId) {
