...
export class MountAwareFileService {
  ...
  constructor({
    ...
    if (!jarvisHome) throw new Error("jarvisHome required");
  ...
  _resolveRootInternal(rootId = "default") {
    ...
    const mount = findLocalFsMount(this._jarvisHome, this._studioId, id);
...
function createServiceResourceIO({ jarvisHome, defaultRoot, studioId }) {
...
function findLocalFsMount(jarvisHome, studioId, rootId) {
  if (!studioId) return null;
  let registry;
  try {
    registry = loadStudioMountRegistry(jarvisHome);
  } catch {
    return null;
  }
