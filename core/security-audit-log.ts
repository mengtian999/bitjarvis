...
export function securityAuditLogPath(jarvisHome) {
  if (!jarvisHome) throw new Error("jarvisHome required");
  return path.join(jarvisHome, "logs", SECURITY_AUDIT_LOG_FILE);
}

export function appendSecurityAuditEvent(jarvisHome, event, {
  now = new Date().toISOString(),
  eventId = `sec_${crypto.randomUUID()}`,
} = {}) {
  if (!jarvisHome) return null;
  const record = normalizeAuditEvent(event, { now, eventId });
  const filePath = securityAuditLogPath(jarvisHome);
  try {
