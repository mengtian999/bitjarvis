...
export class ModelManager {
  ...
  /**
   * @param {object} opts
   * @param {string} opts.jarvisHome - 用户数据根目录
   */
  constructor({ jarvisHome }) {
    ...
    // 新架构模块（init() 后可用）
    this.providerRegistry = new ProviderRegistry(jarvisHome);
    this.executionRouter = null;
  }

  /** 初始化 AuthStorage + ModelRegistry + 新架构模块 */
  init() {
    this._authStorage = AuthStorage.create(path.join(this._jarvisHome, "auth.json"));
    // Same file, same lock: forced OAuth rotation writes through this backend.
    this._authBackend = new FileAuthStorageBackend(path.join(this._jarvisHome, "auth.json"));
    ...
    this._modelRegistry = createModelRegistry(
      this._authStorage,
      path.join(this._jarvisHome, "models.json"),
    );
  ...
  get modelsJsonPath() { return path.join(this._jarvisHome, "models.json"); }
  get authJsonPath() { return path.join(this._jarvisHome, "auth.json"); }
  ...
  /**
   * Jarvis 的 API-key provider 凭证源是 Provider Catalog → models.json。
  ...
  /** Convert Jarvis-visible thinking levels to the Pi SDK session contract. */
  ...
  /**
   * 请求时解析模型与凭证。模型身份始终先从 Jarvis availableModels 解析，
