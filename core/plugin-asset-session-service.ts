...
export function pluginAssetSessionCookieName(pluginId: string) {
  ...
  return `jarvis_plugin_assets_${digest}`;
}
...
}: { jarvisHome?: string; pluginId?: string; principalId?: string; now?: string; ttlMs?: number } = {}) {
...
export function verifyPluginAssetSession({
  ...
  token,
  now = new Date().toISOString(),
}: { jarvisHome?: string; pluginId?: string; token?: string; now?: string } = {}) {
  assertNonEmpty(jarvisHome, "jarvisHome");
  assertNonEmpty(pluginId, "pluginId");
...
function pluginAssetSessionKeyPath(jarvisHome) {
  assertNonEmpty(jarvisHome, "jarvisHome");
  return path.join(securityDirPath(jarvisHome), PLUGIN_ASSET_SESSION_KEY_FILE);
}

function signBody(jarvisHome, body) {
  return crypto
    .createHmac("sha256", readOrCreateSessionKey(jarvisHome))
    .update(body)
    .digest("base64url");
}

function readOrCreateSessionKey(jarvisHome) {
  const filePath = pluginAssetSessionKeyPath(jarvisHome);
  try {
