...
export function loadServerIdentity(jarvisHome) {
  const serverNode = readRequiredIdentityJson(path.join(jarvisHome, SERVER_NODE_FILE), SERVER_NODE_FILE);
  const users = readRequiredIdentityJson(path.join(jarvisHome, USERS_FILE), USERS_FILE);
  const studios = readRequiredStudioRegistry(jarvisHome);
...
export function ensureLocalIdentityRegistries(jarvisHome) {
  const serverNodePath = path.join(jarvisHome, SERVER_NODE_FILE);
  const usersPath = path.join(jarvisHome, USERS_FILE);
  const studiosPath = path.join(jarvisHome, STUDIOS_FILE);
  const legacySpacesPath = path.join(jarvisHome, LEGACY_SPACES_FILE);
  ...
  const foundationRegistries = ensureRemoteAccessFoundationRegistries(jarvisHome, { now });
...
export function ensureRemoteAccessFoundationRegistries(jarvisHome, { now = new Date().toISOString() } = {}) {
  return {
    created: [
      ...ensureDeviceAccessRegistries(jarvisHome, { now }).created,
      ...ensureServerNetworkConfig(jarvisHome, { now }).created,
      ...ensureStudioMountRegistry(jarvisHome, { now }).created,
      ...ensureSecurityRegistries(jarvisHome, { now }).created,
    ],
  };
}

function ensureSecurityRegistries(jarvisHome, { now }) {
  const created = [];
  const grantPath = grantRegistryPath(jarvisHome);
  const leasePath = executionLeaseRegistryPath(jarvisHome);
  const hadGrant = fs.existsSync(grantPath);
  const hadLease = fs.existsSync(leasePath);
  ensureGrantRegistry(jarvisHome, { now });
  ensureExecutionLeaseRegistry(jarvisHome, { now });
...
function readRequiredStudioRegistry(jarvisHome) {
  const studiosPath = path.join(jarvisHome, STUDIOS_FILE);
