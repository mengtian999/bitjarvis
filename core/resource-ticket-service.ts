...
export function resourceTicketKeyPath(jarvisHome: any) {
  assertNonEmpty(jarvisHome, "jarvisHome");
  return path.join(securityDirPath(jarvisHome), RESOURCE_TICKET_KEY_FILE);
}

function signBody(jarvisHome: any, body: any) {
  return crypto
...
function readOrCreateTicketKey(jarvisHome: any) {
  const filePath = resourceTicketKeyPath(jarvisHome);
  try {
