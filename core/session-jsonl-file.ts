...
function projectSessionValue(value, seen, context) {
  if (typeof value === "string") {
    ...
    return `[omitted ${value.length} chars by Jarvis session JSONL guard]`;
  }
  ...
  if (Array.isArray(value)) {
    ...
    if (value.length > DEFAULT_SESSION_JSONL_MAX_ARRAY_ITEMS) {
      items.push(`[omitted ${value.length - DEFAULT_SESSION_JSONL_MAX_ARRAY_ITEMS} items by Jarvis session JSONL guard]`);
    }
    seen.delete(value);
    return items;
  }
  ...
  for (const [key, item] of Object.entries(value)) {
    ...
    if (
      ...
      out[key] = `[omitted ${item.length} chars by Jarvis session JSONL guard]`;
      continue;
    }
 ...
 * Jarvis needs the session file to exist earlier for sidebar, archive, restart,
