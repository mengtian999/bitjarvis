...
export const WEB_SESSION_COOKIE_NAME = "jarvis_session";

const SCHEMA_VERSION = 1;
const SECRET_PREFIX_LENGTH = 18;
const DEFAULT_TTL_MS = 14 * 24 * 60 * 60 * 1000;

export function ensureWebSessionRegistry(jarvisHome, { now = new Date().toISOString() } = {}) {
  const filePath = path.join(jarvisHome, WEB_SESSIONS_FILE);
...
export function loadWebSessionRegistry(jarvisHome) {
  ensureWebSessionRegistry(jarvisHome);
  const registry = readJsonRequired(path.join(jarvisHome, WEB_SESSIONS_FILE), WEB_SESSIONS_FILE);
  return validateWebSessionRegistry(registry, WEB_SESSIONS_FILE);
}

export function createWebSession(jarvisHome, input: { principal?: any; now?: string; ttlMs?: number; userAgent?: string } = {}) {
  assertNonEmptyString(jarvisHome, "jarvisHome");
  ...
  const secret = `jarvis_web_${randomToken(32)}`;
  ...
  const registry = loadWebSessionRegistry(jarvisHome);
  registry.sessions.push(session);
  registry.updatedAt = now;
  persistWebSessionRegistry(jarvisHome, registry);
  return {
...
export function authenticateWebSession(jarvisHome, cookieHeader, { now = new Date().toISOString() } = {}) {
...
export function revokeWebSession(jarvisHome, cookieHeader, { now = new Date().toISOString() } = {}) {
  const secret = parseCookie(cookieHeader, WEB_SESSION_COOKIE_NAME);
...
function persistWebSessionRegistry(jarvisHome, registry) {
  validateWebSessionRegistry(registry, WEB_SESSIONS_FILE);
  writeSecretJson(path.join(jarvisHome, WEB_SESSIONS_FILE), registry);
}
