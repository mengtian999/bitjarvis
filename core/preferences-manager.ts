...
export class PreferencesManager {
  ...
  /** 语音识别默认开箱配置:默认启用 + sherpa-onnx 本地识别(无需凭据/Key)。 */
  getSpeechRecognitionConfig() {
    ...
    return {
      ...
      ...(defaultModel?.provider && defaultModel.id
        ? { defaultModel }
        : { defaultModel: { provider: "sherpa-onnx", id: "sherpa-asr" } }),
  ...
  setSpeechRecognitionConfig(config) {
    ...
    const next = {
      ...
      ...(typeof config?.language === "string" && config.language.trim() ? { language: config.language.trim() } : {}),
    };
    prefs.speechRecognition = next;
    this.savePreferences(prefs);
    return this.getSpeechRecognitionConfig();
  }
  /** 本地语音引擎（sherpa-onnx）二进制/模型目录配置。 */
  getSherpaConfig() {
    const raw = this._cache.sherpa;
    const dir = raw?.dir && typeof raw.dir === "string" && raw.dir.trim() ? raw.dir.trim() : "";
    const bin = raw?.bin && typeof raw.bin === "string" && raw.bin.trim() ? raw.bin.trim() : "";
    return { ...(dir ? { dir } : {}), ...(bin ? { bin } : {}) };
  }

  setSherpaConfig(config) {
    const prefs = this._mutableCopy();
    const next: any = {};
    if (config?.dir && typeof config.dir === "string" && config.dir.trim()) next.dir = config.dir.trim();
    if (config?.bin && typeof config.bin === "string" && config.bin.trim()) next.bin = config.bin.trim();
    if (Object.keys(next).length) prefs.sherpa = next;
    else delete prefs.sherpa;
    this.savePreferences(prefs);
    return this.getSherpaConfig();
  }

  /** 文本转语音默认开箱配置:默认启用 + Edge TTS(免费在线、无需 Key)。 */
  getTtsConfig() {
    const raw = this._cache.tts;
    const defaultModel = raw?.defaultModel && typeof raw.defaultModel === "object" && !Array.isArray(raw.defaultModel)
      ? {
        provider: typeof raw.defaultModel.provider === "string" ? raw.defaultModel.provider : "",
        id: typeof raw.defaultModel.id === "string" ? raw.defaultModel.id : "",
    ...
    return {
      ...
      ...(defaultModel?.provider && defaultModel.id
        ? { defaultModel }
        : { defaultModel: { provider: "microsoft-edge", id: "edge-tts" } }),
      ...(typeof raw?.voice === "string" && raw.voice.trim() ? { voice: raw.voice.trim() } : {}),
      ...(raw?.rate !== undefined && raw.rate !== null ? { rate: raw.rate } : {}),
      ...(typeof raw?.language === "string" && raw.language.trim() ? { language: raw.language.trim() } : {}),
    };
  }
  ...
  setTtsConfig(config) {
    ...
    const next = {
      enabled: config?.enabled === true,
      ...(config?.defaultModel?.provider && config?.defaultModel?.id ? {
        defaultModel: { provider: config.defaultModel.provider, id: config.defaultModel.id },
      } : {}),
      ...(typeof config?.voice === "string" && config.voice.trim() ? { voice: config.voice.trim() } : {}),
      ...(config?.rate !== undefined && config.rate !== null ? { rate: config.rate } : {}),
      ...(typeof config?.language === "string" && config.language.trim() ? { language: config.language.trim() } : {}),
    };
    prefs.tts = next;
    this.savePreferences(prefs);
