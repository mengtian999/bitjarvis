 ...
 *
 * @param {string} jarvisHome - 用户数据根目录（~/.jarvis-dev）
 * @param {string} agentsDir  - agents 目录
 * @param {(msg: string) => void} log - 日志回调
 */
export function migrateToProvidersYaml(jarvisHome: string, agentsDir: string, log: (msg: string) => void = () => {}) {
  const providersPath = path.join(jarvisHome, "added-models.yaml");
  const prefsPath = path.join(jarvisHome, "user", "preferences.json");

  // ── 文件改名迁移：providers.yaml → added-models.yaml ──
  const oldPath = path.join(jarvisHome, "providers.yaml");
