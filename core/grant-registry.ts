...
export function ensureGrantRegistry(jarvisHome, { now = new Date().toISOString() } = {}) {
  const filePath = grantRegistryPath(jarvisHome);
...
export function loadGrantRegistry(jarvisHome) {
  ensureGrantRegistry(jarvisHome);
  return validateGrantRegistry(readJsonRequired(grantRegistryPath(jarvisHome), GRANTS_FILE), GRANTS_FILE);
}

export function createGrant(jarvisHome, input: Record<string, any> = {}) {
  const now = input.now || new Date().toISOString();
  const registry = loadGrantRegistry(jarvisHome);
  ...
  persistGrantRegistry(jarvisHome, registry);
  return clonePlain(grant);
}

export function findActiveGrantsForPrincipal(jarvisHome, principalId, { now = new Date().toISOString() } = {}) {
  ...
  if (changed) persistGrantRegistry(jarvisHome, registry);
  return active;
}

export function revokeGrant(jarvisHome, grantId, { now = new Date().toISOString() } = {}) {
...
export function persistGrantRegistry(jarvisHome, registry) {
  validateGrantRegistry(registry, GRANTS_FILE);
  writeJsonAtomic(grantRegistryPath(jarvisHome), registry);
}

export function grantRegistryPath(jarvisHome) {
  if (!isNonEmptyString(jarvisHome)) throw new Error("jarvisHome required");
  return path.join(jarvisHome, SECURITY_DIR, GRANTS_FILE);
}
