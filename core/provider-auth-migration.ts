 ...
 */
export function migrateLegacyApiKeyAuthToProviders({ jarvisHome, providerRegistry, log = () => {} }: { jarvisHome: string; providerRegistry: any; log?: (msg: string) => void }) {
  if (!jarvisHome) return { migrated: 0, providers: [] };

  const authPath = path.join(jarvisHome, "auth.json");
  ...
  const modelsJsonProvidersRaw = readJson(path.join(jarvisHome, "models.json")).providers || {};
