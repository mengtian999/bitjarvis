...
const JARVIS_CURSOR_BLOOM_COLOR = "#537D96";
const JARVIS_CURSOR_GRADIENT_COLORS = Object.freeze(["#FFFDF8", "#8FAABD", "#2F4A56"]);
const JARVIS_CUA_CURSOR_STYLE = Object.freeze({
  gradient_colors: JARVIS_CURSOR_GRADIENT_COLORS,
  bloom_color: JARVIS_CURSOR_BLOOM_COLOR,
  image_path: "",
});
const JARVIS_AGENT_CURSOR_CONFIG_ENV = "JARVIS_AGENT_CURSOR_CONFIG_JSON";
const JARVIS_AGENT_SOCKET_PATH_ENV = "JARVIS_COMPUTER_USE_SOCKET_PATH";
const JARVIS_CURSOR_MOTION = Object.freeze({
  start_handle: 0.38,
  end_handle: 0.28,
  arc_size: 0.08,
  arc_flow: 0,
...
function helperPath(root: any) {
  return path.posix.join(root, "jarvis-computer-use-helper");
}

function defaultJarvisComputerUseSocketPath(homeDir = os.homedir()) {
  return path.join(homeDir, "Library", "Caches", "jarvis-computer-use", "jarvis-computer-use-helper.sock");
}

function commandIsBundledJarvisHelper(command: any) {
  return path.posix.basename(String(command || "")) === "jarvis-computer-use-helper";
}
...
function resourcesRootFromLegacyJarvisRoot(value: any) {
  const jarvisRoot = normalizedAbsolutePosixPath(value);
  if (!jarvisRoot || path.posix.basename(jarvisRoot) !== "server") return null;
  return macAppResourcesRoot(path.posix.dirname(jarvisRoot));
}
...
function packagedHelperCandidates({ env, jarvisRoot }: any) {
  const roots: string[] = [];
  if (env.JARVIS_DESKTOP_IS_PACKAGED === "1") {
    pushUniquePosixPath(roots, macAppResourcesRoot(env.JARVIS_DESKTOP_RESOURCES_PATH));
    pushUniquePosixPath(roots, resourcesRootFromPackagedAppPath(env.JARVIS_DESKTOP_APP_PATH));
    pushUniquePosixPath(roots, resourcesRootFromPackagedExecPath(env.JARVIS_DESKTOP_EXEC_PATH));
  }
  ...
  pushUniquePosixPath(roots, resourcesRootFromLegacyJarvisRoot(jarvisRoot));
  return roots.map((root) => helperPath(path.posix.join(root, "computer-use", "macos")));
}

function developmentHelperCandidates({ env, jarvisRoot, cwd, arch }: any) {
  if (env.JARVIS_DESKTOP_IS_PACKAGED === "1") return [];
  const candidates: string[] = [];
  const normalizedJarvisRoot = normalizedAbsolutePosixPath(jarvisRoot);
  const normalizedCwd = normalizedAbsolutePosixPath(cwd);
  if (normalizedJarvisRoot) {
    pushUniquePosixPath(
      candidates,
      helperPath(path.posix.join(normalizedJarvisRoot, "dist-computer-use", `mac-${arch}`)),
    );
  }
  if (normalizedCwd) {
    pushUniquePosixPath(
...
export function resolveCuaDriverCommand({
  ...
  const explicitHelper = expandHome(env.JARVIS_COMPUTER_USE_HELPER_PATH, homeDir);
  if (explicitHelper) return explicitHelper;

  const explicitRuntimeRoot = expandHome(env.JARVIS_COMPUTER_USE_RUNTIME_ROOT, homeDir);
  if (explicitRuntimeRoot) return helperPath(explicitRuntimeRoot);

  const packagedCandidates = packagedHelperCandidates({ env, jarvisRoot });
  ...
  const candidates = [
    ...developmentHelperCandidates({ env, jarvisRoot, cwd, arch }),
    env.JARVIS_CUA_DRIVER_PATH,
...
export function createMacosCuaProvider({
  ...
  cursorStyle = JARVIS_CUA_CURSOR_STYLE,
  cursorImagePath = null,
  cursorBloomColor = JARVIS_CURSOR_BLOOM_COLOR,
  cursorEnabled = true,
  cursorMotion = JARVIS_CURSOR_MOTION,
  runner = createCommandRunner(),
  timeoutMs = 30000,
  launchRetryAttempts = 3,
  launchRetryDelayMs = 350,
  socketPath = process.env[JARVIS_AGENT_SOCKET_PATH_ENV] || defaultJarvisComputerUseSocketPath(),
  ...
  const bundledJarvisHelper = commandIsBundledJarvisHelper(command);
  const shouldAutoStartDaemon = autoStartDaemon ?? bundledJarvisHelper;
  const resolvedCursorStyle = normalizeCursorStyle({ cursorStyle, cursorImagePath, cursorBloomColor });
  const nativeCursorEnabled = bundledJarvisHelper && cursorEnabled !== false && Boolean(resolvedCursorStyle);
  const jarvisCursorRuntimeConfig = resolvedCursorStyle && bundledJarvisHelper
    ? {
  ...
  function runEnv(baseEnv: any) {
    return {
      ...(baseEnv || process.env),
      [JARVIS_AGENT_SOCKET_PATH_ENV]: socketPath,
      ...(jarvisCursorRuntimeConfig
        ? { [JARVIS_AGENT_CURSOR_CONFIG_ENV]: JSON.stringify(jarvisCursorRuntimeConfig) }
        : {}),
    };
  }
  ...
  async function stopManagedDaemon() {
    if (!bundledJarvisHelper || !shouldAutoStartDaemon) {
  ...
  async function ensureNativeCursorConfigured() {
    if (!resolvedCursorStyle || !bundledJarvisHelper) return;
      ...
      try {
        ...
        return {
          ...
          reason: err?.code === "ENOENT"
            ? (bundledJarvisHelper ? "bundled-helper-missing" : "binary-not-found")
            : "status-failed",
          command,
