...
export class StudioCronService {
  ...
  /**
   * @param {object} opts
   * @param {string} opts.jarvisHome
   * @param {string} opts.agentsDir
   * @param {() => string} opts.getStudioId
   */
  constructor({ jarvisHome, agentsDir, getStudioId }) {
    if (!jarvisHome) throw new Error("StudioCronService requires jarvisHome");
  ...
  _getStoreForStudio(studioId) {
    ...
    if (!store) {
      const deskDir = path.join(this._jarvisHome, "studios", normalizedStudioId, "desk");
      store = new CronStore(
  ...
  _importLegacyJobs(store, studioId) {
    const studioRunsDir = path.join(this._jarvisHome, "studios", studioId, "desk", "cron-runs");
