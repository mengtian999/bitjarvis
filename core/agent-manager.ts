...
export class AgentManager {
  ...
  _readDeletedAgentInfo(agentId) {
    ...
    return {
      ...
      name,
      agentName: name,
      yuan: tombstone.yuan || cfg.agent?.yuan || "jarvis",
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        agents.push({
          ...
          yuan: cfg.agent?.yuan || "jarvis",
          voice: typeof cfg.agent?.voice === "string" && cfg.agent.voice.trim() ? cfg.agent.voice.trim() : null,
  ...
  async _refreshDescription(agentId) {
    try {
      ...
      const yuan = ag.config?.agent?.yuan || "jarvis";
  ...
  async createAgent({ name, id, yuan, enabledSkills, initialFiles, avatarPath, initialMemory }) {
    ...
    const createStage = (stage) => log.log(`[createAgent] ${name} stage=${stage} +${Date.now() - createStartedAt}ms`);

    const hasExplicitId = id !== undefined && id !== null;
    if (hasExplicitId) assertValidAgentId(id);
    createStage("generate-id:start");
    const agentId = hasExplicitId ? id : await this._generateAgentId(name);
    createStage(`generate-id:done id=${agentId}`);
    ...
    const yuanType = assertKnownYuan(this._d.productDir, yuan || "jarvis");
    ...
    this.invalidateAgentListCache();
    this._rebuildAllAgentSystemPrompts();
    log.log(`创建助手: ${name} (${agentId}) total=${Date.now() - createStartedAt}ms`);
  ...
  async deleteAgent(agentId) {
    ...
    const tombstone = {
      ...
      yuan: ag?.config?.agent?.yuan || this._readAgentYuanFromConfig(agentDir) || "jarvis",
    ...
    if (this._d.jarvisHome) {
      try {
        detachAgentFromBundles({ jarvisHome: this._d.jarvisHome }, agentId);
      } catch (err) {
