 ...
 */
const DIRECT_COMPACTION_IN_PROGRESS = Symbol("jarvisDirectCompactionInProgress");
...
const COMPACTION_TRANSFORM_PROOF_KEY_PREFIX = "__jarvis_compaction_transform_proof_";
...
function createCompactionBoundaryPlaceholder() {
  ...
  return `<jarvis.compaction.boundary:${Date.now().toString(36)}:${compactionBoundaryPlaceholderSequence}>`;
}
...
function hardTruncateCachePreservingCompaction(branchEntries, preparation, {
  ...
  summary = "[由于对话过长且压缩请求本身会超限，早期对话历史已被硬截断（jarvis-cache-preserving-compaction）]",
} = {}) {
