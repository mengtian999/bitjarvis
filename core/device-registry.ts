...
export function ensureDeviceAccessRegistries(jarvisHome, { now = new Date().toISOString() } = {}) {
  const created = [];
  ...
  for (const [file, emptyRegistry, validate] of registries) {
    const filePath = path.join(jarvisHome, file);
...
export function loadDeviceAccessRegistries(jarvisHome) {
  ensureDeviceAccessRegistries(jarvisHome);
  const devices = readJsonRequired(path.join(jarvisHome, DEVICES_FILE), DEVICES_FILE);
  const credentials = readJsonRequired(path.join(jarvisHome, DEVICE_CREDENTIALS_FILE), DEVICE_CREDENTIALS_FILE);
  const pairingSessions = readJsonRequired(path.join(jarvisHome, PAIRING_SESSIONS_FILE), PAIRING_SESSIONS_FILE);
...
export function createDeviceCredential(jarvisHome, input) {
  const now = input?.now || new Date().toISOString();
  const registries = loadDeviceAccessRegistries(jarvisHome);
  const issued = issueDeviceCredential(registries, input, { now });
  persistDeviceAccessRegistries(jarvisHome, registries);
  return cloneIssuedCredential(issued);
}

export function authenticateDeviceCredential(jarvisHome, secret, { now = new Date().toISOString() } = {}) {
...
export function revokeDeviceCredential(jarvisHome, credentialId, { now = new Date().toISOString() } = {}) {
...
export function revokeDevice(jarvisHome, deviceId, { now = new Date().toISOString() } = {}) {
...
export function createPairingSession(jarvisHome, input) {
  const now = input?.now || new Date().toISOString();
...
export function approvePairingSession(jarvisHome, input) {
  const now = input?.now || new Date().toISOString();
...
function issueDeviceCredential(registries, input, { now }) {
  ...
  const secret = `jarvis_dev_${randomToken(32)}`;
...
function persistDeviceAccessRegistries(jarvisHome, registries) {
  ...
  writeSecretJson(path.join(jarvisHome, DEVICES_FILE), registries.devices);
  writeSecretJson(path.join(jarvisHome, DEVICE_CREDENTIALS_FILE), registries.credentials);
  writeSecretJson(path.join(jarvisHome, PAIRING_SESSIONS_FILE), registries.pairingSessions);
}
