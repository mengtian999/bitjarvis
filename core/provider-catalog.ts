...
export class ProviderCatalogStore {
  ...
  constructor(jarvisHome: string) {
    if (!jarvisHome) throw new Error("ProviderCatalogStore requires jarvisHome");
    this._jarvisHome = jarvisHome;
  }

  get catalogPath() {
    return path.join(this._jarvisHome, PROVIDER_CATALOG_FILE);
  }

  get legacyAddedModelsPath() {
    return path.join(this._jarvisHome, LEGACY_ADDED_MODELS_FILE);
  }
  ...
  _writeMigrationBackup(catalog: any) {
    ...
    const backupDir = path.join(
      migrationBackupsRoot(this._jarvisHome),
