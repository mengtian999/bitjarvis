...
export class ProviderRegistry {
  ...
  /**
   * @param {string} jarvisHome - 用户数据根目录（如 ~/.jarvis-dev）
   */
  constructor(jarvisHome) {
    this._jarvisHome = jarvisHome;
    this._catalog = new ProviderCatalogStore(jarvisHome);
    this._localProviderPlugins = new LocalProviderPluginStore(jarvisHome);
  ...
  _merge(plugin, userConfig, isBuiltin) {
    ...
    if (plugin.defaultApiKey) {
      entry.defaultApiKey = plugin.defaultApiKey;
    }
    if (plugin.hideApiReveal === true) {
      entry.hideApiReveal = true;
    }
    return entry;
  }
   ...
   * 缺少字段 → Jarvis/plugin 默认；[] → 明确关闭；非空 → 用户 allowlist。
   */
  ...
  /**
   * 将 Jarvis provider 身份解析为唯一的运行时投影计划。
  ...
  /**
   * 为内置插件 seeding 默认 API Key。
   * 仅当用户尚未配置 api_key（空或不存在）时，将插件声明的 defaultApiKey 写入 catalog。
   * 一旦用户手动修改过密钥，不再覆盖。
   */
  seedDefaultApiKeys(log) {
    const userConfig = this._loadAddedModels();
    let changed = false;
    for (const [id, plugin] of this._plugins) {
      if (!plugin?.defaultApiKey) continue;
      if (!this._isBuiltinPlugin(id, plugin)) continue;
      const uc = userConfig[id] || {};
      if (uc.api_key) continue;
      userConfig[id] = { ...uc, api_key: plugin.defaultApiKey };
      changed = true;
      log?.(`[provider-seed] seeded api_key for builtin provider "${id}"`);
    }
    if (changed) {
      this._saveAddedModels(userConfig, { defaultApiKeySeededAt: new Date().toISOString() });
    }
  }
  ...
  _readOAuthEntry(authJsonKey) {
    try {
      const authPath = path.join(this._jarvisHome, "auth.json");
