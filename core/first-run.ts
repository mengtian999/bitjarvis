 ...
 * first-run.js — 首次运行播种
 *
 * 在 server/engine 启动之前调用，确保 ~/.jarvis/ 结构存在。
...
// 曾经作为内置技能随 skills2set 发布、现已从源码移除的名字。
// syncSkills() 会删除用户 skills 目录中对应的旧目录，避免旧残留持续出现在 UI
// 列表里（只清理已知内置名，绝不触碰用户自装 skill）。
const LEGACY_BUILTIN_SKILL_NAMES = new Set(["hana-plugin-creator"]);
 ...
 * - 默认 agent（jarvis）缺 config → 播种修复；config 损坏 → 先备份再播种
 ...
 *
 * @param {string} jarvisHome - ~/.jarvis 绝对路径
 * @param {string} productDir - 产品模板目录（lib/）
 */
export function ensureFirstRun(jarvisHome, productDir): FirstRunReport {
  // 1. 确保目录结构存在
  const userDir = path.join(jarvisHome, "user");
  fs.mkdirSync(path.join(jarvisHome, "agents"), { recursive: true });
  ...
  // 3. 同步 skills：从 skills2set/ 复制到 ~/.jarvis/skills/
  const skillsSrc = path.join(productDir, "..", "skills2set");
  const skillsDst = path.join(jarvisHome, "skills");
  ...
  touchIfMissing(path.join(jarvisHome, 'user', USER_PROFILE_FILENAME));
...
function seedDefaultAgent(agentsDir, productDir, userDir) {
  ...
  // config.yaml（保持模板默认值：name=Jarvis, yuan=jarvis）
...
/**
 * 同步 skills2set/ → ~/.jarvis/skills/
 * 每次启动都跑，确保新增/更新的 skill 能同步到用户目录
 */
function syncSkills(srcDir, dstDir) {
  fs.mkdirSync(dstDir, { recursive: true });

  // 清理源中已移除的历史内置 skill 残留（例如旧的 hana-plugin-creator），
  // 避免它们永远留在用户 skills 目录、持续出现在 UI 列表里。
  for (const legacyName of LEGACY_BUILTIN_SKILL_NAMES) {
    if (fs.existsSync(path.join(srcDir, legacyName))) continue;
    const legacyDir = path.join(dstDir, legacyName);
    if (!fs.existsSync(legacyDir)) continue;
    try {
      fs.rmSync(legacyDir, { recursive: true, force: true });
      log.log(`已清理历史内置 skill 残留：${legacyName}`);
    } catch (err) {
      errorBus.report(new AppError('SKILL_SYNC_FAILED', {
        cause: err instanceof Error ? err : new Error(String(err)),
        context: { skill: legacyName, phase: "legacy-cleanup" },
      }));
    }
  }
