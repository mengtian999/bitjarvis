...
export function issuePluginIframeTicket({
...
}: { jarvisHome?: string; pluginId?: string; surfacePath?: string; principalId?: string; now?: string; ttlMs?: number } = {}) {
  assertNonEmpty(jarvisHome, "jarvisHome");
  ...
  const signature = signBody(jarvisHome, body);
  return {
    ...payload,
...
export function verifyPluginIframeTicket({
  ...
  surfacePath,
  now = new Date().toISOString(),
}: { jarvisHome?: string; ticket?: string; pluginId?: string; surfacePath?: string; now?: string } = {}) {
  assertNonEmpty(jarvisHome, "jarvisHome");
  assertNonEmpty(pluginId, "pluginId");
  assertNonEmpty(surfacePath, "surfacePath");
  ...
  const expected = signBody(jarvisHome, body);
...
function pluginIframeTicketKeyPath(jarvisHome) {
  assertNonEmpty(jarvisHome, "jarvisHome");
  return path.join(securityDirPath(jarvisHome), PLUGIN_IFRAME_TICKET_KEY_FILE);
}

function signBody(jarvisHome, body) {
  return crypto
    .createHmac("sha256", readOrCreateTicketKey(jarvisHome))
    .update(body)
    .digest("base64url");
}

function readOrCreateTicketKey(jarvisHome) {
  const filePath = pluginIframeTicketKeyPath(jarvisHome);
  try {
