...
const DEFAULT_BASE_URL = "https://apihub.agnes-ai.cn/v1";
...
function videoUrlFromResponse(data) {
  ...
  // Agnes Video 2.5 Flash 文档：任务完成后，最终生成的视频 URL 位于 metadata.url
  if (typeof data?.metadata?.url === "string" && /^https?:\/\//i.test(data.metadata.url)) {
...
export const agnesImageAdapter = {
  ...
  async checkAuth(ctx) {
    try {
      await getCredentials(ctx, { providerId: ctx?.providerId || ctx?.task?.providerId || "agnes" });
      return { ok: true };
    } catch (err) {
...
export const agnesVideoAdapter = {
  ...
  async query(taskId, ctx) {
    const creds = await getCredentials(ctx, { providerId: ctx?.providerId || ctx?.task?.providerId || "agnes" });
