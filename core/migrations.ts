...
const migrations = {
  ...
  // Studio 基础身份：为旧 JARVIS_HOME 补齐 server / legacy owner / default Studio registry
  ...
  // Pi SDK 0.70+ 严格限制 model.input，只允许 text/image；Jarvis 视频能力迁入 compat
  20: migratePiInputSchemaVideoCompat,
  // 刷新高确定性视频模型能力；补齐已升级用户 models.json 里的 Jarvis compat
...
function migrateSubagentExecutorMetadata(ctx) {
  const { agentsDir, jarvisHome, log } = ctx;
  ...
  const deferredTasksPath = path.join(jarvisHome, ".ephemeral", "deferred-tasks.json");
  try {
...
function migrateUserNameToGlobalPreferences(ctx) {
  ...
  const primaryAgentId = preferences.primaryAgent || "jarvis";
...
function migrateMiniMaxTokenPlanAnthropicEndpoint(ctx) {
  const { jarvisHome, log } = ctx;
  const ymlPath = path.join(jarvisHome, "added-models.yaml");
 ...
 *
 * 覆盖位置：
 *   1. ~/.jarvis/added-models.yaml 的 providers.*.models[] 数组（用户主战场）
 *   2. ~/.jarvis/agents/*\/config.yaml 的 models.overrides（历史残留兜底）
 *
 * 幂等：只在发现 vision 字段时改写；image 已存在时保留不覆盖。
 * 配合读时兼容（model-sync.js、provider-registry.js）形成双保险。
 */
function migrateVisionToImage(ctx) {
  const { jarvisHome, agentsDir, log } = ctx;
  let ymlCount = 0;
  let overrideCount = 0;
...
function migrateCronJobsToAutomationReadModel(ctx) {
  ...
  const studiosDir = path.join(jarvisHome, "studios");
  try {
...
function migrateProviderCatalogV2Cutover(ctx) {
  const { jarvisHome, providerRegistry, log } = ctx;
  const store = providerRegistry?._catalog || new ProviderCatalogStore(jarvisHome);
...
function migrateCodexImageGenerationDefaultsToResolutionSchema(ctx) {
  const { jarvisHome, prefs, log } = ctx;
  const preferences = prefs.getPreferences();
  ...
  const pluginChanged = removeCodexImageSizeDefaultFromPluginConfig(jarvisHome, log);
...
function migrateOAuthModelsToProviderCatalog(ctx) {
  const { jarvisHome, prefs, providerRegistry, log } = ctx;
  const store = providerRegistry?._catalog || new ProviderCatalogStore(jarvisHome);
  ...
  if (legacyCodex || canonicalCodex || codexCustom.length > 0) {
    ...
    // 让它进入新的 Jarvis/plugin 默认语义；#44 之后新写入的 [] 才表示明确关闭。
...
function migrationPathIsInsideHome(jarvisHome, candidatePath) {
  const homeKey = filesystemIdentityKeySync(jarvisHome);
...
function migrationRealDirectory(jarvisHome, directory) {
  try {
    return fs.lstatSync(directory).isDirectory()
      && migrationPathIsInsideHome(jarvisHome, directory);
  } catch {
    return false;
  }
}

function migrationRealFile(jarvisHome, filePath) {
  try {
    return fs.lstatSync(filePath).isFile()
      && migrationPathIsInsideHome(jarvisHome, filePath);
  } catch {
    return false;
  }
}

function migrationReadDirectoryEntries(jarvisHome, directory, log) {
  if (!migrationRealDirectory(jarvisHome, directory)) return [];
  try {
...
function migrationWalkRealFiles(jarvisHome, root, accept, log) {
  const files = [];
  const walk = (directory) => {
    for (const entry of migrationReadDirectoryEntries(jarvisHome, directory, log)) {
      // Never follow directory or file symlinks. A user-managed link may point
      // outside JARVIS_HOME, and migration discovery must remain read-only there.
  ...
  if (migrationRealDirectory(jarvisHome, root)) walk(root);
  return files;
}
...
function migrationReadSessionJsonl(filePath, modelIds, log) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    try {
      ...
      // Some older Jarvis-produced snapshots stored the restored model beside
...
function collectCodexModelsFromLegacyPersistence(ctx) {
  const { jarvisHome, agentsDir, prefs, log } = ctx;
  ...
  const agentEntries = migrationReadDirectoryEntries(jarvisHome, agentsDir, log)
    .filter((entry) => entry.isDirectory() && !entry.isSymbolicLink());
  for (const agentEntry of agentEntries) {
    const agentDir = path.join(agentsDir, agentEntry.name);
    const configPath = path.join(agentDir, "config.yaml");
    if (migrationRealFile(jarvisHome, configPath)) {
    ...
    for (const entry of migrationReadDirectoryEntries(jarvisHome, dmDir, log)) {
    ...
    if (migrationRealFile(jarvisHome, cronPath)) {
  ...
  const studiosDir = path.join(jarvisHome, "studios");
  for (const studioEntry of migrationReadDirectoryEntries(jarvisHome, studiosDir, log)) {
  ...
  const channelsDir = path.join(jarvisHome, "channels");
  for (const entry of migrationReadDirectoryEntries(jarvisHome, channelsDir, log)) {
    if (entry.isSymbolicLink() || !entry.isFile() || !entry.name.endsWith(".md")) continue;
...
function writeProviderModelMetadataMigrationBackup({ store, jarvisHome, repairs }) {
  if (!fs.existsSync(store.catalogPath)) {
    throw new Error("provider catalog source is missing before metadata repair");
  }

  const backupRoot = migrationBackupsRoot(jarvisHome);
...
function writeCodexEventIdPollutionRepairBackup({ store, jarvisHome, removed }) {
  if (!fs.existsSync(store.catalogPath)) {
...
function repairPollutedCodexEventIdModels(ctx) {
  ...
  const backupDir = writeCodexEventIdPollutionRepairBackup({ store, jarvisHome, removed });
...
function removeCodexImageSizeDefaultFromPluginConfig(jarvisHome, log) {
  const configPath = path.join(jarvisHome, "plugin-data", "image-gen", "config.json");
...
function migrateGeminiPersistedTasks(jarvisHome, log) {
  const tasksPath = path.join(jarvisHome, "plugin-data", "image-gen", "tasks.json");
...
function migrateGeminiPluginConfig(jarvisHome, log) {
  const configPath = path.join(jarvisHome, "plugin-data", "image-gen", "config.json");
  if (!fs.existsSync(configPath)) return false;
  let config;
  try {
...
function migrateGeminiImagePreviewIdsToStable(ctx) {
  ...
  const pluginConfigChanged = migrateGeminiPluginConfig(jarvisHome, log);
  const tasksChanged = migrateGeminiPersistedTasks(jarvisHome, log);
 ...
 * 全局 skill pool：迁移时复制旧技能到 `{JARVIS_HOME}/skills`，并只把复制后的
...
function backfillLegacySessionFiles(ctx) {
  const { jarvisHome, agentsDir, log } = ctx;
  if (!jarvisHome || !agentsDir) return;

  const registry = new SessionFileRegistry({
    managedCacheRoot: path.join(jarvisHome, "session-files"),
  });
  ...
  for (const sessionPath of sessionPaths) {
    ...
    for (const line of lines) {
      ...
      for (const ref of legacySessionFileRefs(msg)) {
        const ok = registerLegacySessionFile({ registry, sessionId, sessionPath, ref, jarvisHome, log });
        if (ok) registered++;
        else skipped++;
      }
 ...
 *
 * 幂等：视频能力写入 Jarvis compat，Pi-facing input 只保留 text/image；运行期模型对象不保留 video 字段。
 */
 ...
 *
 * Pi SDK models.json 的 input 是外部契约，只允许 text/image。Jarvis 自己的
 * video 能力必须放在 compat.jarvisVideoInput，避免 ModelRegistry 因单个非法
 * 模型把整张模型表判空。
 */
 ...
 * 视频模型仍复用 compat.jarvisVideoInput 表示语义能力，传输能力由运行时根据
 * provider/api/baseUrl 推导。老用户已存在的 models.json 需要重跑一次投影修补，
 * 否则新增的 Kimi 等模型不会拿到 Jarvis 视频能力字段。
 */
 ...
 * 新格式：wx_dm_user@jarvis / tg_dm_user@jarvis
 *
...
function repairModelsJsonPiInputSchema(ctx) {
  const modelsJsonPath = path.join(ctx.jarvisHome, "models.json");
  let raw;
  try {
...
function repairPiModelInputRecord(providerId, record, fallbackModelId) {
  ...
  if (shouldEnableVideo && ensureJarvisVideoInputCompat(record)) patched++;
...
function ensureJarvisVideoInputCompat(record) {
  const compat = record.compat && typeof record.compat === "object" && !Array.isArray(record.compat)
    ? record.compat
    : {};
  if (compat.jarvisVideoInput === true && record.compat === compat) return false;
  record.compat = {
    ...compat,
    jarvisVideoInput: true,
  };
  return true;
}

function promoteAgentVideoOverrides(ctx) {
  const { jarvisHome, agentsDir } = ctx;
  const ymlPath = path.join(jarvisHome, "added-models.yaml");
  const raw = safeReadYAMLSync(ymlPath, null, YAML);
  if (!raw?.providers || typeof raw.providers !== "object") return 0;
...
function registerLegacySessionFile({ registry, sessionId = null, sessionPath, ref, jarvisHome, log }) {
  ...
  try {
    registry.registerFile({
      ...
      storageKind: normalizeLegacyStorageKind(ref, jarvisHome),
    });
    return true;
  } catch (err) {
...
function normalizeLegacyStorageKind(ref, jarvisHome) {
  const storageKind = ref.storageKind || "external";
  if (storageKind !== "managed_cache") return storageKind;

  const managedRoot = path.join(jarvisHome, "session-files");
  // 纯比较，两侧都走共享身份键。
...
function migrateLocalIdentityRegistries(ctx) {
  const { jarvisHome, log } = ctx;
  const { created, migratedFromLegacySpaces } = ensureLocalIdentityRegistries(jarvisHome);
...
function migrateRemoteAccessFoundationRegistries(ctx) {
  const { jarvisHome, log } = ctx;
  const { created } = ensureRemoteAccessFoundationRegistries(jarvisHome);
...
function migrateDurableSubagentRunRegistry(ctx) {
  const { jarvisHome, agentsDir, log } = ctx;
  const store = new SubagentRunStore(path.join(jarvisHome, "subagent-runs.json"));
  let imported = 0;
...
function migrateSubagentThreadRegistry(ctx) {
  const { jarvisHome, log } = ctx;
  const threadStore = new SubagentThreadStore(path.join(jarvisHome, "subagent-threads.json"));
  const runStore = new SubagentRunStore(path.join(jarvisHome, "subagent-runs.json"));
  let importedRuns = 0;
  ...
  const reusableRaw = readJsonForMigration(path.join(jarvisHome, "reusable-subagents.json"));
...
function migrateSubagentDirectThreadSemantics(ctx) {
  const { jarvisHome, log } = ctx;
  const threadsPath = path.join(jarvisHome, "subagent-threads.json");
  const runsPath = path.join(jarvisHome, "subagent-runs.json");
  let threadCount = 0;
  let runCount = 0;
