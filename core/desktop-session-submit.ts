 ...
 * 走 SDK 的 custom entry 通道（与 jarvis-deferred-result 同一模式）。
 ...
 * 禁止盲目前向关联到下一条消息。
 */
export const MESSAGE_ORIGIN_RECORD_TYPE = "jarvis-message-origin";
export const MESSAGE_PRESENTATION_RECORD_TYPE = "jarvis-message-presentation";
export const AGENT_REVIEW_RECORD_TYPE = "jarvis-agent-review-result";
  ...
  try {
    ...
    try {
      ...
      if (typeof engine.preflightSessionInput === "function") {
        ...
        // Compatibility for older embedders. JarvisEngine always takes the guarded path above.
        afterCachePreflight();
        await engine.promptSession(sessionPath, promptText, promptOpts);
      }
...
function registerDisplayAttachments({ jarvisHome, sessionPath, attachments, registerSessionFile }) {
  ...
  for (const attachment of attachments || []) {
    ...
    if (!next.fileId && next.path && path.isAbsolute(next.path) && typeof registerSessionFile === "function") {
      sessionFile = serializeSessionFile(registerSessionFile({
        ...
        storageKind: displayAttachmentStorageKind(jarvisHome, next.path),
...
function displayAttachmentStorageKind(jarvisHome, filePath) {
  if (!jarvisHome) return "external";
  const root = path.resolve(jarvisHome, "session-files");
