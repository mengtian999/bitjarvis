...
function discoverSessionRows({ jarvisHome, agentsDir }) {
  ...
  const addSubagentStoreRecords = (storePath, containerKey, source) => {
    ...
    for (const [recordId, value] of Object.entries(entries)) {
      ...
      if (!entry) continue;
      const sessionPath = storedPathWithin(jarvisHome, entry.childSessionPath || entry.sessionPath, jarvisHome);
  ...
  addSubagentStoreRecords(path.join(jarvisHome, "subagent-runs.json"), "runs", "legacy_subagent_run_store");
  addSubagentStoreRecords(path.join(jarvisHome, "subagent-threads.json"), "threads", "legacy_subagent_thread_store");
...
export function discoverLegacySessions(opts: any = {}) {
  if (!opts.jarvisHome) throw new Error("discoverLegacySessions requires jarvisHome");
  const jarvisHome = path.resolve(opts.jarvisHome);
  const agentsDir = path.resolve(opts.agentsDir || path.join(jarvisHome, "agents"));
  return discoverSessionRows({ jarvisHome, agentsDir });
}
...
export function auditLegacySessionManifests(opts: any = {}) {
  if (!opts.jarvisHome) throw new Error("auditLegacySessionManifests requires jarvisHome");
...
export function migrateLegacySessions(opts: any = {}) {
  if (!opts.jarvisHome) throw new Error("migrateLegacySessions requires jarvisHome");
