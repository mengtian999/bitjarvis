...
export function createSessionManifestCheckpoint(opts: any = {}) {
  if (!opts.jarvisHome) throw new Error("createSessionManifestCheckpoint requires jarvisHome");
  ...
  const checkpointRoot = opts.checkpointRoot
    ...
    : path.join(jarvisHome, "checkpoints", "session-manifest");
  ...
  try {
    for (const name of includes) {
      const source = path.join(jarvisHome, name);
...
export function restoreSessionManifestCheckpoint(opts: any = {}) {
  ...
  const jarvisHome = path.resolve(opts.jarvisHome || receipt.jarvisHome);
  const restoredAt = opts.restoredAt || new Date().toISOString();
  const restoreId = sanitizeTimestamp(restoredAt);
  assertDirectoryWritable(jarvisHome);
  ...
  for (const entry of receipt.includes || []) {
    ...
    const target = path.join(jarvisHome, entry.name);
    safeCopyDir(source, target);
  }
