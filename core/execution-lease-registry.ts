...
export function ensureExecutionLeaseRegistry(jarvisHome, { now = new Date().toISOString() } = {}) {
  const filePath = executionLeaseRegistryPath(jarvisHome);
...
export function loadExecutionLeaseRegistry(jarvisHome, { now = new Date().toISOString() } = {}) {
  ensureExecutionLeaseRegistry(jarvisHome, { now });
  return validateExecutionLeaseRegistry(
    readJsonRequired(executionLeaseRegistryPath(jarvisHome), EXECUTION_LEASES_FILE),
    EXECUTION_LEASES_FILE,
    { now },
  );
}

export function issueExecutionLease(jarvisHome, lease, { now = new Date().toISOString() } = {}) {
  const registry = loadExecutionLeaseRegistry(jarvisHome, { now });
  ...
  persistExecutionLeaseRegistry(jarvisHome, registry);
  return clonePlain(record);
}

export function consumeExecutionLease(jarvisHome, leaseId, { now = new Date().toISOString() } = {}) {
  const registry = loadExecutionLeaseRegistry(jarvisHome, { now });
...
export function revokeExecutionLease(jarvisHome, leaseId, { now = new Date().toISOString() } = {}) {
  const registry = loadExecutionLeaseRegistry(jarvisHome, { now });
  const lease = registry.leases.find((item) => item.leaseId === leaseId);
  if (!lease) throw new Error(`execution lease not found: ${leaseId}`);
...
export function persistExecutionLeaseRegistry(jarvisHome, registry) {
  validateExecutionLeaseRegistry(registry, EXECUTION_LEASES_FILE, { validateExpiry: false });
  writeJsonAtomic(executionLeaseRegistryPath(jarvisHome), registry);
}

export function executionLeaseRegistryPath(jarvisHome) {
  if (!isNonEmptyString(jarvisHome)) throw new Error("jarvisHome required");
  return path.join(jarvisHome, SECURITY_DIR, EXECUTION_LEASES_FILE);
}
