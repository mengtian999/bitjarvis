...
export function pruneStaleCredentialBackups({
  ...
  if (!jarvisHome) throw new Error("credential backup retention requires a data directory");
  ...
  const rollbackTargetIsHealthy = liveCatalogIsUsable(path.join(jarvisHome, LIVE_CATALOG_FILE));
