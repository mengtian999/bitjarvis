...
const REMINDER_HEADER_RE = /^\[jarvis_reminder(?: at \d{4}-\d{2}-\d{2} \d{2}:\d{2})?\]$/;
const SESSION_MODEL_UNAVAILABLE_API = "jarvis-unavailable-model";
...
function cacheContractDebugEnabled() {
  return process.env.JARVIS_CACHE_CONTRACT_DEBUG === "1";
}
...
4. 只有需要你继续处理的后台任务，系统才会以 <jarvis-background-result> 消息送达结果；媒体生成成功由界面和 Bridge 自动处理，不要等待或主动追问。媒体生成失败可能会以 <jarvis-background-result> 送达：只说明失败原因，并询问用户是否要你新生成一张；原地重新生成只由用户在 UI 中操作`
    : `## Background Tasks
...
4. Only background tasks that need your follow-up are delivered via <jarvis-background-result> messages. Successful media generation is handled by the UI and Bridge automatically; do not wait for it or ask about it again. Failed media generation may be delivered via <jarvis-background-result>: explain only why it failed, then ask whether the user wants you to create a new image. In-place regeneration is a UI-only action for the user`;
}
...
const MAX_RECENT_APPROVAL_HISTORY = 20;
...
export class SessionCoordinator {
  ...
  async createSession(sessionMgr: any, cwd: any, memoryEnabled = true, model: any = null, {
    ...
    // 注入器由 Jarvis 持有 session/model 引用，不读取 Pi SDK ctx，避免 restore 后 stale ctx 丢失 sidecar 笔记。
    ...
    const visionAuxiliaryExtension = createVisionContextInjectionExtension({
      path: "jarvis-desktop-vision-context-injection",
      sessionPathRef,
      targetModelRef,
    ...
    const turnContextExtension = createReminderAwareTurnContextExtension({
      path: "jarvis-desktop-session-turn-context",
      sessionPathRef,
    ...
    //
    // allToolNames must cover the COMPLETE active set: Jarvis built-ins
    ...
    Object.assign(sessionEntry, {
      ...
      // Recent user decisions on approval prompts, kept per session runtime so
      // the automatic reviewer can recognise a similar action later in the same
      // session and stop asking. Like capability grants it never reaches the
      // manifest snapshot and dies with the runtime.
  ...
  /**
   * Enforce Jarvis's current model allowlist at the last reusable-session boundary.
   ...
   *
   * When the identity is still allowed, bind the freshly rebuilt Jarvis model
  ...
  /**
   * Record the user's decision on an approval prompt for the remaining life of
   * this session's runtime. The automatic reviewer receives these entries as
   * `recentApprovalHistory` and can recognise a similar action as already
   * sanctioned, which reduces how often an Auto-mode action falls back to a
   * fresh confirmation.
   */
  recordApprovalOutcome(sessionPath: any, entry: any) {
    const sessionEntry = this._getSessionEntryByPath(sessionPath);
    if (!sessionEntry) {
      const error: any = new Error("record approval outcome: session runtime is not loaded");
      error.code = "session_not_loaded";
      error.status = 409;
      throw error;
    }
    if (!entry || typeof entry !== "object") {
      const error: any = new Error("record approval outcome: entry is required");
      error.code = "invalid_entry";
      error.status = 400;
      throw error;
    }
    const toolName = typeof entry.toolName === "string" && entry.toolName.trim()
      ? entry.toolName.trim().slice(0, 120)
      : "";
    if (!toolName) {
      const error: any = new Error("record approval outcome: toolName is required");
      error.code = "invalid_entry";
      error.status = 400;
      throw error;
    }
    const normalized = {
      toolName,
      action: typeof entry.action === "string" && entry.action.trim() ? entry.action.trim().slice(0, 120) : "execute",
      status: entry.status === "rejected" ? "rejected" : "approved",
      ...(typeof entry.target === "string" && entry.target.trim() ? { target: entry.target.trim().slice(0, 240) } : {}),
      ...(typeof entry.reason === "string" && entry.reason.trim() ? { reason: entry.reason.trim().slice(0, 240) } : {}),
      at: Date.now(),
    };
    if (!Array.isArray(sessionEntry.recentApprovalHistory)) {
      sessionEntry.recentApprovalHistory = [];
    }
    sessionEntry.recentApprovalHistory.push(normalized);
    if (sessionEntry.recentApprovalHistory.length > MAX_RECENT_APPROVAL_HISTORY) {
      sessionEntry.recentApprovalHistory = sessionEntry.recentApprovalHistory.slice(-MAX_RECENT_APPROVAL_HISTORY);
    }
    return { ok: true };
  }

  /** Recent user approval decisions for this session (oldest first). */
  getRecentApprovalHistory(sessionPath: any) {
    const entry = this._getSessionEntryByPath(sessionPath);
    const history = Array.isArray(entry?.recentApprovalHistory) ? entry.recentApprovalHistory : [];
   ...
   * Jarvis 侧的 sessionPath 控制权释放出来，再把 SDK abort 和资源清理
   ...
   *   2. unsub — 取消 Jarvis 层的 session 事件转发
   ...
   * 的对象引用。Jarvis 这边 ModelRegistry.refresh() 之后会重建模型对象，
