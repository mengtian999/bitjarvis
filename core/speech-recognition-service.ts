...
/** 请求语言 > config.language(设置页「语音语言」);都未设置返回 undefined(由 adapter 自行处理)。 */
function normalizeLanguageOrDefault(language, configLanguage) {
  const explicit = textOrNull(language);
  if (explicit) return explicit;
  const fromConfig = textOrNull(configLanguage);
...
export class SpeechRecognitionService {
  ...
  /** 免提实时语音用：转写一段内存 WAV（不落库、不发事件），返回文本。 */
  async transcribePcmFile({ filePath, mimeType = "audio/wav", language, providerRef, modelRef }: any = {}) {
    if (!filePath) throw new Error("filePath is required for transcribePcmFile");
    ...
    const targetLanguage = normalizeLanguageOrDefault(language, config.language);
    if (!targetProvider || !targetModel) throw new Error("speech recognition model is not configured");
    ...
    const result = await adapter.transcribe({
      file: { realPath: filePath, filePath, mimeType },
    ...
    return { text: String(result?.text || "").trim(), providerId: target.providerId, modelId: target.model.id };
  }
  ...
  async transcribeAudio(payload: any = {}) {
    ...
    try {
      ...
      const ready = this._updateTranscription({ sessionId, sessionPath }, fileId, {
        ...
        modelId: target.model.id,
        protocolId: target.model.protocolId,
        ...(result.language ? { language: result.language } : targetLanguage ? { language: targetLanguage } : {}),
