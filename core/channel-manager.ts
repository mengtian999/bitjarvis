...
/**
 * 用户向频道提交一条消息(REST /channels/:id/messages 与 /api/voice 频道语音共用):
 * 追加消息 → 解析 @mention → 触发手机投递编排。投递异步执行,失败仅记日志。
 * 返回 { timestamp, senderName, mentionedAgents }。
 */
export async function submitUserChannelMessage(engine: any, hub: any, { name, body }: { name: string; body: string }) {
  const channelsDir = engine?.channelsDir;
  if (!channelsDir) throw new Error("channels unavailable");
  const filePath = path.join(channelsDir, `${name}.md`);
  const resolved = path.resolve(filePath);
  const base = path.resolve(channelsDir);
  if (!resolved.startsWith(base + path.sep) && resolved !== base) {
    throw new Error("Invalid channel id");
  }
  if (!fs.existsSync(filePath)) throw new Error("Channel not found");
  if (!body || !String(body).trim()) throw new Error("body is required");

  const senderName = engine.userName || "user";
  const result = await appendMessage(filePath, senderName, String(body));
  const mentionedAgents = extractMentionedAgentIds(String(body), {
  ...
  if (typeof triggerDelivery === "function") {
    triggerDelivery.call(hub, name, { mentionedAgents })?.catch?.((err: any) => {
      log.error(`触发手机送达失败: ${err?.message || err}`);
    });
  }
  return { timestamp: result.timestamp, senderName, mentionedAgents };
}
