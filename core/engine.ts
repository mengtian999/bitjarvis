/**
 * JarvisEngine — Jarvis 的核心引擎（Thin Facade）
 *
...
} from "../shared/jarvis-runtime-paths.ts";
...
// 底层 error.message（两者都可能带 jarvisHome 绝对路径前缀）——/api/health 可能
...
export class JarvisEngine {
  ...
  declare jarvisHome: any;
  declare _inputDrafts: any;
  declare productDir: any;
  declare userDir: any;
  /**
   * @param {object} dirs
   * @param {string} dirs.jarvisHome
   ...
   */
  constructor({ jarvisHome, productDir, agentId, appVersion, builtinMediaAdapters }) {
    this.jarvisHome = jarvisHome;
    ...
    this.agentsDir = path.join(jarvisHome, "agents");
    this.userDir = path.join(jarvisHome, "user");
    this.channelsDir = path.join(jarvisHome, "channels");
    fs.mkdirSync(this.channelsDir, { recursive: true });
    this._studioCronService = new StudioCronService({
      jarvisHome: this.jarvisHome,
    ...
    this._fileHistory = new FileHistoryService({
      historyRoot: path.join(jarvisHome, "file-history"),
    ...
    this._pluginInstallRecords = new PluginInstallRecords({ jarvisHome });
    ...
    this._inputDrafts = new InputDraftsStore({ jarvisHome: this.jarvisHome });
    this._models = new ModelManager({ jarvisHome });
    ...
    this._mcp = new McpManager({
      dataDir: path.join(this.jarvisHome, PLUGIN_DATA_DIRNAME, "mcp"),
      log: mcpLog,
    }, {
    ...
    this._taskRegistry = new TaskRegistry({
      persistencePath: path.join(this.jarvisHome, ".ephemeral", "plugin-tasks.json"),
      getSessionIdForPath: (sessionPath) => this.getSessionIdForPath(sessionPath),
    });
    ...
    this._checkpointStore = new CheckpointStore(
      path.join(this.jarvisHome, "checkpoints")
    );
    ...
    this._usageLedger = createUsageLedger({
      storagePath: path.join(this.jarvisHome, "usage-ledger.json"),
      eventBus: {
  ...
  _openSessionManifestStore() {
    const dbPath = path.join(this.jarvisHome, "session-manifest.db");
    try {
  ...
  _createResourceLoaderOptions(skillsDir) {
    const cwd = resolveJarvisPiSdkResourceLoaderCwd(this.jarvisHome);
    const agentDir = resolveJarvisPiSdkResourceLoaderAgentDir(this.jarvisHome);
  ...
  recordApprovalOutcome(sessionPath, entry) { return this._sessionCoord?.recordApprovalOutcome(sessionPath, entry); }
  getRecentApprovalHistory(sessionPath) { return this._sessionCoord?.getRecentApprovalHistory(sessionPath) || []; }
  ...
  async init(log: any = () => {}) {
    ...
    const providerSourceStep = runBestEffortStartupMigrationStep("provider-source", () => {
      migrateToProvidersYaml(this.jarvisHome, this.agentsDir, log);
    }, log);
    ...
    if (providerSourceStep.ok) {
      ...
      providerMediaStep = runBestEffortStartupMigrationStep("provider-media", () => {
        migrateProviderMediaConfig(this.jarvisHome, log);
      }, log);
    ...
    runBestEffortStartupMigrationStep("credential-backup-retention", () => {
      pruneStaleCredentialBackups({ jarvisHome: this.jarvisHome, log });
    }, log);
    runBestEffortStartupMigrationStep("credential-custody", () => {
      const healed = healCredentialFileModes({ jarvisHome: this.jarvisHome, log });
      if (healed.failed.length > 0) {
        log(`[credential-custody] ${healed.failed.length} 个文件未能收紧权限，已记录；应用继续启动`);
      }
    }, log);

    // 内置插件 seeding：为声明了 defaultApiKey 的内置 provider 写入初始密钥（仅首次）
    runBestEffortStartupMigrationStep("provider-api-key-seed", () => {
    ...
    this._resourceAccess = new ResourceAccessService({
      ...
      audit: (event) => appendSecurityAuditEvent(this.jarvisHome, event),
    });
    ...
    const t_rl = Date.now();
    const skillsDir = path.join(this.jarvisHome, "skills");
  ...
  async initPlugins(bus) {
    ...
    const userPluginsDir = path.join(this.jarvisHome, "plugins");
    const devPluginsDir = path.join(this.jarvisHome, "plugins-dev");
    const pluginDevRunsDir = path.join(this.jarvisHome, "plugin-dev-runs");
    const pluginDevSourcesDir = path.join(this.jarvisHome, "plugin-dev-sources");
    const pluginDataDir = path.join(this.jarvisHome, PLUGIN_DATA_DIRNAME);
    ...
    // 后台预热运行时媒体能力，避免首次打开「设置 → 多媒体」阻塞在 dreamina 探测上。
    void this._preheatRuntimeMediaCapabilities().catch(() => {});
  }

  _preheatRuntimeMediaCapabilities() {
    const providerRegistry = this._models?.providerRegistry;
    if (!providerRegistry?.refreshRuntimeMediaCapabilities) return Promise.resolve();
    return Promise.allSettled([
      providerRegistry.refreshRuntimeMediaCapabilities({ capability: "image_generation" }),
      providerRegistry.refreshRuntimeMediaCapabilities({ capability: "video_generation" }),
    ]);
  }
  ...
  buildTools(cwd, customTools, opts: any = {}) {
    ...
    result = {
      ...
      tools: wrapWithSessionPermission(result.tools, {
        ...
        getSessionAllowedInvocationCapabilities: (sessionPath) => this.getSessionAllowedInvocationCapabilities(sessionPath),
        recordApprovalOutcome: (sessionPath, entry) => this.recordApprovalOutcome(sessionPath, entry),
        getRecentApprovalHistory: (sessionPath) => this.getRecentApprovalHistory(sessionPath),
        emitEvent: (event, sessionPath) => this._emitEvent(event, sessionPath),
      }),
    ...
    // Object.create(JarvisEngine.prototype) is used by focused tool unit tests;
  ...
  async translateSkillNames(names, lang, opts: any = {}) {
    ...
    return translateSkillNamesWithCache({
      cachePath: getSkillNameTranslationCachePath(this.jarvisHome),
      skills,
      names,
      lang,
