...
export function healCredentialFileModes({ jarvisHome, log = () => {} }: HealOptions): CredentialHealResult {
  ...
  if (!jarvisHome) throw new Error("credential file healer requires a data directory");

  const relative = (target: string) => path.relative(jarvisHome, target) || ".";
  ...
  for (const name of TOP_LEVEL_SECRET_FILES) {
    healFile(path.join(jarvisHome, name));
  }

  for (const agentDir of subdirectories(path.join(jarvisHome, "agents"))) {
  ...
  for (const tree of SECRET_TREES) {
    healTree(path.join(jarvisHome, tree), 0, healDir, healFile);
  }
  ...
  for (const pluginDir of subdirectories(path.join(jarvisHome, PLUGIN_DATA_DIRNAME))) {
  ...
  const checkpointRoot = path.join(jarvisHome, "checkpoints", "session-manifest");
