...
const JARVIS_USER_ENVELOPE_TYPES = new Set([
  MESSAGE_PRESENTATION_RECORD_TYPE,
  MESSAGE_ORIGIN_RECORD_TYPE,
  AGENT_REVIEW_RECORD_TYPE,
]);
export const SESSION_BRANCH_RESET_RECORD_TYPE = "jarvis-session-branch-reset";
 ...
 * branch before Jarvis's contiguous presentation/origin/review envelope; Pi
...
function findUserEnvelopeStartIndex(branch, userIndex) {
  let index = userIndex;
  while (index > 0 && isJarvisUserEnvelopeEntry(branch[index - 1])) index -= 1;
  return index;
}

function isJarvisUserEnvelopeEntry(entry) {
  return entry?.type === "custom" && JARVIS_USER_ENVELOPE_TYPES.has(entry.customType);
}
