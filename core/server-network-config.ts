...
export function ensureServerNetworkConfig(jarvisHome, { now = new Date().toISOString() } = {}) {
  const filePath = path.join(jarvisHome, SERVER_NETWORK_FILE);
...
export function loadServerNetworkConfig(jarvisHome) {
  ensureServerNetworkConfig(jarvisHome);
  const config = readJsonRequired(path.join(jarvisHome, SERVER_NETWORK_FILE), SERVER_NETWORK_FILE);
  return validateServerNetworkConfig(config, SERVER_NETWORK_FILE);
}

export function saveServerNetworkConfig(jarvisHome, config, { now = new Date().toISOString() } = {}) {
  ...
  writeJsonAtomic(path.join(jarvisHome, SERVER_NETWORK_FILE), normalized);
  return normalized;
}

export function resolveServerListenOptions(jarvisHome) {
  const config = loadServerNetworkConfig(jarvisHome);
  return {
    mode: config.mode,
