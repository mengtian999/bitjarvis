...
function pluginSurfaceSessionKeyPath(jarvisHome) {
  assertNonEmpty(jarvisHome, "jarvisHome");
  return path.join(securityDirPath(jarvisHome), PLUGIN_SURFACE_SESSION_KEY_FILE);
}
...
function readOrCreateSessionKey(jarvisHome) {
  const filePath = pluginSurfaceSessionKeyPath(jarvisHome);
  try {
