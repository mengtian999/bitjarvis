...
const signKeysetOverride = process.env.JARVIS_SIGN_KEYSET;
const mainAliases = [];
if (signKeysetOverride) {
  const overridePath = path.resolve(signKeysetOverride);
  if (!fs.existsSync(overridePath)) {
    throw new Error(`[vite.config.main] JARVIS_SIGN_KEYSET points at a missing file: ${overridePath}`);
  }
