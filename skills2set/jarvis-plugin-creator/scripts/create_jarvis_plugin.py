...
SDK_PACKAGES = {
    "@jarvis/plugin-protocol": "plugin-protocol",
    "@jarvis/plugin-runtime": "plugin-runtime",
    "@jarvis/plugin-sdk": "plugin-sdk",
    "@jarvis/plugin-components": "plugin-components",
}

SDK_TARBALLS = {
    "@jarvis/plugin-protocol": "jarvis-plugin-protocol-*.tgz",
    "@jarvis/plugin-runtime": "jarvis-plugin-runtime-*.tgz",
    "@jarvis/plugin-sdk": "jarvis-plugin-sdk-*.tgz",
    "@jarvis/plugin-components": "jarvis-plugin-components-*.tgz",
}
...
def slugify(value: str) -> str:
    ...
    return slug or "jarvis-plugin"


def titleize(value: str) -> str:
    parts = re.split(r"[\s_-]+", value.strip())
    return " ".join(part[:1].upper() + part[1:] for part in parts if part) or "Jarvis Plugin"
...
def find_jarvis_root(*starts: Path) -> Path | None:
    seen: set[Path] = set()
    for start in starts:
...
def choose_sdk_mode(sdk_mode: str, jarvis_root: Path | None, template: str) -> str:
...
def required_sdk_packages(include_tool: bool, include_ui: bool, include_lifecycle: bool, template: str) -> list[str]:
    ...
    if include_tool or include_lifecycle:
        packages.append("@jarvis/plugin-protocol")
        packages.append("@jarvis/plugin-runtime")
    if include_ui:
        if "@jarvis/plugin-protocol" not in packages:
            packages.append("@jarvis/plugin-protocol")
        packages.extend(["@jarvis/plugin-sdk", "@jarvis/plugin-components"])
    return packages
...
def sdk_dependency(
    ...
    if sdk_mode == "workspace":
        if not jarvis_root:
            raise SystemExit("workspace SDK mode requires running from a Jarvis repo. Use --sdk-mode bundled.")
        package_dir = jarvis_root / "packages" / SDK_PACKAGES[package_name]
...
def manifest_for(
    ...
    manifest = {
        ...
        "name": display_name,
        "version": "0.1.0",
        "description": args.description or f"{display_name} plugin for Jarvis.",
...
def package_json_for(
    ...
    for package_name in required_sdk_packages(include_tool, include_ui, include_lifecycle, template):
        dependencies[package_name] = sdk_dependency(plugin_dir, jarvis_root, package_name, sdk_mode, bundled_sdk)
...
export async function execute(input = {{}}, toolCtx) {{
  ...
  const body = typeof input.body === "string" ? input.body : "Generated from a Jarvis plugin.";
...
import fs from "node:fs";
import path from "node:path";
import {{ createMediaDetails, defineTool }} from "@jarvis/plugin-runtime";
...
def create_direct_index(plugin_id: str, display_name: str) -> str:
    ...
    return f"""
const JARVIS_BUS_SKIP = Symbol.for("jarvis.event-bus.skip");
  ...
  async onload() {{
    ...
    if (ctx.bus.handle) {{
      this.register(ctx.bus.handle({js_string(status_type)}, (payload) => {{
        if (payload?.pluginId && payload.pluginId !== ctx.pluginId) return JARVIS_BUS_SKIP;
        return {{
          ok: true,
...
function renderShell(c, ctx, surface) {{
  const jarvisCss = c.req.query("jarvis-css") || "";
  const theme = c.req.query("jarvis-theme") || "inherit";
...
<head>
  ...
  ${{jarvisCss ? `<link rel="stylesheet" href="${{escapeAttr(jarvisCss)}}">` : ""}}
  <link rel="stylesheet" href="${{assetBase}}/panel.css">
</head>
<body data-jarvis-theme="${{escapeAttr(theme)}}" data-surface="${{surface}}">
...
def create_direct_panel_js(display_name: str) -> str:
    return f"""
const PROTOCOL = "jarvis.plugin.ui";
const VERSION = 1;
let seq = 0;

function targetOrigin() {{
  const params = new URLSearchParams(window.location.search);
  const explicit = params.get("jarvis-host-origin");
...
function request(type, payload, timeoutMs = 10000) {{
  const id = `jarvis-plugin-${{Date.now()}}-${{++seq}}`;
...
function normalizePluginApiPath(input) {{
  ...
  const parsed = new URL(`http://jarvis.local/${{stripped}}`);
...
function pluginApiFetch(path, init = {{}}) {{
  ...
  if (!surfaceSession) throw new Error("jarvis.api.fetch requires pluginSurfaceSession in the iframe URL.");
  const headers = new Headers(init.headers || {{}});
  headers.set("X-Jarvis-Plugin-Surface-Session", surfaceSession);
  return fetch(pluginApiUrl(path), {{ ...init, headers }});
}}

const jarvis = {{
  ready: () => event("jarvis.ready"),
  ...
  theme: {{
    getSnapshot: () => {{
      const params = new URLSearchParams(window.location.search);
      return {{ theme: params.get("jarvis-theme") || undefined, cssUrl: params.get("jarvis-css") || undefined }};
    }},
  }},
}};
...
function render() {{
    ...
    <main class="panel">
      <section class="card">
        <header class="card-header">
          <div>
            <h1 id="titleText"></h1>
            <p>A no-build Jarvis plugin panel.</p>
          </div>
  ...
  document.getElementById("copyBtn").addEventListener("click", async () => {{
    await jarvis.clipboard.writeText(titleInput.value);
    await jarvis.toast.show({{ message: "Copied title", type: "success" }});
  }});
  document.getElementById("openBtn").addEventListener("click", () => {{
    jarvis.external.open("https://example.com");
  }});
}}

render();
jarvis.ready();
jarvis.ui.resize({{ height: surface === "widget" ? 320 : 500 }});
"""
...
function Panel() {{
  ...
  useEffect(() => {{
    jarvis.ready();
    jarvis.ui.resize({{ height: surface === 'widget' ? 320 : 520 }});
  }}, [surface]);
  ...
  return (
    <JarvisThemeProvider
      mode={{themeMode}}
      theme={{customTheme || (themeMode === 'jarvis' ? 'warm-paper' : undefined)}}
      className="plugin-panel"
    >
      <CardShell
        title={{title}}
        description="A Jarvis plugin panel using the SDK and shared components."
...
body {
  background: var(--jarvis-plugin-bg, #f8f5ed);
  color: var(--jarvis-plugin-text, #3b3d3f);
}
...
def create_readme(
    ...
    lines = [
        f"# {display_name}",
        "",
        f"Jarvis plugin id: `{plugin_id}`.",
        "",
    ]
    if audience == "beginner" or template == "direct":
        lines.extend([
            "This is a small Jarvis plugin starter. Start by changing labels, button actions, and the sample tool.",
            "",
            "## What to edit first",
            "",
            "- `manifest.json`: the name, description, and permissions Jarvis sees.",
        ])
        if include_provider:
            lines.extend([
                "- `providers/*.js`: the provider declaration Jarvis discovers.",
        ...
        if include_ui:
            lines.extend([
                ...
                "- `ui/Panel.tsx`: React iframe UI built with Jarvis SDK components.",
    ...
    lines.extend([
        "",
        "Install by dragging this folder into Jarvis Settings > Plugins, or place it under the user plugin directory reported by `/api/plugins/settings`.",
        "",
        ...
        "- Browser iframe code may open, pick, or request access to resources through `jarvis.resources.*`, but real file reads and writes belong in server-side plugin tools, routes, or lifecycle code.",
    ...
    if include_ui:
        lines.extend([
            "This plugin requires full-access because Jarvis page and widget contributions are route-backed WebView/iframe UI.",
            ...
            "The starter manifest grants only the host calls used by the sample panel. If you add `jarvis.resources.open()`, `jarvis.resources.pick()`, or `jarvis.resources.requestAccess()` calls, add the matching `resource.open`, `resource.pick`, or `resource.requestAccess` entries under `ui.hostCapabilities`.",
        ])
    if include_provider:
        lines.extend([
...
def scaffold(args: argparse.Namespace) -> Path:
    ...
    jarvis_root = find_jarvis_root(Path.cwd(), parent)
    root_package = read_package_json(jarvis_root)
    sdk_mode = choose_sdk_mode(args.sdk_mode, jarvis_root, template)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create a Jarvis plugin scaffold.")
    ...
    template = choose_scaffold_template(args)
    print(f"Created Jarvis plugin scaffold: {plugin_dir}")
    ...
    if args.kind == "provider":
        ...
        print("  3. Install or drag the plugin folder into Jarvis with full-access enabled.")
        ...
        print("  2. Install or drag the plugin folder into Jarvis; no build step is required.")
    return 0
