...
const ENV_VAR = "JARVIS_PLUGIN_CREATOR_PYTHON";
...
function helpResult() {
  return {
    ...
    code: "help",
    message: [
      "Usage: node skills2set/jarvis-plugin-creator/scripts/check_env.mjs --capability scaffold",
...
function findPython() {
  ...
  if (unsupported) {
    return {
      ...
      attempted,
      message: `Jarvis Plugin Creator scripts require Python ${MIN_PYTHON_VERSION_TEXT}+; found ${unsupported.python.version} at ${unsupported.python.command}.`,
    };
  }
  ...
  return {
    ...
    attempted,
    message: `Jarvis Plugin Creator scripts require Python ${MIN_PYTHON_VERSION_TEXT}+. Install Python or set ${ENV_VAR} to a Python executable, then retry.`,
  };
}
...
function main() {
  ...
  return {
    ...
    message: "Jarvis Plugin Creator environment is ready for scaffolding.",
  };
}
