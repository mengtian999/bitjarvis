...
阶段六的技术底稿。Jarvis 内置 Character Card 系统（`lib/character-cards/service.ts`），本手册的格式与接口均以它的源码为准。
...
- `yuan` 只认 `jarvis` `butter` `ming` `kong`，填错静默回落到 `jarvis`
...
服务发现：端口与令牌在 `~/.jarvis/server-info.json`。

```bash
PORT=$(python3 -c "import json;print(json.load(open('$HOME/.jarvis/server-info.json'))['port'])")
TOKEN=$(python3 -c "import json;print(json.load(open('$HOME/.jarvis/server-info.json'))['token'])")
