...
// 探测失败（如 CLI 未安装）时，短时间内不再重复探测，避免每次打开设置都重试。
...
export function createDreaminaCapabilityDiscovery(options: DiscoveryOptions = {}) {
  ...
  let cachedError: { at: number; error: DreaminaCapabilityError } | null = null;
  ...
  async function discover(_refreshOptions: RefreshOptions): Promise<DreaminaCapabilitySnapshot> {
  ...
  return {
    async refresh(_refreshOptions: RefreshOptions = {}): Promise<DreaminaCapabilitySnapshot> {
      const force = _refreshOptions.force === true;
      if (!force && cachedError && now().getTime() - cachedError.at < NEGATIVE_CACHE_TTL_MS) {
        throw cachedError.error;
      }
      try {
        const snapshot = await discover(_refreshOptions);
        cachedError = null;
        return snapshot;
      } catch (cause) {
        if (cause instanceof DreaminaCapabilityError) {
          cachedError = { at: now().getTime(), error: cause };
        }
        throw cause;
      }
    },
