...
export const OFFICE_PDF_HELPER_FLAG = "--jarvis-office-html-to-pdf";
...
function helperCommandForJob(jobPath, input: any = {}, env = process.env) {
  const execPath = input.helperExecPath
    || env.JARVIS_OFFICE_PDF_HELPER_EXEC
    || env.JARVIS_DESKTOP_EXEC_PATH;
  if (!execPath) {
    throw new Error(
      "Chromium PDF helper is unavailable. Start Jarvis Desktop, or set JARVIS_OFFICE_PDF_HELPER_EXEC to an Electron executable.",
    );
  }

  const helperAppPath = input.helperAppPath
    || env.JARVIS_OFFICE_PDF_HELPER_APP_PATH
    || (env.JARVIS_DESKTOP_IS_PACKAGED === "1" ? null : env.JARVIS_DESKTOP_APP_PATH);
...
function buildJobPayload(input, htmlPath, outputPath) {
  return {
    ...
    embedJarvisFonts: input.embedJarvisFonts !== false,
