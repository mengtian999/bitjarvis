...
function describeDataEpochBlock({ stampEpoch, ownEpoch, stampLastVersion }) {
  ...
  return (
    ...
    + `JARVIS_ALLOW_DATA_DOWNGRADE=1（或对 jarvis serve 传 --allow-data-downgrade）。\n`
    ...
    + `Upgrade, or explicitly accept the risk with JARVIS_ALLOW_DATA_DOWNGRADE=1 `
    + `(or --allow-data-downgrade for jarvis serve).`
  );
}
