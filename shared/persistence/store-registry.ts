...
export const PERSISTENT_STORES: readonly StoreDescriptor[] = Object.freeze([
  defineStore({
    ...
    identityContract: "One high-water stamp belongs to one canonical JARVIS_HOME.",
  ...
  defineStore({
    ...
    affectedByEpochMigration: false,
    identityContract: "transitionId identifies one in-progress transition for one canonical JARVIS_HOME.",
    siteRules: [
  ...
  defineStore({
    ...
    affectedByEpochMigration: false,
    identityContract: "transitionId identifies one published or in-progress checkpoint directory for one canonical JARVIS_HOME; provider-internal .tmp-/.invalid- suffixes on that name are staging, not separate identities.",
    siteRules: rules(
  ...
  defineStore({
    ...
    affectedByEpochMigration: false,
    identityContract: "restoreId identifies one restore transaction's quarantined bytes for one canonical JARVIS_HOME; data-epoch-restores.log is one shared append-only audit trail across every restore for that home.",
    siteRules: rules(
  ...
  defineStore({
    ...
    identityContract: "At most one current server discovery record per JARVIS_HOME.",
    siteRules: [
  ...
  defineStore({
    ...
    checkpointPolicy: "Include with the identity registries for the same JARVIS_HOME.",
    restorePolicy: "Restore before remote transport accepts identity-bound requests.",
    identityContract: "serverNodeId is the stable identity of one JARVIS_HOME server node.",
  ...
  defineStore({
    ...
    identityContract: "The credential belongs to the local userId in the same JARVIS_HOME.",
  ...
  defineStore({
    ...
    identityContract: "One network configuration belongs to one JARVIS_HOME server.",
  ...
  defineStore({
    ...
    identityContract: "One preferences document belongs to the local user in one JARVIS_HOME.",
  ...
  defineStore({
    ...
    openEntry: ["AgentManager.loadAgents", "new Jarvis"],
  ...
  defineStore({
    ...
    openEntry: ["Jarvis.init", "memory ticker and summary stores"],
    migrationEntry: ["Jarvis v1 memories.db import", "core/migrations.ts memory migrations"],
  ...
  defineStore({
    ...
    schemaSource: {
      ...
      extensions: ["core/session-jsonl-file.ts repair contract", "core/session-coordinator.ts Jarvis metadata extensions"],
    },
    ...
    compatibility: "The locked Pi package schema/version plus Jarvis repair and extension readers are canonical; Jarvis does not duplicate Pi's session schema.",
  ...
  defineStore({
    ...
    identityContract: "Usage buckets are keyed by their runtime dimensions inside one JARVIS_HOME ledger.",
  ...
  defineStore({
    ...
    siteRules: [
      ...
      ...rules(["desktop/main.cjs"], "Writes desktop crash or browser-command diagnostics.", ["mkdir", "write-file", "append-file"], "(?:jarvisHome|crashLogPath|browser-cmd[.]log)"),
    ],
  }),
  defineStore({
  ...
  defineStore({
    ...
    siteRules: rules(["lib/pi-sdk/search-tools.ts", "lib/sandbox/win32-runtime-cache.ts"], "Populates a Jarvis-owned regenerable runtime cache."),
  }),
  defineStore({
    ...
    checkpointPolicy: "Excluded; the legacy tree is not Jarvis-owned active state.",
  ...
  defineStore({
    ...
    affectedByEpochMigration: false,
    identityContract: "Each fixed marker records completion for one JARVIS_HOME.",
...
export const PERSISTENCE_EXEMPTIONS: readonly PersistenceExemption[] = Object.freeze([
  ...
  exemption(
    ...
    "desktop/file-text-io.cjs",
    "Writes an absolute artifact/editor path explicitly selected by the caller, outside any implied JARVIS_HOME store.",
    "2027-01-31",
  ),
  exemption(
    ...
    "desktop/auto-updater.cjs",
    "desktop/auto-updater.cjs",
    "Deletes the exact legacy last-update-version source and its now-empty legacy directory outside the active JARVIS_HOME.",
    "2026-10-31",
    ["remove-path"],
    "(?:wrongFile|wrongDir)",
  ),
  exemption(
    ...
    "desktop/auto-updater.cjs",
    "desktop/auto-updater.cjs",
    "Deletes Electron updater's regenerable pending cache under app.getPath(userData), not Jarvis application state.",
    "2027-01-31",
    ["remove-path"],
    "cacheDir",
  ),
  ...
  exemption(
    ...
    "desktop/main.cjs",
    "desktop/main.cjs",
    "Writes an absolute path selected by the renderer or a screenshot directory selected by the user; no JARVIS_HOME store is implied.",
    "2027-01-31",
  ...
  exemption(
    ...
    "server/routes/desk.ts",
    "Deletes a skill directory only after the route proves it belongs to the explicitly mounted workspace skill catalog, outside Jarvis-owned persistence.",
    "2027-01-31",
    ["remove-path"],
    "skillDir",
  ),
  exemption(
    ...
    "server/routes/plugins.ts",
    "One bounded cleanup list contains both JARVIS_HOME .installing staging and operating-system extraction temp; flow-sensitive attribution must replace this exact conditional site.",
    "2026-10-31",
    ["remove-path"],
    "cleanupPath",
  ),
  ...
  exemption(
    ...
    "lib/compat/checks/dirs.ts",
    "Compatibility check creates a fixed set of registered JARVIS_HOME roots; flow attribution remains with the first-run coordinator.",
    "2026-10-31",
  ),
  exemption(
  ...
  exemption(
    ...
    "core/mount-aware-file-service.ts",
    "Creates user-selected workspace or mounted roots whose ownership is recorded by studio-mounts.json rather than by JARVIS_HOME path inventory.",
    "2027-01-31",
  ),
  exemption(
    ...
    "lib/resource-io/providers/local-fs-provider.ts",
    "Writes explicit user-selected local filesystem resource targets outside Jarvis persistence ownership.",
    "2027-01-31",
  ),
  exemption(
    ...
    "lib/file-ref/resource-io.ts",
    "Writes an explicit tool output path supplied by the caller; no JARVIS_HOME store is implied.",
    "2027-01-31",
  ),
  exemption(
    ...
    "shared/default-workspace.ts",
    "Creates the user-selected default workspace outside JARVIS_HOME.",
    "2027-01-31",
  ),
  exemption(
    ...
    "lib/desk/heartbeat.ts",
    "Writes heartbeat/task files inside an explicitly mounted workspace, not a Jarvis-owned persistence root.",
    "2027-01-31",
  ),
  exemption(
  ...
  exemption(
    ...
    "lib/sandbox/win32-exec.ts",
    "Writes a request-scoped Windows command wrapper outside persistent Jarvis state.",
    "2027-01-31",
  ),
  exemption(
