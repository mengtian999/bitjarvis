...
/** Retry EMFILE errors with back-off (Windows fd table exhaustion). */
function emfileRetry(fn, retries = 5) {
  return async (...args) => {
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        return await fn(...args);
      } catch (err) {
        if (err.code === "EMFILE" && attempt < retries) {
...
const safeOpen = emfileRetry((...a) => fs.createReadStream(...a));
const safeStat = emfileRetry((...a) => fsp.lstat(...a));
const safeReaddir = emfileRetry((...a) => fsp.readdir(...a));
const safeMkdir = emfileRetry((...a) => fsp.mkdir(...a));
...
async function packTree(srcDir, archivePath) {
  ...
  await safeMkdir(path.dirname(archivePath), { recursive: true });

  const gzip = zlib.createGzip();
  const out = await emfileRetry((...a) => fs.createWriteStream(...a))(archivePath);
  ...
  async function walk(relDir) {
    ...
    const entries = await safeReaddir(absDir, { withFileTypes: true });
    ...
    for (const entry of entries) {
      ...
      if (stat.isDirectory()) {
        ...
        const fileStream = await safeOpen(absPath);
        let written = 0;
