 ...
 *     -> rollout bucket (dedicated random UUID in JARVIS_HOME)
...
const GITHUB_CHANNEL_BASE = "https://github.com/mengtian999/Bitjarvis/releases/download/channels";
...
//    self-hosted form (`jarvis bundle pull`) ────────────────────────────────
 ...
 * must never cross.
 *
 * Runs only because an operator typed a command (`jarvis bundle pull`) —
 ...
 *     explicitly typing `jarvis bundle pull` is a full-intent action, and
 ...
 * "Restart `jarvis serve` to take effect" remains the operator's step
...
async function downloadAndApplyRendererArtifact(opts) {
  ...
  try {
    ...
    // on purpose: an operator re-running `jarvis bundle pull` against an
