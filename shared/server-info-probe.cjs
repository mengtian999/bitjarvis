 ...
 *
 * Threat model (同宅防线 / same-JARVIS_HOME mutual exclusion):
 * A single JARVIS_HOME can be opened by more than one kernel binary on the
 * same machine — `jarvis serve` (standalone CLI) and the desktop app are the
 ...
 * JARVIS_HOME, just listening on a different port (e.g. `jarvis serve` started
 ...
 * JARVIS_HOME, so a token match is as strong an identity signal as this
 ...
 *                           an `error`/`reason` body). Most likely a Jarvis
 *                           kernel whose token has since rotated, or a
 *                           different JARVIS_HOME's kernel that happens to be
 ...
 *   - "not-jarvis"           — something answered, but neither the 200 nor
 ...
 *                              is provably stale.
 *
 * "not-jarvis" and "dead" both mean the recorded server-info.json cannot be
 ...
 * @returns {Promise<{ status: "alive-same-home" } | { status: "alive-unauthorized" } | { status: "not-jarvis", detail: string } | { status: "dead" }>}
 */
async function probeServerInfo({ info, timeoutMs = DEFAULT_TIMEOUT_MS, fetchImpl, probePath = DEFAULT_PROBE_PATH } = {}) {
  ...
  if (res.status === 200) {
    ...
    return { status: "not-jarvis", detail: `200 response did not match the server-identity shape: ${safeDescribe(body)}` };
  }
  ...
  if (res.status === 403) {
    ...
    return { status: "not-jarvis", detail: `403 response did not match the auth-rejection shape: ${safeDescribe(body)}` };
  }

  return { status: "not-jarvis", detail: `unexpected HTTP status ${res.status}` };
}
 ...
 * either. Only "not-jarvis" and "dead" are safe to clean up and proceed past.
 * @param {string} status
 */
...
function describeForeignServerBlock({ status, info }) {
  ...
  if (status === "alive-same-home") {
    return (
      ...
      + `A Jarvis kernel is already running against this data directory (ownerKind=${ownerKind}, version=${version}, pid=${pid}). Quit it first, then start this one again.`
    );
  }
  if (status === "alive-unauthorized") {
    return (
      `该端口上有一个内核在响应，但无法用本机记录的凭据验证它的身份（token 可能已轮换，或另一个 Jarvis 数据目录的内核占用了这个端口）。请先排查（ownerKind=${ownerKind}, pid=${pid}），确认安全后再启动。\n`
      + `A kernel on that port responded but could not be authenticated with the credentials recorded locally (the token may have rotated, or a kernel from a different JARVIS_HOME is holding that port). Investigate first (ownerKind=${ownerKind}, pid=${pid}) before starting.`
    );
  }
  return null;
}
