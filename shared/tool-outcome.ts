...
/**
 * Read-time compatibility for Jarvis results written before toolError carried
 * Pi's explicit isError bit. Keep this deliberately narrow: plugin-specific
 * details.error values are diagnostics unless they match Jarvis's old helper
 * shape exactly.
 */
export function isKnownLegacyJarvisToolFailure(result: ToolResultLike): boolean {
  const details = recordOf(result.details);
...
export function projectToolResultOutcome(result: ToolResultLike): ToolOutcome {
  ...
  if (!isKnownLegacyJarvisToolFailure(result)) return { status: "succeeded", success: true };
  const error = resultErrorText(result);
...
export function projectKnownLegacyToolFailures(messages: unknown): unknown {
  ...
  const projected = messages.map((message) => {
    ...
    if (
      ...
      || result.isError === true
      || !isKnownLegacyJarvisToolFailure(result)
    ) {
      return message;
    }
    changed = true;
