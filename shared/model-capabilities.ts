...
export function normalizeModelProtocolCompat(value: any): Record<string, any> | null {
  ...
  if (value.jarvisVideoInput === true) out.jarvisVideoInput = true;
  if (value.jarvisAudioInput === true) out.jarvisAudioInput = true;
...
export function getThinkingFormat(model: any, context: any = {}) {
  ...
  // Built-in Anthropic models may arrive without Jarvis's projected compat object.
...
export function modelSupportsVideoInput(model: any): boolean {
  ...
  if (model.compat?.jarvisVideoInput === true) return true;
...
export function modelSupportsAudioInput(model: any): boolean {
  ...
  if (model.compat?.jarvisAudioInput === true) return true;
...
export function resolveModelAudioInputTransport(model: any, context: any = {}) {
  ...
  const explicit = lower(model?.compat?.audioTransport || model?.compat?.jarvisAudioTransport);
  if (explicit) {
...
export function withJarvisVideoInputCompat(model: any, enabled: unknown): any {
  if (!isPlainObject(model) || enabled !== true) return model;

  const compat = isPlainObject(model.compat) ? model.compat : {};
  if (compat.jarvisVideoInput === true) return model;
...
export function withJarvisAudioInputCompat(model: any, enabled: unknown): any {
  if (!isPlainObject(model) || enabled !== true) return model;

  const compat = isPlainObject(model.compat) ? model.compat : {};
  if (compat.jarvisAudioInput === true) return model;
...
export function normalizeVisionCapabilities(value: any): Record<string, any> | null {
  ...
  const outputFormat = ["gemini", "qwen", "anchor", "jarvis"].includes(lower(value.outputFormat))
