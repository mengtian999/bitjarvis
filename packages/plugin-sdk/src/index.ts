...
export interface JarvisPluginSize {
  width?: number;
  height?: number;
}

export interface JarvisPluginThemeSnapshot {
  theme?: string;
  cssUrl?: string;
}

export interface JarvisPluginRequestOptions {
  timeoutMs?: number;
}

export type JarvisToastType = 'success' | 'error' | 'info' | 'warning';

export interface JarvisToastShowInput {
  message: string;
  type?: JarvisToastType;
  duration?: number;
}

export interface JarvisToastShowResult {
  shown: boolean;
}

export type JarvisExternalOpenInput = string | { url: string };

export interface JarvisExternalOpenResult {
  opened: boolean;
}

export type JarvisClipboardWriteTextInput = string | { text: string };

export interface JarvisClipboardWriteTextResult {
  written: boolean;
}

export interface JarvisPluginSdkOptions {
  parentWindow?: Window;
  targetWindow?: Window;
  targetOrigin?: string;
...
export interface JarvisPluginSdk {
  ...
  ui: {
    resize(size: JarvisPluginSize): void;
  };
  theme: {
    getSnapshot(): JarvisPluginThemeSnapshot;
    subscribe(callback: (theme: JarvisPluginThemeSnapshot) => void): () => void;
  };
  host: {
    request<T = unknown>(
      type: string,
      payload?: unknown,
      options?: JarvisPluginRequestOptions,
    ): Promise<T>;
  };
  toast: {
    show(input: JarvisToastShowInput, options?: JarvisPluginRequestOptions): Promise<JarvisToastShowResult>;
  };
  external: {
    open(input: JarvisExternalOpenInput, options?: JarvisPluginRequestOptions): Promise<JarvisExternalOpenResult>;
  };
  clipboard: {
    writeText(
      input: JarvisClipboardWriteTextInput,
      options?: JarvisPluginRequestOptions,
    ): Promise<JarvisClipboardWriteTextResult>;
  };
  resources: {
    open(
...
export class JarvisPluginError extends Error {
  override name = 'JarvisPluginError';
...
function defaultIdFactory(): string {
  ...
  fallbackIdSeq += 1;
  return `jarvis-plugin-${Date.now()}-${fallbackIdSeq}`;
}

function getBrowserWindow(): Window {
  if (typeof window === 'undefined') {
    throw new Error('@jarvis/plugin-sdk requires a browser iframe window.');
  }
  return window;
}
...
function resolveTargetOrigin(targetWindow: Window, explicit?: string): string {
  ...
  const hostOrigin = new URLSearchParams(targetWindow.location.search).get('jarvis-host-origin');
  if (hostOrigin) return hostOrigin;

  return safeOriginFromUrl(targetWindow.document.referrer) ?? '*';
}

function readInitialTheme(targetWindow: Window): JarvisPluginThemeSnapshot {
  const params = new URLSearchParams(targetWindow.location.search);
  return {
    theme: params.get('jarvis-theme') ?? undefined,
    cssUrl: params.get('jarvis-css') ?? undefined,
  };
}
...
function externalOpenPayload(input: JarvisExternalOpenInput): { url: string } {
  return typeof input === 'string' ? { url: input } : input;
}

function clipboardWriteTextPayload(input: JarvisClipboardWriteTextInput): { text: string } {
...
function pluginApiFetch(targetWindow: Window, input: string, init?: RequestInit): Promise<Response> {
  ...
  if (!surfaceSession) {
    throw new Error('jarvis.api.fetch requires pluginSurfaceSession in the iframe URL.');
  }
  const fetchImpl = targetWindow.fetch?.bind(targetWindow) ?? globalThis.fetch?.bind(globalThis);
  if (!fetchImpl) {
    throw new Error('jarvis.api.fetch requires window.fetch.');
  }
...
export function createJarvisPluginSdk(options: JarvisPluginSdkOptions = {}): JarvisPluginSdk {
  ...
  const themeSubscribers = new Set<(theme: JarvisPluginThemeSnapshot) => void>();
  ...
  function onThemeMessage(event: MessageEvent): void {
    ...
    if (message.kind !== 'event' || message.type !== 'jarvis.theme.changed') return;
  ...
  function request<T = unknown>(
    type: string,
    payload?: unknown,
    requestOptions: JarvisPluginRequestOptions = {},
  ): Promise<T> {
    const id = idFactory();
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      const onMessage = (event: MessageEvent) => {
        ...
        if (message.kind === 'error' && message.error) {
          cleanup();
          reject(new JarvisPluginError(message.error));
        }
      };

      const timeout = targetWindow.setTimeout(() => {
        cleanup();
        reject(new JarvisPluginError({
          code: 'TIMEOUT',
  ...
  return {
    ready(payload?: unknown) {
      postEvent('jarvis.ready', payload);
    },
    assets: {
    ...
    theme: {
      ...
      subscribe(callback: (theme: JarvisPluginThemeSnapshot) => void) {
    ...
    toast: {
      show(input: JarvisToastShowInput, options?: JarvisPluginRequestOptions) {
        return request<JarvisToastShowResult>(PLUGIN_UI_CAPABILITY.TOAST_SHOW, input, options);
      },
    },
    external: {
      open(input: JarvisExternalOpenInput, options?: JarvisPluginRequestOptions) {
        return request<JarvisExternalOpenResult>(PLUGIN_UI_CAPABILITY.EXTERNAL_OPEN, externalOpenPayload(input), options);
      },
    },
    clipboard: {
      writeText(input: JarvisClipboardWriteTextInput, options?: JarvisPluginRequestOptions) {
        return request<JarvisClipboardWriteTextResult>(
    ...
    resources: {
      open(input: PluginResourceOpenInput, options?: JarvisPluginRequestOptions) {
        return request<PluginResourceOpenResult>(PLUGIN_UI_CAPABILITY.RESOURCE_OPEN, input, options);
      },
      pick(input: PluginResourcePickInput = {}, options?: JarvisPluginRequestOptions) {
        return request<PluginResourcePickResult>(PLUGIN_UI_CAPABILITY.RESOURCE_PICK, input, options);
      },
      requestAccess(input: PluginResourceRequestAccessInput, options?: JarvisPluginRequestOptions) {
...
let singleton: JarvisPluginSdk | null = null;

function getSingleton(): JarvisPluginSdk {
  singleton ??= createJarvisPluginSdk();
  return singleton;
}

export const jarvis: JarvisPluginSdk = {
  ready(payload?: unknown) {
