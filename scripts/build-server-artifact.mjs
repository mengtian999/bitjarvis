 ...
 *
 * 签名密钥纪律：
 * - `JARVIS_SIGN_KEY=<private-key-path>` 必须设置；未设置时硬报错，绝不静默
 *   跳过或降级（一个没有签名 seed 的安装包是坏的安装包）。
 * - `JARVIS_SIGN_KEYSET=<path>` 是构建期输入：替换"打进当次构建"的 keyset
...
/**
 * 在独立子进程中执行 packTree，获取一个干净的文件描述符表。
 *
 * build-server.mjs 在调用 packTree 之前已经运行了 npm install（634 个包）、
 * nft trace 和清理（移除 25575 个文件），这会累积打开的文件句柄。在 Windows
 * 上，Node.js 默认文件描述符限制很低，导致 EMFILE。通过将 packTree 放在一个
 * 具有自身 fd 表的干净子进程中，我们完全避免了这个问题。
 */
function spawnPackTree(srcDir, archivePath) {
  const helper = path.join(__dirname, "_pack-tree-helper.cjs");
  execFileSync(process.execPath, [helper, srcDir, archivePath], {
    stdio: "inherit",
    env: process.env,
  });
}
 ...
 * 签名模式由 `env.JARVIS_MACHO_SIGN_IDENTITY` 决定：CI（release workflow）由 build.yml 的
...
async function defaultSignMachOFiles(outDir, log, env = process.env) {
  const identity = env.JARVIS_MACHO_SIGN_IDENTITY;
  ...
  const mode = identity ? "Developer ID (JARVIS_MACHO_SIGN_IDENTITY)" : "ad-hoc";
...
/**
 * 解析"打进当次构建"的 keyset：JARVIS_SIGN_KEYSET 覆盖文件，或 repo 默认
...
export function resolveBuildKeyset(env) {
  const override = env.JARVIS_SIGN_KEYSET;
  if (!override) {
    return { keysetPath: null, keyset: loadPinnedKeyset() };
  }
  const keysetPath = path.resolve(override);
  if (!fs.existsSync(keysetPath)) {
    throw new Error(`[build-server] JARVIS_SIGN_KEYSET points at a missing file: ${keysetPath}`);
  }
  ...
  if (
    ...
    throw new Error(`[build-server] JARVIS_SIGN_KEYSET file must be a non-empty array of {keyId, publicKey}: ${keysetPath}`);
  }
  return { keysetPath, keyset: value };
}

/**
 * 校验签名密钥就位：JARVIS_SIGN_KEY 必须设置且指向真实文件。硬报错，绝不
 * 静默跳过或降级（一个没有签名 seed 的安装包是坏的安装包）。
 * @param {NodeJS.ProcessEnv | Record<string, string | undefined>} env
 * @returns {string} signKeyPath
 */
function requireSignKeyPath(env) {
  const signKeyPath = env.JARVIS_SIGN_KEY;
  if (!signKeyPath) {
    throw new Error(
      "[build-server] JARVIS_SIGN_KEY is not set. The installer seed MUST be signed; "
        + "building an unsigned seed is not a thing. Set JARVIS_SIGN_KEY=<private-key-path> "
        + "(local validation: generate a throwaway pair with scripts/artifact-keygen.mjs "
        + "and point JARVIS_SIGN_KEYSET at its matching keyset file).",
    );
  }
  if (!fs.existsSync(signKeyPath)) {
    throw new Error(`[build-server] JARVIS_SIGN_KEY points at a missing file: ${signKeyPath}`);
  }
  return signKeyPath;
}
...
export async function packServerArchive({ outDir, artifactOutDir, version, platform, arch, env = process.env, log = console.log, deps = {} }) {
  ...
  // env 显式传参（不在签名器里抓 process.env），JARVIS_MACHO_SIGN_IDENTITY
  ...
  // 在子进程中执行 packTree 以获得干净的 fd 表（避免 Windows EMFILE）
  spawnPackTree(outDir, archivePath);
...
export async function packRendererArtifact({ rendererDistDir, artifactOutDir, version, log = console.log, deps = {} }) {
  ...
  const archivePath = path.join(artifactOutDir, archiveName);
  // 在子进程中执行 packTree 以获得干净的 fd 表（避免 Windows EMFILE）
  spawnPackTree(rendererDistDir, archivePath);
  ...
  if (!fs.existsSync(archivePath)) {
    throw new Error(
      `[build-server] prebuilt renderer archive path invalid: ${archivePath} does not exist. `
        + "JARVIS_PREBUILT_RENDERER_BOX (or the prebuiltRendererArchive option) must point at the renderer "
...
export async function packDualKindSeed({
  ...
  // （或环境变量 JARVIS_PREBUILT_RENDERER_BOX）指向共享 job 已经打好的箱子，
  // 就跳过现场打包，直接复用那份字节——四个平台安装包内嵌的种子从此和货架
  // 上的归档字节完全一致。留空（本地开发者手跑 / 未设置该环境变量）时行为
  // 与过去完全一致：现场从 rendererDistDir 打包。
  prebuiltRendererArchive = env.JARVIS_PREBUILT_RENDERER_BOX || undefined,
}) {
  ...
  if (keysetPath) {
    log(`[build-server] seed: using JARVIS_SIGN_KEYSET override for THIS build: ${keysetPath}`);
  }
