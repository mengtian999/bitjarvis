...
const IDENT = [
  "-c", "user.email=smoke@jarvis.invalid",
  "-c", "user.name=jarvis-smoke",
];
...
export function main(argv = process.argv.slice(2), env = process.env) {
  ...
  const workRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jarvis-mingit-smoke-"));
  ...
  try {
    const results = [
      ...
      run("git config write", git, ["-C", repoDir, "config", "jarvis.smoke", "1"], runOptions),
      run("git config read", git, ["-C", repoDir, "config", "jarvis.smoke"], runOptions),
