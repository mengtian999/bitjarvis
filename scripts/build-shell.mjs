 ...
 *
 * Structural guarantee: JARVIS_SIGN_KEY (the private signing key path) is
 ...
 * JARVIS_SIGN_KEY either (it only ever reads the public JARVIS_SIGN_KEYSET
...
const SIGN_KEY_ENV_VAR = "JARVIS_SIGN_KEY";
 ...
 * that JARVIS_SIGN_KEY cannot leak into anything build:shell runs.
 */
  ...
  if (!fs.existsSync(manifestPath)) {
    throw new Error(
      `[build-shell] no signed seed kit found at ${artifactOutDir} (expected ${manifestFileName}). `
        ...
        + `Run \`npm run build:server\` (with ${SIGN_KEY_ENV_VAR}/JARVIS_SIGN_KEYSET set) to produce one, or copy an `
