 ...
 *     jarvis-server             ← shell wrapper（设置 JARVIS_ROOT 并启动）
 *     jarvis                    ← shell wrapper（server-first CLI）
...
// 打包模式下 JARVIS_ROOT 指向 resources/server，不能依赖 renderer asar 里的 assets。
...
// JARVIS_SIGN_KEY 未设置时这里硬报错（安装包必须携带签名 seed）；本地验证用
// artifact-keygen.mjs 生成一次性密钥对，配 JARVIS_SIGN_KEYSET 指向其 keyset。
// full 专属：开源产物不装箱、不签名、全程不读 JARVIS_SIGN_KEY。
