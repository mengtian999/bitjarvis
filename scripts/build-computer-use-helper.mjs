...
export function resolveComputerUseHelperBuildArch({
  ...
  return explicitArg || env.JARVIS_COMPUTER_USE_HELPER_ARCH || arch;
}
...
export function buildComputerUseHelper({
  ...
  env = process.env,
  arch = env.JARVIS_COMPUTER_USE_HELPER_ARCH || process.arch,
} = {}) {
  ...
  const packageDir = path.join(rootDir, "desktop", "native", "JarvisComputerUseHelper");
  ...
  const source = path.join(binPath, "jarvis-computer-use-helper");
  ...
  const target = path.join(outDir, "jarvis-computer-use-helper");
