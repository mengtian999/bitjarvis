 ...
 * binary into `outDir` (renamed to `jarvis-server.exe` on Windows so
...
export function prepareNodeRuntime({
  ...
  const destNode = path.join(outDir, isWin ? "jarvis-server.exe" : "node");
 ...
 * the `JARVIS_SERVER_BUNDLE_ENTRY` environment variable (read by
...
export function buildViteServerBundle({ rootDir, viteBundleDir, bundleOutDir, entry, log = (msg) => console.log(msg) }) {
  ...
  execSync("npx vite build --config vite.config.server.js", {
    cwd: rootDir,
    stdio: "inherit",
    env: entry ? { ...process.env, JARVIS_SERVER_BUNDLE_ENTRY: entry } : process.env,
  });
...
export async function pruneServerNodeModulesViaNft({
  ...
  // Temporary local-build override: the @vercel/nft trace walks every file in
  // node_modules and can OOM on memory-constrained machines (e.g. many Gradle
  // daemons holding multi-GB heaps). nft pruning is purely a size optimization
  // (removes unused node_modules files); skipping it does NOT affect runtime
  // correctness. Set JARVIS_SERVER_SKIP_NFT=1 to skip the prune step.
  if (process.env.JARVIS_SERVER_SKIP_NFT === "1") {
    log("[build-server] skipping nft trace (JARVIS_SERVER_SKIP_NFT=1) — node_modules kept whole");
    return;
  }
...
/**
 * Writes the shell/cmd wrapper scripts that set JARVIS_ROOT + JARVIS_SERVER_ENTRY
...
export function writeServerWrapperScripts({
  ...
  if (isWin) {
    ...
    fs.writeFileSync(
      path.join(outDir, "jarvis-server.cmd"),
      `@echo off\r\nset "JARVIS_ROOT=%~dp0"\r\nset "JARVIS_SERVER_ENTRY=%~dp0${winServerEntry}"\r\n"%~dp0jarvis-server.exe" "%~dp0bootstrap.js" %*\r\n`,
    );
    fs.writeFileSync(
      path.join(outDir, "jarvis.cmd"),
      `@echo off\r\nset "JARVIS_ROOT=%~dp0"\r\nset "JARVIS_SERVER_ENTRY=%~dp0${winServerEntry}"\r\n"%~dp0jarvis-server.exe" "%~dp0${winCliEntry}" %*\r\n`,
    );
  } else {
    const wrapper = path.join(outDir, "jarvis-server");
    fs.writeFileSync(wrapper, [
      "#!/bin/sh",
      'DIR="$(cd "$(dirname "$0")" && pwd)"',
      'export JARVIS_ROOT="$DIR"',
      `export JARVIS_SERVER_ENTRY="$DIR/${serverEntryRelPath}"`,
    ...
    const cliWrapper = path.join(outDir, "jarvis");
