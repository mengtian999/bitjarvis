...
import { applyDevEnvironment, defaultDevJarvisHome } from "./dev-env.js";
import {
...
const jarvisHome = defaultDevJarvisHome();
...
async function waitForServerInfo({ timeoutMs = 90_000, intervalMs = 200 } = {}) {
  ...
  while (Date.now() < deadline) {
    if (!isChildAlive(serverProcess)) {
      throw new Error("Jarvis server exited before writing server-info.json");
    }
    try {
...
function spawnServer() {
  ...
  serverEnv.JARVIS_ROOT = rootDir;
  serverEnv.JARVIS_SERVER_ENTRY = path.join(rootDir, "server", "main-full.ts");
  serverEnv.JARVIS_CREATE_STARTUP_SESSION = "0";
  serverEnv.JARVIS_PORT = process.env.JARVIS_PORT || "0";
...
function spawnVite(clientConfig, serverInfo) {
  ...
  viteEnv.JARVIS_DEV_WEB = "1";
  viteEnv.JARVIS_DEV_WEB_CLIENT_PORT = clientConfig.serverPort;
  viteEnv.JARVIS_DEV_WEB_API_BASE_URL = clientConfig.apiBaseUrl;
  viteEnv.JARVIS_DEV_WEB_SERVER_URL = `http://127.0.0.1:${serverInfo.port}`;
  viteEnv.JARVIS_DEV_WEB_SERVER_TOKEN = serverInfo.token;
