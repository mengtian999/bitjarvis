#!/usr/bin/env node
/**
 * Build the self-contained Windows JarvisCore archive published alongside the
 * desktop installer.
 *
...
export const STANDALONE_LAYOUT_ROOT = "JarvisCore";
...
export function standaloneArtifactNames(version, arch = STANDALONE_ARCH) {
  ...
  const stem = `JarvisCore-${version}-Windows-${arch}`;
...
export function standaloneWrapperContents() {
  const common = [
    "@echo off",
    "setlocal",
    'set "JARVIS_ROOT=%~dp0server"',
    'set "JARVIS_SERVER_ENTRY=%~dp0server\\bundle\\index.js"',
    'set "JARVIS_WIN32_SANDBOX_HELPER=%~dp0sandbox\\windows\\jarvis-win-sandbox.exe"',
    'set "PATH=%~dp0git\\cmd;%~dp0git\\usr\\bin;%~dp0git\\mingw64\\bin;%PATH%"',
  ];
  return {
    jarvis: [...common, '"%~dp0server\\jarvis-server.exe" "%~dp0server\\bundle\\cli.js" %*', ""].join("\r\n"),
    server: [...common, '"%~dp0server\\jarvis-server.exe" "%~dp0server\\bootstrap.js" %*', ""].join("\r\n"),
  };
}
...
export async function buildWindowsStandaloneArtifact(opts = {}) {
  ...
  const helperPath = path.resolve(
    opts.helperPath ?? path.join(rootDir, "dist-sandbox", `win-${arch}`, "jarvis-win-sandbox.exe"),
  );
  ...
  const stagingDir = fs.mkdtempSync(path.join(os.tmpdir(), "jarvis-standalone-"));
  ...
  try {
    ...
    const stagedHelper = path.join(layoutRoot, "sandbox", "windows", "jarvis-win-sandbox.exe");
    ...
    fs.writeFileSync(path.join(layoutRoot, "jarvis.cmd"), wrappers.jarvis, "utf8");
    fs.writeFileSync(path.join(layoutRoot, "jarvis-server.cmd"), wrappers.server, "utf8");
    ...
    const manifest = {
      ...
      layout: {
        ...
        sandboxHelper: `${STANDALONE_LAYOUT_ROOT}/sandbox/windows/jarvis-win-sandbox.exe`,
      },
