 ...
 * lib/known-models.json（Jarvis 策展的模型参考词典）。
 *
 * 行为：
 *   - 只做交集对表：遍历 Jarvis 词典每个 (provider, modelId)，在 pi 目录里找
 *     同 provider 同 id 的条目；provider key 字面比对，不做模糊映射。
 *   - 字段白名单（只碰这四项）：
 *       pi.contextWindow      → jarvis.context
 *       pi.maxTokens          → jarvis.maxOutput
 *       pi.input 含 "image"   → jarvis.image（词典缺省 false）
 *       pi.reasoning          → jarvis.reasoning（词典缺省 false）
...
// 显式排除表：记录已经人工审核、必须保留 Jarvis 现值的上游差异。
// 键格式与报告行一致（provider/modelId.field）。命中的差异保持 Jarvis 现值，
...
const excludedHits = []; // 同结构 — 命中排除表，保持 Jarvis 现值
...
for (const [provider, models] of Object.entries(dict)) {
  ...
  for (const [modelId, jarvisEntry] of Object.entries(models)) {
    jarvisTotal += 1;
    ...
    matched += 1;
    for (const { jarvisKey, read, jarvisDefault } of FIELD_MAP) {
      const piValue = read(piEntry);
      if (piValue === undefined || piValue === null) continue; // pi 侧无数据，不动
      const jarvisValue = Object.prototype.hasOwnProperty.call(jarvisEntry, jarvisKey)
        ? jarvisEntry[jarvisKey]
        : jarvisDefault;
      if (jarvisValue !== piValue) {
        const entry = { provider, modelId, field: jarvisKey, oldValue: jarvisValue, newValue: piValue };
        if (EXCLUDED_UPDATES.has(`${provider}/${modelId}.${jarvisKey}`)) {
...
console.log("");
console.log("═══ excluded（用户审核口径，保持 Jarvis 现值）═══");
...
console.log("");
console.log("═══ 统计 ═══");
console.log(`Jarvis 词典条目: ${jarvisTotal}`);
