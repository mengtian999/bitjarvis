...
function parseArgs(argv) {
  const options = { json: false, failOnAnomaly: false, jarvisHome: process.env.JARVIS_HOME || null };
  for (let index = 0; index < argv.length; index += 1) {
    const arg = argv[index];
    if (arg === "--jarvis-home" || arg === "--jarvis-home") {
      options.jarvisHome = argv[++index] || null;
    } else if (arg === "--json") {
      options.json = true;
...
function helpText() {
  return [
    ...
    "",
    "Usage:",
    "  JARVIS_HOME=/path/to/data node scripts/session-manifest-audit.mjs [--json] [--fail-on-anomaly]",
    "  node scripts/session-manifest-audit.mjs --jarvis-home /path/to/data [--json]",
    "",
...
function humanReport(report, jarvisHome) {
  const lines = [
    `Session manifest audit (${jarvisHome})`,
...
try {
  ...
  if (!options.jarvisHome) throw new Error("JARVIS_HOME or --jarvis-home is required");
  const jarvisHome = path.resolve(options.jarvisHome);
  index = openReadonlyManifestIndex(path.join(jarvisHome, "session-manifest.db"));
  const report = auditLegacySessionManifests({ jarvisHome, store: index });
  process.stdout.write(options.json
    ? `${JSON.stringify(report, null, 2)}\n`
    : `${humanReport(report, jarvisHome)}\n`);
