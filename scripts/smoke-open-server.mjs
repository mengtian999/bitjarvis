 ...
 * wrapper execs into) under a throwaway JARVIS_HOME, waits for it to bind and
...
function nodeBinPath(serverDir, isWin) {
  return path.join(serverDir, isWin ? "jarvis-server.exe" : "node");
}
...
function spawnOpenServer({ serverDir, isWin, jarvisHome, extraEnv = {} }) {
  ...
  const child = spawn(bin, [bootstrapPath], {
    ...
    env: {
      ...process.env,
      JARVIS_ROOT: serverDir,
      JARVIS_SERVER_ENTRY: path.join(serverDir, "bundle", "index.js"),
...
export async function runPositiveSmoke({ rootDir = ROOT, platform = process.platform, arch = process.arch } = {}) {
  ...
  const jarvisHome = fs.mkdtempSync(path.join(os.tmpdir(), "jarvis-open-smoke-"));
  const { child, getStderr } = spawnOpenServer({ serverDir, isWin, jarvisHome });
  ...
  try {
    ...
    fs.rmSync(jarvisHome, { recursive: true, force: true });
  }
}
...
export async function runNegativeSmoke({ rootDir = ROOT, platform = process.platform, arch = process.arch } = {}) {
  ...
  const jarvisHome = fs.mkdtempSync(path.join(os.tmpdir(), "jarvis-open-smoke-neg-"));
  const { child, getStderr } = spawnOpenServer({ serverDir, isWin, jarvisHome });
