 ...
 * 以单个签名归档进箱、首启在 JARVIS_HOME 解压，该机器整块删除。双 artifact 路径 起
  ...
  if (!manifestPath) {
    throw new Error(
      `[fix-modules] seed manifest missing from packaged resources: ${path.join(seedDir, "seed-train-<platform>-<arch>.json")}. `
        + "Run npm run build:server (with JARVIS_SIGN_KEY) before electron-builder.",
    );
  }
...
exports.default = async function (context) {
  ...
  if (platformName === "mac") {
    const computerUseHelper = path.join(resourcesDir, "computer-use", "macos", "jarvis-computer-use-helper");
