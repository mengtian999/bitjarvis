...
function analyzeFile(file) {
  ...
  const flags = {
    ...
    hasFsTmp: /\bmkdtemp(?:Sync)?\b|\btmpdir\b|\bJARVIS_HOME\b|\.jarvis(?:-dev)?/.test(haystack),
    ...
    hasPersistence: /migration|persist|store|registry|sidecar|sqlite|jsonl|yaml|atomic|write|read|cache|JARVIS_HOME|mkdtemp/i.test(haystack),
    ...
    hardcodesJarvisHome: /\.jarvis(?:-dev)?/.test(haystack),
  };
...
function decide(flags, category) {
  ...
  if (category === "persistence" || category === "adapter" || category === "architecture" || category === "ui-state") {
    return flags.touchesPrivateMock || flags.hardcodesJarvisHome ? "SHRINK" : "KEEP";
  }
  if (category === "scaffold") return "QUARANTINE";
  if (flags.touchesPrivateMock || flags.hardcodesJarvisHome) return "SHRINK";
  return "QUARANTINE";
}
...
function deleteOrMergeCandidate(flags, category, decision) {
  ...
  if (flags.hardcodesJarvisHome) candidates.push("replace hardcoded Jarvis data directory with injected temp JARVIS_HOME");
