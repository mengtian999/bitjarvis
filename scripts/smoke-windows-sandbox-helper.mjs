...
export function windowsSandboxHelperPath({ rootDir = ROOT, arch = "x64" } = {}) {
  return path.join(rootDir, "dist-sandbox", `win-${arch}`, "jarvis-win-sandbox.exe");
}
...
export function smokeWindowsSandboxHelper({
  ...
  const smokeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jarvis-windows-sandbox-ci-"));
  const workDir = path.join(smokeRoot, "work");
  const jarvisHome = path.join(smokeRoot, "jarvis-home");
...
// construction. A fresh jarvisHome is provisioned per case so the required
...
async function runSandboxedCommandForSmoke({ rootDir, arch, env, command, timeout }) {
  ...
  const smokeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jarvis-windows-sandbox-parity-"));
  const workDir = path.join(smokeRoot, "work");
  const jarvisHome = path.join(smokeRoot, "jarvis-home");
  fs.mkdirSync(workDir, { recursive: true });
  fs.mkdirSync(jarvisHome, { recursive: true });
  try {
    ...
    const exec = createWin32Exec({ sandbox: { policy, jarvisHome: jarvisHome, helperPath } });
    const chunks = [];
