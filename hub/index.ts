 ...
 * Hub — 消息调度中枢
 *
 * 同进程模式：Hub 和 JarvisEngine 跑在同一个 Node 进程里。
...
export class Hub {
  ...
  async send(text, opts: any = {}) {
    ...
    if (
      o.images?.length
      && this._engine.jarvisHome
      && !o.inboundFiles?.length
      && !hasDisplayImageAttachments(o.displayMessage)
    ) {
      const attachDir = path.join(this._engine.jarvisHome, "attachments");
